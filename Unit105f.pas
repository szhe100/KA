...
procedure TsetexpayB2.setupdatestatus;    // 编辑状态
begin
speedbutton2.enabled:=false;   // refresh disable
speedbutton5.enabled:=false;   // refresh disable
end;

procedure TsetexpayB2.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure TsetexpayB2.FormActivate(Sender: TObject);
begin
with homebank do
begin
...
procedure TsetexpayB2.FormCloseQuery(Sender: TObject;
...
procedure TsetexpayB2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TsetexpayB2.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpayB2.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TsetexpayB2.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
with bill_dtl do
...
procedure TsetexpayB2.FormCreate(Sender: TObject);
begin
...
procedure TsetexpayB2.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure TsetexpayB2.DSbillStateChange(Sender: TObject);
begin
with bill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpayB2.billBeforeEdit(DataSet: TDataSet);
begin
with bill do
begin
...
procedure TsetexpayB2.billCalcFields(DataSet: TDataSet);
begin
with bill do
...
procedure TsetexpayB2.billReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure TsetexpayB2.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpayB2.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TsetexpayB2.dxDBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
if key in ['a'..'z'] then key:=chr(ord(key)-32);   // turn lower charactor to upper
end;

procedure TsetexpayB2.SpeedButton1Click(Sender: TObject);
begin
with dm.pubqry do
begin
    if active then close;
...
procedure TsetexpayB2.SpeedButton2Click(Sender: TObject);
var s: string;
begin
with dm.pubqry do
begin
...
procedure TsetexpayB2.SpeedButton3Click(Sender: TObject);
begin
with dm.pubqry do
begin
    if active then close;
...
procedure TsetexpayB2.DSbill_dtlStateChange(Sender: TObject);
begin
with bill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpayB2.bill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
showmessage(dataset.name+':'+e.message);
...
procedure TsetexpayB2.hello1Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag);
end;

procedure TsetexpayB2.dxDBGrid2CustomDrawCell(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
  AColumn: TdxTreeListColumn; ASelected, AFocused, ANewItemRow: Boolean;
  var AText: String; var AColor: TColor; AFont: TFont;
  var AAlignment: TAlignment; var ADone: Boolean);
...
procedure TsetexpayB2.bill_dtlBeforePost(DataSet: TDataSet);
begin
with bill_dtl do
begin
...
procedure TsetexpayB2.Button1Click(Sender: TObject);
begin
