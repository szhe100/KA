...
procedure Tsetexpay.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetexpay.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetexpay.FormActivate(Sender: TObject);
begin
with homebank do
begin
...
procedure Tsetexpay.FormCloseQuery(Sender: TObject;
...
procedure Tsetexpay.FormClose(Sender: TObject;
...
procedure Tsetexpay.SpeedButton8Click(Sender: TObject);
begin
...
procedure Tsetexpay.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
setprogress(1);
...
procedure Tsetexpay.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
with bill_dtl do
...
//    commandtext:=commandtext+' f.bank_name,broker=g.zname,carry_dt1=dbo.fn_getcarrydt(a.bod_id),h.carry_dt2,'; //j.carry_dt,';
    commandtext:=commandtext+' f.bank_name,broker=g.zname,h.carry_dt2,'; //j.carry_dt,';
...
//    commandtext:=commandtext+' left join (select a.bod_cd,b.med_id,bank_id1=a.bank_id,a.carry_dt from tb_bill a,tb_bill_dtl b where a.bod_type_id=30 and a.bod_status_id=1 and a.bod_id=b.bod_id) j on a.bod_id =j.med_id';
    commandtext:=commandtext+' where bod_type_id=37';
...
procedure Tsetexpay.FormCreate(Sender: TObject);
begin
...
procedure Tsetexpay.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetexpay.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetexpay.DSbillStateChange(Sender: TObject);
begin
with bill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetexpay.billBeforeEdit(DataSet: TDataSet);
begin
with bill do
begin
...
procedure Tsetexpay.billCalcFields(DataSet: TDataSet);
begin
with bill do
...
procedure Tsetexpay.billReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetexpay.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetexpay.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetexpay.dxDBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
if key in ['a'..'z'] then key:=chr(ord(key)-32);   // turn lower charactor to upper
end;

procedure Tsetexpay.SpeedButton1Click(Sender: TObject);
begin
...
procedure Tsetexpay.SpeedButton2Click(Sender: TObject);
var s: string;
begin
with bill do
begin
...
procedure Tsetexpay.DSbill_dtlStateChange(Sender: TObject);
begin
with bill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetexpay.bill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
showmessage(dataset.name+':'+e.message);
...
procedure Tsetexpay.bill_dtlAfterPost(DataSet: TDataSet);
begin
if bill_dtl.ChangeCount>0 then myupdaterefresh(bill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetexpay.dxDBGrid2CustomDrawCell(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
  AColumn: TdxTreeListColumn; ASelected, AFocused, ANewItemRow: Boolean;
  var AText: String; var AColor: TColor; AFont: TFont;
  var AAlignment: TAlignment; var ADone: Boolean);
...
procedure Tsetexpay.bill_dtlCalcFields(DataSet: TDataSet);
begin
with bill_dtl do
begin
