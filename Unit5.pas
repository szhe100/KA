...
type
  Tsetbusimate = class(TForm)
    ...
    dxDBGrid1Cremark: TdxDBGridColumn;
    busimateCremark: TStringField;
    Label34: TLabel;
    dxDBEdit20: TdxDBEdit;
    Label35: TLabel;
    dxDBGrid1tax_code: TdxDBGridColumn;
    busimateBEZEI: TStringField;
    busimateCITY1: TStringField;
    dxDBGrid1BEZEI: TdxDBGridColumn;
    dxDBGrid1CITY1: TdxDBGridColumn;
    Edit1: TEdit;
    Label36: TLabel;
    busimatedist3: TStringField;
    ...
    procedure busimatedistrictChange(Sender: TField);
    procedure busimateBeforeEdit(DataSet: TDataSet);
    ...
    procedure dxLookupTreeView1MouseMove(Sender: TObject;
      Shift: TShiftState; X, Y: Integer);
    procedure dxDBLookupTreeView1CloseUp(Sender: TObject; Accept: Boolean);
...
begin
    ...
    dxDBLookupTreeView1.text:= fieldbyname('cdistrict').asstring;
end;
end;
...
//    commandtext:='select cdistrict=dbo.fn_getdistrict(a.district),settletype=dbo.fn_obj_desc(a.settle_type),';
    commandtext:='select cdistrict=dbo.fn_getdistrict1(a.district),settletype=dbo.fn_obj_desc(a.settle_type),';
    ...
    begin
        case radiogroup1.itemindex of
            ...
            3:  commandtext:=commandtext+' and a.mate_code='''+trim(dxedit1.text)+'''';
        end;
        if radiogroup1.itemindex<3 then commandtext:=commandtext+' like ''%'+trim(dxedit1.text)+'%''';
    end;
...
//    if Trim(dxLookupTreeView1.text)<>'' then commandtext:=commandtext+' and a.region='+dm.district.fieldbyname('code').asstring;
    if Trim(dxLookupTreeView1.text)<>'' then commandtext:=commandtext+' and (a.BEZEI like '''+dm.district.fieldbyname('name0').asstring+'%'' or a.CITY1 like '''+dm.district.fieldbyname('name0').asstring+'%'')';
...
begin
    ...
    if curuserid<>fieldbyname('creat_by').asinteger then raise Exception.Create('非本人设置记录，不可删除');
...
//    if fieldbyname('tax_code').asstring='' then raise Exception.Create('请填写统一社会信用代码');
...
begin
    ...
    begin
        ...
        open;
        if recordcount>0 then raise Exception.Create('已有相同名称的商业公司，不可重复录入');
    end;
end;
with busimate do
begin
...
//            commandtext:='select top 1 code from vi_district1 where prov_name+'' ''+city_name='''+s3+'''';
            commandtext:='select top 1 code from vi_district1 where name='''+s3+''' or prov_name+'' ''+name='''+s3+'''';
//            commandtext:=' inner join vi_district1 h on b.f5=h.name or b.f5=h.prov_name+'' ''+h.name';
            open;
            if recordcount=0 then id3:=0 else id3:=fieldbyname('code').asinteger;
            if active then close;
            commandtext:='if exists (select 1 from tb_busimate where mate_type_id=2 and mate_name='''+s2+''' and creat_by>0)';
            ...
            commandtext:=commandtext+' where mate_type_id=2 and mate_name='''+s2+''' and creat_by>0';   //creat_by>0 KA中录入的记录 即不修改来自SAP的记录
            commandtext:=commandtext+' if not exists (select 1 from tb_busimate where mate_type_id=2 and mate_name='''+s2+''')';
            commandtext:=commandtext+' insert into tb_busimate (mate_code,mate_name,district,region,qry_code,mate_type_id,zname1,address1,phone1,zname2,address2,phone2,creat_by,creat_dt)';
            commandtext:=commandtext+' values ('''+s1+''','''+s2+''','+inttostr(id3)+','+inttostr(id3)+',dbo.fn_getpy('''+s2+'''),2,'''+s4+''','''+s5+''','''+s6+''','''+s7+''','''+s8+''','''+s9+''','+inttostr(curuserid)+',getdate())';
//edit1.text:=commandtext;            
            execute;
        end;
        j:=j+1;
...
procedure Tsetbusimate.busimatedistrictChange(Sender: TField);
begin
with busimate do
begin
    fieldbyname('BEZEI').asstring:=dm.district.fieldbyname('prov_name').asstring;
    fieldbyname('CITY1').asstring:=dm.district.fieldbyname('city_name').asstring;
end;
end;

procedure Tsetbusimate.busimateBeforeEdit(DataSet: TDataSet);
begin
{
with busimate do
begin
...
procedure Tsetbusimate.dxLookupTreeView1DropDown(Sender: TObject);
begin
setprogress(1);
end;

procedure Tsetbusimate.dxDBLookupTreeView1DropDown(Sender: TObject);
begin
setprogress(1);
end;

procedure Tsetbusimate.dxDBLookupTreeView1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
setprogress(0);
end;

procedure Tsetbusimate.dxLookupTreeView1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
setprogress(0);
end;

procedure Tsetbusimate.dxDBLookupTreeView1CloseUp(Sender: TObject;
  Accept: Boolean);
begin
(Sender as TdxDBLookupTreeView).text:=dm.district.fieldbyname('cdistrict').asstring;
end;

procedure Tsetbusimate.dxLookupTreeView1CloseUp(Sender: TObject;
  Accept: Boolean);
begin
