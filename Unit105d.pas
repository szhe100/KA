...
procedure TsetexpayB.setupdatestatus;    // 编辑状态
begin
...
procedure TsetexpayB.setunupdatestatus;  // 浏览状态
begin
...
procedure TsetexpayB.FormActivate(Sender: TObject);
begin
with homebank do
begin
...
procedure TsetexpayB.FormCloseQuery(Sender: TObject;
...
procedure TsetexpayB.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TsetexpayB.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpayB.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TsetexpayB.SpeedButton8Click(Sender: TObject);
begin
...
procedure TsetexpayB.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
setprogress(1);
...
procedure TsetexpayB.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
with bill_dtl do
...
procedure TsetexpayB.FormCreate(Sender: TObject);
begin
...
procedure TsetexpayB.SpeedButton7Click(Sender: TObject);
begin
...
procedure TsetexpayB.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure TsetexpayB.DSbillStateChange(Sender: TObject);
begin
with bill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpayB.billBeforeEdit(DataSet: TDataSet);
begin
with bill do
begin
...
procedure TsetexpayB.billCalcFields(DataSet: TDataSet);
begin
with bill do
...
procedure TsetexpayB.billReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure TsetexpayB.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpayB.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TsetexpayB.dxDBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
if key in ['a'..'z'] then key:=chr(ord(key)-32);   // turn lower charactor to upper
end;

procedure TsetexpayB.SpeedButton1Click(Sender: TObject);
begin
...
procedure TsetexpayB.SpeedButton2Click(Sender: TObject);
var s: string;
begin
with bill do
begin
...
procedure TsetexpayB.DSbill_dtlStateChange(Sender: TObject);
begin
with bill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpayB.bill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
showmessage(dataset.name+':'+e.message);
...
procedure TsetexpayB.bill_dtlAfterPost(DataSet: TDataSet);
begin
if bill_dtl.ChangeCount>0 then myupdaterefresh(bill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure TsetexpayB.dxDBGrid2CustomDrawCell(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
  AColumn: TdxTreeListColumn; ASelected, AFocused, ANewItemRow: Boolean;
  var AText: String; var AColor: TColor; AFont: TFont;
  var AAlignment: TAlignment; var ADone: Boolean);
...
procedure TsetexpayB.bill_dtlBeforePost(DataSet: TDataSet);
begin
with bill_dtl do
begin
...
procedure TsetexpayB.bill_dtlCalcFields(DataSet: TDataSet);
begin
with bill_dtl do
begin
...
begin
    ...
    if active then close;
    commandtext:=' select top 3 a.bod_cd from tb_bill a';
    commandtext:=commandtext+' left join (select b.med_id,amot=sum(b.amot) from tb_bill a,tb_bill_dtl b where a.bod_type_id=37 and (a.bod_status_id=1 or a.bod_id in ('+t+')) and a.bod_id=b.bod_id and b.type_id=36 group by b.med_id) d on a.bod_id=d.med_id';
    commandtext:=commandtext+' where a.bod_id in (select med_id from tb_bill_dtl where bod_id in ('+t+') and type_id=36)';
    commandtext:=commandtext+'  and d.amot>a.bod_amot';   //and d.med_id=a.bod_id
edit1.text:=commandtext;
    open;
    if recordcount>0 then
    begin
        s:='下列关联应付记账实付金额(可能包含之前已核销金额)超过应付金额，不可送审核'+#13+#10+'------------------------------------------------';
        first;
        while not eof do
        begin
            s:=s+#13+#10+'应付记账单:'+fieldbyname('bod_cd').asstring+' '+fieldbyname('med_name').asstring+' '+fieldbyname('specifi').asstring;
    ...
    commandtext:=commandtext+' left join (select b.med_id,amot=sum(b.amot) from tb_bill a,tb_bill_dtl b where a.bod_type_id=37 and (a.bod_status_id=1 or a.bod_id='+bill.fieldbyname('bod_id').asstring+') and a.bod_id=b.bod_id and b.type_id=1 group by b.med_id) d on d.med_id=h.rec_id'; //b.dtl_id=d.med_id';
    commandtext:=commandtext+' where a.bod_type_id=1';
    commandtext:=commandtext+'  and b.dtl_id in (select med_id from tb_bill_dtl where bod_id in ('+t+') and type_id=1)';
    commandtext:=commandtext+'  and d.amot>b.qty*(isnull(c.amot,0)+isnull(c.fee,0))';  //and b.dtl_id=d.med_id
edit2.text:=commandtext;
    open;
    if recordcount>0 then
    begin
        s:='下列关联品种实付金额(可能包含之前已核销金额)超过应付金额，不可送审核'+#13+#10+'------------------------------------------------';
        first;
        while not eof do
        begin
            s:=s+#13+#10+'分销采购单:'+fieldbyname('bod_cd').asstring+' '+fieldbyname('med_name').asstring+' '+fieldbyname('specifi').asstring;
