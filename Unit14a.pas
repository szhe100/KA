...
type
  Tpickmed_bat1 = class(TForm)
    ...
    DBGrid1gmp: TdxDBGridCheckColumn;
...
procedure Tpickmed_bat1.Edit1KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
    fwid: boolean;
...
begin
    ...
    begin
        ...
        commandtext:=commandtext+'  agent=dbo.fn_mate_name(a.mate_id),a.med_id,a.bat_cd,a.qty,a.price,b.med_name,b.med_code,b.chm_name,b.specifi,b.pdt_place,';
...
//        if pickmed_bat1.tag=0 then commandtext:=commandtext+' and (a.qty<>0)';
        commandtext:=commandtext+' and (a.qty<>0) and a.mate_id='+inttostr(fmateid);
        if fwid  // 宽字符串
...
procedure Tpickmed_bat1.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickmed_bat1.DBGrid1DblClick(Sender: TObject);
begin
   modalresult:=mrOK;
end;

procedure Tpickmed_bat1.wwDBNavigator1FirstClick(Sender: TObject);
begin
edit1.setfocus;
end;

procedure Tpickmed_bat1.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
var flook: boolean;
    i,j: word;
    s: string;
begin
if (edit1.text='') or (query1.active=false) or (pickmed_bat1.tag=1) then
begin
    pickmed_bat1.tag:=0;
    exit;
end;
...
procedure Tpickmed_bat1.FormActivate(Sender: TObject);
begin
edit1.setfocus;
edit1.selectall;
end;

procedure Tpickmed_bat1.BitBtn3Click(Sender: TObject);
begin
pickmed_bat1.tag:=1;  // <Esc> (cancel) 退出标志
end;

procedure Tpickmed_bat1.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickmed_bat1.DSquery1DataChange(Sender: TObject; Field: TField);
begin
if activecontrol=dbgrid1 then
edit1.text:=query1.fieldbyname('pick_ord').asstring;
end;

procedure Tpickmed_bat1.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
