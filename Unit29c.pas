...
begin
    ...
    begin
        ...
        commandtext:='select creat_dt=cast(convert(char(10),a.creat_dt,20) as datetime),j.carry_dt,check_dt1=j.check_dt,a.*,bod_cd1=j.bod_cd,mate_name='''',mate_name1='''',creater=d.zname,checker=e.zname,broker=f.zname,carryer=g.zname,receiver=h.zname,storager=i.zname,';
//        commandtext:='select creat_dt=cast(convert(char(10),a.creat_dt,20) as datetime),j.carry_dt,check_dt1=j.check_dt,a.*,bod_cd1=j.bod_cd,b.mate_name,mate_name1=c.mate_name,creater=d.zname,checker=e.zname,broker=f.zname,carryer=g.zname,receiver=h.zname,storager=i.zname,';
        commandtext:=commandtext+' cdistrict=dbo.fn_getdistrict1(f.district),receiver1=f.receiver,f.bank_name,f.depo_acco,j.bank_id1,bank_name1=dbo.fn_mate_name(j.bank_id1)';
        commandtext:=commandtext+' from tb_bill a';
//        commandtext:=commandtext+' left join (select b.bod_id,c.src_id,c.dst_id from tb_bill_dtl b,tb_bill_stadtl h,tb_bill c where b.med_id=h.rec_id and h.bod_id=c.bod_id) k on a.bod_id=k.bod_id';
//        commandtext:=commandtext+' left join tb_busimate b on k.src_id =b.mate_id';
//        commandtext:=commandtext+' left join tb_busimate c on k.dst_id =c.mate_id';
...
with qry1 do
    begin
        ...
        commandtext:=commandtext+' cdistrict=dbo.fn_getdistrict1(c.district),receiver1=f.receiver,f.bank_name,f.depo_acco,j.bank_id1,bank_name1=dbo.fn_mate_name(j.bank_id1),';
        commandtext:=commandtext+' dist1=dbo.fn_getdistrictname (c.district,1),'; //dbo.fn_getdistrictlevel (c.district,1),';
...
//        commandtext:=commandtext+' level1=dbo.fn_obj_desc(k.level_id1),level2=dbo.fn_obj_desc(k.level_id2),level3=dbo.fn_obj_desc(k.level_id3),';
        commandtext:=commandtext+' level1=k.VKBUR_TXT,level2='''',level3=k.ZPLOT_T,leader=k.BU_SORT1';
        commandtext:=commandtext+' ,a.channel_id,channel=dbo.fn_obj_desc1(11,y.channel_id),a.channel_dtl_id,channel_dtl=dbo.fn_obj_desc1(12,y.channel_dtl_id)';
...
//        commandtext:=commandtext+'  c.med_id,m.med_code,med_name=case when m.med_name is null then ''应付记账'' else m.med_name end,m.specifi,m.pdt_place,med_unit=dbo.fn_obj_desc(m.unit_id)';
        ...
        commandtext:=commandtext+'  left join (select b.bod_id,c.bod_type_id,d.channel_id,d.channel_dtl_id, bod_cd2=c.bod_cd,bod_desc2=c.bod_desc,d.qty,b.price,b.amot,carry_dt1=c.carry_dt,d.sta_id,';
        commandtext:=commandtext+'  broker=dbo.fn_staff_name(case when h.sta_id is null then d.sta_id else h.sta_id end),c.src_id,c.dst_id,';
        commandtext:=commandtext+'  d.med_id,m.material_code1, med_code,med_name,specifi,pdt_place,med_unit=dbo.fn_obj_desc(m.unit_id)';
        commandtext:=commandtext+'  from tb_bill_dtl b,tb_bill c,tb_bill_dtl d,tb_bill_stadtl h,tb_medicine m';
        commandtext:=commandtext+'  where b.med_id=h.rec_id and c.bod_id=d.bod_id and d.dtl_id=h.dtl_id and d.med_id=m.med_id';
        commandtext:=commandtext+'  union all select b.bod_id,c.bod_type_id,c.channel_id,c.channel_dtl_id,bod_cd2=c.bod_cd,bod_desc2=c.bod_desc,qty=0,b.price,b.amot,carry_dt1=c.carry_dt,c.broker_id,broker=dbo.fn_staff_name(c.broker_id),c.src_id,c.dst_id,';
        commandtext:=commandtext+'  c.med_id,m.material_code1,m.med_code,med_name=case when m.med_name is null then ''未关联品种'' else m.med_name end,m.specifi,m.pdt_place,med_unit=dbo.fn_obj_desc(m.unit_id)';
        commandtext:=commandtext+'  from tb_bill_dtl b inner join tb_bill c on b.sta_id=c.bod_id left join tb_medicine m on c.med_id=m.med_id) y on a.bod_id=y.bod_id';
      ...
      //	commandtext:=commandtext+' left join tb_busiframe k on k.rec_id=dbo.fn_getbusiframerecid5(c.district,y.med_id,y.dst_id,y.carry_dt1)';
        commandtext:=commandtext+' left join SAP_ZSD_BUS k on k.rec_id=dbo.fn_getSAP_ZSD_BUSrecid(y.channel_id,y.channel_dtl_id,c.district,c.mate_code,y.material_code1,y.carry_dt1)';
