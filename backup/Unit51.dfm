object setreport06: Tsetreport06
  Left = 191
  Top = 106
  AutoScroll = False
  Caption = #21307#33647#20195#34920#36129#29486#27611#21033#25552#25104#25490#21517#34920
  ClientHeight = 514
  ClientWidth = 695
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  DesignSize = (
    695
    514)
  PixelsPerInch = 96
  TextHeight = 12
  object Shape6: TShape
    Left = 2
    Top = 26
    Width = 73
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label25: TLabel
    Left = 674
    Top = 9
    Width = 7
    Height = 12
    Alignment = taRightJustify
    Anchors = [akTop, akRight]
    Font.Charset = GB2312_CHARSET
    Font.Color = clMaroon
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = [fsBold]
    ParentFont = False
  end
  object SpeedButton2: TSpeedButton
    Left = 513
    Top = 2
    Width = 25
    Height = 25
    Hint = #36716#20986#25253#34920
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton2Click
  end
  object Shape4: TShape
    Left = 2
    Top = 2
    Width = 73
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label3: TLabel
    Left = 14
    Top = 8
    Width = 49
    Height = 14
    AutoSize = False
    Caption = #21457#29983#26085#26399
    Transparent = True
    WordWrap = True
  end
  object Label1: TLabel
    Left = 181
    Top = 8
    Width = 12
    Height = 12
    Caption = #33267
  end
  object SpeedButton5: TSpeedButton
    Left = 513
    Top = 45
    Width = 175
    Height = 25
    Hint = #21047#26032
    Anchors = [akTop, akRight]
    Caption = '&R '#21047#26032
    Glyph.Data = {
      
      3393333777333777337333333999993333333333377777333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Spacing = 10
    OnClick = SpeedButton5Click
  end
  object Label7: TLabel
    Left = 15
    Top = 32
    Width = 48
    Height = 12
    Caption = #19994' '#21153' '#21592
    Transparent = True
  end
  object Shape1: TShape
    Left = 2
    Top = 50
    Width = 73
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label2: TLabel
    Left = 15
    Top = 56
    Width = 48
    Height = 12
    Caption = #25152#22312#22320#21306
    Transparent = True
  end
  object dxDBGrid1: TdxDBGrid
    Left = 0
    Top = 74
    Width = 695
    Height = 440
    Bands = <
      item
      end>
    DefaultLayout = True
    HeaderPanelRowCount = 1
    KeyField = 'broker_id'
    ShowGroupPanel = True
    ShowSummaryFooter = True
    SummaryGroups = <
      item
        DefaultGroup = True
        SummaryItems = <
          item
            ColumnName = 'dxDBGrid1broker'
            SummaryFormat = '###,###'#20154';; '
            SummaryType = cstCount
          end
          item
            SummaryField = 'bal0'
            SummaryFormat = '###,###,##0.00;; '
            SummaryType = cstSum
          end
          item
            ColumnName = 'dxDBGrid1drawamot'
            SummaryField = 'drawamot'
            SummaryFormat = '###,###,##0.00;; '
            SummaryType = cstSum
          end
          item
            SummaryField = 'rec'
            SummaryFormat = '###,###,##0.00;; '
            SummaryType = cstSum
          end
          item
            SummaryField = 'pay'
            SummaryFormat = '###,###,##0.00;; '
            SummaryType = cstSum
          end
          item
            ColumnName = 'dxDBGrid1profit'
            SummaryField = 'bal1'
            SummaryFormat = '###,###,##0.00;; '
            SummaryType = cstSum
          end>
        Name = 'dxDBGrid1SummaryGroup2'
      end>
    SummarySeparator = ', '
    Align = alBottom
    PopupMenu = PopupMenu1
    TabOrder = 3
    DataSource = DSqry
    Filter.Active = True
    Filter.DropDownCount = 30
    Filter.Criteria = {00000000}
    HighlightColor = clAqua
    HighlightTextColor = clBlack
    LookAndFeel = lfUltraFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabThrough, edgoVertThrough]
    OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
    ShowRowFooter = True
    Anchors = [akLeft, akTop, akRight, akBottom]
    object dxDBGrid1ord: TdxDBGridColumn
      Alignment = taCenter
      Font.Charset = GB2312_CHARSET
      Font.Color = clMaroon
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'ord'
      DisableFilter = True
    end
    object dxDBGrid1district: TdxDBGridColumn
      Width = 90
      BandIndex = 0
      RowIndex = 0
      FieldName = 'district'
    end
    object dxDBGrid1team_name: TdxDBGridColumn
      DisableEditor = True
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'team_name'
    end
    object dxDBGrid1broker: TdxDBGridMaskColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'broker'
      SummaryFooterType = cstCount
      SummaryFooterFormat = '###,###'#20154';; '
      DisableFilter = True
    end
    object dxDBGrid1saleamot: TdxDBGridColumn
      BandIndex = 0
      RowIndex = 0
      FieldName = 'saleamot'
      SummaryFooterType = cstSum
      SummaryFooterField = 'saleamot'
      SummaryFooterFormat = '###,###,##0.00;; '
      SummaryType = cstSum
      SummaryField = 'saleamot'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1profit: TdxDBGridColumn
      Width = 100
      BandIndex = 0
      RowIndex = 0
      FieldName = 'profit'
      SummaryFooterType = cstSum
      SummaryFooterField = 'profit'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryField = 'profit'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1drawamot: TdxDBGridMaskColumn
      Color = 15132390
      Width = 100
      BandIndex = 0
      RowIndex = 0
      FieldName = 'drawamot'
      SummaryFooterType = cstSum
      SummaryFooterField = 'drawamot'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryField = 'drawamot'
    end
    object dxDBGrid1rate: TdxDBGridColumn
      Alignment = taCenter
      Width = 60
      BandIndex = 0
      RowIndex = 0
      FieldName = 'rate'
      DisableFilter = True
    end
  end
  object dxDateEdit1: TdxDateEdit
    Left = 77
    Top = 4
    Width = 90
    Style.BorderStyle = xbsSingle
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 1
    StyleController = StyleController
    Date = -700000.000000000000000000
  end
  object dxDateEdit2: TdxDateEdit
    Left = 205
    Top = 4
    Width = 90
    Style.BorderStyle = xbsSingle
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 2
    StyleController = StyleController
    Date = -700000.000000000000000000
  end
  object wwDBNavigator2: TwwDBNavigator
    Left = 538
    Top = 2
    Width = 150
    Height = 25
    DataSource = DSqry
    Options = []
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    Anchors = [akTop, akRight]
    ParentShowHint = False
    object wwNavButton1: TwwNavButton
      Left = 0
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 0
      Style = nbsFirst
    end
    object wwNavButton2: TwwNavButton
      Left = 25
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Prior'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 1
      Style = nbsPrior
    end
    object wwNavButton3: TwwNavButton
      Left = 50
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Next'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 2
      Style = nbsNext
    end
    object wwNavButton4: TwwNavButton
      Left = 75
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 3
      Style = nbsLast
    end
    object wwNavButton5: TwwNavButton
      Left = 100
      Top = 0
      Width = 25
      Height = 25
      Hint = #35774#32622#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1SaveBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 4
      Style = nbsSaveBookmark
    end
    object wwNavButton6: TwwNavButton
      Left = 125
      Top = 0
      Width = 25
      Height = 25
      Hint = #36820#22238#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1RestoreBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 5
      Style = nbsRestoreBookmark
    end
  end
  object dxButtonEdit4: TdxButtonEdit
    Left = 77
    Top = 27
    Width = 218
    Style.BorderStyle = xbsSingle
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 0
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit4ButtonClick
    ExistButtons = True
  end
  object dxLookupEdit1: TdxLookupEdit
    Left = 77
    Top = 51
    Width = 218
    Font.Charset = GB2312_CHARSET
    Font.Color = clMaroon
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Style.BorderStyle = xbsSingle
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 5
    StyleController = StyleController
    DropDownRows = 20
    ClearKey = 46
    ListFieldName = 'dist_name'
    CanDeleteText = True
    KeyFieldName = 'dist_id'
    ListSource = DM.DSedistrict
    LookupKeyValue = Null
  end
  object DSqry: TDataSource
    DataSet = qry
    OnDataChange = DSqryDataChange
    Left = 381
    Top = 208
  end
  object qry: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    ReadOnly = True
    RemoteServer = DM.RemoteServer
    Left = 352
    Top = 208
    object qryteam_id: TAutoIncField
      FieldName = 'team_id'
      ReadOnly = True
    end
    object qryteam_name: TStringField
      DisplayLabel = #19994#21153#32452
      FieldName = 'team_name'
    end
    object qrybroker_id: TIntegerField
      FieldName = 'broker_id'
    end
    object qrybroker: TStringField
      DisplayLabel = #19994#21153#21592
      FieldName = 'broker'
      Size = 100
    end
    object qrydistrict: TStringField
      DisplayLabel = #22320#21306
      FieldName = 'district'
      Size = 50
    end
    object qrysaleamot: TBCDField
      DisplayLabel = #38144#21806#37329#39069
      FieldName = 'saleamot'
      DisplayFormat = '###,###,##0.00;; '
      Precision = 15
      Size = 2
    end
    object qrydrawamot: TBCDField
      DisplayLabel = #25237#36164#27454#37329#39069
      FieldName = 'drawamot'
      ReadOnly = True
      DisplayFormat = '###,###,##0.00;; '
      Precision = 32
      Size = 2
    end
    object qryprofit: TBCDField
      DisplayLabel = #27611#21033#28070
      FieldName = 'profit'
      DisplayFormat = '###,###,##0.00;; '
      Precision = 15
      Size = 2
    end
    object qryord: TLargeintField
      DisplayLabel = #25490#21517
      FieldName = 'ord'
      ReadOnly = True
    end
    object qryrate: TBCDField
      DisplayLabel = #25152#21344#27604#20363
      FieldName = 'rate'
      ReadOnly = True
      DisplayFormat = '##,##0.##%;; '
      Precision = 32
      Size = 18
    end
  end
  object StyleController: TdxEditStyleController
    BorderColor = 14065456
    BorderStyle = xbsSingle
    ButtonStyle = btsSimple
    Edges = [edgBottom]
    HotTrack = True
    Left = 321
    Top = 201
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingAGE;Data Source=.\heyang'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'sp_brokerexgbal1 0,0,'#39'2011-1-1'#39','#39'2011-5-11'#39)
    Left = 371
    Top = 34
    object ADOQuery1broker_id: TIntegerField
      FieldName = 'broker_id'
    end
    object ADOQuery1broker: TStringField
      FieldName = 'broker'
    end
    object ADOQuery1district: TStringField
      FieldName = 'district'
      Size = 50
    end
    object ADOQuery1saleamot: TBCDField
      FieldName = 'saleamot'
      Precision = 15
      Size = 2
    end
    object ADOQuery1drawamot: TBCDField
      FieldName = 'drawamot'
      Precision = 15
      Size = 2
    end
    object ADOQuery1profit: TBCDField
      FieldName = 'profit'
      Precision = 15
      Size = 2
    end
    object ADOQuery1sta_id: TAutoIncField
      FieldName = 'sta_id'
      ReadOnly = True
    end
    object ADOQuery1dept_id: TIntegerField
      FieldName = 'dept_id'
    end
    object ADOQuery1team_id: TIntegerField
      FieldName = 'team_id'
    end
    object ADOQuery1zname: TStringField
      FieldName = 'zname'
    end
    object ADOQuery1zcode: TStringField
      FieldName = 'zcode'
      Size = 10
    end
    object ADOQuery1sex: TBooleanField
      FieldName = 'sex'
    end
    object ADOQuery1personalID: TStringField
      FieldName = 'personalID'
    end
    object ADOQuery1birthdate: TDateTimeField
      FieldName = 'birthdate'
    end
    object ADOQuery1hiredate: TDateTimeField
      FieldName = 'hiredate'
    end
    object ADOQuery1firedate: TDateTimeField
      FieldName = 'firedate'
    end
    object ADOQuery1origin: TStringField
      FieldName = 'origin'
    end
    object ADOQuery1degree: TStringField
      FieldName = 'degree'
    end
    object ADOQuery1title: TStringField
      FieldName = 'title'
    end
    object ADOQuery1zpost: TStringField
      FieldName = 'zpost'
    end
    object ADOQuery1phone: TStringField
      FieldName = 'phone'
      Size = 50
    end
    object ADOQuery1fax: TStringField
      FieldName = 'fax'
    end
    object ADOQuery1residence: TStringField
      FieldName = 'residence'
      Size = 50
    end
    object ADOQuery1notes: TMemoField
      FieldName = 'notes'
      BlobType = ftMemo
    end
    object ADOQuery1password: TStringField
      FieldName = 'password'
    end
    object ADOQuery1zright: TMemoField
      FieldName = 'zright'
      BlobType = ftMemo
    end
    object ADOQuery1operative: TBooleanField
      FieldName = 'operative'
    end
    object ADOQuery1stop_yn: TBooleanField
      FieldName = 'stop_yn'
    end
    object ADOQuery1stop_by: TIntegerField
      FieldName = 'stop_by'
    end
    object ADOQuery1stop_for: TStringField
      FieldName = 'stop_for'
    end
    object ADOQuery1stop_dt: TDateTimeField
      FieldName = 'stop_dt'
    end
    object ADOQuery1use_priority: TIntegerField
      FieldName = 'use_priority'
    end
    object ADOQuery1receipt: TBooleanField
      FieldName = 'receipt'
    end
    object ADOQuery1broker_1: TBooleanField
      FieldName = 'broker_1'
    end
    object ADOQuery1creat_dt: TDateTimeField
      FieldName = 'creat_dt'
    end
    object ADOQuery1creat_by: TIntegerField
      FieldName = 'creat_by'
    end
    object ADOQuery1storage: TBooleanField
      FieldName = 'storage'
    end
    object ADOQuery1account: TBooleanField
      FieldName = 'account'
    end
    object ADOQuery1supervisor: TBooleanField
      FieldName = 'supervisor'
    end
    object ADOQuery1purchase: TBooleanField
      FieldName = 'purchase'
    end
    object ADOQuery1biller: TBooleanField
      FieldName = 'biller'
    end
    object ADOQuery1printable: TBooleanField
      FieldName = 'printable'
    end
    object ADOQuery1exportable: TBooleanField
      FieldName = 'exportable'
    end
    object ADOQuery1sta_type_id: TIntegerField
      FieldName = 'sta_type_id'
    end
    object ADOQuery1mate_id: TIntegerField
      FieldName = 'mate_id'
    end
    object ADOQuery1qry_code: TStringField
      FieldName = 'qry_code'
    end
    object ADOQuery1pick_ord: TIntegerField
      FieldName = 'pick_ord'
    end
    object ADOQuery1opr_scale: TWordField
      FieldName = 'opr_scale'
    end
    object ADOQuery1med_scale: TWordField
      FieldName = 'med_scale'
    end
    object ADOQuery1depo_bank: TIntegerField
      FieldName = 'depo_bank'
    end
    object ADOQuery1bank_name: TStringField
      FieldName = 'bank_name'
      Size = 50
    end
    object ADOQuery1depo_acco: TStringField
      FieldName = 'depo_acco'
    end
    object ADOQuery1district_1: TIntegerField
      FieldName = 'district_1'
    end
    object ADOQuery1receiver: TStringField
      FieldName = 'receiver'
    end
    object ADOQuery1Email: TStringField
      FieldName = 'Email'
    end
    object ADOQuery1mobile_no: TStringField
      FieldName = 'mobile_no'
    end
    object ADOQuery1ord: TLargeintField
      FieldName = 'ord'
      ReadOnly = True
    end
    object ADOQuery1rate: TBCDField
      FieldName = 'rate'
      ReadOnly = True
      Precision = 32
      Size = 18
    end
    object ADOQuery1team_id_1: TIntegerField
      FieldName = 'team_id_1'
    end
    object ADOQuery1team_name: TStringField
      FieldName = 'team_name'
      ReadOnly = True
    end
  end
  object PopupMenu1: TPopupMenu
    Left = 133
    Top = 174
    object N3: TMenuItem
      Caption = #26597#25214'...'
      OnClick = N3Click
    end
    object N4: TMenuItem
      Caption = '-'
    end
    object N1: TMenuItem
      Caption = #38544#34255#26412#21015
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #26174#31034#25152#26377#21015
      OnClick = N2Click
    end
    object N5: TMenuItem
      Caption = '-'
    end
    object N6: TMenuItem
      Caption = #26684#24335#22797#21407
      OnClick = N6Click
    end
  end
end
