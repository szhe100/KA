...
type
  Tsetruleapply5 = class(TForm)
    ...
    dxDBGrid2bonus: TdxDBGridColumn;
...
procedure Tsetruleapply5.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetruleapply5.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetruleapply5.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsetruleapply5.FormCloseQuery(Sender: TObject;
...
procedure Tsetruleapply5.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetruleapply5.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetruleapply5.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetruleapply5.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=53 and a.bod_status_id in (1,2)';
...
procedure Tsetruleapply5.FormCreate(Sender: TObject);
begin
...
procedure Tsetruleapply5.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
begin
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active=true then close;
                commandtext:='select c.rec_id,c.amot,c.rate,c.bonus,c.remark,c.valid_dt,price1=c.price,';
                commandtext:=commandtext+' b.bod_id,b.dtl_id,b.med_id,b.price,med_code,med_name,specifi,pdt_place,med_unit=dbo.fn_obj_desc(m.unit_id),med_type=dbo.fn_med_type(m.med_id)';
                commandtext:=commandtext+' from tb_bill_dtl b,tb_busiframe8 c,tb_medicine m';
...
procedure Tsetruleapply5.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetruleapply5.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetruleapply5.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetruleapply5.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetruleapply5.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetruleapply5.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetruleapply5.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetruleapply5.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetruleapply5.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetruleapply5.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetruleapply5.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetruleapply5.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetruleapply5.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetruleapply5.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetruleapply5.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsetruleapply5.SpeedButton2Click(Sender: TObject);
begin
...
begin
    if active=true then close;
    commandtext:='sp_updatestock53 53,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsetruleapply5.SpeedButton3Click(Sender: TObject);
begin
