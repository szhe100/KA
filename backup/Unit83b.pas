...
var
  setagentaccount2: Tsetagentaccount2;
...
procedure Tsetagentaccount2.setupdatestatus;    // 编辑状态
begin
speedbutton2.enabled:=false;    // examine disable
speedbutton5.enabled:=false;   // refresh disable
end;

procedure Tsetagentaccount2.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetagentaccount2.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsetagentaccount2.FormCloseQuery(Sender: TObject;
...
procedure Tsetagentaccount2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetagentaccount2.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetagentaccount2.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetagentaccount2.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetagentaccount2.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetagentaccount2.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=14 and a.bod_status_id in (1,3,4) and busi_type=1';  // a.comp_id='+inttostr(compid)+' and
...
procedure Tsetagentaccount2.FormCreate(Sender: TObject);
begin
N2.Visible:=False;
...
procedure Tsetagentaccount2.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetagentaccount2.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetagentaccount2.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetagentaccount2.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetagentaccount2.SpeedButton2Click(Sender: TObject);
begin
...
procedure Tsetagentaccount2.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetagentaccount2.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetagentaccount2.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetagentaccount2.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
...
procedure Tsetagentaccount2.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetagentaccount2.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetagentaccount2.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetagentaccount2.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetagentaccount2.dxDBGrid1CustomDrawCell(Sender: TObject;
...
begin
    if (ANode.Values[dxdbgrid1cbod_status.Index] ='制单') or (ANode.Values[dxdbgrid1cbod_status.Index] ='财务复核') then Acolor := $00A3FEFC;
...
procedure Tsetagentaccount2.N1Click(Sender: TObject);
...
begin
    ...
    begin
        if active=true then close;
        commandtext:='update tb_bill set bod_status_id=4,receipt_by='+inttostr(curuserid)+',receipt_dt=getdate() where bod_id in ('+t+')';
        execute;
        SpeedButton5Click(nil);
...
procedure Tsetagentaccount2.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetagentaccount2.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetagentaccount2.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
{
with cdsbill_dtl do
begin
...
procedure Tsetagentaccount2.SpeedButton6Click(Sender: TObject);
begin
