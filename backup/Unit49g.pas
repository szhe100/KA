...
type
  Tsetallotstopapply3 = class(TForm)
...
var
  setallotstopapply3: Tsetallotstopapply3;
...
procedure Tsetallotstopapply3.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetallotstopapply3.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetallotstopapply3.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsetallotstopapply3.FormCloseQuery(Sender: TObject;
...
procedure Tsetallotstopapply3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetallotstopapply3.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetallotstopapply3.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with cdsbill_dtl do
...
procedure Tsetallotstopapply3.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetallotstopapply3.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=25 and a.bod_status_id in (1,4)';   // a.comp_id='+inttostr(compid)+' and
...
procedure Tsetallotstopapply3.FormCreate(Sender: TObject);
begin
...
procedure Tsetallotstopapply3.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetallotstopapply3.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetallotstopapply3.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetallotstopapply3.SpeedButton2Click(Sender: TObject);
begin
...
begin
    if active=true then close;
    commandtext:='sp_updatestock22 22,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsetallotstopapply3.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetallotstopapply3.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetallotstopapply3.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetallotstopapply3.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetallotstopapply3.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetallotstopapply3.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetallotstopapply3.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetallotstopapply3.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetallotstopapply3.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetallotstopapply3.N1Click(Sender: TObject);
...
procedure Tsetallotstopapply3.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetallotstopapply3.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetallotstopapply3.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetallotstopapply3.brokermedtlCalcFields(DataSet: TDataSet);
begin
with brokermedtl do
begin
...
procedure Tsetallotstopapply3.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetallotstopapply3.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsetallotstopapply3.SpeedButton3Click(Sender: TObject);
begin
