object setreport21: Tsetreport21
  Left = 149
  Top = 117
  AutoScroll = False
  Caption = #20445#35777#37329#24405#20837
  ClientHeight = 551
  ClientWidth = 1007
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  DesignSize = (
    1007
    551)
  PixelsPerInch = 96
  TextHeight = 12
  object Shape9: TShape
    Left = 259
    Top = 56
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Shape5: TShape
    Left = 259
    Top = 77
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Shape11: TShape
    Left = 848
    Top = 0
    Width = 60
    Height = 22
    Anchors = [akTop, akRight]
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Shape1: TShape
    Left = 1
    Top = 78
    Width = 50
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object SpeedButton2: TSpeedButton
    Left = 251
    Top = 0
    Width = 25
    Height = 25
    Hint = #36716#20986#25253#34920
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton2Click
  end
  object Label1: TLabel
    Left = 15
    Top = 84
    Width = 24
    Height = 12
    Caption = #21697#31181
    Transparent = True
  end
  object SpeedButton3: TSpeedButton
    Left = 836
    Top = 74
    Width = 152
    Height = 25
    Hint = #21047#26032
    Anchors = [akTop, akRight]
    Flat = True
    Glyph.Data = {
      
      3393333777333777337333333999993333333333377777333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton3Click
  end
  object Shape2: TShape
    Left = 259
    Top = 35
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label3: TLabel
    Left = 273
    Top = 40
    Width = 36
    Height = 12
    Caption = #22320'  '#21306
    Transparent = True
  end
  object Label19: TLabel
    Left = 177
    Top = 38
    Width = 24
    Height = 12
    Caption = #21253#21547
  end
  object SpeedButton5: TSpeedButton
    Left = 423
    Top = 75
    Width = 25
    Height = 25
    Hint = #25353#35774#32622#25968#25454#20462#25913#24403#21069#35760#24405
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3055577777555557F7F555000555555999555577755555577755}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    OnClick = SpeedButton5Click
  end
  object Shape3: TShape
    Left = 544
    Top = 78
    Width = 45
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
    Visible = False
  end
  object Label2: TLabel
    Left = 554
    Top = 83
    Width = 24
    Height = 12
    Caption = #23458#25143
    Transparent = True
    Visible = False
  end
  object Shape4: TShape
    Left = 1
    Top = 60
    Width = 50
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label4: TLabel
    Left = 3
    Top = 65
    Width = 48
    Height = 12
    Caption = #21830#19994#20844#21496
    Transparent = True
  end
  object Label5: TLabel
    Left = 273
    Top = 83
    Width = 60
    Height = 12
    Caption = #19994#21153#36127#36131#20154
    Transparent = True
  end
  object SpeedButton6: TSpeedButton
    Left = 622
    Top = 31
    Width = 110
    Height = 25
    Hint = 
      #20174#31532'2'#34892#24320#22987#23548#20837#65292#23548#20837#26684#24335': '#24180#24230','#24402#23646','#31614#35746#26085#26399','#20107#19994#37096','#21306#22495#32463#29702','#30465#20221','#22478#24066','#19994#21153#21592','#23458#25143','#30446#26631#21307#38498','#33647#21697#32534#30721','#13#20379#36135#20215','#25512 +
      #24191#26381#21153#36153',1'#26376',2'#26376',3'#26376',4'#26376',5'#26376',6'#26376',7'#26376',8'#26376',9'#26376',10'#26376',11'#26376',12'#26376','#13#39318#25209#25552#36135#37327','#30446#26631#38144#37327','#39044#35745#24320#21457#26376#20221','#36215 +
      #22987#26085#26399','#32467#26463#26085#26399','#20854#20182#32422#23450','#20445#35777#37329
    Caption = #23548#20837'&Excel'
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Visible = False
    OnClick = SpeedButton6Click
  end
  object Label15: TLabel
    Left = 836
    Top = 56
    Width = 138
    Height = 12
    Anchors = [akTop, akRight]
    Caption = #27880':'#26412#26597#35810#21463#25480#26435#22320#21306#38480#21046
    Font.Charset = GB2312_CHARSET
    Font.Color = clBlack
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
  end
  object Label16: TLabel
    Left = 854
    Top = 5
    Width = 48
    Height = 12
    Anchors = [akTop, akRight]
    Caption = #35774#32622#26085#26399
    Font.Charset = GB2312_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label17: TLabel
    Left = 885
    Top = 30
    Width = 12
    Height = 12
    Anchors = [akTop, akRight]
    Caption = #33267
  end
  object Label10: TLabel
    Left = 259
    Top = 61
    Width = 60
    Height = 12
    Caption = #21830#19994#19994#21153#21592
    Transparent = True
  end
  object dxButtonEdit1: TdxButtonEdit
    Left = 54
    Top = 80
    Width = 200
    Font.Charset = GB2312_CHARSET
    Font.Color = clNavy
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 0
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit1ButtonClick
    ExistButtons = True
  end
  object dxLookupTreeView1: TdxLookupTreeView
    Left = 319
    Top = 36
    Width = 100
    Height = 20
    CanSelectParents = True
    DropDownRows = 30
    ParentColor = False
    TabOrder = 1
    TabStop = True
    TreeViewColor = clWindow
    TreeViewCursor = crDefault
    TreeViewFont.Charset = DEFAULT_CHARSET
    TreeViewFont.Color = clWindowText
    TreeViewFont.Height = -11
    TreeViewFont.Name = 'MS Sans Serif'
    TreeViewFont.Style = []
    TreeViewImages = ImageList
    TreeViewIndent = 19
    TreeViewReadOnly = True
    TreeViewShowButtons = True
    TreeViewShowHint = False
    TreeViewShowLines = True
    TreeViewShowRoot = True
    TreeViewSortType = stNone
    OnCloseUp = dxLookupTreeView1CloseUp
    OnKeyDown = dxLookupTreeView1KeyDown
    DividedChar = '.'
    ListSource = DM.DSdistrict
    KeyField = 'rec_id'
    ListField = 'zdesc'
    Options = [trCanDBNavigate, trSmartRecordCopy, trCheckHasChildren]
    ParentField = 'parent'
    TextStyle = tvtsShort
    Alignment = taLeftJustify
  end
  object RadioGroup1: TRadioGroup
    Left = 1
    Top = 25
    Width = 175
    Height = 33
    Columns = 4
    ItemIndex = 0
    Items.Strings = (
      #31616#30721
      #21697#21517
      #21378#23478
      #32534#30721)
    TabOrder = 2
  end
  object dxEdit1: TdxEdit
    Left = 198
    Top = 34
    Width = 56
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = bts3D
    Style.ButtonTransparence = ebtNone
    Style.HotTrack = False
    Style.Shadow = False
    TabOrder = 3
    OnKeyDown = dxEdit1KeyDown
  end
  object dxCheckEdit1: TdxCheckEdit
    Left = 456
    Top = 79
    Width = 97
    Font.Charset = GB2312_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.Edges = []
    TabOrder = 4
    Anchors = [akTop, akRight]
    Caption = #25353#27492#26465#20214#26597#35810
  end
  object Edit1: TEdit
    Left = 499
    Top = 2
    Width = 165
    Height = 20
    TabOrder = 5
    Text = 'Edit1'
    Visible = False
  end
  object wwDBNavigator1: TwwDBNavigator
    Left = 1
    Top = 0
    Width = 250
    Height = 25
    DataSource = DSqry
    Options = []
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    ParentShowHint = False
    object wwDBNavigator1First: TwwNavButton
      Left = 0
      Top = 0
      Width = 25
      Height = 25
      Hint = #39318#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 0
      Style = nbsFirst
    end
    object wwDBNavigator1Prior: TwwNavButton
      Left = 25
      Top = 0
      Width = 25
      Height = 25
      Hint = #21069#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Prior'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 1
      Style = nbsPrior
    end
    object wwDBNavigator1Next: TwwNavButton
      Left = 50
      Top = 0
      Width = 25
      Height = 25
      Hint = #21518#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Next'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 2
      Style = nbsNext
    end
    object wwDBNavigator1Last: TwwNavButton
      Left = 75
      Top = 0
      Width = 25
      Height = 25
      Hint = #23614#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 3
      Style = nbsLast
    end
    object wwDBNavigator1Insert: TwwNavButton
      Left = 100
      Top = 0
      Width = 25
      Height = 25
      Hint = #26032#22686
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Insert'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 4
      Style = nbsInsert
    end
    object wwDBNavigator1Delete: TwwNavButton
      Left = 125
      Top = 0
      Width = 25
      Height = 25
      Hint = #21024#38500
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Delete'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 5
      Style = nbsDelete
    end
    object wwDBNavigator1Post: TwwNavButton
      Left = 150
      Top = 0
      Width = 25
      Height = 25
      Hint = #23384#20837
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Post'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      OnClick = wwDBNavigator1PostClick
      Index = 6
      Style = nbsPost
    end
    object wwDBNavigator1Cancel: TwwNavButton
      Left = 175
      Top = 0
      Width = 25
      Height = 25
      Hint = #21462#28040
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Cancel'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      OnClick = wwDBNavigator1CancelClick
      Index = 7
      Style = nbsCancel
    end
    object wwDBNavigator1SaveBookmark: TwwNavButton
      Left = 200
      Top = 0
      Width = 25
      Height = 25
      Hint = #35774#32622#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1SaveBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 8
      Style = nbsSaveBookmark
    end
    object wwDBNavigator1RestoreBookmark: TwwNavButton
      Left = 225
      Top = 0
      Width = 25
      Height = 25
      Hint = #36820#22238#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1RestoreBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 9
      Style = nbsRestoreBookmark
    end
  end
  object dxButtonEdit2: TdxButtonEdit
    Left = 593
    Top = 80
    Width = 200
    Font.Charset = GB2312_CHARSET
    Font.Color = clNavy
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 7
    Visible = False
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit2ButtonClick
    ExistButtons = True
  end
  object dxButtonEdit3: TdxButtonEdit
    Left = 54
    Top = 60
    Width = 200
    Font.Charset = GB2312_CHARSET
    Font.Color = clNavy
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 8
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit3ButtonClick
    ExistButtons = True
  end
  object dxButtonEdit4: TdxButtonEdit
    Left = 319
    Top = 79
    Width = 100
    Font.Charset = GB2312_CHARSET
    Font.Color = clNavy
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 9
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit4ButtonClick
    ExistButtons = True
  end
  object Button1: TButton
    Left = 515
    Top = 38
    Width = 75
    Height = 25
    Caption = 'Button1'
    TabOrder = 10
    Visible = False
    OnClick = Button1Click
  end
  object Panel1: TPanel
    Left = 0
    Top = 103
    Width = 1007
    Height = 448
    Align = alBottom
    Anchors = [akLeft, akTop, akRight, akBottom]
    BevelOuter = bvNone
    Caption = 'Panel1'
    TabOrder = 11
    object Splitter1: TSplitter
      Left = 0
      Top = 326
      Width = 1007
      Height = 2
      Cursor = crVSplit
      Align = alBottom
    end
    object dxDBGrid1: TdxDBGrid
      Left = 0
      Top = 0
      Width = 1007
      Height = 326
      Bands = <
        item
        end>
      DefaultLayout = True
      HeaderPanelRowCount = 1
      KeyField = 'rec_id'
      ShowGroupPanel = True
      ShowSummaryFooter = True
      SummaryGroups = <>
      SummarySeparator = ', '
      Align = alClient
      PopupMenu = PopupMenu1
      TabOrder = 0
      OnKeyDown = dxDBGrid1KeyDown
      DataSource = DSqry
      Filter.Active = True
      Filter.DropDownCount = 30
      Filter.Criteria = {00000000}
      HideSelectionColor = clAqua
      HideSelectionTextColor = clBlack
      HighlightColor = clAqua
      HighlightTextColor = clBlack
      LookAndFeel = lfUltraFlat
      OptionsBehavior = [edgoAutoSort, edgoEditing, edgoEnterShowEditor, edgoEnterThrough, edgoImmediateEditor, edgoMultiSelect, edgoShowHourGlass, edgoTabs, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanNavigation, edgoLoadAllRecords, edgoUseBookmarks]
      OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
      OnCustomDrawCell = dxDBGrid1CustomDrawCell
      object dxDBGrid1rec_id: TdxDBGridColumn
        Alignment = taCenter
        Caption = #24207#21495
        DisableEditor = True
        Width = 40
        BandIndex = 0
        RowIndex = 0
        FieldName = 'rec_id'
        SummaryFooterType = cstCount
        SummaryFooterField = 'rec_id'
        DisableFilter = True
        SummaryType = cstCount
        SummaryField = 'rec_id'
      end
      object dxDBGrid1flow_cd: TdxDBGridColumn
        Color = 13303293
        Width = 70
        BandIndex = 0
        RowIndex = 0
        FieldName = 'flow_cd'
      end
      object dxDBGrid1Ltype: TdxDBGridLookupColumn
        Alignment = taCenter
        Color = 13303293
        Width = 46
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Ltype'
      end
      object dxDBGrid1Lcusttype: TdxDBGridLookupColumn
        Alignment = taCenter
        Color = 13303293
        Width = 69
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Lcusttype'
      end
      object dxDBGrid1Lbusitype: TdxDBGridLookupColumn
        Color = 13303293
        Width = 69
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Lbusitype'
      end
      object dxDBGrid1dist1: TdxDBGridColumn
        DisableEditor = True
        Width = 50
        BandIndex = 0
        RowIndex = 0
        FieldName = 'dist1'
      end
      object dxDBGrid1dist2: TdxDBGridColumn
        DisableEditor = True
        Width = 50
        BandIndex = 0
        RowIndex = 0
        FieldName = 'dist2'
      end
      object dxDBGrid1mate_name: TdxDBGridColumn
        Caption = #21830#19994#20844#21496
        DisableEditor = True
        Font.Charset = GB2312_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = #23435#20307
        Font.Style = []
        Width = 100
        BandIndex = 0
        RowIndex = 0
        FieldName = 'mate_name'
      end
      object dxDBGrid1agent: TdxDBGridColumn
        DisableEditor = True
        Width = 81
        BandIndex = 0
        RowIndex = 0
        FieldName = 'agent'
      end
      object dxDBGrid1remark: TdxDBGridColumn
        Color = 14155775
        Width = 80
        BandIndex = 0
        RowIndex = 0
        FieldName = 'remark'
      end
      object dxDBGrid1broker: TdxDBGridColumn
        DisableEditor = True
        Width = 58
        BandIndex = 0
        RowIndex = 0
        FieldName = 'broker'
      end
      object dxDBGrid1payer: TdxDBGridColumn
        Color = 13303293
        Width = 58
        BandIndex = 0
        RowIndex = 0
        FieldName = 'payer'
      end
      object dxDBGrid1Cchecked: TdxDBGridCheckColumn
        DisableEditor = True
        Width = 57
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Cchecked'
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object dxDBGrid1med_code: TdxDBGridColumn
        Alignment = taCenter
        DisableEditor = True
        Font.Charset = GB2312_CHARSET
        Font.Color = clRed
        Font.Height = -12
        Font.Name = #23435#20307
        Font.Style = []
        TabStop = False
        Width = 50
        BandIndex = 0
        RowIndex = 0
        FieldName = 'med_code'
      end
      object dxDBGrid1med_name: TdxDBGridMaskColumn
        DisableEditor = True
        Font.Charset = GB2312_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = #23435#20307
        Font.Style = []
        TabStop = False
        Width = 100
        BandIndex = 0
        RowIndex = 0
        FieldName = 'med_name'
      end
      object dxDBGrid1specifi: TdxDBGridMaskColumn
        DisableEditor = True
        TabStop = False
        Width = 80
        BandIndex = 0
        RowIndex = 0
        FieldName = 'specifi'
        DisableFilter = True
      end
      object dxDBGrid1occur_dt: TdxDBGridDateColumn
        Alignment = taCenter
        Color = 13303293
        TabStop = False
        Width = 72
        BandIndex = 0
        RowIndex = 0
        FieldName = 'occur_dt'
      end
      object dxDBGrid1Linsuretype: TdxDBGridLookupColumn
        Alignment = taCenter
        Color = 13303293
        Width = 81
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Linsuretype'
      end
      object dxDBGrid1insure_amot: TdxDBGridColumn
        Caption = #20445#35777#37329#37329#39069
        Color = 13303293
        Width = 66
        BandIndex = 0
        RowIndex = 0
        FieldName = 'insure_amot'
        DisableFilter = True
      end
      object dxDBGrid1freceipt: TdxDBGridCheckColumn
        Color = 13303293
        Width = 93
        BandIndex = 0
        RowIndex = 0
        FieldName = 'freceipt'
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object dxDBGrid1receipt_cd: TdxDBGridColumn
        Color = 13303293
        Width = 80
        BandIndex = 0
        RowIndex = 0
        FieldName = 'receipt_cd'
        DisableFilter = True
      end
      object dxDBGrid1creater: TdxDBGridMaskColumn
        DisableEditor = True
        TabStop = False
        Width = 45
        BandIndex = 0
        RowIndex = 0
        FieldName = 'creater'
        DisableFilter = True
      end
      object dxDBGrid1creat_dt: TdxDBGridDateColumn
        DisableEditor = True
        TabStop = False
        Width = 65
        BandIndex = 0
        RowIndex = 0
        FieldName = 'creat_dt'
        DisableFilter = True
      end
      object dxDBGrid1checker: TdxDBGridColumn
        DisableEditor = True
        Width = 69
        BandIndex = 0
        RowIndex = 0
        FieldName = 'checker'
      end
      object dxDBGrid1check_dt: TdxDBGridColumn
        DisableEditor = True
        Width = 90
        BandIndex = 0
        RowIndex = 0
        FieldName = 'check_dt'
        DisableFilter = True
      end
    end
    object dxDBGrid2: TdxDBGrid
      Left = 0
      Top = 328
      Width = 1007
      Height = 120
      Bands = <
        item
        end>
      DefaultLayout = True
      HeaderPanelRowCount = 1
      KeyField = 'dtl_id'
      ShowSummaryFooter = True
      SummaryGroups = <>
      SummarySeparator = ', '
      Align = alBottom
      PopupMenu = PopupMenu2
      TabOrder = 1
      OnEnter = dxDBGrid2Enter
      DataSource = DSqrydtl
      Filter.Active = True
      Filter.DropDownCount = 30
      Filter.Criteria = {00000000}
      HideSelectionColor = clAqua
      HideSelectionTextColor = clBlack
      HighlightColor = clAqua
      HighlightTextColor = clBlack
      LookAndFeel = lfUltraFlat
      OptionsBehavior = [edgoAutoSort, edgoEditing, edgoEnterShowEditor, edgoEnterThrough, edgoImmediateEditor, edgoShowHourGlass, edgoTabs, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCanAppend, edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoLoadAllRecords, edgoResetColumnFocus, edgoUseBookmarks]
      OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
      object dxDBGrid2mate_name: TdxDBGridButtonColumn
        Color = 13303293
        Font.Charset = GB2312_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = #23435#20307
        Font.Style = []
        Width = 280
        BandIndex = 0
        RowIndex = 0
        FieldName = 'mate_name'
        SummaryFooterType = cstCount
        SummaryFooterField = 'mate_id1'
        SummaryFooterFormat = '###,##0'#23478';; '
        Buttons = <
          item
            Default = True
          end>
        OnButtonClick = dxDBGrid2mate_nameButtonClick
        DisableFilter = True
        SummaryType = cstCount
        SummaryField = 'mate_id1'
        SummaryFormat = '###,##0'#23478';; '
      end
      object dxDBGrid2insure_amot: TdxDBGridColumn
        Color = 13303293
        Width = 67
        BandIndex = 0
        RowIndex = 0
        FieldName = 'insure_amot'
        SummaryFooterType = cstSum
        SummaryFooterField = 'qty2'
        SummaryFooterFormat = '###,##0;; '
        DisableFilter = True
        SummaryType = cstSum
        SummaryField = 'qty2'
        SummaryFormat = '###,##0;; '
      end
      object dxDBGrid2zdesc: TdxDBGridColumn
        Color = 13303293
        Width = 200
        BandIndex = 0
        RowIndex = 0
        FieldName = 'zdesc'
      end
      object dxDBGrid2creater: TdxDBGridMaskColumn
        DisableEditor = True
        TabStop = False
        Width = 45
        BandIndex = 0
        RowIndex = 0
        FieldName = 'creater'
        DisableFilter = True
      end
      object dxDBGrid2creat_dt: TdxDBGridDateColumn
        Caption = #35774#32622#26102#38388
        DisableEditor = True
        TabStop = False
        Width = 120
        BandIndex = 0
        RowIndex = 0
        FieldName = 'creat_dt'
        DisableFilter = True
      end
    end
  end
  object dxDateEdit3: TdxDateEdit
    Left = 909
    Top = 1
    Width = 80
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 12
    Anchors = [akTop, akRight]
    Date = -700000.000000000000000000
  end
  object dxDateEdit4: TdxDateEdit
    Left = 909
    Top = 26
    Width = 80
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 13
    Anchors = [akTop, akRight]
    Date = -700000.000000000000000000
  end
  object Edit2: TEdit
    Left = 693
    Top = 17
    Width = 121
    Height = 20
    TabOrder = 14
    Text = 'Edit2'
    Visible = False
  end
  object dxButtonEdit5: TdxButtonEdit
    Left = 319
    Top = 58
    Width = 100
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 15
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit5ButtonClick
    ExistButtons = True
  end
  object qry: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    AfterInsert = qryAfterInsert
    BeforeEdit = qryBeforeEdit
    BeforePost = qryBeforePost
    AfterPost = qryAfterPost
    BeforeDelete = qryBeforeDelete
    AfterDelete = qryAfterPost
    OnCalcFields = qryCalcFields
    Left = 78
    Top = 180
    object qryrec_id: TAutoIncField
      FieldName = 'rec_id'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      ReadOnly = True
    end
    object qrymed_id: TIntegerField
      FieldName = 'med_id'
    end
    object qrycreat_by: TIntegerField
      FieldName = 'creat_by'
    end
    object qrycreat_dt: TDateTimeField
      DisplayLabel = #35774#32622#26085#26399
      FieldName = 'creat_dt'
    end
    object qrydist1: TStringField
      DisplayLabel = #30465#20221
      FieldName = 'dist1'
      ReadOnly = True
      Size = 100
    end
    object qrydist2: TStringField
      DisplayLabel = #22478#24066
      FieldName = 'dist2'
      ReadOnly = True
      Size = 100
    end
    object qrymate_name: TStringField
      DisplayLabel = #23458#25143
      FieldName = 'mate_name'
      Size = 100
    end
    object qrycomp_id: TWordField
      FieldName = 'comp_id'
    end
    object qrymed_code: TStringField
      DisplayLabel = #32534#30721
      FieldName = 'med_code'
    end
    object qrymed_name: TStringField
      DisplayLabel = #21697#21517
      FieldName = 'med_name'
      Size = 50
    end
    object qryspecifi: TStringField
      DisplayLabel = #35268#26684
      FieldName = 'specifi'
      Size = 50
    end
    object qrypdt_place: TStringField
      DisplayLabel = #29983#20135#20225#19994
      FieldName = 'pdt_place'
      Size = 30
    end
    object qrymed_unit: TStringField
      DisplayLabel = #21333#20301
      FieldName = 'med_unit'
      Size = 30
    end
    object qrymed_type: TStringField
      DisplayLabel = #21058#22411
      FieldName = 'med_type'
      ReadOnly = True
      Size = 10
    end
    object qryflow_cd: TStringField
      DisplayLabel = #27969#31243#32534#21495
      FieldName = 'flow_cd'
      Size = 10
    end
    object qrymate_id: TIntegerField
      FieldName = 'mate_id'
    end
    object qrycreater: TStringField
      DisplayLabel = #35774#32622#20154
      FieldName = 'creater'
    end
    object qrypayer: TStringField
      DisplayLabel = #20184#27454#20154
      FieldName = 'payer'
    end
    object qrybroker_id: TIntegerField
      FieldName = 'agent_id'
    end
    object qryagent: TStringField
      DisplayLabel = #19994#21153#36127#36131#20154
      FieldName = 'agent'
    end
    object qrydistrict_id: TIntegerField
      FieldName = 'district_id'
    end
    object qryinsure_amot: TBCDField
      DisplayLabel = #20445#35777#37329
      DisplayWidth = 16
      FieldName = 'insure_amot'
      DisplayFormat = '###,###,##0.##'
      EditFormat = '########0.##'
      Precision = 15
      Size = 2
    end
    object qrychm_name: TStringField
      FieldName = 'chm_name'
      Size = 50
    end
    object qryqtyperpack: TBCDField
      FieldName = 'qtyperpack'
      Precision = 18
      Size = 0
    end
    object qryqtyperbox: TBCDField
      FieldName = 'qtyperbox'
      Precision = 18
      Size = 0
    end
    object qrymodify_dt: TDateTimeField
      FieldName = 'modify_dt'
    end
    object qrymodify_by: TIntegerField
      FieldName = 'modify_by'
    end
    object qrycheck_by: TIntegerField
      FieldName = 'check_by'
    end
    object qrycheck_dt: TDateTimeField
      DisplayLabel = #23457#26680#26102#38388
      FieldName = 'check_dt'
    end
    object qrychecker: TStringField
      DisplayLabel = #36130#21153#23457#26680
      FieldName = 'checker'
    end
    object qryfreceipt: TBooleanField
      DisplayLabel = #26159#21542#24320#20855#25910#25454
      FieldName = 'freceipt'
    end
    object qryreceipt_cd: TStringField
      DisplayLabel = #25910#25454#32534#30721
      FieldName = 'receipt_cd'
    end
    object qryoccur_dt: TDateTimeField
      DisplayLabel = #25910'/'#36864#27454#26085#26399
      FieldName = 'occur_dt'
    end
    object qrybroker_id2: TIntegerField
      FieldName = 'broker_id'
    end
    object qrybroker: TStringField
      DisplayLabel = #19994#21153#21592
      FieldName = 'broker'
    end
    object qrytype_id: TIntegerField
      FieldName = 'type_id'
    end
    object qryLtype1: TStringField
      DisplayLabel = #31867#22411
      FieldKind = fkLookup
      FieldName = 'Ltype'
      LookupDataSet = type1
      LookupKeyFields = 'obj_id'
      LookupResultField = 'zdesc'
      KeyFields = 'type_id'
      Lookup = True
    end
    object qrycust_type_id: TIntegerField
      FieldName = 'cust_type_id'
    end
    object qryLtype2: TStringField
      DisplayLabel = #23458#25143#31867#22411
      FieldKind = fkLookup
      FieldName = 'Lcusttype'
      LookupDataSet = type2
      LookupKeyFields = 'obj_id'
      LookupResultField = 'zdesc'
      KeyFields = 'cust_type_id'
      Lookup = True
    end
    object qryinsure_type_id: TIntegerField
      FieldName = 'insure_type_id'
    end
    object qryLinsuretype: TStringField
      DisplayLabel = #20445#35777#37329#31867#22411
      FieldKind = fkLookup
      FieldName = 'Linsuretype'
      LookupDataSet = type3
      LookupKeyFields = 'obj_id'
      LookupResultField = 'zdesc'
      KeyFields = 'insure_type_id'
      Lookup = True
    end
    object qrybusi_type_id: TIntegerField
      FieldName = 'busi_type_id'
    end
    object qryLbusitype: TStringField
      DisplayLabel = #19994#21153#31867#21035
      FieldKind = fkLookup
      FieldName = 'Lbusitype'
      LookupDataSet = type4
      LookupKeyFields = 'obj_id'
      LookupResultField = 'zdesc'
      KeyFields = 'busi_type_id'
      Size = 10
      Lookup = True
    end
    object qryremark: TStringField
      DisplayLabel = #22791#27880
      FieldName = 'remark'
      Size = 60
    end
    object qryCchecked: TBooleanField
      DisplayLabel = #24050#23457#26680
      FieldKind = fkCalculated
      FieldName = 'Cchecked'
      Calculated = True
    end
  end
  object DSqry: TDataSource
    DataSet = qry
    OnDataChange = DSqryDataChange
    Left = 106
    Top = 180
  end
  object PopupMenu1: TPopupMenu
    Left = 316
    Top = 220
    object N7: TMenuItem
      Caption = #22797#21046#35760#24405
      OnClick = N7Click
    end
    object N8: TMenuItem
      Caption = #31896#36148#35760#24405
      OnClick = N8Click
    end
    object N9: TMenuItem
      Caption = '-'
    end
    object N10: TMenuItem
      Caption = #36130#21153#23457#26680
      OnClick = N10Click
    end
    object N11: TMenuItem
      Caption = '-'
    end
    object N3: TMenuItem
      Caption = #26597#25214'...'
      OnClick = N3Click
    end
    object N4: TMenuItem
      Caption = '-'
    end
    object N1: TMenuItem
      Caption = #38544#34255#26412#21015
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #26174#31034#25152#26377#21015
      OnClick = N2Click
    end
    object N5: TMenuItem
      Caption = '-'
    end
    object N6: TMenuItem
      Caption = #26684#24335#22797#21407
      OnClick = N6Click
    end
  end
  object ImageList: TImageList
    Left = 483
    Top = 14
    Bitmap = {
      
      000000000000}
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingageYPH;Data Source=.'
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <>
    SQL.Strings = (
      
        'select top 1 signer=dbo.fn_staff_name(sign_by),sign_by from tb_r' +
        'eport01 ')
    Left = 185
    Top = 295
    object ADOQuery1sign_by: TIntegerField
      FieldName = 'sign_by'
    end
    object ADOQuery1signer: TStringField
      FieldName = 'signer'
      ReadOnly = True
      Size = 100
    end
  end
  object type1: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    Left = 205
    Top = 166
    object type1obj_id: TAutoIncField
      FieldName = 'obj_id'
      ReadOnly = True
    end
    object type1zdesc: TStringField
      DisplayLabel = #20837#24211#31867#21035
      FieldName = 'zdesc'
      Size = 30
    end
  end
  object type2: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    Left = 205
    Top = 194
    object AutoIncField1: TAutoIncField
      FieldName = 'obj_id'
      ReadOnly = True
    end
    object StringField1: TStringField
      DisplayLabel = #20837#24211#31867#21035
      FieldName = 'zdesc'
      Size = 30
    end
  end
  object broker: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    Left = 425
    Top = 19
    object brokersta_id: TAutoIncField
      DisplayLabel = #32534#21495
      FieldName = 'sta_id'
      ReadOnly = True
    end
    object brokerzname: TStringField
      DisplayLabel = #22995#21517
      FieldName = 'zname'
    end
  end
  object DSbroker: TDataSource
    DataSet = broker
    Left = 453
    Top = 19
  end
  object DSqrydtl: TDataSource
    DataSet = qrydtl
    OnStateChange = DSqrydtlStateChange
    Left = 110
    Top = 465
  end
  object qrydtl: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    BeforeEdit = qryBeforeEdit
    BeforePost = qrydtlBeforePost
    AfterPost = qrydtlAfterPost
    BeforeDelete = qrydtlBeforeDelete
    AfterDelete = qrydtlAfterPost
    Left = 81
    Top = 464
    object qrydtldtl_id: TAutoIncField
      FieldName = 'dtl_id'
      ReadOnly = True
    end
    object qrydtlrec_id: TAutoIncField
      FieldName = 'rec_id'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object qrydtlmate_id1: TIntegerField
      FieldName = 'mate_id'
    end
    object qrydtlmate_name1: TStringField
      DisplayLabel = #21307#38498#21517#31216
      FieldName = 'mate_name'
      Size = 100
    end
    object qrydtlinsure_amot: TBCDField
      DisplayLabel = #20445#35777#37329#37329#39069
      DisplayWidth = 16
      FieldName = 'insure_amot'
      DisplayFormat = '###,###,##0.##'
      EditFormat = '########0.##'
      Precision = 15
      Size = 2
    end
    object qrydtlzdesc: TStringField
      DisplayLabel = #22791#27880
      FieldName = 'zdesc'
      Size = 50
    end
    object qrydtlcreat_by: TIntegerField
      FieldName = 'creat_by'
    end
    object qrydtlcreat_dt: TDateTimeField
      DisplayLabel = #35774#32622#26085#26399
      FieldName = 'creat_dt'
    end
    object qrydtlcreater: TStringField
      DisplayLabel = #35774#32622#20154
      FieldName = 'creater'
    end
    object qrydtlmodify_by: TIntegerField
      FieldName = 'modify_by'
    end
    object qrydtlmodify_dt: TDateTimeField
      FieldName = 'modify_dt'
    end
  end
  object PopupMenu2: TPopupMenu
    Left = 449
    Top = 472
    object MenuItem9: TMenuItem
      Caption = #26684#24335#22797#21407
      OnClick = MenuItem9Click
    end
  end
  object level2: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    Left = 733
    Top = 58
    object level2obj_id: TAutoIncField
      FieldName = 'obj_id'
      ReadOnly = True
    end
    object level2zdesc: TStringField
      DisplayLabel = #20107#19994#37096
      DisplayWidth = 50
      FieldName = 'zdesc'
      Size = 50
    end
  end
  object DSlevel2: TDataSource
    DataSet = level2
    Left = 762
    Top = 58
  end
  object ADOQuery2: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingageYPH;Data Source=.'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select top 1 type_id3,task_amot,flow_cd from tb_report01')
    Left = 496
    Top = 205
    object ADOQuery2type_id3: TWordField
      FieldName = 'type_id3'
    end
    object ADOQuery2task_amot: TBCDField
      FieldName = 'task_amot'
      Precision = 15
      Size = 2
    end
    object ADOQuery2flow_cd: TStringField
      FieldName = 'flow_cd'
      Size = 10
    end
  end
  object type3: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    Left = 205
    Top = 224
    object AutoIncField2: TAutoIncField
      FieldName = 'obj_id'
      ReadOnly = True
    end
    object StringField2: TStringField
      DisplayLabel = #20837#24211#31867#21035
      FieldName = 'zdesc'
      Size = 30
    end
  end
  object type4: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    Left = 205
    Top = 255
    object AutoIncField3: TAutoIncField
      FieldName = 'obj_id'
      ReadOnly = True
    end
    object StringField3: TStringField
      DisplayLabel = #20837#24211#31867#21035
      FieldName = 'zdesc'
      Size = 30
    end
  end
end
