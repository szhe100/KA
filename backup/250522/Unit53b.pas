...
procedure Tsetruleapply2.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetruleapply2.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetruleapply2.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsetruleapply2.FormCloseQuery(Sender: TObject;
...
procedure Tsetruleapply2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetruleapply2.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetruleapply2.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with cdsbill_dtl do
...
procedure Tsetruleapply2.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetruleapply2.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.comp_id='+inttostr(compid)+' and a.bod_type_id=20 and a.bod_status_id in (1,3,4)';
...
procedure Tsetruleapply2.FormCreate(Sender: TObject);
begin
...
procedure Tsetruleapply2.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetruleapply2.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetruleapply2.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetruleapply2.SpeedButton2Click(Sender: TObject);
begin
...
procedure Tsetruleapply2.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetruleapply2.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetruleapply2.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetruleapply2.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetruleapply2.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetruleapply2.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetruleapply2.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetruleapply2.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetruleapply2.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetruleapply2.N1Click(Sender: TObject);
...
procedure Tsetruleapply2.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetruleapply2.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetruleapply2.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetruleapply2.brokermedtlCalcFields(DataSet: TDataSet);
begin
with brokermedtl do
begin
...
procedure Tsetruleapply2.SpeedButton6Click(Sender: TObject);
begin
