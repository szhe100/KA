...
function InternalFindShowForm1(FormClass: TFormClass;
  const Caption: string; Restore: Boolean; Formtag: Integer): TForm;
function FindShowForm1(FormClass: TFormClass; const Caption: string; Formtag: Integer): TForm;
...
function InternalFindShowForm(FormClass: TFormClass;
  const Caption: string; Restore: Boolean): TForm; //; Formtag: Integer
var
  I: Integer;
begin
  Result := nil;
...
begin
  ...
  begin
    ...
    if Caption <> '' then Result.Caption := Caption;
    Result.tag:=Formtag;
//    application.ShowException(Result.tag);//  .showmessage(inttostr(Result.tag));
  end;
  with Result do
  begin
    if Restore and (WindowState = wsMinimized) then WindowState := wsNormal;
///使form在背景区域居中    
    if WindowState = wsNormal then SetBounds((screen.Width-Width) div 2 -5,(screen.Height-Height) div 2 -75,Width,Height);
///////////////////////////
    Show;
  end;
end;
...
function FindShowForm1(FormClass: TFormClass; const Caption: string; Formtag: Integer): TForm;
begin
  Result := InternalFindShowForm1(FormClass, Caption, True, Formtag);
end;
