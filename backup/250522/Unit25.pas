...
type
  Tsetcheckstock = class(TForm)
    ...
    Label1: TLabel;
    dxDBGrid2: TdxDBGrid;
    dxDBGrid2material_code1: TdxDBGridMaskColumn;
...
begin
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active then close;
                commandtext:='select a.*,b.med_name,b.material_code1,b.specifi,b.pdt_place,b.med_code,b.qtyperpack,med_unit=c.zdesc';
                commandtext:=commandtext+' ,a.channel_id,channel=dbo.fn_obj_desc(a.channel_id),a.channel_dtl_id,channel_dtl=dbo.fn_obj_desc(a.channel_dtl_id)';
...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1发生日期, 2业务类别, 3商业公司, 4单据摘要, 5商务代表, 6定价参考物流编码, 7批号, 8数量, 9单价, 10渠道编码, 11子渠道编码
//    sbodcd:='导入数据已生成下列单据'+#13#10+'---------------------------';
    ...
    XL.Quit;
//从第2行开始导入，格式: 1发生日期, 2业务类别, 3商业公司, 4单据摘要, 5商务代表, 6定价参考物流编码, 7批号, 8数量, 9单价, 10渠道, 11子渠道
    sql1:=sql;  //导入数据无错误时，用sql1 插入数据
    sql:=sql+'select top 5 * from ( select top 5 info=''第''+line_no+''行无发生日期或数据无效'' from @tab where f1='''' or try_cast(f1 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无业务类别或数据无效'' from @tab where f2='''' or (f2<>''商业'' and f2<>''分销'')'; //商业  分销
    sql:=sql+' union all select top 5 ''第''+line_no+''行无商业公司或数据无效'' from @tab where f3='''' or not exists (select 1 from tb_busimate where mate_type_id=2 and f3=mate_name)';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无单据摘要'' from @tab where f4=''''';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无商务代表或数据无效'' from @tab where f5='''' or not exists (select 1 from tb_staff where sta_type_id=0 and broker=1 and f5=zname)';//商务代表
    sql:=sql+' union all select top 5 ''第''+line_no+''行无定价参考物流编码或数据无效'' from @tab where f6='''' or not exists (select 1 from tb_medicine where f6=material_code1)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无数量或数据无效'' from @tab where f8='''' or try_cast(f8 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无单价或数据无效'' from @tab where f9='''' or try_cast(f9 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无渠道或数据无效'' from @tab a where f10='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.zdesc=a.f10)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无子渠道或数据无效'' from @tab a where f11='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.zdesc=a.f11)';
    sql:=sql+' ) a';
//从第2行开始导入，格式: 1发生日期, 2业务类别, 3商业公司, 4单据摘要, 5商务代表, 6定价参考物流编码, 7批号, 8数量, 9单价, 10渠道, 11子渠道
...
select @bodcd,@bcd
        ...
        sql:=sql+'  select top 1 @lineno=line_no from @tab order by f1,f2,f3,f5';
        sql:=sql+'  insert into tb_bill_dtl (bod_id,med_id,bat_cd,price,qty,amot,channel_id,channel_dtl_id)';
        sql:=sql+'  select @bodid,m.med_id,bat_cd=a.f7,price=cast(a.f9 as decimal(15,4)),qty=cast(a.f8 as decimal(15,4)),amot=round(cast(a.f8 as decimal(15,4))*cast(a.f9 as decimal(15,4)),2),c.obj_id,d.obj_id';
        sql:=sql+'      from @tab a inner join (select top 1 * from @tab b where line_no=@lineno) b on a.f1=b.f1 and a.f2=b.f2 and a.f3=b.f3 and a.f5=b.f5';
        sql:=sql+'      inner join tb_medicine m on a.f6=m.material_code1';
        ...
        sql:=sql+'  delete from @tab where line_no in (select b.line_no from @tab b,(select top 1 * from @tab where line_no=@lineno) a where a.f1=b.f1 and a.f2=b.f2 and a.f3=b.f3 and a.f5=b.f5)';
...
edit1.text:=commandtext;
        try
            ...
            begin
                ...
                //s:=fields[1].asstring;
                s:='已成功导入下列盘点单'+#13#10+'-------------------------------'+#13#10+fields[1].asstring+#13#10+'-------------------------------'+#13#10+'合计:'+fields[0].asstring+'单';
            end;
        finally
            setprogress(0);
        end;
