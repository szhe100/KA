...
procedure Tsetmedexport3.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetmedexport3.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetmedexport3.FormActivate(Sender: TObject);
begin
{
with dm.broker do
begin
...
procedure Tsetmedexport3.FormCloseQuery(Sender: TObject;
...
procedure Tsetmedexport3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetmedexport3.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedexport3.dxDBGrid2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
...
procedure Tsetmedexport3.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
exit;
...
procedure Tsetmedexport3.dxDBGrid2Enter(Sender: TObject);
begin
...
procedure Tsetmedexport3.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetmedexport3.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetmedexport3.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetmedexport3.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' agent=j.zname,level1=g.zdesc,level2=h.zdesc,level3=i.zdesc';
...
procedure Tsetmedexport3.FormCreate(Sender: TObject);
begin
...
procedure Tsetmedexport3.SpeedButton6Click(Sender: TObject);
var mid : integer;
begin
...
procedure Tsetmedexport3.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetmedexport3.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
//        wwRadioGroup2.enabled:=(active=true) and (recordcount>0) and (fieldbyname('creat_by').asinteger=curuserid);
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active=true then close;
                commandtext:='select a.*,leader=dbo.fn_staff_name(k.sta_id),broker=d.zname,b.med_name,b.specifi,b.pdt_place,b.med_code,b.site_code,b.qtyperpack,b.qtyperbox,med_unit=c.zdesc ';
...
//                commandtext:=commandtext+' left join tb_staff e on a.lead_id =e.sta_id';
                commandtext:=commandtext+' inner join tb_bill f on a.bod_id =f.bod_id';
                commandtext:=commandtext+' left join tb_busimate g on f.dst_id=g.mate_id';
                commandtext:=commandtext+' left join tb_busiframe k on k.rec_id=dbo.fn_getbusiframerecid5(g.district,a.med_id,f.dst_id,f.carry_dt)';
...
procedure Tsetmedexport3.Edit1KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
    fwid: boolean; //  东方语言 宽字符 (2bytes)
...
procedure Tsetmedexport3.Edit1Click(Sender: TObject);
begin
(sender as TEdit).selectall;
//edit1.selectall;
end;

procedure Tsetmedexport3.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
procedure Tsetmedexport3.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedexport3.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedexport3.SpeedButton2Click(Sender: TObject);
var s: string;
begin
with cdsbill do
begin
...
//edit3.text:=commandtext;
    ...
    begin
        first;
        s:='下列品种尚未设置业务员费用规则(注意生效日期)，请先设置'+ #13#10;
        s:=s+'-------------------------------------------------------';  //+ #13#10
...
//edit3.text:=commandtext;
    ...
    begin
        ...
        begin
            first;
            s:='下列品种尚未设置业务结构(注意生效日期)，请先设置'+ #13#10;
            s:=s+'-----------------------------------------------------';  //+ #13#10
...
select top 1 lead_id,broker_id=sta_id,med_id,price1,
	...
	order by rec_id desc
        ...
        commandtext:='select top 5 m.med_code,m.med_name,m.specifi,m.pdt_place';
        commandtext:=commandtext+' from tb_bill a,tb_bill_dtl b,tb_medicine m';
        commandtext:=commandtext+' left join tb_busimate c on c.mate_id=a.dst_id';
        commandtext:=commandtext+' left join tb_busiframe k on k.rec_id=dbo.fn_getbusiframerecid5(c.district,b.med_id,a.dst_id,a.carry_dt)';
        commandtext:=commandtext+' where a.bod_id='+ cdsbill.fieldbyname('bod_id').asstring;
        commandtext:=commandtext+'  and a.bod_id=b.bod_id and b.med_id=m.med_id';
        commandtext:=commandtext+'  and (isnull(k.sta_id,0)=0 or isnull(b.sta_id,0)=0';
//        commandtext:=commandtext+'  and (isnull(b.lead_id,0)=0 or isnull(b.sta_id,0)=0 ';
        commandtext:=commandtext+'  or not exists (select 1 from tb_brokermed f where a.dst_id=f.mate_id and b.med_id=f.med_id'; //f.comp_id='+inttostr(compid)+' and 
        commandtext:=commandtext+'  and f.valid=1 and valid_dt<=a.carry_dt) )';
edit3.text:=commandtext;        
        try
            setprogress(1);
            open;
        ...
        begin
            first;
            s:='下列品种无当前区域经理或业务员数据(可能尚未设置业务规则或已停用或过期)'+ #13#10; //主管及，本单不可审核
            s:=s+'-----------------------------------------------------------------------';  //+ #13#10
...
procedure Tsetmedexport3.SpeedButton4Click(Sender: TObject);
begin
...
procedure Tsetmedexport3.SpeedButton3Click(Sender: TObject);
var s: string;
begin
with dm.pubqry do
begin
...
procedure Tsetmedexport3.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedexport3.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedexport3.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetmedexport3.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
    then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
...
procedure Tsetmedexport3.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetmedexport3.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedexport3.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetmedexport3.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetmedexport3.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedexport3.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetmedexport3.dxDBGrid2DblClick(Sender: TObject);
var key:char;
begin
exit;
key:=#32;
...
procedure Tsetmedexport3.Edit2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
    fwid: boolean; //  东方语言 宽字符 (2bytes)
...
procedure Tsetmedexport3.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
    SFileName,sbodcd,sb: string;
    J,mi,mid,id1,id2,id3,id4,id5,id6,id10,matetypeid: integer;
    rate: single;
    dt: TDatetime;
    price,price1: real;
begin
...
//导入格式: 1供货单位id1,2购货单位id2,3单据摘要,4发生日期,5单据类别id3,6商业公司药品编码id4,7数量,8批号,9有效期至,10业务类别   //中标价,11
    ...
    begin
        id1:=0;id2:=0;id3:=0;id4:=0;id5:=0;id6:=0; id10:=0; matetypeid:=0;  price:=0;  price1:=0;
        with dm.pubqry do
        begin
            if active=true then close;
            ...
            begin
                if active=true then close;
                commandtext:='select top 1 mate_id,mate_type_id from tb_busimate where mate_type_id in (1,2) and mate_name ='''+Trim(sheet.cells[j,2].text)+'''';
                open;
                if recordcount>0 then
                begin
                ...
                begin
                    ...
                    commandtext:=commandtext+' where a.type_id=2 and a.comp_id=1 and a.mate_id=b.mate_id';
            ...
            if (sheet.cells[j,5].text='销售退货') or (sheet.cells[j,5].text='退货') then id3:= 2;
            if id3=0 then id3:= 1;
            ...
            begin
{
                ...
                begin
                    if active=True then close;
                    commandtext:='select top 1 a.med_id,a.rate from tb_linkword a,tb_medicine b';
...
{
            commandtext:='select top 1 case when a.price4>0 then a.price4 else case when a.price>0 then a.price else case when a.price5>0 then a.price5';
            commandtext:=commandtext+' else case when a.price6>0 then a.price6 else a.price7 end end end end';
            commandtext:=commandtext+' from tb_medata a,tb_busimate b where type_id=1 and a.med_id='+inttostr(id4)+' and b.mate_id='+inttostr(id2); //+' and a.price>0';
            commandtext:=commandtext+' and dbo.fn_treeischild(b.district,a.id1)=1';
}
//edit3.text:=commandtext;
            open;
...
//导入格式: 1供货单位id1,2购货单位id2,3单据摘要,4发生日期,5单据类别id3,6商业公司药品编码id4,7数量,8批号,9有效期至,10中标价,11业务类别
            if (id1=0) or (id2=0) or (id4=0) or (sheet.cells[j,7].text='')
...
//            if (id1=0) or (id2=0) or (id4=0) or ((matetypeid=1) and ((id5=0) or (id6=0))) or (sheet.cells[j,7].text='') or (price1<=0) then  // or (sheet.cells[j,8].text='')
            begin
                ...
                if id1=0 then sbodcd:=sbodcd+',未取得供货单位';
                if id2=0 then sbodcd:=sbodcd+',未取得购货单位';
...
//exit;
//开始导入    
//导入格式: 1供货单位id1,2购货单位id2,3单据摘要,4发生日期,5单据类别id3,6商业公司药品编码id4,7数量,8批号,9有效期至,10中标价,11业务类别 id11
    ...
    begin
        ...
        begin
            ...
            if (sheet.cells[j,5].text='') or (sheet.cells[j,5].text='销售出库') or (sheet.cells[j,5].text='出库') then id3:= 1;
            if (sheet.cells[j,5].text='销售退货') or (sheet.cells[j,5].text='退货') or (strtofloat(cleardeli(sheet.cells[j,7].text))<0) then id3:= 2;
            if id3=0 then id3:= 1;
            if (sheet.cells[j,10].text='分销') then id10:= 2 else id10:= 1;
            if sheet.cells[j,6].text<>'' then
            begin
...
//导入格式: 1供货单位id1,2购货单位id2,3单据摘要,4发生日期,5单据类别id3,6商业公司药品编码id4,7数量,8批号,9有效期至,10中标价,11业务类别 id11
            ...
            if active=true then close;
            commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,cate_id,busi_type,creat_by,creat_dt,src_id,dst_id,bod_desc,carry_dt)';
...
//导入格式: 1供货单位id1,2购货单位id2,3单据摘要,4发生日期,5单据类别id3,6商业公司药品编码id4,7数量,8批号,9有效期至,10中标价,11业务类别
            if active=true then close;
            commandtext:='insert into tb_bill_dtl (bod_id,med_id,price,price2,qty,amot,sta_id,lead_id,bat_cd,valid_dt)';
...
procedure Tsetmedexport3.N1Click(Sender: TObject);
...
{       ///后面取中标价
        ...
        begin
            first;
            s:='下列品种无主管及业务员数据(可能尚未设置业务规则或已停用)'+ #13#10;
            s:=s+'--------------------------------------------------------------';  //+ #13#10
...
procedure Tsetmedexport3.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetmedexport3.N3Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t : string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetmedexport3.dxDBButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedexport3.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetmedexport3.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetmedexport3.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tsetmedexport3.Button1Click(Sender: TObject);
begin
...
procedure Tsetmedexport3.N5Click(Sender: TObject);
begin
if pubqry.active=true then pubqry.close;
pubqry.commandtext:='select bod_id from tb_bill where bod_type_id=2 and isnull(mate_id1,0)=0 and isnull(mate_id2,0)=0 and isnull(mate_id3,0)=0 and isnull(mate_id4,0)=0 and isnull(mate_id5,0)=0';
pubqry.open;
...
procedure Tsetmedexport3.dxEdit1Click(Sender: TObject);
begin
dxEdit1.SelectAll;
dxEdit1.SetFocus;
end;

procedure Tsetmedexport3.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
