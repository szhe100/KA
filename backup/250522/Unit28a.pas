...
type
  Tpickstaff = class(TForm)
    ...
    DBGrid1csex: TdxDBGridMaskColumn;
...
procedure Tpickstaff.Edit1KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
    fwid: boolean;
...
begin
    ...
    begin
        ...
        commandtext:='select top 200 pick_ord=cast(ROW_NUMBER() OVER(ORDER BY zname) as int),sta_id,zname,zcode,origin,degree,title,zpost,phone,fax,residence,';
        commandtext:=commandtext+' csex=case sex when 1 then ''男'' else ''女'' end,sta_type_id,qry_code from tb_staff ';
        commandtext:=commandtext+' where sta_type_id=0 and comp_id='+inttostr(compid);
...
//        commandtext:=commandtext+' order by zname';
        open;
        if recordcount=0 then begin beep; raise Exception.Create('无所查询员工'); end;
        label1.caption:= formatfloat('[#,###]',query1.recordcount);
    end;
end;
end;

procedure Tpickstaff.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickstaff.DBGrid1DblClick(Sender: TObject);
begin
   modalresult:=mrOK;
end;

procedure Tpickstaff.wwDBNavigator1FirstClick(Sender: TObject);
begin
edit1.setfocus;
end;

procedure Tpickstaff.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
var flook: boolean;
    i,j: word;
    s: string;
begin
if (edit1.text='') or (query1.active=false) or (pickbroker.tag=1) then
begin
    pickbroker.tag:=0;
    exit;
end;
...
procedure Tpickstaff.FormActivate(Sender: TObject);
begin
edit1.setfocus;
edit1.selectall;
end;

procedure Tpickstaff.BitBtn3Click(Sender: TObject);
begin
pickbroker.tag:=1;  // <Esc> (cancel) 退出标志
end;

procedure Tpickstaff.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickstaff.DSquery1DataChange(Sender: TObject; Field: TField);
begin
if activecontrol=dbgrid1 then
edit1.text:=query1.fieldbyname('pick_ord').asstring;
end;

procedure Tpickstaff.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
