...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1渠道,2子渠道,3业务省份,4业务城市,5定价参考物料编码, 6中标价/开票价, 7金额,8定价单位,9启用日期
    mi:=0;j:=2;  //第1行开始 每行一单
    ...
    sql:=sql+' select top 5 * from ( select top 5 info=''第''+line_no+''行无渠道或数据无效'' from @tab a where f1='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.obj_code=a.f1)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无子渠道或数据无效'' from @tab a where f2='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.obj_code=a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无业务省份或数据无效'' from @tab a where f3='''' or not exists (select 1 from tb_district b where b.prov_name=f3 )';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无业务城市'' from @tab where f4=''''';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无定价参考物料编码或数据无效'' from @tab a where f5='''' or not exists (select 1 from tb_medicine b where b.material_code1=f5)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无中标价/开票价或数据无效'' from @tab where f6='''' or try_cast(f6 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无金额或数据无效'' from @tab where f7='''' or try_cast(f7 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无定价单位或数据无效'' from @tab where f8='''''; // or try_cast(f8 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无启用日期或数据无效'' from @tab where f9='''' or try_cast(f9 as datetime) is null';
...
//rec_id,comp_id,type_id,channel_id,channel_dtl_id,cust_group_id,district_id,med_id,price,price_unit,price1,valid_from,valid_to,creat_by,creat_dt
        sql:=sql1+' insert into tb_medprice (comp_id,type_id,channel_id,channel_dtl_id,district_id,med_id,price,price_unit,price1,valid_from,valid_to,creat_by,creat_dt)';
        sql:=sql+'  select comp_id='+inttostr(compid)+',type_id=3,channel_id=b.obj_id,channel_dtl_id=c.obj_id,district_id=case when e.city_code<>'''' then e.city_code else d.prov_code end,'; //type_id=3 设置学术标准政策
        ...
        sql:=sql+' from @tab a';
        sql:=sql+' left join tb_object b on b.obj_type_id=11 and b.obj_code= a.f1';
        sql:=sql+' left join tb_object c on c.obj_type_id=12 and c.obj_code= a.f2';
...
//        sql:=sql+' left join (select distinct city_code,city_name=prov_name+'' ''+city_name from tb_district) d on d.city_name= a.f3+'' ''+a.f4';
        ...
        sql:=sql+' left join tb_medicine m on m.material_code1=a.f5';
...
begin
    ...
    commandtext:=commandtext+' (3,'+dm.district.fieldbyname('code').asstring+','+inttostr(dxbuttonedit1.tag)+','+inttostr(curuserid)+',getdate())';
