...
type
  Tsettrustruleapply1 = class(TForm)
...
var
  settrustruleapply1: Tsettrustruleapply1;
...
procedure Tsettrustruleapply1.setupdatestatus;    // 编辑状态
begin
...
procedure Tsettrustruleapply1.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsettrustruleapply1.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsettrustruleapply1.FormCloseQuery(Sender: TObject;
...
procedure Tsettrustruleapply1.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsettrustruleapply1.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsettrustruleapply1.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with cdsbill_dtl do
...
procedure Tsettrustruleapply1.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsettrustruleapply1.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.comp_id='+inttostr(compid)+' and a.bod_type_id=24 and a.bod_status_id in (1,2,3,4)';
...
procedure Tsettrustruleapply1.FormCreate(Sender: TObject);
begin
...
procedure Tsettrustruleapply1.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsettrustruleapply1.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsettrustruleapply1.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsettrustruleapply1.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsettrustruleapply1.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsettrustruleapply1.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsettrustruleapply1.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsettrustruleapply1.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsettrustruleapply1.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsettrustruleapply1.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsettrustruleapply1.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsettrustruleapply1.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsettrustruleapply1.N1Click(Sender: TObject);
...
procedure Tsettrustruleapply1.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsettrustruleapply1.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsettrustruleapply1.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsettrustruleapply1.brokermedtlCalcFields(DataSet: TDataSet);
begin
with brokermedtl do
begin
...
procedure Tsettrustruleapply1.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsettrustruleapply1.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsettrustruleapply1.SpeedButton2Click(Sender: TObject);
begin
