...
var
  setexpaybook2a: Tsetexpaybook2a;
...
procedure Tsetexpaybook2a.setupdatestatus;    // 编辑状态
begin
speedbutton5.enabled:=false;   // refresh disable
speedbutton6.enabled:=false;   // insert disable
end;

procedure Tsetexpaybook2a.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetexpaybook2a.FormActivate(Sender: TObject);
begin
{
with dm.homebank do
...
procedure Tsetexpaybook2a.FormCloseQuery(Sender: TObject;
...
procedure Tsetexpaybook2a.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetexpaybook2a.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetexpaybook2a.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetexpaybook2a.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where bod_type_id=43 and a.bod_status_id in (1,2) and cate_id=1';
...
procedure Tsetexpaybook2a.FormCreate(Sender: TObject);
begin
...
procedure Tsetexpaybook2a.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetexpaybook2a.DSbillStateChange(Sender: TObject);
begin
with bill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetexpaybook2a.billBeforeEdit(DataSet: TDataSet);
begin
with bill do
begin
    if fieldbyname('bod_status_id').asinteger=1 then raise Exception.Create('本单已非制单状态,不可更改'+#13#10+'确需更改，须先返回制单状态');
//    if curuserid<>fieldbyname('creat_by').asinteger then raise Exception.Create('非本人单据，请勿制单');
end;
end;

procedure Tsetexpaybook2a.billCalcFields(DataSet: TDataSet);
begin
with bill do
begin
...
procedure Tsetexpaybook2a.billReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetexpaybook2a.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetexpaybook2a.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetexpaybook2a.dxDBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
if key in ['a'..'z'] then key:=chr(ord(key)-32);   // turn lower charactor to upper
end;

procedure Tsetexpaybook2a.SpeedButton2Click(Sender: TObject);
begin
with bill do
begin
...
procedure Tsetexpaybook2a.SpeedButton3Click(Sender: TObject);
begin
...
procedure Tsetexpaybook2a.SpeedButton6Click(Sender: TObject);
begin
