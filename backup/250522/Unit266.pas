...
type
  Tsetbusiframe = class(TForm)
    ...
    dxDBGrid1DATE_TO: TdxDBGridColumn;
...
//    commandtext:='select a.*,channel=b.zdesc,channel_dtl=c.zdesc from SAP_ZSD_BUS a'; //zname1=dbo.fn_staff_name(b.sta_id),zname2=dbo.fn_staff_name(c.sta_id),zname3=dbo.fn_staff_name(d.sta_id),';
...
//    commandtext:=commandtext+' left join (select obj_id,obj_code,zdesc from tb_object where obj_type_id=11) b on a.VTWEG=b.obj_code';
//    commandtext:=commandtext+' left join (select obj_id,obj_code,zdesc from tb_object where obj_type_id=12) c on a.KDGRP=c.obj_code';
...
//        if Trim(dxLookupTreeView1.text)<>'' then commandtext:=commandtext+' and dbo.fn_treeischild(a.district_id,'+dm.district.fieldbyname('rec_id').asstring+')=1';
        if Trim(dxLookupEdit1.text)<>'' then commandtext:=commandtext+' and a.VTWEG='''+Trim(dxLookupEdit1.text)+'''';
        if Trim(dxLookupEdit2.text)<>'' then commandtext:=commandtext+' and a.KDGRP='''+Trim(dxLookupEdit2.text)+'''';
