...
//            and ((fieldbyname('creat_by').asinteger=curuserid) or (fieldbyname('check_by').asinteger=curuserid))
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active then close;
                commandtext:='select a.*,leader=e.zname,broker=d.zname,b.med_name,b.specifi,b.pdt_place,b.med_code,b.material_code1,b.site_code,b.qtyperpack,b.qtyperbox,med_unit=c.zdesc';
...
begin
    ...
    sbodcd:='';
    j:=2; mi:=0;  //第2行开始 每行一单
//导入格式: 1供货单位id1,2购货单位id2,3单据摘要,4发生日期,5单据类别id3,6定价参考物料编码id4,7数量,8批号,9有效期至,10业务类别,11商业单价
            ...
            begin
{
                ...
                begin
                    ...
                    commandtext:=commandtext+'  and a.med_id=b.med_id and (a.zdesc='''+Trim(sheet.cells[j,6].text)+''' or b.material_code1='''+Trim(sheet.cells[j,6].text)+''')';
