...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1交货单号, 2订单编号, 3渠道, 4子渠道, 5业务负责人, 6供货单位, 7商业公司, 8销售人员, 9单据摘要, 10发运日期, 11定价参考物料编码, 12单价, 13数量, 14批号, 15有效期至
...
//        sql:=sql+''','''+Trim(sheet.cells[j,11].text)+''','''+Trim(sheet.cells[j,12].text)+''','''+Trim(sheet.cells[j,13].text)+''','''+Trim(sheet.cells[j,14].text)+''','''+Trim(sheet.cells[j,15].text)+'''';
    ...
    XL.Quit;
//从第2行开始导入，格式: 1交货单号, 2订单编号, 3渠道编码, 4子渠道编码, 5业务负责人, 6供货单位, 7商业公司, 8销售人员, 9单据摘要, 10发运日期, 11定价参考物料编码, 12单价, 13数量, 14批号, 15有效期至
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行无订单编号'' from @tab where f2=''''';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无渠道编码或数据无效'' from @tab a where f3='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.obj_code=a.f3)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无子渠道编码或数据无效'' from @tab a where f4='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.obj_code=a.f4)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无业务负责人或数据无效'' from @tab a where f5='''' or not exists (select 1 from tb_busimate b where mate_type_id=4 and b.mate_name=a.f5)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无供货单位或数据无效'' from @tab a where f6='''' or not exists (select 1 from tb_busimate b where mate_type_id=2 and b.mate_name=a.f6)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无商业公司或数据无效'' from @tab a where f7='''' or not exists (select 1 from tb_busimate b where mate_type_id=2 and b.mate_name=a.f7)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无销售人员或数据无效'' from @tab a where f8='''' or not exists (select 1 from tb_staff b where sta_type_id=1 and b.zname=a.f8)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无发运日期或数据无效'' from @tab where f10='''' or try_cast(f10 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无定价参考物流编码或数据无效'' from @tab a where f11='''' or not exists (select 1 from tb_medicine b where b.material_code1=a.f11)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无单价或数据无效'' from @tab where f12='''' or try_cast(f12 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无数量或数据无效'' from @tab where f13='''' or try_cast(f13 as decimal(15,4)) is null';
...
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无有效期至或数据无效'' from @tab where f15='''' or try_cast(f15 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行有效期数据无效'' from @tab where f15<>'''' and try_cast(f15 as datetime) is null';
...
//        sql:=sql+' declare @cnt int'; // @bodcd varchar(20),@bodid int,@lineno varchar(10),@mi int,@bcd varchar(2000)';
        sql:=sql+' insert into SAP_ZSD_015 (BEZEI,GBELN,VBELN,VTWEG,VTEXT,KDGRP,ZKDGRP,NAME_FIRST,ZVKORG,NAME1,ZSALESNAM,bod_desc,WADAT_IST,carry_dt,MATNR,ZKONV,KONV,price,KWMENG,qty,KZWI5,amot,CHARG,VFDAT,AUDAT,creat_dt,ZTERNAM,creat_by)';
        ...
        sql:=sql+'  CHARG=f14,VFDAT=f15,AUDAT=format( cast(getdate() as datetime),''yyyy-MM-dd HH:mm:ss''),creat_dt=getdate(),ZTERNAM=dbo.fn_staff_name('+inttostr(curuserid)+'),creat_by='+inttostr(curuserid);
        sql:=sql+' from @tab a';
