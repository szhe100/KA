...
begin
    ...
    begin
        ...
        commandtext:='select a.*,leader=dbo.fn_staff_name(a.lead_id),leader1=dbo.fn_staff_name(k.sta_id),b.med_code,b.med_name,b.material_code,b.material_code1,b.specifi,b.pdt_place,med_unit=c.zdesc,b.qtyperpack,b.qtyperbox,';
//        commandtext:=commandtext+' f.check_dt,stoper=dbo.fn_staff_name(a.stop_by),price0=dbo.fn_getfeerate1(d.district,a.med_id,a.price1),fee1=0.0000,'; //dbo.fn_gettrustfee1(a.mate_id,a.med_id,a.price1),
        ...
        commandtext:=commandtext+' b.chm_name,med_type=dbo.fn_med_type(b.med_id),broker=d.zname,e.mate_name,e.trusted,creater=g.zname,e.district,dist_name=dbo.fn_getdistrictname (e.district,1)+'' ''+dbo.fn_getdistrictname (e.district,2),'; //dbo.fn_getdistrict(e.district),';
...
begin
    ...
    begin
        ...
        commandtext:=commandtext+' a.*,leader=dbo.fn_staff_name(a.lead_id),leader1=dbo.fn_staff_name(k.sta_id),b.med_code,b.material_code1,b.med_name,b.specifi,b.pdt_place,med_unit=c.zdesc,b.qtyperpack,b.qtyperbox,';
...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1申请摘要, 2渠道, 3子渠道, 4业务员, 5商业公司, 6医院, 7定价参考物料编码, 8明细业务员, 9费用金额, 10中标价, 11医院托管结算率, 12启用日期
    mi:=0;j:=2;  //第1行开始 每行一单
    ...
    begin
        ...
        sql:=sql+''','''+Trim(sheet.cells[j,6].text)+''','''+Trim(sheet.cells[j,7].text)+''','''+Trim(sheet.cells[j,8].text)+''','''+Trim(sheet.cells[j,9].text)+''','''+Trim(sheet.cells[j,10].text)+''',replace('''+Trim(sheet.cells[j,11].text)+''',''%'',''''),'''+Trim(sheet.cells[j,12].text)+'''';
        j:=j+1;
    end;
    XL.Quit;
    sql1:=sql;  //导入数据无错误时，用sql1 插入数据
//从第2行开始导入，格式: 1申请摘要, 2渠道, 3子渠道, 4业务员, 5商业公司, 6医院, 7定价参考物料编码, 8明细业务员, 9费用金额, 10中标价, 11医院托管结算率, 12启用日期
    sql:=sql+' select top 10 * from ( select top 5 info=''第''+line_no+''行无渠道或数据无效'' from @tab a where a.f2='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.obj_code=a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无子渠道或数据无效'' from @tab a where a.f3='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.obj_code=a.f3)';
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行无定价参考物料编码或数据无效'' from @tab a where f7='''' or not exists (select 1 from tb_medicine m where m.material_code1 =a.f7)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行明细业务员无效'' from @tab a where f8<>'''' and not exists (select 1 from tb_staff b where sta_type_id=1 and b.zname =a.f8)';
...
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无医院托管结算率或数据无效'' from @tab where f11<>'''' and try_cast(f11 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行医院托管结算率数据无效'' from @tab where f11<>'''' and try_cast(f11 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无启用日期或数据无效'' from @tab where f12='''' or try_cast(f12 as datetime) is null';
...
edit1.text:=commandtext;        
        ...
        sql:=sql1;  //取前脚本
        sql:=sql+' update a set channel_id=b.obj_id,channel_dtl_id=c.obj_id,sta_id=d.sta_id,sta_id1=case when g.sta_id>0 then g.sta_id else d.sta_id end, mate_id1=e.mate_id,mate_id=f.mate_id,med_id=m.med_id';
        sql:=sql+' from @tab a';
...
// 以下为 无主要数据 的 insert into tb_brokermed
        sql:=sql+' insert into tb_brokermed (comp_id,bod_desc,channel_id,channel_dtl_id,sta_id,mate_id1,mate_id,med_id,price1,rate1,valid_dt,creat_by,creat_dt)';
        sql:=sql+' select distinct comp_id=1,bod_desc=f1,channel_id=a.channel_id,channel_dtl_id=a.channel_dtl_id,a.sta_id,';
        sql:=sql+' mate_id1=a.mate_id1,a.mate_id,a.med_id,price1=cast(f10 as decimal(15,4)),rate1=case when f11='''' then 0 else cast(f11 as decimal(15,4)) end,valid_dt=cast(f12 as datetime),';
        ...
        sql:=sql+'       and k.valid_dt=cast(a.f12 as datetime))';  // and k.rate1=cast(a.f11 as decimal(15,4))
        sql:=sql+' set @cnt2=@@ROWCOUNT';

        // 以下为 有明细数据 的 update tb_brokermedtl 价格类数据
        sql:=sql+' update b set rate=cast(f9 as decimal(15,4)),modify_by='+inttostr(curuserid)+',modify_dt=getdate()';
        sql:=sql+' from @tab a';
        ...
        sql:=sql+'       and k.valid_dt=cast(f12 as datetime)'; // and k.rate1=cast(a.f11 as decimal(15,4))';
        sql:=sql+' inner join tb_brokermedtl b on k.rec_id=b.rela_id and b.sta_id=a.sta_id1';
        sql:=sql+' where isnull(b.rate,0)<>cast(f9 as decimal(15,4))';
