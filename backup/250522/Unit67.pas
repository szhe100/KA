...
begin
    ...
    begin
        ...
        commandtext:='select a.*,mate_name1=d.mate_name,m.med_code,m.material_code1,m.med_name,m.chm_name,m.specifi,m.pdt_place,qtyperpack,med_unit=dbo.fn_obj_desc(m.unit_id),med_type=dbo.fn_med_type(m.med_id),creater=c.zname';
...
//从第2行开始导入,导入格式: 1商业公司, 2客户名称, 3流向词语
    ...
    sql:=sql+' select top 5 * from ( select top 5 info=''第''+line_no+''行无商业公司'' from @tab a where f1='''' or not exists (select 1 from tb_busimate b where b.mate_type_id=2 and b.mate_name=a.f1)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无客户名称'' from @tab a where f2='''' or not exists (select 1 from tb_busimate b where b.mate_type_id=1 and b.mate_name=a.f2)';
...
{ 此处不设修改功能
        ...
        sql:=sql+'  select type_id=2,comp_id='+inttostr(compid)+',mate_id1=a.mate_id1,mate_id=a.mate_id,zdesc=a.f3,creat_by='+inttostr(curuserid)+',creat_dt=getdate()';
        sql:=sql+' from @tab a';
...
begin
    ...
    setprogress(1);
    m1:=0; m2:=0; j:=2;  
//从第2行开始导入,导入格式: 1商业公司, 2定价参考物料编码, 3流向词语, 4单位系数
...
//从第2行开始导入,导入格式: 1商业公司, 2定价参考物料编码, 3流向词语, 4单位系数
    sql1:=sql;  //导入数据无错误时，用sql1 插入数据
    sql:=sql+'select top 5 * from ( select top 5 info=''第''+line_no+''行无商业公司或数据无效'' from @tab a where a.f1='''' or not exists (select 1 from tb_busimate b where b.mate_type_id=2 and a.f1=b.mate_name)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无定价参考物料编码或数据无效'' from @tab a where a.f2='''' or not exists (select 1 from tb_medicine b where b.material_code1=a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无流向词语'' from @tab where f3='''''; // or try_cast(f4 as datetime) is null
...
{ 此处不设修改功能
        ...
        sql:=sql+'  select type_id=1,comp_id='+inttostr(compid)+',mate_id1=a.mate_id1,med_id=a.med_id,zdesc=a.f3,rate=cast(f4 as decimal(6,2)),creat_by='+inttostr(curuserid)+',creat_dt=getdate()';
        sql:=sql+' from @tab a';
