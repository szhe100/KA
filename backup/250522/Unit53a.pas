...
type
  Tsetruleapply1 = class(TForm)
    ...
    dxDBGrid2trusted: TdxDBGridCheckColumn;
...
procedure Tsetruleapply1.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetruleapply1.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetruleapply1.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsetruleapply1.FormCloseQuery(Sender: TObject;
...
procedure Tsetruleapply1.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetruleapply1.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetruleapply1.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with cdsbill_dtl do
...
procedure Tsetruleapply1.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetruleapply1.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    close;
    commandtext:='select a.*,leader=c.zname,creater=d.zname,checker=e.zname,broker=f.zname,carryer=g.zname,receiver=h.zname';
    commandtext:=commandtext+' from tb_bill a';
    commandtext:=commandtext+' left join tb_staff c on a.src_id =c.sta_id';
    ...
    commandtext:=commandtext+' where a.bod_type_id=20 and a.bod_status_id in (1,2,3,4)'; //a.comp_id='+inttostr(compid)+' and 
    commandtext:=commandtext+'  and a.creat_dt>= '''+datetostr(dxdateedit1.date)+''' and a.creat_dt< dateadd(day,1,'''+datetostr(dxdateedit4.date)+''')';
    if fdistltd then
    begin
{
        commandtext:=commandtext+' and exists (select 1 from tb_bill_dtl b,tb_brokermed r,tb_busimate s,tb_trustdistrict t where a.bod_id=b.bod_id and b.med_id=r.rec_id and r.mate_id=s.mate_id';
        commandtext:=commandtext+'  and t.sta_id='+inttostr(curuserid)+' and dbo.fn_ischildnode(s.district,t.dist_id)=1)';
}
        commandtext:=commandtext+' and exists (select 1 from tb_bill_dtl b,tb_brokermed r,tb_busimate s,tb_trustdistrict t,tb_treedata i where a.bod_id=b.bod_id and b.med_id=r.rec_id and r.mate_id=s.mate_id';
        commandtext:=commandtext+'  and t.sta_id='+inttostr(curuserid)+'  and i.rec_id=s.district and (i.rec_id=t.dist_id or i.parent=t.dist_id))';
//        commandtext:=commandtext+'  and exists (select 1 from tb_trustdistrict s,tb_treedata t where s.sta_id='+inttostr(curuserid)+' and t.rec_id=a.district and (t.rec_id=s.dist_id or t.parent=s.dist_id))';
    end;
    if fmedltd then
    begin
{
        commandtext:=commandtext+' and exists (select 1 from tb_bill_dtl x,tb_bill m,tb_bill_dtl n,tb_bill_stadtl l,tb_trustmed t ';
        commandtext:=commandtext+'  where a.bod_id=x.bod_id and x.med_id=l.rec_id and m.bod_id=n.bod_id and n.dtl_id=l.dtl_id'; // and d.med_id=m.med_id and b.bod_id='+cdsbill.fieldbyname('bod_id').asstring;
        commandtext:=commandtext+'      and t.sta_id='+inttostr(curuserid)+' and n.med_id=t.med_id)';
}
        commandtext:=commandtext+' and exists (select 1 from tb_bill_dtl b,tb_brokermed r,tb_trustmed t where a.bod_id=b.bod_id and b.med_id=r.rec_id';
        commandtext:=commandtext+'  and t.sta_id='+inttostr(curuserid)+' and r.med_id=t.med_id)';
    end;
...
procedure Tsetruleapply1.FormCreate(Sender: TObject);
begin
...
procedure Tsetruleapply1.DSbillDataChange(Sender: TObject; Field: TField);
begin
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                ...
                commandtext:=commandtext+' a.*,leader=dbo.fn_staff_name(k.sta_id),m.med_code,m.med_name,m.chm_name,m.specifi,m.pdt_place,med_unit=c.zdesc,m.qtyperpack,m.qtyperbox,';
...
procedure Tsetruleapply1.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetruleapply1.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetruleapply1.SpeedButton2Click(Sender: TObject);
begin
{
...
if MessageBox(0,'确定本单通过业务审核,送财务复核','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
setprogress(1);
...
procedure Tsetruleapply1.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetruleapply1.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetruleapply1.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetruleapply1.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetruleapply1.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetruleapply1.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetruleapply1.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetruleapply1.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetruleapply1.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetruleapply1.N1Click(Sender: TObject);
...
begin
    if active=true then close;
    commandtext:='update tb_bill set bod_status_id=3,carry_by='+inttostr(curuserid)+',carry_dt=getdate() where bod_id in ('+t+') and bod_status_id=2';
    execute;
end;
SpeedButton5Click(nil);
end;

procedure Tsetruleapply1.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetruleapply1.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetruleapply1.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetruleapply1.brokermedtlCalcFields(DataSet: TDataSet);
begin
with brokermedtl do
begin
...
procedure Tsetruleapply1.SpeedButton6Click(Sender: TObject);
begin
