...
begin
    ...
    XL.Quit;
//从第2行开始导入,导入格式:1发生日期, 2商业公司, 3业务负责人, 4金额, 5单据摘要, 6关联定价参考物料编码(选填), 7渠道, 8子渠道
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行无商业公司或数据无效'' from @tab a where f2='''' or not exists (select 1 from tb_busimate b where mate_type_id=2 and b.mate_name =a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无业务负责人或数据无效'' from @tab a where f3='''' or not exists (select 1 from tb_busimate b where mate_type_id=4 and b.mate_name =a.f3)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无金额或数据无效'' from @tab a where f4='''' or try_cast(f4 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行定价参考物料编码无效'' from @tab a where f6='''' or not exists (select 1 from tb_medicine m where m.material_code1 =a.f6)';
...
//从第2行开始导入,导入格式:1发生日期, 2商业公司, 3业务负责人, 4金额, 5单据摘要, 6关联定价参考物料编码(选填), 7渠道, 8子渠道
        ...
        sql:=sql+' insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,carry_dt,dst_id,agent_id,bod_amot,bod_desc,med_id,channel_id,channel_dtl_id,creat_by,creat_dt)';
        ...
        sql:=sql+' bod_amot=cast(f4 as decimal(15,2)),bod_desc=f5,med_id=m.med_id,channel_id=e.obj_id,channel_dtl_id=f.obj_id,';
        ...
        sql:=sql+' left join (select mate_id,mate_name from tb_busimate where mate_type_id=4) d on d.mate_name=a.f3';
...
//exit;
        ...
        begin
            SpeedButton5Click(nil);
            if (mi>0) and (bill.active) then bill.Last;  //如有新增，locate到最后记录
            MessageBox(0,pchar('已新增'+inttostr(mi)+'条记录(单据)'),'请注意',MB_OK+MB_ICONInformation);
        end;
    end;
end;

procedure Tsetexpaybook.dxDBButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickchannel do
begin
...
procedure Tsetexpaybook.dxDBButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
