...
//    commandtext:=commandtext+' a.*,h.price10,broker='''',c.mate_name,agent=d.mate_name,m.med_code,m.material_code1,m.med_name,m.chm_name,m.specifi,med_unit=dbo.fn_obj_desc(m.unit_id),m.qtyperpack,'; //pdt_place=dbo.fn_mate_name(m.pdt_id),
    commandtext:=commandtext+' a.*,price10=dbo.fn_getprice1(c.district,a.med_id,a.valid_dt),broker='''',c.mate_name,agent=d.mate_name,m.med_code,m.material_code1,m.med_name,m.chm_name,m.specifi,med_unit=dbo.fn_obj_desc(m.unit_id),m.qtyperpack,'; //pdt_place=dbo.fn_mate_name(m.pdt_id),
...
begin
    ...
    setprogress(1);
//导入 格式: 1商业公司,2业务负责人,3产品编码,4生效日期,5渠道编码,6子渠道编码,7税率,8销售单价,9考核单价,10应付金额/盒,11促销率,12促销费/盒,13明细业务负责人(选填), 14备注
    mi:=0;j:=2;  //第1行开始 每行一单
    ...
    sql:=sql+' f11 varchar(100),f12 varchar(100),f13 varchar(100),f14 varchar(100),';
...
//    sql:=sql+' channel_id int,channel_dtl_id int,sta_id int,sta_id1 int,mate_id int,mate_id1 int,med_id int)';
    ...
    begin
        ...
        sql:=sql+'replace('''+Trim(sheet.cells[j,11].text)+''',''%'',''''),replace('''+Trim(sheet.cells[j,12].text)+''','','',''''),'''+Trim(sheet.cells[j,13].text)+''','''+Trim(sheet.cells[j,14].text)+'''';
        j:=j+1;
    end;
    XL.Quit;
    sql1:=sql;  //导入数据无错误时，用sql1 插入数据
//导入 格式: 1商业公司,2业务负责人,3产品编码,4生效日期,5渠道编码,6子渠道编码,7税率,8销售单价,9考核单价,10应付金额/盒,11促销率,12促销费/盒,13明细业务负责人(选填), 14备注
    sql:=sql+' select top 10 * from ( select top 5 info=''第''+line_no+''行无商业公司或数据无效'' from @tab a where f1='''' or not exists (select 1 from tb_busimate b where mate_type_id=2 and b.mate_name =a.f1)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无业务负责人或数据无效'' from @tab a where f2='''' or not exists (select 1 from tb_busimate b where mate_type_id=4 and b.mate_name =a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无定价参考物料编码或数据无效'' from @tab a where f3='''' or not exists (select 1 from tb_medicine m where m.material_code1 =a.f3)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无启用日期或数据无效'' from @tab where f4='''' or try_cast(f4 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无渠道或数据无效'' from @tab a where a.f5='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.obj_code=a.f5)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无子渠道或数据无效'' from @tab a where a.f6='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.obj_code=a.f6)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行税率数据无效'' from @tab where f7<>'''' and try_cast(f7 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无销售单价或数据无效'' from @tab where f8='''' or try_cast(f8 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无考核单价或数据无效'' from @tab where f9='''' or try_cast(f9 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无应付金额/盒或数据无效'' from @tab a where f10='''' or try_cast(f10 as decimal(15,4)) is null';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行促销率数据无效'' from @tab where f11<>'''' and try_cast(f11 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行促销费/盒数据无效'' from @tab where f12='''' or try_cast(f12 as decimal(15,4)) is null';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无明细业务负责人或数据无效'' from @tab a where f13='''' or not exists (select 1 from tb_busimate b where mate_type_id=4 and b.mate_name =a.f13)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行明细业务负责人数据无效'' from @tab a where f13<>'''' and not exists (select 1 from tb_busimate b where mate_type_id=4 and b.mate_name =a.f13)';
    sql:=sql+' ) a order by info';
//导入 格式: 1商业公司,2业务负责人,3产品编码,4生效日期,5渠道编码,6子渠道编码,7税率,8销售单价,9考核单价,10应付金额/盒,11促销率,12促销费/盒,13明细业务负责人, 14备注
    with dm.pubqry do
    begin
        if active then close;
        commandtext:=sql;
...
//导入 格式: 1商业公司,2业务负责人,3产品编码,4生效日期,5渠道编码,6子渠道编码,7税率,8销售单价,9考核单价,10应付金额/盒,11促销率,12促销费/盒, 13明细业务负责人, 14备注
        sql:=sql1;  //取前脚本
        sql:=sql+' update a set channel_id=b.obj_id,channel_dtl_id=c.obj_id,mate_id=d.mate_id,mate_id1=e.mate_id,mate_id2=case when f.mate_id>0 then f.mate_id else e.mate_id end,med_id=m.med_id';
        sql:=sql+' from @tab a';
        sql:=sql+' left join tb_object b on b.obj_type_id=11 and b.obj_code=a.f5';
        sql:=sql+' left join tb_object c on c.obj_type_id=12 and c.obj_code=a.f6';
        sql:=sql+' left join tb_busimate d on d.mate_type_id=2 and d.mate_name =a.f1';
        sql:=sql+' left join tb_busimate e on e.mate_type_id=4 and e.mate_name =a.f2';
        sql:=sql+' left join tb_busimate f on f.mate_type_id=4 and f.mate_name =a.f13';
        sql:=sql+' left join tb_medicine m on m.material_code1 =a.f3';
...
// 以下为 有主要数据 的 update tb_busiframe3 价格类数据
        ...
        sql:=sql+'  rate1=case when a.f11='''' then 0 else cast(a.f11 as decimal(15,4)) end,remark=a.f13';
        ...
        sql:=sql+'    and k.valid_dt=cast(a.f4 as datetime)'; // and rate1=cast(f11 as decimal(15,4))';
        sql:=sql+' set @cnt1=@@ROWCOUNT';

//导入 格式: 1商业公司,2业务负责人,3产品编码,4生效日期,5渠道编码,6子渠道编码,7税率,8销售单价,9考核单价,10应付金额/盒,11促销率,12促销费/盒,13明细业务负责人, 14备注
// 以下为 无主要数据 的 insert into tb_brokermed
        sql:=sql+' insert into tb_busiframe3 (comp_id,channel_id,channel_dtl_id,mate_id,agent_id,med_id,price1,price2,valid_dt,creat_by,creat_dt)'; //remark, rate, amot,fee,rate1,
        ...
        sql:=sql+' price1=cast(a.f8 as decimal(15,4)),price2=cast(a.f9 as decimal(15,4)),'; //rate=case when f7='''' then 0 else cast(a.f7 as decimal(15,4)) end, amot=cast(a.f10 as decimal(15,4)),rate1=case when f11='''' then 0 else cast(a.f11 as decimal(15,4)) end,';
        sql:=sql+' valid_dt=cast(a.f4 as datetime),  creat_by='+inttostr(curuserid)+',creat_dt=getdate()'; //remark=a.f13,
        sql:=sql+' from @tab a';
...
//维护 备注
        ...
        sql:=sql+' update b set amot=cast(f10 as decimal(15,4)),fee=cast(f12 as decimal(15,4)),modify_by='+inttostr(curuserid)+',modify_dt=getdate()';
        sql:=sql+' from @tab a';
        sql:=sql+' inner join tb_busiframe3 k on k.channel_id=a.channel_id and k.channel_dtl_id=a.channel_dtl_id and k.mate_id=a.mate_id and k.agent_id=a.mate_id1 and k.med_id=a.med_id and k.valid_dt=cast(a.f4 as datetime)';
...
// 以下为 无明细数据 的 insert tb_busiframe3_dtl
        sql:=sql+' insert into tb_busiframe3_dtl (rela_id,agent_id,rate,fee,creat_by,creat_dt)';
        sql:=sql+' select rela_id=k.rec_id,agent_id=a.mate_id2,amot=cast(f10 as decimal(15,4)),fee=cast(f12 as decimal(15,4)),creat_by='+inttostr(curuserid)+',creat_dt=getdate()';
        sql:=sql+' from @tab a';
        sql:=sql+' inner join tb_busiframe3 k on k.channel_id=a.channel_id and k.channel_dtl_id=a.channel_dtl_id and k.mate_id=a.mate_id and k.agent_id=a.mate_id1 and k.med_id=a.med_id and k.valid_dt=cast(a.f4 as datetime)';
...
if dxButtonEdit5.tag=0 then raise Exception.Create('请选择业务负责人');
with dm.pubqry do
begin
    if active then close;
...
edit2.text:=commandtext;
    open;
    if recordcount>0 then raise Exception.Create('本组已有该业务负责人，不可重复设置');
    if active then close;
...
if MessageBox(0,'确定删除本业务负责人成员规则','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;
