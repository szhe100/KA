...
var
  setcheckstock3: Tsetcheckstock3;
...
procedure Tsetcheckstock3.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetcheckstock3.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetcheckstock3.FormActivate(Sender: TObject);
begin
if cdsbill.active=false then SpeedButton5Click(nil);
end;

procedure Tsetcheckstock3.FormCloseQuery(Sender: TObject;
...
procedure Tsetcheckstock3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetcheckstock3.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetcheckstock3.dxDBGrid2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
...
procedure Tsetcheckstock3.dxDBGrid2Enter(Sender: TObject);
begin
...
procedure Tsetcheckstock3.FormKeyPress(Sender: TObject; var Key: Char);
//var fapp: boolean;
begin
...
procedure Tsetcheckstock3.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=10 and a.bod_status_id in (1,2) and a.busi_type=1'; //a.comp_id='+inttostr(compid)+' and    // busi_type: 1 商业公司盘点单；2 连锁终端盘点单
...
procedure Tsetcheckstock3.FormCreate(Sender: TObject);
begin
...
procedure Tsetcheckstock3.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetcheckstock3.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
procedure Tsetcheckstock3.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetcheckstock3.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetcheckstock3.SpeedButton2Click(Sender: TObject);
var s: string;
begin
with cdsbill do
begin
...
procedure Tsetcheckstock3.SpeedButton3Click(Sender: TObject);
begin
...
procedure Tsetcheckstock3.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetcheckstock3.cdsbillBeforeScroll(DataSet: TDataSet);
begin
exit;
with cdsbill do
begin
...
procedure Tsetcheckstock3.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetcheckstock3.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetcheckstock3.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetcheckstock3.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetcheckstock3.cdsbillBeforePost(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetcheckstock3.cdsbillReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetcheckstock3.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetcheckstock3.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetcheckstock3.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetcheckstock3.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetcheckstock3.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetcheckstock3.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
...
procedure Tsetcheckstock3.N1Click(Sender: TObject);
...
procedure Tsetcheckstock3.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetcheckstock3.dxDBButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickbroker1 do
begin
...
procedure Tsetcheckstock3.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetcheckstock3.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetcheckstock3.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tsetcheckstock3.SpeedButton6Click(Sender: TObject);
begin
