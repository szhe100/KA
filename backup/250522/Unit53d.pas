...
procedure Tsetrulestop1.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetrulestop1.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetrulestop1.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsetrulestop1.FormCloseQuery(Sender: TObject;
...
procedure Tsetrulestop1.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetrulestop1.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetrulestop1.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with cdsbill_dtl do
...
procedure Tsetrulestop1.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetrulestop1.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=21 and a.bod_status_id in (1,2,3,4)';  //a.comp_id='+inttostr(compid)+' and 
    commandtext:=commandtext+'  and a.creat_dt>= '''+datetostr(dxdateedit1.date)+''' and a.creat_dt< dateadd(day,1,'''+datetostr(dxdateedit4.date)+''')';
{
    ...
    if fdistltd then
    begin
		commandtext:=commandtext+' and exists (select 1 from tb_bill_dtl b,tb_brokermed r,tb_busimate c,tb_trustdistrict t';
        commandtext:=commandtext+' 		where a.bod_id=b.bod_id and b.med_id=r.rec_id and r.mate_id=c.mate_id and t.sta_id='+inttostr(curuserid)+' and dbo.fn_ischildnode(c.district,t.dist_id)=1)';
{
...
procedure Tsetrulestop1.FormCreate(Sender: TObject);
begin
...
procedure Tsetrulestop1.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetrulestop1.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetrulestop1.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetrulestop1.SpeedButton2Click(Sender: TObject);
begin
...
procedure Tsetrulestop1.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetrulestop1.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetrulestop1.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetrulestop1.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetrulestop1.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetrulestop1.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetrulestop1.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetrulestop1.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetrulestop1.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetrulestop1.N1Click(Sender: TObject);
...
procedure Tsetrulestop1.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetrulestop1.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetrulestop1.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetrulestop1.SpeedButton6Click(Sender: TObject);
begin
