...
type
  Tsetbrokerfeeapply2 = class(TForm)
...
var
  setbrokerfeeapply2: Tsetbrokerfeeapply2;
...
procedure Tsetbrokerfeeapply2.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetbrokerfeeapply2.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetbrokerfeeapply2.FormActivate(Sender: TObject);
begin
with dm.district do
...
procedure Tsetbrokerfeeapply2.FormCloseQuery(Sender: TObject;
...
procedure Tsetbrokerfeeapply2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetbrokerfeeapply2.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetbrokerfeeapply2.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetbrokerfeeapply2.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetbrokerfeeapply2.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetbrokerfeeapply2.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=17 and a.bod_status_id in (1,4,5)'; // a.comp_id='+inttostr(compid)+' and
...
procedure Tsetbrokerfeeapply2.FormCreate(Sender: TObject);
begin
...
procedure Tsetbrokerfeeapply2.DSbillDataChange(Sender: TObject; Field: TField);
begin
                ...
                if active then close;
{
                ...
                commandtext:=commandtext+' m.med_code,med_name=case when m.med_name is null then ''应付记账'' else m.med_name end,m.chm_name,m.specifi,m.pdt_place,med_unit=dbo.fn_obj_desc(m.unit_id),med_type='''',o.type_id2';
                commandtext:=commandtext+' from tb_bill_dtl b';
...
procedure Tsetbrokerfeeapply2.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetbrokerfeeapply2.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetbrokerfeeapply2.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetbrokerfeeapply2.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetbrokerfeeapply2.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetbrokerfeeapply2.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetbrokerfeeapply2.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetbrokerfeeapply2.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetbrokerfeeapply2.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetbrokerfeeapply2.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetbrokerfeeapply2.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetbrokerfeeapply2.N1Click(Sender: TObject);
...
begin
    if active then close;
    commandtext:='update tb_bill set bod_status_id=5,storage_by='+inttostr(curuserid)+',storage_dt=getdate() where bod_id in ('+t+') and bod_status_id=4';
    execute;
end;
...
procedure Tsetbrokerfeeapply2.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetbrokerfeeapply2.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetbrokerfeeapply2.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetbrokerfeeapply2.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsetbrokerfeeapply2.N3Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'列表1.xls', dxDBGrid1.SaveToXLS, self.tag)
end;

procedure Tsetbrokerfeeapply2.dxEdit2KeyDown(Sender: TObject;
...
procedure Tsetbrokerfeeapply2.SpeedButton2Click(Sender: TObject);
var s: string;
begin
if cdsbill_dtl.RecordCount=0 then raise Exception.Create('无明细数据，不可批准');
with dm.pubqry do
begin
    if active then close;
...
procedure Tsetbrokerfeeapply2.SpeedButton3Click(Sender: TObject);
begin
with dm.pubqry do
begin
    if active then close;
    commandtext:='select top 1 bod_cd from tb_bill a,tb_bill_dtl b where a.bod_type_id=18 and a.bod_id=b.bod_id and b.med_id='+cdsbill.fieldbyname('bod_id').asstring;
    open;
    if recordcount>0 then raise Exception.Create('本单已生成'+fieldbyname('bod_cd').asstring+'号出纳付款单，不可反审');
end;
if MessageBox(0,'确定反审核本单(取消批准)','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
