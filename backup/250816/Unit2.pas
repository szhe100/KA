...
begin
    ...
    commandtext:=commandtext+' dist3=dbo.fn_getdistrictname (a.district,3),'; //dbo.fn_getdistrictlevel (a.district,2),';
...
begin
    ...
    if (fieldbyname('mate_code').asstring='') then raise Exception.Create('请输入主数据编码');
...
begin
    if active then close;
    commandtext:='select top 1 mate_name from tb_busimate where mate_type_id=1 and mate_code='''+customer.fieldbyname('mate_code').asstring+'''';
    if customer.fieldbyname('mate_id').asinteger>0 then commandtext:=commandtext+' and mate_id<>'+customer.fieldbyname('mate_id').asstring;
    open;
    if recordcount>0 then raise Exception.Create(fieldbyname('mate_name').asstring+'已使用编码'+customer.fieldbyname('mate_code').asstring+'，不可重复');
{
    if customer.fieldbyname('mate_id').asinteger=0 then //新录入记录，尚未有mate_id
    begin
        commandtext:='select top 1 mate_name from tb_busimate where mate_type_id=1 and mate_code='''+customer.fieldbyname('mate_code').asstring+'''';
        open;
        if recordcount>0 then raise Exception.Create(fieldbyname('mate_name').asstring+'已使用编码'+customer.fieldbyname('mate_code').asstring+'，不可重复');
    ...
    begin
        commandtext:='select top 1 mate_name from tb_busimate where mate_type_id=1 and mate_code='''+customer.fieldbyname('mate_code').asstring+''' and mate_id<>'+customer.fieldbyname('mate_id').asstring;
        open;
        if recordcount>0 then raise Exception.Create(fieldbyname('mate_name').asstring+'已使用编码'+customer.fieldbyname('mate_code').asstring+'，不可重复');
    end;
}
...
//从第2行开始导入，导入格式: 1医院主数据编码, 2客户名称, 3级别, 4性质, 5所在地区(省份 城市), 6地址, 7电话, 8特色科室
    sql:='declare @tab table (line_no varchar(3),f1 varchar(100),f2 varchar(300),f3 varchar(100),f4 varchar(100),f5 varchar(100),f6 varchar(200),f7 varchar(100),f8 varchar(100),id3 int,id4 int )';
...
//    sql:=sql+' select top 10 * from ( select top 5 info=line_no+''行无医院主数据编码或编码重复'' from @tab a where f1='''' or exists (select 1 from tb_busimate b where b.mate_type_id=1 and b.mate_code=f1 and b.mate_name<>a.f2)';
//    sql:=sql+' select top 10 * from ( select top 5 info=line_no+''行 无医院主数据编码或编码重复'' from @tab a where f1='''' or exists (select 1 from tb_busimate b where b.mate_type_id=1 and b.mate_code=f1 and b.mate_name<>a.f2)';
    sql:=sql+' select top 10 * from ( select top 5 info=line_no+''行 无医院主数据编码'' from @tab a where f1='''''; // or exists (select 1 from tb_busimate b where b.mate_type_id=1 and b.mate_code=f1 and b.mate_name<>a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无客户(医院)名称或名称超长'' from @tab a where f2='''' or len(f2)>300'; // or exists (select 1 from tb_busimate b where b.mate_type_id=1 and b.mate_name=a.f2 and b.mate_code<>a.f1)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行  无级别或数据无效'' from @tab where f3='''' or not (f3 like ''%一级%'' or f3 like ''%二级%'' or f3 like ''%三级%'' or f3 like ''%特三甲%'' or f3 like ''%社区%'' or f3 like ''%基层%'' or f3 like ''%诊所%'' or f3 like ''%药店%'')';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无性质或数据无效'' from @tab where f4='''' or not (f4 like ''%公立%'' or f4 like ''%民营%'' or f4 like ''%连锁%'' or f4 like ''%电商%'' or f4 like ''%厂矿职工%'' or f4 like ''%第三终端%'')';   //公立 民营 连锁 电商 厂矿职工 第三终端
//    sql:=sql+' union all select top 5 ''第''+line_no+''行 无地区编码或数据无效'' from @tab a where f5='''' or not exists (select 1 from tb_district b where a.f5=prov_code or a.f5=b.city_code or a.f5=b.county_code)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无所在地区(省份 城市)或数据无效'' from @tab a where f5='''' or not exists (select 1 from vi_district1 b where f5=b.name or f5=b.prov_name+'' ''+b.name)'; // f5=b.prov_name+'' ''+b.city_name or f5=b.prov_name+'' ''+b.county_name)';
    sql:=sql+' order by cast(line_no as int)) a ';
edit1.text:=sql;
edit2.text:=sql;
//abort;
//从第2行开始导入，导入格式: 1医院主数据编码, 2客户名称, 3级别, 4性质, 5所在地区(省份 城市), 6地址, 7电话, 8特色科室
    ...
    begin
        ...
        sql:=sql+'  id4=case when f4 like ''%公立%'' then 1 when f4 like ''%民营%'' then 2 when f4 like ''%连锁%'' then 3 when f4 like ''%电商%'' then 4 when f4 like ''%厂矿职工%'' then 5 when f4 like ''%第三终端%'' then 6 end';
        ////公立 民营 连锁 电商 厂矿职工 第三终端
        sql:=sql+' from @tab a';
//从第2行开始导入，导入格式: 1医院主数据编码, 2客户名称, 3级别, 4性质, 5所在地区(省份 城市), 6地址, 7电话, 8特色科室
...
//        sql:=sql+' update a set mate_name=b.f2,busi_type_id=b.id3,kind_id=b.id4,district=case when h.city_code<>'''' then h.city_code else h.prov_code end,address=b.f6,phone=b.f7,remark1=b.f8';
        sql:=sql+' update a set mate_name=b.f2,busi_type_id=b.id3,kind_id=b.id4,district=h.code,address=b.f6,phone=b.f7,remark1=b.f8';
...
//        sql:=sql+' inner join vi_district1 h on b.f5=h.prov_name+'' ''+h.city_name or b.f5=h.prov_name+'' ''+h.county_name';
        sql:=sql+' inner join vi_district1 h on b.f5=h.name or b.f5=h.prov_name+'' ''+h.name';
        ...
        sql:=sql+'  select mate_type_id=1,mate_code=f1,mate_name=f2,busi_type_id=a.id3,kind_id=a.id4,district=h.code,';
        sql:=sql+'  address=a.f6,phone=a.f7,remark1=a.f8,creat_by='+inttostr(curuserid)+',creat_dt=getdate()';
        sql:=sql+' from @tab a';
        sql:=sql+' inner join vi_district1 h on a.f5=h.name or a.f5=h.prov_name+'' ''+h.name';
//        sql:=sql+' left join (select distinct prov_code,prov_name,city_code,city_name from tb_district) h on a.f5=h.prov_name+'' ''+h.city_name';
        sql:=sql+' where not exists (select 1 from tb_busimate b where mate_type_id=1 and (a.f1=b.mate_code or a.f2=b.mate_name))';
...
begin
    ...
    case fieldbyname('kind_id').asinteger of   //公立 民营 连锁 电商 厂矿职工 第三终端
        1:  fieldbyname('ckind').asstring:='公立';
        2:  fieldbyname('ckind').asstring:='民营';
        3:  fieldbyname('ckind').asstring:='连锁';
        4:  fieldbyname('ckind').asstring:='电商';
        5:  fieldbyname('ckind').asstring:='厂矿职工';
        6:  fieldbyname('ckind').asstring:='第三终端';
    end;
    fieldbyname('cremark').asstring:=fieldbyname('remark').asstring;
    fieldbyname('cremark1').asstring:=fieldbyname('remark1').asstring;
    fieldbyname('cdistrict').asstring:=fieldbyname('dist1').asstring+' '+fieldbyname('dist2').asstring+' '+fieldbyname('dist3').asstring;
end;
end;
...
(Sender as TdxLookupTreeView).width:=100;
end;
