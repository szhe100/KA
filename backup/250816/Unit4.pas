...
with broker do
begin
    if fieldbyname('zcode').asstring='' then raise exception.create('请获取编码');
...
begin
    ...
    begin
        broker.fieldbyname('mate_id').asinteger:= query1.fieldbyname('mate_code').asstring+' '+query1.fieldbyname('mate_id').asinteger;
...
//从第2行开始导入，导入格式: 格式: 1业务员编码,2业务员名称,3所在地区,4电子邮箱,5联系电话(选填),6收款人,7开户银行,8帐户, 9性别(选填), 10身份证号(选填)
        ...
        sql:=sql+' select zcode=cast(isnull(@cid,200000)+row_number() over(order by line_no) as varchar(7)),zname=f2,qry_code=dbo.fn_getpy(f2),';
