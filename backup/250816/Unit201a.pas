...
type
  Tqryclassimp1 = class(TForm)
    ...
    dxDBGrid1CKWMENG: TdxDBGridColumn;
    dxDBGrid1CZPR03: TdxDBGridColumn;
    dxDBGrid1CZKHJE: TdxDBGridColumn;
    dxDBGrid1price: TdxDBGridColumn;
    dxDBGrid1CKZWI5: TdxDBGridColumn;
    dxDBGrid1CZREC: TdxDBGridColumn;
    ...
    Shape7: TShape;
    qryprice: TBCDField;
    qryNKDGRP: TStringField;
    qryZNKDGRP: TStringField;
    dxDBGrid1NKDGRP: TdxDBGridColumn;
    dxDBGrid1ZNKDGRP: TdxDBGridColumn;
...
uses datamodu, Unit7, Unit16b, Unit16c;
...
begin
    ...
    commandtext:='select price=cast(case when cast(isnull(KONV,0) as decimal(15,4))=0 then case when cast(isnull(KWMENG,0) as decimal(15,4))=0 then 0 else cast(KZWI5 as decimal(15,4))/cast(KWMENG as decimal(15,4)) end';
    commandtext:=commandtext+'  else cast(isnull(KONV,0) as decimal(15,4)) end as decimal(15,4)),';
//  price=cast(KONV as decimal(15,4)),
    commandtext:=commandtext+' a.* from SAP_ZSD_015 a';
...
//    if Trim(dxEdit3.text)<>'' then commandtext:=commandtext+' and a.VBELN='''+Trim(dxEdit3.text)+'''';
...
//    if Trim(dxbuttonedit3.text)<>'' then commandtext:=commandtext+' and a.NAME_FIRST='''+Trim(dxbuttonedit3.text)+'''';    //ASSIGNED_BP
    if Trim(dxbuttonedit3.text)<>'' then commandtext:=commandtext+' and a.ASSIGNED_BP=(select top 1 mate_code from tb_busimate where mate_id='+inttostr(dxbuttonedit3.tag)+')';    //ASSIGNED_BP
...
//edit1.text:=commandtext;
    commandtext:=commandtext+'  order by a.creat_dt';
edit1.text:=commandtext;
    try
        setprogress(1);
        open;
...
with qry do
begin
    if fieldbyname('KWMENG').asstring='' then fieldbyname('CKWMENG').asfloat:=0 else fieldbyname('CKWMENG').asfloat:= fieldbyname('KWMENG').asfloat;  //销售数量
    if fieldbyname('ZKHJE').asstring='' then fieldbyname('CZKHJE').asfloat:=0 else fieldbyname('CZKHJE').asfloat:= fieldbyname('ZKHJE').asfloat;  //考核金额
    fieldbyname('CKZWI5').asfloat:= fieldbyname('KZWI5').asfloat; //销售金额
    if fieldbyname('ZREC').asstring='' then fieldbyname('CZREC').asfloat:=0 else fieldbyname('CZREC').asfloat:= fieldbyname('ZREC').asfloat; //已收款金额
{
    if fieldbyname('CZKHJE').asfloat=0
        then fieldbyname('CZKONV').asfloat:= 0     //销售单价
        else fieldbyname('CZKONV').asfloat:= fieldbyname('CKZWI5').asfloat/fieldbyname('CKWMENG').asfloat;
}
    if fieldbyname('ZPR03').asstring='' then fieldbyname('CZPR03').asfloat:=0 else fieldbyname('CZPR03').asfloat:= fieldbyname('ZPR03').asfloat; //考核单价
{
    if fieldbyname('price').asfloat=0 then
    begin
        if fieldbyname('CKWMENG').asfloat=0 then fieldbyname('Cprice').asfloat:=0 else fieldbyname('Cprice').asfloat:= fieldbyname('KZWI5').asfloat/fieldbyname('CKWMENG').asfloat; //考核单价
    end;
}
end;
{
