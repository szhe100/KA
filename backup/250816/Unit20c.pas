...
type
  Tsetmatestock = class(TForm)
    ...
    stockqryqty0: TBCDField;
    stockqryimp_qty: TBCDField;
...
begin
    ...
    begin
        ...
        commandtext:='sp_qrymedstock '+inttostr(my)+','+inttostr(mm)+',';
...
//导入Excel文件生成单据(格式:1商业公司标准名称id1,2商业公司药品编码id2,3库存日期dt,4月期末库存数量)
    ...
    begin
        with dm.pubqry do
        begin
            if sheet.cells[j,1].text='' then serror:=serror+#13#10+'第'+inttostr(j)+'行 无商业公司标准名称'
            else
            begin
                if active then close;
                commandtext:='select top 1 mate_id from tb_busimate where mate_type_id=2 and mate_name='''+sheet.cells[j,1].text+'''';
                open;
                if recordcount>0 then id1:=fieldbyname('mate_id').asinteger else serror:=serror+#13#10+'第'+inttostr(j)+'行 商业公司标准名称有误';
            end;
            if sheet.cells[j,2].text='' then serror:=serror+#13#10+'第'+inttostr(j)+'行 无商业公司药品编码'
            else
            begin
                if active then close;
                commandtext:='select top 1 med_id from tb_linkword where zdesc='''+sheet.cells[j,2].text+'''';
                open;
                if recordcount>0 then id2:=fieldbyname('med_id').asinteger else serror:=serror+#13#10+'第'+inttostr(j)+'行 商业公司药品编码有误';
            end;
...
begin
{
    ...
    fieldbyname('cqtydiff').asfloat:=fieldbyname('bal').asfloat-fieldbyname('imp_qty').asfloat;
    fieldbyname('cadj_bal1').asfloat:=fieldbyname('bal').asfloat+fieldbyname('adj_qty1').asfloat;
    fieldbyname('cadj_bal2').asfloat:=fieldbyname('bal').asfloat+fieldbyname('adj_qty2').asfloat;
    fieldbyname('Cadj_diff').asfloat:=fieldbyname('cadj_bal1').asfloat-fieldbyname('cadj_bal2').asfloat;
...
procedure Tsetmatestock.SpeedButton2Click(Sender: TObject);
var
    XL,Sheet: Variant;
    SFileName: string;
    mi,J,m1,m2: integer; 
...
//从第2行开始导入，格式: 1商业公司标准名称, 2商业公司药品编码, 3生产批号(选填), 4库存日期, 5月期末库存数
    mi:=0;j:=2;  //第1行开始 每行一单
    sql:='declare @tab table (line_no varchar(3),f1 varchar(100),f2 varchar(100),f3 varchar(100),f4 varchar(100),f5 varchar(100),mate_id int,med_id int)';
    ...
    sql:=sql+' select top 10 * from ( select top 5 info=''第''+line_no+''行 无商业公司标准名称或数据无效'' from @tab a where f1='''' or not exists (select 1 from tb_busimate b where b.mate_type_id=2 and b.mate_name=a.f1)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 商业公司药品编码或数据无效'' from @tab a where f2='''' or not exists (select 1 from tb_linkword b,tb_busimate c where b.type_id=1 and b.mate_id1=c.mate_id and c.mate_type_id=2 and c.mate_name=f1 and b.zdesc=a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 库存日期或数据无效'' from @tab where f4='''' or try_cast(f4 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 月期末库存数或数据无效'' from @tab where f5='''' or try_cast(f5 as decimal(15,2)) is null';
    sql:=sql+' ) a';
//从第2行开始导入，格式: 1商业公司标准名称, 2商业公司药品编码, 3生产批号(选填), 4库存日期, 5月期末库存数
edit1.text:=sql;
//    setprogress(0);
...
//从第2行开始导入，格式: 1商业公司标准名称, 2商业公司药品编码, 3生产批号(选填), 4库存日期, 5月期末库存数
        ...
        sql:=sql+' update a set mate_id=b.mate_id,med_id=c.med_id';
        sql:=sql+' from @tab a left join (select mate_id,mate_name from tb_busimate where mate_type_id=2) b on a.mate_name=a.f1';
        sql:=sql+' inner join tb_linkword c on c.type_id=1 and c.mate_id1=b.mate_id and c.zdesc=a.f2';

        sql:=sql+' update a set occur_dt=cast(f4 as datetime),qty=cast(f5 as decimal(15,2)),modify_by='+inttostr(curuserid)+',modify_dt=getdate()';
        sql:=sql+' from tb_matestock a,@tab b';
        sql:=sql+' where a.mate_id=b.mate_id and a.med_id=b.med_id and a.bat_cd=b.f3';  // where ';
        sql:=sql+' set @cnt1=@@ROWCOUNT';

        sql:=sql+' insert into tb_matestock (mate_id,med_id,bat_cd,occur_dt,qty,creat_by,creat_dt)';
        sql:=sql+' select mate_id,med_id,bat_cd,occur_dt,qty,creat_by='+inttostr(curuserid)+',creat_dt=getdate() from @tab b ';
        sql:=sql+' where not exists (select 1 from tb_matestock a where a.mate_id=b.mate_id and a.med_id=b.med_id and a.bat_cd=b.f3)';
...
//exit;
    ...
    begin
        //SpeedButton3Click(nil);
        //medprice.Last;
        MessageBox(0,pchar('已新增'+inttostr(m1)+'条, 更新'+inttostr(m2)+'条记录'),'请注意',MB_OK+MB_ICONInformation);
    end;
end;
