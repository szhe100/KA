...
type
  Tsetmedexport = class(TForm)
    ...
    Label26: TLabel;
    Label29: TLabel;
    cdsbill_dtlterminal_id: TStringField;
...
begin
    ...
    setprogress(1);
//新格式:1供货单位, 2购货单位, 3单据摘要, 4发生日期, 5单据类别, 6物料编码, 7数量, 8批号, 9有效期至, 10商业价, 11渠道名称, 12子渠道名称, 13终端ID号
    j:=2;  //第1行开始 每行一单   //mi:=0;
    sql:='declare @tab table (line_no varchar(3),f1 varchar(300),f2 varchar(300),f3 varchar(100),f4 varchar(100),f5 varchar(100),f6 varchar(100),f7 varchar(100),f8 varchar(100),f9 varchar(100),f10 varchar(100),f11 varchar(100),f12 varchar(100),f13 varchar(100))';
...
//    sql:=sql+' union all select top 5 ''第''+line_no+''行 无业务类别或数据无效'' from @tab where f10='''' or f10 not in (''商业'',''分销'')';
    ...
    sql:=sql+' order by line_no) a'; // order by info';
//新格式:1供货单位, 2购货单位, 3单据摘要, 4发生日期, 5单据类别, 6物料编码, 7数量, 8批号, 9有效期至, 10商业价, 11渠道名称, 12子渠道名称, 13终端ID号
...
//        sql:=sql+'  insert into tb_bill_dtl (bod_id,med_id,bat_cd,valid_dt,price,qty,amot,channel_id,channel_dtl_id)';
        ...
        sql:=sql+'  insert into tb_bill_dtl (bod_id,med_id,bat_cd,valid_dt,price,price3,qty,amot,channel_id,channel_dtl_id,terminal_id)';
        ...
        sql:=sql+'      amot=round(cast(a.f10 as decimal(15,4))*cast(a.f7 as decimal(15,4)),2),c.obj_code,d.obj_code,a.f13';
