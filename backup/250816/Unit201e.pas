...
begin
    ...
    commandtext:=commandtext+' left join (select mate_id,mate_code from tb_busimate where mate_type_id=2) b on a.KUNNR=b.mate_code';  //
//    commandtext:=commandtext+' left join tb_medprice c on c.rec_id=dbo.fn_getmedpricerecid(a.MATNR,a.VTWEG,a.KDGRP,a.ZREGIO,a.ZCITYNUM,a.carry_dt)';  //
    commandtext:=commandtext+' left join tb_medprice c on c.rec_id=dbo.fn_getmedpricerecid2(b.mate_id,a.MATNR,a.carry_dt)';  //fn_getmedpricerecid2](@mateid int,@materialcode varchar(20),@dt datetime)
...
//    commandtext:=commandtext+' left join tb_medprice j on j.rec_id=dbo.fn_getmedpricerecid(m.material_code,g.obj_code,h.obj_code,i.prov_code,i.city_code,a.carry_dt)';
    commandtext:=commandtext+' left join tb_medprice j on j.rec_id=dbo.fn_getmedpricerecid2(d.mate_id,m.material_code,a.carry_dt)';  //fn_getmedpricerecid2](@mateid int,@materialcode varchar(20),@dt datetime)
...
procedure Tqryclassimp5.dxDateEdit1DblClick(Sender: TObject);
...
procedure Tqryclassimp5.dxDateEdit2DblClick(Sender: TObject);
var Year, Month, Day: Word;
begin
...
procedure Tqryclassimp5.dxLookupTreeView1CloseUp(Sender: TObject;
  Accept: Boolean);
begin
