...
type
  Tsetmedimport = class(TForm)
    ...
    dxDBGrid1ZVTWEG: TdxDBGridColumn;
...
//    commandtext:=commandtext+' where VTWEG in (''10'',''19'')';
...
//    commandtext:='update tb_bill set bod_type_id=bod_type_id+100,delete_by='+inttostr(curuserid)+',delete_dt=getdate() where bod_id='+cdsbill.fieldbyname('bod_id').asstring;
    commandtext:='delete from tb_bill where rec_id='+cdsbill.fieldbyname('rec_id').asstring;
    execute;
    close;
end;
cdsbill.delete;
...
//        gifimage1.visible:=(fieldbyname('bod_status_id').asinteger=1);  //审核 图章显示
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('rec_id').asinteger then
            ...
            begin
                if active then close;
                commandtext:='select a.*,m.med_name,m.specifi,m.pdt_place from SAP_ZSD_015 a inner join tb_medicine m on a.MATNR=m.material_code';
//                commandtext:='select a.*,m.med_name,m.specifi,m.pdt_place from SAP_ZSD_015 a inner join tb_medicine m on a.MATNR=m.material_code1';
...
procedure Tsetmedimport.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1交货单号, 2订单编号, 3渠道, 4子渠道, 5业务负责人, 6供货单位, 7商业公司, 8销售人员, 9单据摘要, 10发运日期, 11物料编码, 12单价, 13数量, 14批号, 15有效期至, 16考核单价, 17考核金额, 18收款金额
//    sbodcd:='导入数据已生成下列单据'+#13#10+'---------------------------';
    ...
    sql:=sql+' f11 varchar(100),f12 varchar(100),f13 varchar(100),f14 varchar(100),f15 varchar(100),f16 varchar(100),f17 varchar(100),f18 varchar(100))';
    sql:=sql+' insert into @tab (line_no,f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13,f14,f15,f16,f17,f18)';
...
//        sql:=sql+''','''+Trim(sheet.cells[j,11].text)+''','''+Trim(sheet.cells[j,12].text)+''','''+Trim(sheet.cells[j,13].text)+''','''+Trim(sheet.cells[j,14].text)+''','''+Trim(sheet.cells[j,15].text)+'''';
        sql:=sql+''','''+Trim(sheet.cells[j,11].text)+''',replace('''+Trim(sheet.cells[j,12].text)+''','','',''''),replace('''+Trim(sheet.cells[j,13].text)+''','','',''''),'''+Trim(sheet.cells[j,14].text)+''','''+Trim(sheet.cells[j,15].text); //+'''';
        sql:=sql+''',replace('''+Trim(sheet.cells[j,16].text)+''','','',''''),replace('''+Trim(sheet.cells[j,17].text)+''','','',''''),replace('''+Trim(sheet.cells[j,17].text)+''','','','''')';
        j:=j+1;
    end;
    XL.Quit;
//从第2行开始导入，格式: 1交货单号, 2订单编号, 3渠道, 4子渠道, 5业务负责人, 6供货单位, 7商业公司, 8销售人员, 9单据摘要, 10发运日期, 11定价参考物料编码, 12单价, 13数量, 14批号, 15有效期至, 16考核单价, 17考核金额, 18收款金额
    sql1:=sql;  //导入数据无错误时，用sql1 插入数据
    sql:=sql+'select top 10 * from ( select top 5 info=''第''+line_no+''行无交货单号或已有该单号'' from @tab a where f1='''' or exists (select 1 from vi_ZSD_015 b where b.GBELN=a.f1)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无订单编号'' from @tab where f2=''''';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无渠道编码或数据无效'' from @tab a where f3='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.obj_code=a.f3)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无子渠道编码或数据无效'' from @tab a where f4='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.obj_code=a.f4)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无业务负责人或数据无效'' from @tab a where f5='''' or not exists (select 1 from tb_busimate b where mate_type_id=4 and b.mate_name=a.f5)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无供货单位或数据无效'' from @tab a where f6='''' or not exists (select 1 from tb_busimate b where mate_type_id=2 and b.mate_name=a.f6)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无商业公司或数据无效'' from @tab a where f7='''' or not exists (select 1 from tb_busimate b where mate_type_id=2 and b.mate_name=a.f7)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无销售人员或数据无效'' from @tab a where f8='''' or not exists (select 1 from tb_staff b where sta_type_id=0 and dept_id=43 and b.zname=a.f8)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无发运日期或数据无效'' from @tab where f10='''' or try_cast(f10 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无物料编码或数据无效'' from @tab a where f11='''' or not exists (select 1 from tb_medicine b where b.material_code=a.f11)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无单价或数据无效'' from @tab where f12='''' or try_cast(f12 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无数量或数据无效'' from @tab where f13='''' or try_cast(f13 as decimal(15,4)) is null';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无批号'' from @tab where f14=''''';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无有效期至或数据无效'' from @tab where f15='''' or try_cast(f15 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 有效期数据无效'' from @tab where f15<>'''' and try_cast(f15 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无考核单价或数据无效'' from @tab where f16='''' or try_cast(f16 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无考核金额或数据无效'' from @tab where f17='''' or try_cast(f17 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无收款金额或数据无效'' from @tab where f18='''' or try_cast(f18 as decimal(15,4)) is null';
...
//        sql:=sql+' declare @cnt int'; // @bodcd varchar(20),@bodid int,@lineno varchar(10),@mi int,@bcd varchar(2000)';
        sql:=sql+' insert into SAP_ZSD_015 (BEZEI,GBELN,VBELN,VTWEG,VTEXT,KDGRP,ZKDGRP,NAME_FIRST,ZVKORG,NAME1,ZSALESNAM,bod_desc,WADAT_IST,carry_dt,MATNR,ZKONV,KONV,price,KWMENG,qty,KZWI5,amot,CHARG,VFDAT,AUDAT,ZPR03,amot1,amot2, creat_dt,ZTERNAM,creat_by)';
        sql:=sql+' select BEZEI=''标准订单'',GBELN=f1,VBELN=f2,VTWEG=a.f3,VTEXT=c.zdesc,KDGRP=a.f4,ZKDGRP=d.zdesc,NAME_FIRST=f5,ZVKORG=f6,NAME1=f7,ZSALESNAM=f8,bod_desc=f9,WADAT_IST=format( cast(f10 as datetime),''yyyy-MM-dd HH:mm:ss''),carry_dt=cast(f10 as datetime),';
        ...
        sql:=sql+'  CHARG=f14,VFDAT=f15,AUDAT=format( cast(getdate() as datetime),''yyyy-MM-dd HH:mm:ss''),ZPR03=f16,amot1=cast(f18 as decimal(15,4)),amot2=cast(f17 as decimal(15,4)),';
        sql:=sql+'  creat_dt=getdate(),ZTERNAM=dbo.fn_staff_name('+inttostr(curuserid)+'),creat_by='+inttostr(curuserid);
        sql:=sql+' from @tab a';
        sql:=sql+' left join (select obj_id,obj_code,zdesc from tb_object where obj_type_id=11) c on c.obj_code=a.f3';
        sql:=sql+' left join (select obj_id,obj_code,zdesc from tb_object where obj_type_id=12) d on d.obj_code=a.f4';
...
edit1.text:=commandtext;
        try
            ...
            if mi=0 then MessageBox(0,pchar('无可导入单据记录(已有交货单号不再重复导入)'),'请注意',MB_OK+MB_ICONInformation)
            else
            begin
                SpeedButton5Click(nil);
                cdsbill.Last;
