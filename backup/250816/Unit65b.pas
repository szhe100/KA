...
type
  Tsettrustruleapply2 = class(TForm)
...
var
  settrustruleapply2: Tsettrustruleapply2;
...
procedure Tsettrustruleapply2.setupdatestatus;    // 编辑状态
begin
...
procedure Tsettrustruleapply2.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsettrustruleapply2.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsettrustruleapply2.FormCloseQuery(Sender: TObject;
...
procedure Tsettrustruleapply2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsettrustruleapply2.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsettrustruleapply2.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with cdsbill_dtl do
...
procedure Tsettrustruleapply2.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsettrustruleapply2.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=24 and a.bod_status_id in (1,3,4)';  //a.comp_id='+inttostr(compid)+' and 
...
procedure Tsettrustruleapply2.FormCreate(Sender: TObject);
begin
...
procedure Tsettrustruleapply2.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsettrustruleapply2.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsettrustruleapply2.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsettrustruleapply2.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsettrustruleapply2.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsettrustruleapply2.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsettrustruleapply2.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsettrustruleapply2.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsettrustruleapply2.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsettrustruleapply2.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsettrustruleapply2.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsettrustruleapply2.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsettrustruleapply2.N1Click(Sender: TObject);
...
procedure Tsettrustruleapply2.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsettrustruleapply2.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsettrustruleapply2.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsettrustruleapply2.brokermedtlCalcFields(DataSet: TDataSet);
begin
with brokermedtl do
begin
...
procedure Tsettrustruleapply2.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsettrustruleapply2.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsettrustruleapply2.SpeedButton2Click(Sender: TObject);
begin
