...
type
  Tsetmedprice5 = class(TForm)
    ...
    medpricemate_code: TStringField;
    medpricemate_name: TStringField;
...
procedure Tsetmedprice5.FormActivate(Sender: TObject);
begin
{
with dm.district do
...
procedure Tsetmedprice5.FormCloseQuery(Sender: TObject;
...
procedure Tsetmedprice5.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetmedprice5.SpeedButton3Click(Sender: TObject);
var mi: integer;
begin
with medprice do
begin
    ...
    commandtext:='select top 20000 med_unit=dbo.fn_obj_desc(m.unit_id),a.*,b.mate_code,b.mate_name,m.*,';
...
//    commandtext:=commandtext+' channel=b.zdesc,channel_dtl=c.zdesc,cust_group=e.zdesc,creater=f.zname';
    ...
    commandtext:=commandtext+' inner join tb_busimate b on a.mate_id=b.mate_id';
...
//        commandtext:=commandtext+' inner join (select channel_id,channel_dtl_id,district_id,med_id,rec_id=max(rec_id) from tb_medprice where type_id=3 group by channel_id,channel_dtl_id,district_id,med_id) h on a.rec_id=h.rec_id';
        commandtext:=commandtext+' inner join (select a.mate_id,m.material_code1,rec_id=max(rec_id) from tb_medprice a,tb_medicine m where a.type_id=5 and a.med_id=m.med_id group by a.mate_id,m.material_code1) h on a.rec_id=h.rec_id';
    end;
    commandtext:=commandtext+' where type_id=5'; //设置商业采购推广规则
    if dxCheckEdit1.Checked then
    begin
        if Trim(dxButtonEdit1.text)<>'' then commandtext:=commandtext+' and m.material_code1=(select top 1 material_code1 from tb_medicine where med_id='+inttostr(dxButtonEdit1.tag)+')';
        if Trim(dxButtonEdit3.text)<>'' then commandtext:=commandtext+' and b.mate_id='+inttostr(dxButtonEdit3.tag);
...
procedure Tsetmedprice5.FormCreate(Sender: TObject);
begin
{
...
procedure Tsetmedprice5.N3Click(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure Tsetmedprice5.N1Click(Sender: TObject);
begin
with dxdbgrid1 do Columns[GetAbsoluteColumnIndex(FocusedColumn)].Visible:=false;
end;

procedure Tsetmedprice5.N2Click(Sender: TObject);
var i,mi: integer;
begin
setprogress(1);
with dxdbgrid1 do
...
procedure Tsetmedprice5.N6Click(Sender: TObject);
begin
if FileExists(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini')
    then dxDBGrid1.LoadFromIniFile(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini');//加载
end;

procedure Tsetmedprice5.medpriceAfterPost(DataSet: TDataSet);
begin
if medprice.ChangeCount>0 then myupdaterefresh(medprice,'tb_medprice','rec_id','');
end;

procedure Tsetmedprice5.medpriceBeforePost(DataSet: TDataSet);
begin
{
with medprice do
begin
...
procedure Tsetmedprice5.SpeedButton6Click(Sender: TObject);
var
    XL,Sheet: Variant;
    SFileName: string;
    mi,J: integer; 
...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1渠道名称,2子渠道名称,3省份,4城市,5商业公司, 6关联编码, 7中标价/开票价, 8金额, 9定价单位,10启用日期,11备注
    mi:=0;j:=2;  //第1行开始 每行一单
    ...
    begin
        ...
        sql:=sql+''','''+Trim(sheet.cells[j,6].text)+''','''+Trim(sheet.cells[j,7].text)+''','''+Trim(sheet.cells[j,8].text)+''','''+Trim(sheet.cells[j,9].text)+''','''+Trim(sheet.cells[j,10].text)+''','''+Trim(sheet.cells[j,11].text)+'''';      //+''','''+Trim(sheet.cells[j,10].text
        j:=j+1;
    end;
    XL.Quit;
...
//从第2行开始导入，格式: 1渠道名称,2子渠道名称,3省份,4城市,5商业公司, 6关联编码, 7中标价/开票价, 8金额, 9定价单位,10启用日期,11备注
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无商业公司或数据无效'' from @tab a where f5='''' or not exists (select 1 from tb_busimate b where b.mate_type_id=2 and b.mate_name=f5)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无物料编码或数据无效'' from @tab a where f6='''' or not exists (select 1 from tb_medicine b where b.material_code=f6)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无中标价/开票价或数据无效'' from @tab where f7='''' or try_cast(f7 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无金额或数据无效'' from @tab where f8='''' or try_cast(f8 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无定价单位或数据无效'' from @tab where f9='''''; // or try_cast(f8 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无启用日期或数据无效'' from @tab where f10='''' or try_cast(f10 as datetime) is null';
...
//rec_id,comp_id,type_id,channel_id,channel_dtl_id,cust_group_id,district_id,med_id,price,price_unit,price1,valid_from,valid_to,creat_by,creat_dt
        ...
        sql:=sql1+' insert into tb_medprice (comp_id,type_id,channel_id,channel_dtl_id,district_id,mate_id,med_id,price,price_unit,price1,valid_from,zdesc,creat_by,creat_dt)';   //valid_to,

        sql:=sql+'  select comp_id='+inttostr(compid)+',type_id=5,channel_id=b.obj_code,channel_dtl_id=c.obj_code,district_id=case when d.county_code<>'''' then d.county_code when d.city_code<>'''' then d.city_code else d.prov_code end,'; //type_id=3 设置学术标准政策
        sql:=sql+'      f.mate_id,m.med_id,price=f8,price_unit=f9,price1=f7,valid_from=cast(f10 as datetime),zdesc=f11,'; //,valid_to=cast(f10 as datetime),';
...
//        sql:=sql+' left join (select distinct city_code,city_name=prov_name+'' ''+city_name from tb_district) d on d.city_name= a.f3+'' ''+a.f4';
        sql:=sql+' left join (select prov_code,prov_name,city_code,city_name,county_code,county_name from tb_district) d on d.prov_name= a.f3 and (d.city_name=a.f4 and d.county_name='''' or d.county_name=a.f4)';
        sql:=sql+' left join (select mate_id,mate_name from tb_busimate where mate_type_id=2) f on f.mate_name=a.f5';
        sql:=sql+' left join tb_medicine m on m.material_code=a.f6';
...
procedure Tsetmedprice5.dxButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedprice5.DSmedpriceDataChange(Sender: TObject;
  Field: TField);
begin
with medprice do
begin
...
procedure Tsetmedprice5.dxLookupTreeView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if key=vk_delete then dxLookupTreeView1.text:='';
end;

procedure Tsetmedprice5.SpeedButton4Click(Sender: TObject);
var mi: integer;
    ds: string;
begin
...
if MessageBox(0,'确定加入新的商业采购推广规则','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
if dxLookupTreeView1.Text='' then ds:='null' else ds:=dm.district.fieldbyname('code').asstring;
with dm.pubqry do
begin
    if active then close;
    commandtext:='insert into tb_medprice (type_id,district_id,mate_id,med_id,creat_by,creat_dt) values';
    commandtext:=commandtext+' (5,'+ds+','+inttostr(dxbuttonedit3.tag)+','+inttostr(dxbuttonedit1.tag)+','+inttostr(curuserid)+',getdate())';
...
procedure Tsetmedprice5.medpriceBeforeDelete(DataSet: TDataSet);
begin
...
procedure Tsetmedprice5.medpriceCalcFields(DataSet: TDataSet);
begin
with medprice do
begin
...
procedure Tsetmedprice5.SpeedButton2Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag);
end;

procedure Tsetmedprice5.dxLookupTreeView1DropDown(Sender: TObject);
begin
setprogress(1);
end;

procedure Tsetmedprice5.dxLookupTreeView1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
setprogress(0);
end;

procedure Tsetmedprice5.medpriceBeforeEdit(DataSet: TDataSet);
begin
with medprice do
begin
...
procedure Tsetmedprice5.dxButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
