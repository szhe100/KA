...
procedure Tsetbusimate7.FormActivate(Sender: TObject);
begin
with dm.staff do
...
procedure Tsetbusimate7.FormCloseQuery(Sender: TObject;
...
procedure Tsetbusimate7.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetbusimate7.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2
end;

procedure Tsetbusimate7.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetbusimate7.wwDBNavigator1InsertClick(Sender: TObject);
begin
with pagecontrol1 do
    if activepage=tabsheet1 then activepage:=tabsheet2;
end;

procedure Tsetbusimate7.DSbusimateDataChange(Sender: TObject;
  Field: TField);
begin
with busimate do
...
procedure Tsetbusimate7.SpeedButton2Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag)
end;

procedure Tsetbusimate7.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetbusimate7.SpeedButton5Click(Sender: TObject);
var mi: integer;
begin
with busimate do
begin
    ...
    commandtext:=commandtext+' where a.mate_type_id=7';
...
procedure Tsetbusimate7.DSbusimateStateChange(Sender: TObject);
begin
with busimate do
    if active=true then label25.caption:=formatfloat('[###,###]',recordcount);
end;

procedure Tsetbusimate7.dxDBEdit5Change(Sender: TObject);
var sw: widestring;
    c,spy:string;
    i:integer;
begin
...
procedure Tsetbusimate7.busimateAfterPost(DataSet: TDataSet);
var i: integer;
begin
with busimate do
begin
...
procedure Tsetbusimate7.busimateBeforeDelete(DataSet: TDataSet);
begin
with busimate do
...
procedure Tsetbusimate7.busimateBeforePost(DataSet: TDataSet);
begin
with busimate do
begin
...
begin
    ...
    begin
        if active=True then close;
        commandtext:='select top 1 1 from tb_busimate where mate_type_id=7 and mate_name='''+busimate.fieldbyname('mate_name').asstring+'''';
...
begin
    ...
    begin
        ...
        fieldbyname('mate_type_id').asinteger:=7; // 快递公司
...
procedure Tsetbusimate7.SpeedButton6Click(Sender: TObject);
var
    XL,Sheet: Variant;
    SFileName: string;
    mi,J: integer;
        ...
        begin
{
            ...
            if active=true then close;
            commandtext:='if exists (select 1 from tb_busimate where mate_type_id=7 and mate_name='''+s1+''')';
            commandtext:=commandtext+' update tb_busimate set phone='''+s2+'''';
            commandtext:=commandtext+' where mate_type_id=7 and mate_name='''+s1+'''';
            commandtext:=commandtext+' else insert into tb_busimate (mate_name,qry_code,mate_type_id,phone,creat_by,creat_dt)';
            commandtext:=commandtext+' values ('''+s1+''',dbo.fn_getpy('''+s2+'''),7,'''+s2+''','+inttostr(curuserid)+',getdate())';
            execute;
        end;
        j:=j+1;
    end;
    XL.Quit;
    SpeedButton5Click(nil);
    MessageBox(0,pchar('Excel数据已导入(共导入'+inttostr(mi)+'个快递公司,已有快递公司则更新数据)'),'请注意',MB_OK+MB_ICONInformation);
end;

procedure Tsetbusimate7.PageControl1Change(Sender: TObject);
begin
...
procedure Tsetbusimate7.busiframeBeforeDelete(DataSet: TDataSet);
begin
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetbusimate7.busiframeAfterPost(DataSet: TDataSet);
begin
if busiframe.ChangeCount>0 then myupdaterefresh(busiframe,'tb_agentdata','rec_id','');
end;

procedure Tsetbusimate7.dxLookupTreeView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if key=vk_delete then dxLookupTreeView1.text:='';
end;

procedure Tsetbusimate7.busimateAfterDelete(DataSet: TDataSet);
begin
with busimate do
begin
