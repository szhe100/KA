...
with busimate do
begin
    if (fieldbyname('mate_name').asstring='') then raise Exception.Create('供应商名称不可为空');
    if (fieldbyname('district').asinteger=0) then raise Exception.Create('请设置供应商所属地区');
end;
...
begin
    ...
    begin
        ...
        open;
        if recordcount>0 then raise Exception.Create('已有相同名称的供应商，不可重复录入');
    end;
end;
with busimate do
begin
    ...
    begin
        ...
        fieldbyname('mate_type_id').asinteger:=3; // 1医院 2商业公司 3供应商
...
begin
    ...
    if MessageBox(0,'确定导入供应商','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
    setprogress(1);
//从第2行开始导入，导入格式: 1供应商编码,2供应商名称,3所在地区,4属性,5联系人,6联系电话
    mi:=0;j:=2;  //第1行开始 每行一单
...
//edit1.text:=commandtext;            
    ...
    XL.Quit;
    SpeedButton5Click(nil);
    MessageBox(0,pchar('Excel数据已导入(共导入'+inttostr(mi)+'个供应商)'),'请注意',MB_OK+MB_ICONInformation);
end;
...
with busimate do
    case fieldbyname('busi_type_id').asinteger of
        1:  fieldbyname('Cbusitype').asstring:='厂家';
        2:  fieldbyname('Cbusitype').asstring:='子公司';
    end;
end;
