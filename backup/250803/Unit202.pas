...
procedure Tqryclassexp.FormCloseQuery(Sender: TObject;
...
procedure Tqryclassexp.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tqryclassexp.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
begin
    ...
    commandtext:='select b.dtl_id,a.bod_id,a.bod_cd,a.bod_status_id,d.med_code,d.material_code,d.material_code1,d.med_name,d.specifi,d.pdt_place,d.qtyperpack,';
//    commandtext:=commandtext+' cdistrict=dbo.fn_getdistrictname (j.district,1)+'' ''+dbo.fn_getdistrictname (j.district,2),'; //dbo.fn_getdistrict(j.district),';
    commandtext:=commandtext+' cdistrict=dbo.fn_getdistrict1(j.district),'; //dbo.fn_getdistrict(j.district),';
...
//    commandtext:=commandtext+' level1=dbo.fn_obj_desc(k.level_id1),level2=dbo.fn_obj_desc(k.level_id2),level3=dbo.fn_obj_desc(k.level_id3),';
    ...
    commandtext:=commandtext+' k.VKGRP,k.VKGRP_TXT,k.PARTNER,leader=k.BU_SORT1,a.data_type_id,data_type=dbo.fn_obj_desc(a.data_type_id),';  //  leader=p.zname customer=dbo.fn_mate_name(a.dst_id),
...
//    commandtext:=commandtext+' mate_name1=dbo.fn_mate_name(a.src_id),mate_name2=dbo.fn_mate_name(a.dst_id),mate_name3=f.mate_name,mate_name4=g.mate_name,mate_name5=h.mate_name,';
    commandtext:=commandtext+' mate_name1='''','; //dbo.fn_getlevelmatename(d.material_code1,j.district,1,a.carry_dt),';
    commandtext:=commandtext+' mate_name2='''','; //dbo.fn_getlevelmatename(d.material_code1,j.district,2,a.carry_dt),';
...
//    commandtext:=commandtext+' b.bat_cd,broker=dbo.fn_staff_name(h.sta_id),creater=dbo.fn_staff_name(a.creat_by),checker=dbo.fn_staff_name(a.check_by)';
...
//    commandtext:=commandtext+' left join tb_brokermed h on h.rec_id =dbo.fn_getbrokermedrecid(a.src_id,a.dst_id,b.med_id,a.carry_dt)';
...
//        if Trim(dxLookupTreeView1.text)<>'' then commandtext:=commandtext+' and (c.prov_name like '''+dm.district.fieldbyname('name0').asstring+'%'' or c.city_name like '''+dm.district.fieldbyname('name0').asstring+'%'')';
    ...
    if Trim(dxEdit6.text)<>'' then commandtext:=commandtext+' and (d.material_code='''+Trim(dxEdit6.text)+''' or d.material_code1='''+Trim(dxEdit6.text)+''')'; //d.material_code,d.material_code1
...
procedure Tqryclassexp.dxButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tqryclassexp.dxButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tqryclassexp.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tqryclassexp.dxEdit1Click(Sender: TObject);
begin
...
procedure Tqryclassexp.dxButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tqryclassexp.FormActivate(Sender: TObject);
begin
with dm.district do
...
procedure Tqryclassexp.dxButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickbroker do
    if showmodal= mrOK then
...
procedure Tqryclassexp.N3Click(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure Tqryclassexp.N1Click(Sender: TObject);
begin
with dxdbgrid1 do Columns[GetAbsoluteColumnIndex(FocusedColumn)].Visible:=false;
end;

procedure Tqryclassexp.N2Click(Sender: TObject);
var i,mi: integer;
begin
setprogress(1);
with dxdbgrid1 do
...
procedure Tqryclassexp.N6Click(Sender: TObject);
begin
if FileExists(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini')
    then dxDBGrid1.LoadFromIniFile(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini');//加载
end;

procedure Tqryclassexp.qryCalcFields(DataSet: TDataSet);
begin
with qry do
begin
...
procedure Tqryclassexp.SpeedButton3Click(Sender: TObject);
var st,ss,sm: string;
begin
...
procedure Tqryclassexp.SpeedButton1Click(Sender: TObject);
...
procedure Tqryclassexp.SpeedButton4Click(Sender: TObject);
//var h: boolean;
begin
Timer1.Enabled:=True;
...
procedure Tqryclassexp.Timer1Timer(Sender: TObject);
//var a,b:PAnsiChar;
var h:HWND;
begin
...
procedure Tqryclassexp.dxDBGrid1DblClick(Sender: TObject);
begin
...
procedure Tqryclassexp.SpeedButton6Click(Sender: TObject);
var dtlid: integer; //,mi
    sbodid: string;
begin
with qry do
...
procedure Tqryclassexp.dxLookupTreeView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if key=vk_delete then dxLookupTreeView1.text:='';
end;

procedure Tqryclassexp.N7Click(Sender: TObject);
begin
with qry do
	if (active=False) or (recordcount=0) then exit;
with dm.pubqry do
...
procedure Tqryclassexp.dxDBGrid1CustomDrawFooter(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
...
procedure Tqryclassexp.dxLookupTreeView1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
setprogress(0);
end;
