...
with pickmedicine.query1 do
    if (active=false) or (recordcount=0) then raise Exception.Create('请选择盘点品种');
with cdsbill_dtl do
begin
...
//    fieldbyname('med_code').asstring:=pickmedicine.query1.fieldbyname('med_code').asstring;
    fieldbyname('material_code').asstring:=pickmedicine.query1.fieldbyname('material_code').asstring;
...
//    commandtext:=commandtext+' ,a.channel_id,channel=dbo.fn_obj_desc(a.channel_id),a.channel_dtl_id,channel_dtl=dbo.fn_obj_desc(a.channel_dtl_id)';
    commandtext:=commandtext+' ,channel=dbo.fn_obj_desc(a.channel_id),channel_dtl=dbo.fn_obj_desc(a.channel_dtl_id)';
    ...
    commandtext:=commandtext+' where a.bod_type_id=10 and a.busi_type=1'; // and a.comp_id='+inttostr(compid); //busi_type: 1 商业公司盘点单；2 连锁终端盘点单
...
begin
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active then close;
                commandtext:='select a.*,b.med_name,b.material_code,b.material_code1,b.specifi,b.pdt_place,b.med_code,b.qtyperpack,med_unit=c.zdesc';
...
//    commandtext:=commandtext+'  and isnull(b.price,0)=0 and b.med_id=m.med_id';
    commandtext:=commandtext+'  and (isnull(b.price,0)=0 or isnull(b.qty,0)=0 or b.qty<>CAST(b.qty AS INT)) and b.med_id=m.med_id';
    try
        setprogress(1);
        open;
    finally
    ...
    begin
        first;
        s:='下列品种价格或数量为0,或数量非整数, 本单不可送审核'+ #13#10;
        s:=s+'-----------------------------------------------------';  //+ #13#10
...
with cdsbill do
begin
    if fieldbyname('bod_status_id').asinteger=1 then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
    if (curuserid<>fieldbyname('creat_by').asinteger) and (fieldbyname('bod_cd').asstring='') then raise Exception.Create('非本人新增单据，请勿制单');
end;
end;
...
with cdsbill_dtl do
begin
//    if fieldbyname('qty').asfloat=0 then raise Exception.Create('数量不可为零');
    if (fieldbyname('price').asfloat=0) or (fieldbyname('qty').asfloat=0) then raise Exception.Create('货品单价或数量不可为零');
    if int(fieldbyname('qty').asfloat)<>fieldbyname('qty').asfloat then raise Exception.Create('数量不可有小数');
end;
}
end;
...
//    sbodcd:='导入数据已生成下列单据'+#13#10+'---------------------------';
    ...
    begin
        ...
        sql:=sql+''','''+Trim(sheet.cells[j,6].text)+''','''+Trim(sheet.cells[j,7].text)+''','''+Trim(sheet.cells[j,8].text)+''','''+Trim(sheet.cells[j,9].text)+''','''+Trim(sheet.cells[j,10].text)+''','''+Trim(sheet.cells[j,11].text)+'''';
        j:=j+1;
    end;
    XL.Quit;
//从第2行开始导入，格式: 1发生日期, 2业务类别, 3商业公司, 4单据摘要, 5商务人员, 6物料编码, 7批号, 8数量, 9单价, 10渠道名称, 11子渠道名称
    sql1:=sql;  //导入数据无错误时，用sql1 插入数据
    sql:=sql+'select top 5 * from ( select top 5 info=''第''+line_no+''行 无发生日期或数据无效'' from @tab where f1='''' or try_cast(f1 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无业务类别或数据无效'' from @tab where f2='''' or (f2<>''商业'' and f2<>''分销'')'; //商业  分销
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无商业公司或数据无效'' from @tab where f3='''' or not exists (select 1 from tb_busimate where mate_type_id=2 and f3=mate_name)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无商务人员或数据无效'' from @tab where f5='''' or not exists (select 1 from tb_staff where sta_type_id=0 and broker=1 and f5=zname)';//商务人员
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无物料编码或数据无效'' from @tab where f6='''' or not exists (select 1 from tb_medicine where f6=material_code)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无数量或数据无效(需为整数)'' from @tab where f8='''' or try_cast(f8 as int) is null';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无数量或数据无效'' from @tab where f8='''' or try_cast(f8 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无单价或数据无效'' from @tab where f9='''' or try_cast(f9 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无渠道(名称)或数据无效'' from @tab where f10='''' or not exists (select 1 from tb_object where obj_type_id=11 and zdesc= f10)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无子渠道(名称)或数据无效'' from @tab where f11='''' or not exists (select 1 from tb_object where obj_type_id=12 and zdesc= f11)';
{
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无渠道(名称)或数据无效'' from @tab a where f10='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.zdesc=a.f10)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无子渠道(名称)或数据无效'' from @tab a where f11='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.zdesc=a.f11)';
}
    sql:=sql+' ) a order by info';
//从第2行开始导入，格式: 1发生日期, 2业务类别, 3商业公司, 4单据摘要, 5商务代表, 6物料编码, 7批号, 8数量, 9单价, 10渠道名称, 11子渠道名称
...
select @bodcd,@bcd
        ...
        sql:=sql+'      select top 1 '+inttostr(compid)+',bod_type_id=10,bod_status_id=0,bod_cd= @bodcd,busi_type=1,creat_by='+inttostr(curuserid)+',creat_dt=getdate(),carry_dt=a.f1,'; //cast(convert(char(10),getdate(),20) as datetime),';
        sql:=sql+'      bod_desc=a.f4,cate_id=case a.f2 when ''商业'' then 1 when ''分销'' then 2 end,dst_id=b.mate_id,broker_id=c.sta_id';
        sql:=sql+'      from @tab a left join tb_busimate b on b.mate_type_id=2 and a.f3=b.mate_name';
        sql:=sql+'          left join tb_staff c on c.sta_type_id=0 and c.broker=1 and a.f5=c.zname';
        ...
        sql:=sql+'  insert into tb_bill_dtl (bod_id,med_id,bat_cd,price,qty,amot,channel_id,channel_dtl_id)';
        sql:=sql+'  select @bodid,m.med_id,bat_cd=a.f7,price=cast(a.f9 as decimal(15,4)),qty=cast(a.f8 as decimal(15,4)),amot=round(cast(a.f8 as decimal(15,4))*cast(a.f9 as decimal(15,4)),2),c.obj_code,d.obj_code';
        ...
        sql:=sql+'      inner join (select obj_id,obj_code,zdesc from tb_object where obj_type_id=11) c on a.f10=c.zdesc';
        sql:=sql+'      inner join (select obj_id,obj_code,zdesc from tb_object where obj_type_id=12) d on a.f11=d.zdesc';
...
procedure Tsetcheckstock.dxDBButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
begin
    ...
    if showmodal= mrOK then
    begin
        if query1.fieldbyname('stop_yn').asboolean then raise Exception.Create('该商业公司已停用，不可制单');
...
procedure Tsetcheckstock.dxDBButtonEdit1KeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#32) and ((Sender as TdxDBButtonEdit).Text<>'') then //
begin
