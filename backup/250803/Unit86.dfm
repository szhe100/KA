object setexpaycash: Tsetexpaycash
  Left = 191
  Top = 106
  AutoScroll = False
  Caption = #20854#20182#25903#20986#20184#27454#21046#21333
  ClientHeight = 426
  ClientWidth = 732
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 12
  object PageControl1: TPageControl
    Left = 0
    Top = 45
    Width = 732
    Height = 381
    ActivePage = TabSheet2
    Align = alBottom
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 0
    OnChange = PageControl1Change
    object TabSheet1: TTabSheet
      Caption = #21015#34920
      object dxDBGrid1: TdxDBGrid
        Left = 0
        Top = 0
        Width = 724
        Height = 354
        Bands = <
          item
          end>
        DefaultLayout = True
        HeaderPanelRowCount = 1
        KeyField = 'bod_id'
        ShowGroupPanel = True
        ShowSummaryFooter = True
        SummaryGroups = <>
        SummarySeparator = ', '
        Align = alClient
        PopupMenu = PopupMenu1
        TabOrder = 0
        OnDblClick = dxDBGrid1DblClick
        DataSource = DSbill
        Filter.Active = True
        Filter.DropDownCount = 30
        Filter.Criteria = {00000000}
        HideSelectionColor = clAqua
        HideSelectionTextColor = clBlack
        HighlightColor = clAqua
        HighlightTextColor = clBlack
        LookAndFeel = lfUltraFlat
        OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSelect, edgoTabThrough, edgoVertThrough]
        OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoSmartReload, edgoUseBookmarks]
        OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
        OnCustomDrawCell = dxDBGrid1CustomDrawCell
        object dxDBGrid1creat_dt: TdxDBGridDateColumn
          DisableEditor = True
          Width = 68
          BandIndex = 0
          RowIndex = 0
          FieldName = 'creat_dt'
          DisableFilter = True
        end
        object dxDBGrid1bod_cd: TdxDBGridMaskColumn
          Alignment = taCenter
          DisableEditor = True
          Font.Charset = GB2312_CHARSET
          Font.Color = clPurple
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          Width = 65
          BandIndex = 0
          RowIndex = 0
          FieldName = 'bod_cd'
          SummaryFooterType = cstCount
          SummaryFooterFormat = '##,###'#21333
          DisableFilter = True
        end
        object dxDBGrid1Cbod_status: TdxDBGridMaskColumn
          Alignment = taCenter
          Caption = #29366#24577
          DisableEditor = True
          Font.Charset = GB2312_CHARSET
          Font.Color = clGreen
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          Width = 45
          BandIndex = 0
          RowIndex = 0
          FieldName = 'Cbod_status'
        end
        object dxDBGrid1bank_name: TdxDBGridColumn
          DisableEditor = True
          Width = 120
          BandIndex = 0
          RowIndex = 0
          FieldName = 'bank_name'
        end
        object dxDBGrid1carry_dt: TdxDBGridColumn
          Caption = #21457#29983#26085#26399
          DisableEditor = True
          Width = 65
          BandIndex = 0
          RowIndex = 0
          FieldName = 'carry_dt'
          DisableFilter = True
        end
        object dxDBGrid1bod_desc: TdxDBGridMaskColumn
          DisableEditor = True
          Width = 80
          BandIndex = 0
          RowIndex = 0
          FieldName = 'bod_desc'
          DisableFilter = True
        end
        object dxDBGrid1bod_amot: TdxDBGridColumn
          DisableEditor = True
          Width = 70
          BandIndex = 0
          RowIndex = 0
          FieldName = 'bod_amot'
          SummaryFooterType = cstSum
          SummaryFooterField = 'bod_amot'
          SummaryFooterFormat = '###,###,##0.00;; '
          DisableFilter = True
          SummaryType = cstSum
          SummaryField = 'bod_amot'
          SummaryFormat = '###,###,##0.00;; '
        end
        object dxDBGrid1creater: TdxDBGridMaskColumn
          Alignment = taCenter
          Caption = #21046#21333#30003#35831
          DisableEditor = True
          Width = 69
          BandIndex = 0
          RowIndex = 0
          FieldName = 'creater'
        end
        object dxDBGrid1carryer: TdxDBGridColumn
          DisableEditor = True
          Width = 57
          BandIndex = 0
          RowIndex = 0
          FieldName = 'carryer'
        end
        object dxDBGrid1checker: TdxDBGridMaskColumn
          Alignment = taCenter
          Caption = #25209#20934#20154
          DisableEditor = True
          Width = 57
          BandIndex = 0
          RowIndex = 0
          FieldName = 'checker'
        end
        object dxDBGrid1check_dt: TdxDBGridDateColumn
          Caption = #25209#20934#26102#38388
          DisableEditor = True
          Width = 65
          BandIndex = 0
          RowIndex = 0
          FieldName = 'check_dt'
          DisableFilter = True
        end
        object dxDBGrid1bod_id: TdxDBGridColumn
          DisableEditor = True
          Visible = False
          BandIndex = 0
          RowIndex = 0
          FieldName = 'bod_id'
        end
        object dxDBGrid1bod_status_id: TdxDBGridColumn
          Visible = False
          BandIndex = 0
          RowIndex = 0
          FieldName = 'bod_status_id'
        end
      end
    end
    object TabSheet2: TTabSheet
      Caption = #26126#32454
      ImageIndex = 1
      object Panel1: TPanel
        Left = 0
        Top = 0
        Width = 724
        Height = 354
        Align = alClient
        BevelOuter = bvNone
        Color = 16707839
        Font.Charset = GB2312_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = #23435#20307
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        DesignSize = (
          724
          354)
        object DBText8: TDBText
          Left = 492
          Top = 315
          Width = 42
          Height = 12
          Anchors = [akRight, akBottom]
          AutoSize = True
          DataField = 'checker'
          DataSource = DSbill
        end
        object Label1: TLabel
          Left = 247
          Top = 3
          Width = 168
          Height = 20
          Caption = #20854#20182#25903#20986#20184#27454#21046#21333
          Font.Charset = GB2312_CHARSET
          Font.Color = clWindowText
          Font.Height = -20
          Font.Name = #26999#20307'_GB2312'
          Font.Style = [fsBold, fsUnderline]
          ParentFont = False
        end
        object Label2: TLabel
          Left = 8
          Top = 40
          Width = 54
          Height = 12
          Caption = #21333#25454#32534#21495':'
        end
        object Label3: TLabel
          Left = 8
          Top = 58
          Width = 54
          Height = 12
          Caption = #21333#25454#29366#24577':'
        end
        object Label4: TLabel
          Left = 8
          Top = 75
          Width = 54
          Height = 12
          Caption = #21046#21333#26085#26399':'
        end
        object DBText9: TDBText
          Left = 62
          Top = 39
          Width = 49
          Height = 12
          AutoSize = True
          DataField = 'bod_cd'
          DataSource = DSbill
          Font.Charset = GB2312_CHARSET
          Font.Color = clPurple
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = [fsBold]
          ParentFont = False
        end
        object DBText10: TDBText
          Left = 62
          Top = 58
          Width = 48
          Height = 12
          AutoSize = True
          DataField = 'Cbod_status'
          DataSource = DSbill
          Font.Charset = GB2312_CHARSET
          Font.Color = clGreen
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          ParentFont = False
        end
        object DBText14: TDBText
          Left = 63
          Top = 75
          Width = 48
          Height = 12
          AutoSize = True
          DataField = 'creat_dt'
          DataSource = DSbill
          Font.Charset = GB2312_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          ParentFont = False
        end
        object GIFimage2: TRxGIFAnimator
          Left = 138
          Top = 18
          Width = 77
          Height = 40
          Image.Data = {
            
            08AD28B4680E6947C3B9A001D561532F0500003B}
        end
        object GIFimage1: TRxGIFAnimator
          Left = 527
          Top = 10
          Width = 77
          Height = 40
          Image.Data = {
            
            EDFC858328A602003B}
        end
        object Label14: TLabel
          Left = 217
          Top = 75
          Width = 54
          Height = 12
          Caption = #21333#25454#25688#35201':'
        end
        object DBText6: TDBText
          Left = 408
          Top = 320
          Width = 42
          Height = 12
          AutoSize = True
          DataField = 'mate_name'
          DataSource = DSbill
          Font.Charset = GB2312_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          ParentFont = False
          Visible = False
        end
        object DBText7: TDBText
          Left = 408
          Top = 337
          Width = 42
          Height = 12
          AutoSize = True
          DataField = 'mate_name1'
          DataSource = DSbill
          Font.Charset = GB2312_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          ParentFont = False
          Visible = False
        end
        object Label15: TLabel
          Left = 353
          Top = 320
          Width = 54
          Height = 12
          Caption = #21830#19994#20844#21496':'
          Visible = False
        end
        object Label7: TLabel
          Left = 353
          Top = 337
          Width = 54
          Height = 12
          Caption = #21307'    '#38498':'
          Visible = False
        end
        object Label13: TLabel
          Left = 543
          Top = 75
          Width = 54
          Height = 12
          Caption = #21457#29983#26085#26399':'
        end
        object Label5: TLabel
          Left = 205
          Top = 57
          Width = 66
          Height = 12
          Caption = #25512#24191#26381#21153#21830':'
        end
        object DBText15: TDBText
          Left = 62
          Top = 315
          Width = 48
          Height = 12
          Anchors = [akRight, akBottom]
          AutoSize = True
          DataField = 'creater'
          DataSource = DSbill
        end
        object Label9: TLabel
          Left = 8
          Top = 315
          Width = 54
          Height = 12
          Anchors = [akRight, akBottom]
          Caption = #21046' '#21333' '#20154':'
        end
        object Label17: TLabel
          Left = 438
          Top = 315
          Width = 54
          Height = 12
          Anchors = [akRight, akBottom]
          Caption = #23457' '#26680' '#20154':'
        end
        object Label16: TLabel
          Left = 547
          Top = 315
          Width = 54
          Height = 12
          Anchors = [akRight, akBottom]
          Caption = #23457#26680#26102#38388':'
        end
        object DBText12: TDBText
          Left = 602
          Top = 315
          Width = 48
          Height = 12
          Anchors = [akRight, akBottom]
          AutoSize = True
          DataField = 'check_dt'
          DataSource = DSbill
          Font.Charset = GB2312_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          ParentFont = False
        end
        object DBText3: TDBText
          Left = 192
          Top = 315
          Width = 42
          Height = 12
          Anchors = [akRight, akBottom]
          AutoSize = True
          DataField = 'creat_dt'
          DataSource = DSbill
        end
        object Label6: TLabel
          Left = 137
          Top = 315
          Width = 54
          Height = 12
          Anchors = [akRight, akBottom]
          Caption = #21046#21333#26102#38388':'
        end
        object dxDBEdit1: TdxDBEdit
          Left = 269
          Top = 70
          Width = 235
          Color = 16707839
          Style.BorderColor = 14065456
          Style.BorderStyle = xbsSingle
          Style.ButtonStyle = btsSimple
          Style.Edges = [edgBottom]
          Style.Shadow = False
          TabOrder = 1
          DataField = 'bod_desc'
          DataSource = DSbill
        end
        object Edit1: TEdit
          Left = 220
          Top = 317
          Width = 121
          Height = 20
          TabOrder = 2
          Text = 'Edit1'
          Visible = False
        end
        object dxDBDateEdit1: TdxDBDateEdit
          Left = 595
          Top = 71
          Width = 90
          Color = 16707839
          Style.ButtonTransparence = ebtAlways
          Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
          TabOrder = 3
          DataField = 'carry_dt'
          DataSource = DSbill
          StyleController = StyleController
        end
        object dxDBLookupEdit2: TdxDBLookupEdit
          Left = 602
          Top = 24
          Width = 100
          Font.Charset = GB2312_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          ParentFont = False
          Style.BorderColor = 12615680
          Style.BorderStyle = xbsSingle
          Style.ButtonStyle = btsSimple
          Style.ButtonTransparence = ebtAlways
          TabOrder = 4
          Visible = False
          DataField = 'bank_id'
          DataSource = DSbill
          DropDownRows = 20
          ListFieldName = 'bank_name'
          KeyFieldName = 'bank_id'
          ListSource = DShomebank
          LookupKeyValue = Null
        end
        object dxDBButtonEdit3: TdxDBButtonEdit
          Left = 271
          Top = 53
          Width = 232
          Style.BorderColor = 12615680
          Style.BorderStyle = xbsSingle
          Style.ButtonStyle = btsSimple
          Style.ButtonTransparence = ebtAlways
          TabOrder = 0
          OnEnter = dxDBButtonEdit3Enter
          OnKeyDown = dxDBButtonEdit3KeyDown
          OnKeyPress = dxDBButtonEdit3KeyPress
          DataField = 'bank_name'
          DataSource = DSbill
          Buttons = <
            item
              Default = True
            end>
          OnButtonClick = dxDBButtonEdit3ButtonClick
          ExistButtons = True
        end
        object dxDBGrid2: TdxDBGrid
          Left = 5
          Top = 93
          Width = 701
          Height = 214
          Bands = <
            item
            end>
          DefaultLayout = True
          HeaderPanelRowCount = 1
          KeyField = 'bod_id'
          ShowSummaryFooter = True
          SummaryGroups = <>
          SummarySeparator = ', '
          PopupMenu = PopupMenu1
          TabOrder = 5
          DataSource = DSbill_dtl
          Filter.Active = True
          Filter.DropDownCount = 30
          Filter.Criteria = {00000000}
          HideSelectionColor = clAqua
          HideSelectionTextColor = clBlack
          HighlightColor = clAqua
          HighlightTextColor = clBlack
          LookAndFeel = lfUltraFlat
          OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSelect, edgoTabThrough, edgoVertThrough]
          OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoSmartReload, edgoUseBookmarks]
          OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
          object dxDBGrid2creat_dt: TdxDBGridColumn
            DisableEditor = True
            Width = 68
            BandIndex = 0
            RowIndex = 0
            FieldName = 'creat_dt'
            DisableFilter = True
          end
          object dxDBGrid2bod_cd: TdxDBGridMaskColumn
            Alignment = taCenter
            Caption = #20184#27454#30003#35831#21333
            DisableEditor = True
            Font.Charset = GB2312_CHARSET
            Font.Color = clPurple
            Font.Height = -12
            Font.Name = #23435#20307
            Font.Style = []
            Width = 66
            BandIndex = 0
            RowIndex = 0
            FieldName = 'bod_cd'
            SummaryFooterType = cstCount
            SummaryFooterFormat = '##,###'#21333
            DisableFilter = True
          end
          object dxDBGrid2agent: TdxDBGridColumn
            DisableEditor = True
            Font.Charset = GB2312_CHARSET
            Font.Color = clNavy
            Font.Height = -12
            Font.Name = #23435#20307
            Font.Style = []
            Width = 70
            BandIndex = 0
            RowIndex = 0
            FieldName = 'agent'
            DisableFilter = True
          end
          object dxDBGrid2mate_name: TdxDBGridColumn
            DisableEditor = True
            Font.Charset = GB2312_CHARSET
            Font.Color = clNavy
            Font.Height = -12
            Font.Name = #23435#20307
            Font.Style = []
            Width = 200
            BandIndex = 0
            RowIndex = 0
            FieldName = 'mate_name'
            DisableFilter = True
          end
          object dxDBGrid2bod_desc: TdxDBGridMaskColumn
            DisableEditor = True
            Width = 173
            BandIndex = 0
            RowIndex = 0
            FieldName = 'bod_desc'
            DisableFilter = True
          end
          object dxDBGrid2amot: TdxDBGridColumn
            Caption = #20184#27454#37329#39069
            DisableEditor = True
            Width = 90
            BandIndex = 0
            RowIndex = 0
            FieldName = 'amot'
            SummaryFooterType = cstSum
            SummaryFooterField = 'amot'
            SummaryFooterFormat = '###,###,##0.00;; '
            DisableFilter = True
            SummaryType = cstSum
            SummaryField = 'amot'
            SummaryFormat = '###,###,##0.00;; '
          end
          object dxDBGrid2bod_id: TdxDBGridColumn
            DisableEditor = True
            Visible = False
            BandIndex = 0
            RowIndex = 0
            FieldName = 'bod_id'
          end
        end
      end
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 732
    Height = 42
    Align = alTop
    TabOrder = 1
    object SpeedButton2: TSpeedButton
      Left = 323
      Top = 1
      Width = 40
      Height = 40
      Hint = #36865#23457#26680
      Caption = '&K'#36865#23457
      Flat = True
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      Spacing = 2
      OnClick = SpeedButton2Click
    end
    object SpeedButton5: TSpeedButton
      Left = 163
      Top = 1
      Width = 40
      Height = 40
      Hint = #21047#26032
      Caption = '&R'#21047#26032
      Flat = True
      Glyph.Data = {
        
        3393333777333777337333333999993333333333377777333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      Spacing = 2
      OnClick = SpeedButton5Click
    end
    object SpeedButton7: TSpeedButton
      Left = 283
      Top = 1
      Width = 40
      Height = 40
      Hint = #21024#38500
      Caption = '&D'#21024#38500
      Flat = True
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      Spacing = 2
      OnClick = SpeedButton7Click
    end
    object SpeedButton8: TSpeedButton
      Left = 203
      Top = 1
      Width = 40
      Height = 40
      Hint = #20445#23384
      Caption = '&S'#20445#23384
      Flat = True
      Glyph.Data = {
        
        93337FFFF7737777733300000033333333337777773333333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      Spacing = 2
      OnClick = SpeedButton8Click
    end
    object SpeedButton9: TSpeedButton
      Left = 243
      Top = 1
      Width = 40
      Height = 40
      Hint = #21462#28040
      Caption = '&C'#21462#28040
      Flat = True
      Glyph.Data = {
        
        3333333777333777333333333999993333333333377777333333}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      Spacing = 2
      OnClick = SpeedButton9Click
    end
    object wwDBNavigator1: TwwDBNavigator
      Left = 2
      Top = 1
      Width = 160
      Height = 40
      AutosizeStyle = asSizeNavButtons
      DataSource = DSbill
      ShowHint = True
      RepeatInterval.InitialDelay = 500
      RepeatInterval.Interval = 100
      ParentShowHint = False
      object wwDBNavigator1First: TwwNavButton
        Left = 0
        Top = 0
        Width = 40
        Height = 40
        Hint = #26368#21069
        ImageIndex = -1
        NumGlyphs = 2
        ShowText = True
        Spacing = 4
        Transparent = False
        Caption = #26368#21069
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 0
        Style = nbsFirst
      end
      object wwDBNavigator1Prior: TwwNavButton
        Left = 40
        Top = 0
        Width = 40
        Height = 40
        Hint = #21521#21069
        ImageIndex = -1
        NumGlyphs = 2
        ShowText = True
        Spacing = 4
        Transparent = False
        Caption = #21521#21069
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 1
        Style = nbsPrior
      end
      object wwDBNavigator1Next: TwwNavButton
        Left = 80
        Top = 0
        Width = 40
        Height = 40
        Hint = #21521#21518
        ImageIndex = -1
        NumGlyphs = 2
        ShowText = True
        Spacing = 4
        Transparent = False
        Caption = #21521#21518
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 2
        Style = nbsNext
      end
      object wwDBNavigator1Last: TwwNavButton
        Left = 120
        Top = 0
        Width = 40
        Height = 40
        Hint = #26368#21518
        ImageIndex = -1
        NumGlyphs = 2
        ShowText = True
        Spacing = 4
        Transparent = False
        Caption = #26368#21518
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 3
        Style = nbsLast
      end
    end
    object Panel3: TPanel
      Left = 619
      Top = 1
      Width = 112
      Height = 40
      Align = alRight
      BevelOuter = bvLowered
      Color = 14277081
      TabOrder = 1
      DesignSize = (
        112
        40)
      object Label19: TLabel
        Left = 2
        Top = 5
        Width = 30
        Height = 12
        Hint = #21015#34920#20013#21015#20986#21333#25454#30340#21551#22987#26085#26399
        Caption = #33539#22260':'
        ParentShowHint = False
        ShowHint = True
      end
      object Label22: TLabel
        Left = 1
        Top = 21
        Width = 30
        Height = 15
        Alignment = taCenter
        Anchors = [akTop, akRight]
        AutoSize = False
        Caption = '[]'
        Font.Charset = ANSI_CHARSET
        Font.Color = clPurple
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object dxDateEdit1: TdxDateEdit
        Left = 32
        Top = 1
        Width = 85
        Hint = #21015#34920#20013#21015#20986#21333#25454#30340#21551#22987#26085#26399
        Color = 14277081
        ParentShowHint = False
        ShowHint = True
        Style.ButtonTransparence = ebtAlways
        TabOrder = 0
        StyleController = StyleController
        Date = -700000.000000000000000000
      end
      object dxDateEdit4: TdxDateEdit
        Left = 32
        Top = 20
        Width = 85
        Hint = #21015#34920#20013#21015#20986#21333#25454#30340#32456#27490#26085#26399
        Color = 14277081
        ParentShowHint = False
        ShowHint = True
        Style.ButtonTransparence = ebtAlways
        TabOrder = 1
        StyleController = StyleController
        Date = -700000.000000000000000000
      end
    end
    object dxCheckEdit1: TdxCheckEdit
      Left = 407
      Top = 12
      Width = 74
      TabOrder = 2
      Caption = #26412#20154#21333#25454
      StyleController = StyleController
      State = cbsChecked
    end
  end
  object DSbill_dtl: TDataSource
    DataSet = cdsbill_dtl
    OnStateChange = DSbill_dtlStateChange
    Left = 342
    Top = 227
  end
  object DSbill: TDataSource
    DataSet = cdsbill
    OnStateChange = DSbillStateChange
    OnDataChange = DSbillDataChange
    Left = 342
    Top = 193
  end
  object StyleController: TdxEditStyleController
    BorderColor = 14065456
    BorderStyle = xbsSingle
    ButtonStyle = btsSimple
    Edges = [edgBottom]
    HotTrack = True
    Left = 642
    Top = 44
  end
  object cdsbill: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    BeforeEdit = cdsbillBeforeEdit
    BeforeDelete = cdsbillBeforeEdit
    OnCalcFields = cdsbillCalcFields
    Left = 304
    Top = 192
    object cdsbillbod_id: TIntegerField
      FieldName = 'bod_id'
    end
    object cdsbillbod_cd: TStringField
      DisplayLabel = #21333#25454#21495
      FieldName = 'bod_cd'
    end
    object cdsbillsrc_id: TIntegerField
      FieldName = 'src_id'
    end
    object cdsbilldst_id: TIntegerField
      FieldName = 'dst_id'
    end
    object cdsbillmate_name: TStringField
      DisplayLabel = #37197#36865#21830#19994#20844#21496
      FieldName = 'mate_name'
      Size = 100
    end
    object cdsbillmate_name1: TStringField
      DisplayLabel = #19968#32423#20379#24212#21830
      FieldName = 'mate_name1'
      Size = 100
    end
    object cdsbillbank_id: TIntegerField
      FieldName = 'bank_id'
    end
    object cdsbillbank_name: TStringField
      DisplayLabel = #25512#24191#26381#21153#21830
      FieldName = 'bank_name'
      Size = 50
    end
    object cdsbillbod_status_id: TIntegerField
      FieldName = 'bod_status_id'
    end
    object cdsbillbod_type_id: TIntegerField
      FieldName = 'bod_type_id'
    end
    object cdsbillbusi_type: TIntegerField
      FieldName = 'busi_type'
    end
    object cdsbillbod_amot: TBCDField
      DisplayLabel = #21333#25454#37329#39069
      FieldName = 'bod_amot'
      DisplayFormat = '###,###,##0.00;; '
      Precision = 15
    end
    object cdsbillcreat_by: TIntegerField
      FieldName = 'creat_by'
    end
    object cdsbillcreat_dt: TDateTimeField
      DisplayLabel = #21046#21333#26085#26399
      FieldName = 'creat_dt'
    end
    object cdsbillcheck_by: TIntegerField
      FieldName = 'check_by'
    end
    object cdsbillcheck_dt: TDateTimeField
      DisplayLabel = #23457#26680#26085#26399
      FieldName = 'check_dt'
    end
    object cdsbillrelative_id: TIntegerField
      FieldName = 'relative_id'
    end
    object cdsbillCbod_status: TStringField
      DisplayLabel = #21333#25454#29366#24577
      FieldKind = fkCalculated
      FieldName = 'Cbod_status'
      Calculated = True
    end
    object cdsbillcreater: TStringField
      DisplayLabel = #21046#21333
      FieldName = 'creater'
    end
    object cdsbillchecker: TStringField
      DisplayLabel = #23457#26680
      FieldName = 'checker'
    end
    object cdsbillsettle_type: TIntegerField
      FieldName = 'settle_type'
    end
    object cdsbillbroker_id: TIntegerField
      FieldName = 'broker_id'
    end
    object cdsbillbroker: TStringField
      DisplayLabel = #21307#33647#20195#34920
      FieldName = 'broker'
    end
    object cdsbillprncount: TIntegerField
      FieldName = 'prncount'
    end
    object cdsbillbod_desc: TStringField
      DisplayLabel = #25688#35201
      FieldName = 'bod_desc'
      Size = 100
    end
    object cdsbillcarry_by: TIntegerField
      FieldName = 'carry_by'
    end
    object cdsbillcarry_dt: TDateTimeField
      DisplayLabel = #22797#26680#26102#38388
      FieldName = 'carry_dt'
    end
    object cdsbillcarryer: TStringField
      DisplayLabel = #22797#26680#20154
      FieldName = 'carryer'
    end
    object cdsbillreceipt_by: TIntegerField
      FieldName = 'receipt_by'
    end
    object cdsbillreceipt_dt: TDateTimeField
      FieldName = 'receipt_dt'
    end
    object cdsbillreceiver: TStringField
      FieldName = 'receiver'
      ReadOnly = True
      Size = 100
    end
    object cdsbillstorage_by: TIntegerField
      FieldName = 'storage_by'
    end
    object cdsbillstorage_dt: TDateTimeField
      FieldName = 'storage_dt'
    end
    object cdsbillstorager: TStringField
      FieldName = 'storager'
      ReadOnly = True
      Size = 100
    end
  end
  object cdsbill_dtl: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    BeforeInsert = cdsbill_dtlBeforeEdit
    AfterInsert = cdsbill_dtlAfterInsert
    BeforeEdit = cdsbill_dtlBeforeEdit
    AfterPost = cdsbill_dtlAfterPost
    BeforeDelete = cdsbill_dtlBeforeDelete
    AfterDelete = cdsbill_dtlAfterPost
    Left = 303
    Top = 227
    object cdsbill_dtldtl_id: TIntegerField
      FieldName = 'dtl_id'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object cdsbill_dtlmed_id: TIntegerField
      FieldName = 'med_id'
    end
    object cdsbill_dtlbod_id: TIntegerField
      FieldName = 'bod_id'
    end
    object cdsbill_dtlbod_cd: TStringField
      DisplayLabel = #21333#25454#21495
      FieldName = 'bod_cd'
    end
    object cdsbill_dtlagent_id: TIntegerField
      FieldName = 'agent_id'
    end
    object cdsbill_dtlagent: TStringField
      DisplayLabel = #19994#21153#36127#36131#20154
      FieldName = 'agent'
      Size = 100
    end
    object cdsbill_dtlsrc_id: TIntegerField
      FieldName = 'src_id'
    end
    object cdsbill_dtldst_id: TIntegerField
      FieldName = 'dst_id'
    end
    object cdsbill_dtlmate_name: TStringField
      DisplayLabel = #21830#19994#20844#21496
      FieldName = 'mate_name'
      Size = 100
    end
    object cdsbill_dtlbod_status_id: TIntegerField
      FieldName = 'bod_status_id'
    end
    object cdsbill_dtlbod_type_id: TIntegerField
      FieldName = 'bod_type_id'
    end
    object cdsbill_dtlbusi_type: TIntegerField
      FieldName = 'busi_type'
    end
    object cdsbill_dtlbod_amot: TBCDField
      DisplayLabel = #24212#20184#37329#39069
      FieldName = 'bod_amot'
      DisplayFormat = '###,###,##0.00;; '
      Precision = 15
    end
    object cdsbill_dtlcreat_by: TIntegerField
      FieldName = 'creat_by'
    end
    object cdsbill_dtlcreat_dt: TDateTimeField
      DisplayLabel = #21046#21333#26085#26399
      FieldName = 'creat_dt'
    end
    object cdsbill_dtlcheck_by: TIntegerField
      FieldName = 'check_by'
    end
    object cdsbill_dtlcheck_dt: TDateTimeField
      DisplayLabel = #23457#26680#26085#26399
      FieldName = 'check_dt'
    end
    object cdsbill_dtlrelative_id: TIntegerField
      FieldName = 'relative_id'
    end
    object cdsbill_dtlCbod_status: TStringField
      DisplayLabel = #21333#25454#29366#24577
      FieldKind = fkCalculated
      FieldName = 'Cbod_status'
      Calculated = True
    end
    object cdsbill_dtlcreater: TStringField
      DisplayLabel = #21046#21333
      FieldName = 'creater'
    end
    object cdsbill_dtlchecker: TStringField
      DisplayLabel = #23457#26680
      FieldName = 'checker'
    end
    object cdsbill_dtlsettle_type: TIntegerField
      FieldName = 'settle_type'
    end
    object cdsbill_dtlbroker_id: TIntegerField
      FieldName = 'broker_id'
    end
    object cdsbill_dtlbroker: TStringField
      DisplayLabel = #19994#21153#21592
      FieldName = 'broker'
    end
    object cdsbill_dtlprncount: TIntegerField
      FieldName = 'prncount'
    end
    object cdsbill_dtlbod_desc: TStringField
      DisplayLabel = #25688#35201
      FieldName = 'bod_desc'
      Size = 30
    end
    object cdsbill_dtlcarry_by: TIntegerField
      FieldName = 'carry_by'
    end
    object cdsbill_dtlcarry_dt: TDateTimeField
      DisplayLabel = #22797#26680#26102#38388
      FieldName = 'carry_dt'
    end
    object cdsbill_dtlcarryer: TStringField
      DisplayLabel = #22797#26680#20154
      FieldName = 'carryer'
    end
    object cdsbill_dtlreceipt_by: TIntegerField
      FieldName = 'receipt_by'
    end
    object cdsbill_dtlreceipt_dt: TDateTimeField
      FieldName = 'receipt_dt'
    end
    object cdsbill_dtlreceiver: TStringField
      FieldName = 'receiver'
      ReadOnly = True
      Size = 100
    end
    object cdsbill_dtlstorage_by: TIntegerField
      FieldName = 'storage_by'
    end
    object cdsbill_dtlstorage_dt: TDateTimeField
      FieldName = 'storage_dt'
    end
    object cdsbill_dtlstorager: TStringField
      FieldName = 'storager'
      ReadOnly = True
      Size = 100
    end
    object cdsbill_dtlprice: TBCDField
      DisplayLabel = #24212#20184#36153#29992
      FieldName = 'price'
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Precision = 15
      Size = 6
    end
    object cdsbill_dtlamot: TBCDField
      DisplayLabel = #23454#20184#36153#29992
      FieldName = 'amot'
      DisplayFormat = '###,###,##0.00##;; '
      Precision = 15
    end
  end
  object pubqry: TClientDataSet
    Aggregates = <>
    CommandText = 
      'select distinct b.zname,c.zdesc from tb_useright a,tb_staff b,tb' +
      '_object c'
    Params = <>
    ProviderName = 'pvdpubquery'
    RemoteServer = DM.RemoteServer
    Left = 562
    Top = 240
  end
  object DSpubqry: TDataSource
    DataSet = pubqry
    Left = 609
    Top = 240
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingageYPH;Data Source=.'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select top 1 bank_id from tb_bill')
    Left = 514
    Top = 211
    object ADOQuery1bank_id: TIntegerField
      FieldName = 'bank_id'
    end
  end
  object PopupMenu1: TPopupMenu
    Left = 37
    Top = 216
    object N1: TMenuItem
      Caption = #25209#37327#36865#23457
      OnClick = N1Click
    end
  end
  object homebank: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    Left = 477
    Top = 106
    object homebankbank_id: TAutoIncField
      FieldName = 'bank_id'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object homebankbank_name: TStringField
      DisplayLabel = #38134#34892'/'#31185#30446#21517#31216
      FieldName = 'bank_name'
      Size = 50
    end
    object homebankbank_acnt: TStringField
      DisplayLabel = #38134#34892#24080#25143
      FieldName = 'bank_acnt'
    end
  end
  object DShomebank: TDataSource
    DataSet = homebank
    Left = 505
    Top = 106
  end
end
