...
type
  Tsettrustruleapply3 = class(TForm)
...
var
  settrustruleapply3: Tsettrustruleapply3;
...
procedure Tsettrustruleapply3.setupdatestatus;    // 编辑状态
begin
...
procedure Tsettrustruleapply3.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsettrustruleapply3.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsettrustruleapply3.FormCloseQuery(Sender: TObject;
...
procedure Tsettrustruleapply3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsettrustruleapply3.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsettrustruleapply3.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with cdsbill_dtl do
...
procedure Tsettrustruleapply3.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsettrustruleapply3.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=24 and a.bod_status_id in (1,4)';  // a.comp_id='+inttostr(compid)+' and
...
procedure Tsettrustruleapply3.FormCreate(Sender: TObject);
begin
...
procedure Tsettrustruleapply3.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsettrustruleapply3.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsettrustruleapply3.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsettrustruleapply3.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsettrustruleapply3.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsettrustruleapply3.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsettrustruleapply3.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsettrustruleapply3.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsettrustruleapply3.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsettrustruleapply3.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsettrustruleapply3.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsettrustruleapply3.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsettrustruleapply3.N1Click(Sender: TObject);
...
procedure Tsettrustruleapply3.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsettrustruleapply3.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsettrustruleapply3.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsettrustruleapply3.brokermedtlCalcFields(DataSet: TDataSet);
begin
with brokermedtl do
begin
...
procedure Tsettrustruleapply3.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsettrustruleapply3.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsettrustruleapply3.SpeedButton2Click(Sender: TObject);
begin
...
begin
    if active then close;
    commandtext:='sp_updatestock24 24,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsettrustruleapply3.SpeedButton3Click(Sender: TObject);
begin
