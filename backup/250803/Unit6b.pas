...
procedure Tsetmedprice.FormActivate(Sender: TObject);
begin
with dm.district do
...
procedure Tsetmedprice.FormCloseQuery(Sender: TObject;
...
procedure Tsetmedprice.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetmedprice.SpeedButton3Click(Sender: TObject);
var mi: integer;
begin
with medprice do
begin
...
//    commandtext:=commandtext+'	dist1=dbo.fn_getdistrictname (a.district_id,1),dist2=dbo.fn_getdistrictname (a.district_id,2),';
    commandtext:=commandtext+' dist1=dbo.fn_getdistrict1(a.ZREGIO),'; //  dbo.fn_getdistrictlevel (a.district_id,1),';
    commandtext:=commandtext+' dist2=dbo.fn_getdistrict1(a.ZCITYNUM),'; //  dbo.fn_getdistrictlevel (a.district_id,2),';
    commandtext:=commandtext+' creater=c.zname';
    commandtext:=commandtext+' from tb_medprice a';
//    commandtext:=commandtext+' inner join tb_medicine m on a.med_id =m.med_id';
    commandtext:=commandtext+' inner join tb_medicine m on (a.MATNR =m.material_code or a.med_id=m.med_id)';
    commandtext:=commandtext+' left join tb_staff c on a.creat_by =c.sta_id';
    if dxCheckEdit2.Checked then
    begin
//        commandtext:=commandtext+' inner join (select ZREGIO,ZCITYNUM,MATNR,rec_id=max(rec_id) from tb_medprice where type_id=1 group by ZREGIO, ZCITYNUM,MATNR) h on a.rec_id=h.rec_id';
        commandtext:=commandtext+' inner join (select ZREGIO,ZCITYNUM,MATNR,valid_from=max(valid_from) from tb_medprice where type_id=1 group by ZREGIO, ZCITYNUM,MATNR) h ';
        commandtext:=commandtext+'	on a.ZREGIO=h.ZREGIO and a.ZCITYNUM=h.ZCITYNUM and a.MATNR=h.MATNR and a.valid_from=h.valid_from';
    end;
//    commandtext:=commandtext+' left join (select prov_code,prov_name,city_name from tb_district where level_id=1) d on a.ZREGIO=d.prov_code '; //a.district_id =d.rec_id';
    commandtext:=commandtext+' where type_id=1'; //a.comp_id='+inttostr(compid);
...
//    if dxCheckEdit2.Checked then commandtext:=commandtext+' 
...
procedure Tsetmedprice.FormCreate(Sender: TObject);
begin
{
...
procedure Tsetmedprice.N3Click(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure Tsetmedprice.N1Click(Sender: TObject);
begin
with dxdbgrid1 do Columns[GetAbsoluteColumnIndex(FocusedColumn)].Visible:=false;
end;

procedure Tsetmedprice.N2Click(Sender: TObject);
var i,mi: integer;
begin
setprogress(1);
with dxdbgrid1 do
...
procedure Tsetmedprice.N6Click(Sender: TObject);
begin
if FileExists(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini')
    then dxDBGrid1.LoadFromIniFile(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini');//加载
end;

procedure Tsetmedprice.medpriceAfterPost(DataSet: TDataSet);
begin
if medprice.ChangeCount>0 then myupdaterefresh(medprice,'tb_medprice','rec_id','');
end;

procedure Tsetmedprice.medpriceBeforePost(DataSet: TDataSet);
begin
{
with medprice do
begin
...
procedure Tsetmedprice.SpeedButton6Click(Sender: TObject);
var
    XL,Sheet: Variant;
    SFileName: string;
    mi,J,unitid: integer;
...
begin
    ...
    setprogress(1);
//从第2行开始导入，导入格式: 1物料编码,2通用名,3商品名,4规格,5生产企业,6计量单位,7整件内装数
...
//    SpeedButton3Click(nil);
    setprogress(0);
    MessageBox(0,pchar('数据已导入(共导入'+inttostr(mi)+'个品种,物料编码重复品种未导入。可刷新查询)'),'请注意',MB_OK+MB_ICONInformation);
end;

procedure Tsetmedprice.dxButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedprice.DSmedpriceDataChange(Sender: TObject;
  Field: TField);
begin
with medprice do
begin
...
procedure Tsetmedprice.dxLookupTreeView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if key=vk_delete then dxLookupTreeView1.text:='';
end;

procedure Tsetmedprice.SpeedButton4Click(Sender: TObject);
var mi: integer;
begin
...
begin
    ...
    commandtext:=commandtext+' select top 1 type_id=1,comp_id='+inttostr(compid)+',district_id=case when b.city_name='''' then b.prov_code else b.city_code end,ZREGIO=b.prov_code,ZCITYNUM=case when b.city_name='''' then '''' else b.city_code end,b.prov_name,';
    ...
    commandtext:=commandtext+'select @@identity';
{
    ...
    commandtext:=commandtext+' (1,'+inttostr(compid)+','+dm.district.fieldbyname('code').asstring+','+inttostr(dxbuttonedit1.tag)+','+inttostr(curuserid)+',getdate())';
//    commandtext:=commandtext+' (1,'+inttostr(compid)+','+dm.district.fieldbyname('rec_id').asstring+','+inttostr(dxbuttonedit1.tag)+','+inttostr(curuserid)+',getdate())';
    commandtext:=commandtext+'select @@identity';
}
    open;
...
procedure Tsetmedprice.medpriceBeforeDelete(DataSet: TDataSet);
begin
...
procedure Tsetmedprice.SpeedButton2Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag);
end;

procedure Tsetmedprice.dxLookupTreeView1DropDown(Sender: TObject);
begin
setprogress(1);
end;

procedure Tsetmedprice.dxLookupTreeView1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
setprogress(0);
end;

procedure Tsetmedprice.medpriceBeforeEdit(DataSet: TDataSet);
begin
with medprice do
begin
