object pickchannel_dtl: Tpickchannel_dtl
  Left = 334
  Top = 143
  HorzScrollBar.Visible = False
  VertScrollBar.Visible = False
  AutoScroll = False
  BorderIcons = []
  Caption = #26597#35810#23376#28192#36947
  ClientHeight = 361
  ClientWidth = 281
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  OnActivate = FormActivate
  OnCloseQuery = FormCloseQuery
  OnKeyDown = Edit1KeyDown
  DesignSize = (
    281
    361)
  PixelsPerInch = 96
  TextHeight = 12
  object wwDBNavigator1: TwwDBNavigator
    Left = 107
    Top = 1
    Width = 96
    Height = 22
    AutosizeStyle = asSizeNavButtons
    DataSource = DSquery1
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    ParentShowHint = False
    object wwDBNavigator1First: TwwNavButton
      Left = 0
      Top = 0
      Width = 24
      Height = 22
      Hint = #39318#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      ParentShowHint = False
      ShowHint = True
      OnClick = wwDBNavigator1FirstClick
      Index = 0
      Style = nbsFirst
    end
    object wwDBNavigator1PriorPage: TwwNavButton
      Left = 24
      Top = 0
      Width = 24
      Height = 22
      Hint = #21069#19968#39029
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1PriorPage'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      ParentShowHint = False
      ShowHint = True
      OnClick = wwDBNavigator1FirstClick
      Index = 1
      Style = nbsPriorPage
    end
    object wwDBNavigator1NextPage: TwwNavButton
      Left = 48
      Top = 0
      Width = 24
      Height = 22
      Hint = #19979#19968#39029
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1NextPage'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      ParentShowHint = False
      ShowHint = True
      OnClick = wwDBNavigator1FirstClick
      Index = 2
      Style = nbsNextPage
    end
    object wwDBNavigator1Last: TwwNavButton
      Left = 72
      Top = 0
      Width = 24
      Height = 22
      Hint = #23614#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      ParentShowHint = False
      ShowHint = True
      OnClick = wwDBNavigator1FirstClick
      Index = 3
      Style = nbsLast
    end
  end
  object BitBtn2: TBitBtn
    Left = 214
    Top = 1
    Width = 22
    Height = 22
    Anchors = [akTop, akRight]
    Caption = ' '
    TabOrder = 1
    Kind = bkOK
    Spacing = 0
  end
  object BitBtn3: TBitBtn
    Left = 237
    Top = 1
    Width = 22
    Height = 22
    Anchors = [akTop, akRight]
    Caption = ' '
    TabOrder = 2
    OnClick = BitBtn3Click
    Kind = bkCancel
    Spacing = 0
  end
  object DBGrid1: TdxDBGrid
    Left = 0
    Top = 25
    Width = 281
    Height = 336
    Bands = <
      item
      end>
    DefaultLayout = True
    HeaderPanelRowCount = 1
    KeyField = 'obj_id'
    ShowSummaryFooter = True
    SummaryGroups = <>
    SummarySeparator = ', '
    Align = alBottom
    TabOrder = 3
    OnDblClick = DBGrid1DblClick
    OnKeyPress = DBGrid1KeyPress
    DataSource = DSquery1
    Filter.Criteria = {00000000}
    HighlightColor = clAqua
    HighlightTextColor = clBlack
    LookAndFeel = lfUltraFlat
    OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoRowSelect, edgoUseBitmap]
    Anchors = [akLeft, akTop, akRight, akBottom]
    object DBGrid1pick_ord: TdxDBGridMaskColumn
      Alignment = taCenter
      Font.Charset = GB2312_CHARSET
      Font.Color = clMaroon
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 30
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pick_ord'
      SummaryFooterType = cstCount
      SummaryFooterField = 'obj_id'
      SummaryType = cstCount
      SummaryField = 'obj_id'
    end
    object DBGrid1obj_code: TdxDBGridColumn
      Alignment = taCenter
      Width = 60
      BandIndex = 0
      RowIndex = 0
      FieldName = 'obj_code'
    end
    object DBGrid1zdesc: TdxDBGridColumn
      Caption = #28192#36947
      DisableEditor = True
      Width = 155
      BandIndex = 0
      RowIndex = 0
      FieldName = 'zdesc'
      SummaryFooterField = 'obj_id'
      SummaryField = 'obj_id'
    end
  end
  object Edit1: TdxEdit
    Left = 2
    Top = 3
    Width = 100
    Hint = #21487#36755#20837#31616#30721#25110#20013#25991#21517#31216#12289#31616#31216
    ImeMode = imClose
    ParentShowHint = False
    ShowHint = True
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 4
    OnKeyDown = Edit1KeyDown
    OnKeyPress = Edit1KeyPress
  end
  object DSquery1: TDataSource
    DataSet = query1
    OnDataChange = DSquery1DataChange
    Left = 66
    Top = 132
  end
  object query1: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    Left = 34
    Top = 132
    object query1pick_ord: TIntegerField
      DisplayLabel = #36873#21495
      FieldName = 'pick_ord'
    end
    object query1obj_id: TAutoIncField
      FieldName = 'obj_id'
      ReadOnly = True
    end
    object query1obj_code: TStringField
      DisplayLabel = #32534#30721
      FieldName = 'obj_code'
    end
    object query1zdesc: TStringField
      DisplayLabel = #23567#21306
      FieldName = 'zdesc'
      Size = 30
    end
    object query1qry_code: TWideStringField
      DisplayLabel = #31616#30721
      FieldName = 'qry_code'
      ReadOnly = True
      Size = 4000
    end
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingageYPH;Data Source=.'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select top 1 obj_id,zdesc,qry_code=dbo.fn_getpy(zdesc)'
      ' from tb_object where comp_id=1 and obj_type_id=23 ')
    Left = 188
    Top = 146
    object ADOQuery1obj_id: TAutoIncField
      FieldName = 'obj_id'
      ReadOnly = True
    end
    object ADOQuery1zdesc: TStringField
      FieldName = 'zdesc'
      Size = 30
    end
    object ADOQuery1qry_code: TWideStringField
      FieldName = 'qry_code'
      ReadOnly = True
      Size = 4000
    end
  end
end
