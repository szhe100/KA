...
//    sql:=sql+' select top 10 * from ( select top 5 info=line_no+''行无医院主数据编码或编码重复'' from @tab a where f1='''' or exists (select 1 from tb_busimate b where b.mate_type_id=1 and b.mate_code=f1 and b.mate_name<>a.f2)';
    sql:=sql+' select top 10 * from ( select top 5 info=line_no+''行无医院主数据编码或编码重复'' from @tab a where f1='''' or exists (select 1 from tb_busimate b where b.mate_type_id=1 and b.mate_code=f1 and b.mate_name<>a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无客户(医院)名称或名称重复'' from @tab a where f2='''' or exists (select 1 from tb_busimate b where b.mate_type_id=1 and b.mate_name=a.f2 and b.mate_code<>a.f1)';
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行无性质或数据无效'' from @tab where f4='''' or not (f4 like ''%公立%'' or f4 like ''%民营%'' or f4 like ''%连锁%'' or f4 like ''%电商%'' or f4 like ''%厂矿职工%'' or f4 like ''%第三终端%'')';   //公立 民营 连锁 电商 厂矿职工 第三终端
