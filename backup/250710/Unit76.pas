...
uses datamodu, Unit28, Unit7, Unit16;   //Unit28f, Unit28g, 
...
begin
    ...
    close;
    commandtext:='select a.*,c.mate_name,m.med_code,m.material_code,m.material_code1,m.med_name,m.specifi,creater=d.zname,checker=e.zname,broker=b.zname';
...
begin
    ...
    fieldbyname('Cmed').asstring:=fieldbyname('material_code').asstring+' '+fieldbyname('med_name').asstring+' '+fieldbyname('specifi').asstring;
end;
end;
...
begin
    ...
    XL.Quit;
//从第2行开始导入,导入格式:1发生日期,2业务员,3付款金额,4单据摘要,5物料编码,6医院(选填) ,7渠道,8子渠道
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无业务员或数据无效'' from @tab a where f2='''' or not exists (select 1 from tb_staff b where sta_type_id=1 and b.zname =a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无付款金额或数据无效'' from @tab a where f3='''' or try_cast(f3 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无物料编码或数据无效'' from @tab a where f5='''' or not exists (select 1 from tb_medicine m where m.material_code =a.f5)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 医院数据无效'' from @tab a where f6<>'''' and not exists (select 1 from tb_busimate b where mate_type_id=1 and b.mate_name =a.f6)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无渠道或数据无效'' from @tab a where a.f7='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.obj_code=a.f7)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无子渠道或数据无效'' from @tab a where a.f8='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.obj_code=a.f8)';
...
//        sql:=sql+' select top 1 @cid=cast(bod_cd as int) from tb_bill where bod_type_id=19 order by bod_cd desc';  //取最大单据号
        sql:=sql+' select top 1 @cid=currentid from tb_billid where comp_id='+inttostr(compid)+' and bod_type_id=19';  //取最大单据号
...
//从第2行开始导入,导入格式:1发生日期,2业务员,3付款金额,4单据摘要,5物料编码,6医院(选填) ,7渠道,8子渠道
        ...
        sql:=sql+' left join (select med_id,material_code,material_code1 from tb_medicine) m on m.material_code=a.f5';
        ...
        sql:=sql+' set @cid=@@ROWCOUNT';
        sql:=sql+' update tb_billid set currentid=currentid+@cid where comp_id='+inttostr(compid)+' and bod_type_id=19';
        sql:=sql+' select @cid';
edit1.text:=sql;
//exit;
        try
...
begin
    ...
    begin
        ...
        cdsbill.fieldbyname('material_code').asstring:= query1.fieldbyname('material_code').asstring;
    ...
    begin
        ...
        cdsbill.fieldbyname('material_code').asstring:= '';
