...
procedure Tpickbusimate3.Edit1KeyPress(Sender: TObject; var Key: Char);
var sk: widestring;
begin
    ...
    if sk='' then exit;
{
    ...
    begin
        ...
        commandtext:=commandtext+' mate_id,mate_name,mate_type_id,deputy,phone,address,broker_id,broker=dbo.fn_staff_name(broker_id),';
        commandtext:=commandtext+' settle_type,stop_yn,qry_code from tb_busimate';
        commandtext:=commandtext+' where mate_type_id in (1,2)'; // and comp_id='+inttostr(compid);
...
//        commandtext:=commandtext+' order by mate_name';
        open;
        if recordcount=0 then raise Exception.Create('无所查询商业公司与医院');
    end;
end;
end;

procedure Tpickbusimate3.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickbusimate3.DBGrid1DblClick(Sender: TObject);
begin
   modalresult:=mrOK;
end;

procedure Tpickbusimate3.wwDBNavigator1FirstClick(Sender: TObject);
begin
edit1.setfocus;
end;

procedure Tpickbusimate3.FormCloseQuery(Sender: TObject;
...
procedure Tpickbusimate3.FormActivate(Sender: TObject);
begin
edit1.setfocus;
edit1.selectall;
end;

procedure Tpickbusimate3.BitBtn3Click(Sender: TObject);
begin
self.tag:=1;  // <Esc> (cancel) 退出标志
end;

procedure Tpickbusimate3.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickbusimate3.DSquery1DataChange(Sender: TObject; Field: TField);
begin
if activecontrol=dbgrid1 then
edit1.text:=query1.fieldbyname('pick_ord').asstring;
end;

procedure Tpickbusimate3.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
