...
//从第2行开始导入，格式: 1渠道编码,2子渠道编码,3业务省份,4业务城市,5物料编码,6中标价/终端开票价,7开票扣率,8考核价,9推广比率,10金额,11定价单位,12启用日期, 13备注
    ...
    sql:=sql+'select top 10 * from ( select top 5 info=''第''+line_no+''行无渠道编码或数据无效'' from @tab a where f1='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.obj_code=a.f1)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无子渠道编码或数据无效'' from @tab a where f2='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.obj_code=a.f2)';
...
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无业务城市'' from @tab where f4=''''';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无物料编码'' from @tab a where f5='''' or not exists (select 1 from tb_medicine b where b.material_code=a.f5)';
...
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无考核价或数据无效'' from @tab where f8='''' or try_cast(f8 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行考核价数据无效'' from @tab where f8<>'''' and try_cast(f8 as decimal(15,4)) is null';
...
//从第2行开始导入，格式: 1渠道编码,2子渠道编码,3业务省份,4业务城市,5定价参考物料编码,6中标价/终端开票价,7开票扣率,8考核价,9推广比率,10金额,11定价单位,12启用日期, 13备注
edit2.text:=sql;
    with dm.pubqry do
    begin
