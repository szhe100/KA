...
//    commandtext:='select z.rec_id,z.ckd_amot3,z.amot1,y.* from ('; //select h.rec_id,b.dtl_id,a.bod_id,a.carry_dt,a.bod_type_id,bod_type=''分销采购'',';
    ...
    commandtext:=commandtext+' i.type_id,i.type_id2,i.rate,i.rate1,fee=cast(i.fee as decimal(15,2)),amot1a=cast(i.amot as decimal(15,2)),cprice1=i.price1,cprice2=i.price2,fee_type_id=0,';
...
//    commandtext:=commandtext+' left join tb_busiframe3 i on i.rec_id=dbo.fn_getbusiframe3recid(a.VTWEG,a.KDGRP,b.mate_id,c.mate_id,a.MATNR,cast(a.KONV as decimal(15,4)),cast(ZPR03 as decimal(15,4)),a.carry_dt)';
    commandtext:=commandtext+' left join (select c.rec_id,c.type_id,c.rate,c.rate1,c.price1,c.price2,c.type_id2,agent_id1=c.agent_id,d.agent_id,d.amot,d.fee,agent_code=e.mate_code,agent=e.mate_name';
    commandtext:=commandtext+'   from tb_busiframe3 c left join tb_busiframe3_dtl d on c.rec_id=d.rela_id left join tb_busimate e on d.agent_id=e.mate_id) i';
...
//    if Trim(dxEdit1.text)<>'' then commandtext:=commandtext+' and a.GBELN='''+Trim(dxEdit1.text)+'''';
    ...
    commandtext:=commandtext+' a.broker_id,broker=dbo.fn_staff_name(a.broker_id),agent_code=f.mate_code,agent=f.mate_name,a.agent_id,ASSIGNED_BP=f.mate_code,agent1='''',stoppay=isnull(f.stoppay,0),';
