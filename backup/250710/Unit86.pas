...
var
  setexpaycash: Tsetexpaycash;

implementation

uses datamodu, Unit16, Unit31;

{$R *.DFM}

procedure Tsetexpaycash.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetexpaycash.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetexpaycash.FormActivate(Sender: TObject);
begin
{
with homebank do
begin
...
procedure Tsetexpaycash.FormCloseQuery(Sender: TObject;
...
procedure Tsetexpaycash.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetexpaycash.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetexpaycash.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetexpaycash.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetexpaycash.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetexpaycash.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    close;
    commandtext:='select a.*,bank_name=c.mate_name,mate_name='''',mate_name1='''',creater=d.zname,checker=e.zname,broker='''',carryer='''',receiver='''',storager=''''';
    commandtext:=commandtext+' from tb_bill a';
//    commandtext:=commandtext+' left join tb_bank c on a.bank_id =c.bank_id';
    commandtext:=commandtext+' left join tb_busimate c on a.bank_id =c.mate_id';
    commandtext:=commandtext+' left join tb_staff d on a.creat_by =d.sta_id';
    commandtext:=commandtext+' left join tb_staff e on a.check_by =e.sta_id';
    commandtext:=commandtext+' where a.bod_type_id=30';     // a.comp_id='+inttostr(compid)+' and
...
procedure Tsetexpaycash.FormCreate(Sender: TObject);
begin
...
procedure Tsetexpaycash.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetexpaycash.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
begin
    ...
    begin
        ...
        dxDBLookupEdit2.enabled:=(active) and (recordcount>0) and (fieldbyname('creat_by').asinteger=curuserid);
...
procedure Tsetexpaycash.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetexpaycash.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetexpaycash.SpeedButton2Click(Sender: TObject);
//var s: string;
begin
{
with cdsbill do
...
procedure Tsetexpaycash.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetexpaycash.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetexpaycash.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetexpaycash.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
...
procedure Tsetexpaycash.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetexpaycash.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetexpaycash.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetexpaycash.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetexpaycash.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetexpaycash.N1Click(Sender: TObject);
var mi,i,j,k,l : integer;  //
    s,t : string;
begin
...
procedure Tsetexpaycash.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetexpaycash.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetexpaycash.dxDBButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickbusimate10 do
begin
    if not (cdsbill.state in [dsinsert,dsedit]) then cdsbill.edit;
    if showmodal= mrOK then
    begin
        cdsbill.fieldbyname('bank_id').asinteger:= query1.fieldbyname('mate_id').asinteger;
        cdsbill.fieldbyname('bank_name').asstring:= query1.fieldbyname('mate_name').asstring;
    end
    else
    begin
...
procedure Tsetexpaycash.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetexpaycash.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetexpaycash.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#32) and ((Sender as TdxDBButtonEdit).Text<>'') then //
begin
    pickbusimate10.Edit1.Text:=(Sender as TdxDBButtonEdit).Text;
    pickbusimate10.Edit1.OnKeyPress(Sender,Key);
