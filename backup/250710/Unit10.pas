...
//从第2行开始导入，格式: 1交货单号, 2订单编号, 3渠道, 4子渠道, 5业务负责人, 6供货单位, 7商业公司, 8销售人员, 9单据摘要, 10发运日期, 11定价参考物料编码, 12单价, 13数量, 14批号, 15有效期至, 16考核单价, 17考核金额, 18收款金额
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 订单编号'' from @tab where f2=''''';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 渠道编码或数据无效'' from @tab a where f3='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.obj_code=a.f3)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 子渠道编码或数据无效'' from @tab a where f4='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.obj_code=a.f4)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 业务负责人或数据无效'' from @tab a where f5='''' or not exists (select 1 from tb_busimate b where mate_type_id=4 and b.mate_name=a.f5)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 供货单位或数据无效'' from @tab a where f6='''' or not exists (select 1 from tb_busimate b where mate_type_id=2 and b.mate_name=a.f6)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 商业公司或数据无效'' from @tab a where f7='''' or not exists (select 1 from tb_busimate b where mate_type_id=2 and b.mate_name=a.f7)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 销售人员或数据无效'' from @tab a where f8='''' or not exists (select 1 from tb_staff b where sta_type_id=1 and b.zname=a.f8)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 发运日期或数据无效'' from @tab where f10='''' or try_cast(f10 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无  物料编码或数据无效'' from @tab a where f11='''' or not exists (select 1 from tb_medicine b where b.material_code=a.f11)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无单价或数据无效'' from @tab where f12='''' or try_cast(f12 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 数量或数据无效'' from @tab where f13='''' or try_cast(f13 as decimal(15,4)) is null';
