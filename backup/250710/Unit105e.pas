...
procedure TsetexpayB1.setupdatestatus;    // 编辑状态
begin
speedbutton2.enabled:=false;   // refresh disable
speedbutton5.enabled:=false;   // refresh disable
end;

procedure TsetexpayB1.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure TsetexpayB1.FormActivate(Sender: TObject);
begin
with homebank do
begin
...
procedure TsetexpayB1.FormCloseQuery(Sender: TObject;
...
procedure TsetexpayB1.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TsetexpayB1.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpayB1.FormKeyPress(Sender: TObject; var Key: Char);
//var fapp: boolean;
begin
...
procedure TsetexpayB1.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
with bill_dtl do
...
procedure TsetexpayB1.FormCreate(Sender: TObject);
begin
...
procedure TsetexpayB1.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure TsetexpayB1.DSbillStateChange(Sender: TObject);
begin
with bill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpayB1.billBeforeEdit(DataSet: TDataSet);
begin
with bill do
begin
...
procedure TsetexpayB1.billCalcFields(DataSet: TDataSet);
begin
with bill do
...
procedure TsetexpayB1.billReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure TsetexpayB1.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpayB1.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TsetexpayB1.dxDBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
if key in ['a'..'z'] then key:=chr(ord(key)-32);   // turn lower charactor to upper
end;

procedure TsetexpayB1.SpeedButton1Click(Sender: TObject);
begin
with dm.pubqry do
begin
    if active then close;
...
procedure TsetexpayB1.DSbill_dtlStateChange(Sender: TObject);
begin
with bill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpayB1.bill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
showmessage(dataset.name+':'+e.message);
...
procedure TsetexpayB1.SpeedButton2Click(Sender: TObject);
var s: string;
begin
with dm.pubqry do
begin
...
procedure TsetexpayB1.dxDBGrid2CustomDrawCell(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
  AColumn: TdxTreeListColumn; ASelected, AFocused, ANewItemRow: Boolean;
  var AText: String; var AColor: TColor; AFont: TFont;
  var AAlignment: TAlignment; var ADone: Boolean);
...
procedure TsetexpayB1.bill_dtlBeforePost(DataSet: TDataSet);
begin
with bill_dtl do
begin
...
procedure TsetexpayB1.hello1Click(Sender: TObject);
begin
