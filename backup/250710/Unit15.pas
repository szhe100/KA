...
//新格式:1供货单位, 2购货单位, 3单据摘要, 4发生日期, 5单据类别, 6物料编码, 7数量, 8批号, 9有效期至, 10商业价, 11渠道, 12子渠道
    j:=2;  //第1行开始 每行一单   //mi:=0;
    sql:='declare @tab table (line_no varchar(3),f1 varchar(100),f2 varchar(100),f3 varchar(100),f4 varchar(100),f5 varchar(100),f6 varchar(100),f7 varchar(100),f8 varchar(100),f9 varchar(100),f10 varchar(100),f11 varchar(100),f12 varchar(100))';
...
//新格式:1供货单位, 2购货单位, 3单据摘要, 4发生日期, 5单据类别, 6物料编码, 7数量, 8批号, 9有效期至, 10商业价, 11渠道, 12子渠道
    ...
    sql:=sql+'select top 10 * from ( select top 5 info=''第''+line_no+''行 无供货单位或数据无效'' from @tab where f1='''' or not exists (select 1 from tb_busimate where mate_type_id=2 and f1=mate_name)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无购货单位或数据无效'' from @tab where f2='''' or not exists (select 1 from tb_busimate where mate_type_id in (1,2) and f2=mate_name)';
...
//    sql:=sql+' union all select top 5 ''第''+line_no+''行 无业务类别或数据无效'' from @tab where f10='''' or f10 not in (''商业'',''分销'')';
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无渠道(名称)或数据无效'' from @tab where f11='''' or not exists (select 1 from tb_object where obj_type_id=11 and zdesc= f11)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无子渠道(名称)或数据无效'' from @tab where f12='''' or not exists (select 1 from tb_object where obj_type_id=12 and zdesc= f12)';
{
...
//        sql:=sql+'  insert into tb_bill_dtl (bod_id,med_id,bat_cd,valid_dt,price,qty,amot,channel_id,channel_dtl_id)';
        ...
        sql:=sql+'      amot=round(cast(a.f10 as decimal(15,4))*cast(a.f7 as decimal(15,4)),2),c.obj_id,d.obj_id';
