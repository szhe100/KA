...
begin
    ...
    commandtext:='select a.*,zname1=f.zname,d.mate_name,cdistrict=dbo.fn_getdistrict(a.district),stoper=e.zname,creater=b.zname,modifier=c.zname';
...
begin
    ...
    setprogress(1);
//从第2行开始导入，导入格式: 1业务员编码,2业务员名称,3所在地区,4电子邮箱,5联系电话,6收款人,7开户银行,8帐户, 9性别(非必填), 10身份证号(非必填)
    mi:=0;j:=2;  //第1行开始 每行一单
    ...
    begin
        ...
        if s9='男' then s9:='1' else s9:='0';
        s10:=Trim(sheet.cells[j,10].text);
        if
        with dm.pubqry do
        begin
            if active then close;
            commandtext:='select top 1 1 from tb_staff where sta_type_id=1 and zcode='''+s1+'''';   // comp_id='+inttostr(compid)+' and
            open;
            if recordcount=0 then// continue; //已有该编码医药代表，不再加入
            begin
                if active then close;
                ...
                if active then close;
                commandtext:='insert into tb_staff (zcode,zname,qry_code,comp_id,sta_type_id,district,Email,phone,receiver,bank_name,depo_acco,creat_by,creat_dt)';
                commandtext:=commandtext+' values ('''+s1+''','''+s2+''',dbo.fn_getpy('''+s2+'''),'+inttostr(compid)+',1,'+inttostr(id3)+','''+s4+''','''+s5+''','''+s6+''','''+s7+''','''+s8+''','+inttostr(curuserid)+',getdate())';
{
                commandtext:='insert into tb_staff (zcode,zname,qry_code,comp_id,sta_type_id,district,Email,phone,receiver,bank_name,depo_acco,sex,personalID,creat_by,creat_dt)';
                commandtext:=commandtext+' values ('''+s1+''','''+s2+''',dbo.fn_getpy('''+s2+'''),'+inttostr(compid)+',1,'+inttostr(id3)+','''+s4+''','''+s5+''','''+s6+''','''+s7+''','''+s8+''','+s9+','''+s10+''','+inttostr(curuserid)+',getdate())';
}
...
//abort;               
    ...
    XL.Quit;
    SpeedButton5Click(nil);
    MessageBox(0,pchar('Excel数据已导入(共导入'+inttostr(mi)+'个业务员,编码重复业务员未导入)'),'请注意',MB_OK+MB_ICONInformation);
end;
