...
procedure Tsetmedexport4.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetmedexport4.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetmedexport4.FormActivate(Sender: TObject);
begin
with dm.broker do
begin
...
procedure Tsetmedexport4.FormCloseQuery(Sender: TObject;
...
procedure Tsetmedexport4.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetmedexport4.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedexport4.dxDBGrid2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
...
procedure Tsetmedexport4.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
exit;
...
procedure Tsetmedexport4.dxDBGrid2Enter(Sender: TObject);
begin
...
procedure Tsetmedexport4.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetmedexport4.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetmedexport4.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetmedexport4.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=2 and a.cate_id=3';   // cate_id 1:商业 2:分销 3:终端战略 4:终端普通 5:两票商业 6:连锁终端销售出库单 
...
procedure Tsetmedexport4.FormCreate(Sender: TObject);
begin
...
procedure Tsetmedexport4.SpeedButton6Click(Sender: TObject);
var mid : integer;
begin
...
begin
    ...
    commandtext:=commandtext+' values ('+inttostr(compid)+',2,0,3,1,'+inttostr(curuserid)+',getdate(),cast(convert(char(10),getdate(),20) as datetime))';
...
procedure Tsetmedexport4.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetmedexport4.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetmedexport4.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
procedure Tsetmedexport4.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedexport4.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedexport4.SpeedButton2Click(Sender: TObject);
begin
with cdsbill do
begin
...
procedure Tsetmedexport4.SpeedButton4Click(Sender: TObject);
begin
...
procedure Tsetmedexport4.SpeedButton3Click(Sender: TObject);
var s: string;
begin
with dm.pubqry do
begin
...
procedure Tsetmedexport4.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedexport4.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedexport4.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetmedexport4.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
    then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
...
procedure Tsetmedexport4.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetmedexport4.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedexport4.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetmedexport4.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetmedexport4.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedexport4.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetmedexport4.dxDBGrid2DblClick(Sender: TObject);
var key:char;
begin
exit;
key:=#32;
...
procedure Tsetmedexport4.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
...
begin
    ...
    sbodcd:='';
    j:=2; mi:=0;  //第2行开始 每行一单
//格式:1商业公司id1, 2药店名称id2, 3发生日期dt1, 4本公司产品编码id4, 5数量, 6批号, 7有效期dt2, 8流向类别(纯销/分销)id8, 9单据摘要
...
//格式:1商业公司id1, 2药店名称id2, 3发生日期dt1, 4本公司产品编码id4, 5数量, 6批号, 7有效期dt2, 8流向类别(纯销/分销)id8, 9单据摘要
    ...
    begin
        ...
        begin
            ...
            if Trim(sheet.cells[j,8].text)='分销' then id8:=2 else id8:=1;
            if strtoint(cleardeli(sheet.cells[j,4].text))<0 then id11:=2 else id11:=1; //id11=1 销售，id11=2 退货
            ...
            commandtext:=commandtext+' values ('+inttostr(compid)+','''+sb+''',2,0,3,'+inttostr(id11)+','+inttostr(id8)+','+inttostr(curuserid)+',getdate(),';
            commandtext:=commandtext+inttostr(id1)+','+ inttostr(id2)+','+inttostr(id9)+','''+Trim(sheet.cells[j,9].text)+''','''+datetimetostr(dt1)+''')';        //+','+inttostr(id10)
            commandtext:=commandtext+' select @@identity';
...
//格式:1商业公司id1, 2药店名称id2, 3发生日期dt1, 4本公司产品编码id4, 5数量, 6批号, 7有效期dt2, 8流向类别(纯销/分销)id8, 9单据摘要
            if active=true then close;
            commandtext:='insert into tb_bill_dtl (bod_id,lead_id,med_id,qty,bat_cd,valid_dt)';   //,valid_dt
            commandtext:=commandtext+' values ('+inttostr(mid)+','+inttostr(id10)+','+inttostr(id4)+',';
            if sheet.cells[j,5].text='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,5].text)+',';
...
procedure Tsetmedexport4.N1Click(Sender: TObject);
...
{       ///后面取中标价
        ...
        begin
            first;
            s:='下列品种尚未设置终端业务结构(请注意生效日期)，本单不可审核'+ #13#10;
            s:=s+'-----------------------------------------------------';  //+ #13#10
...
procedure Tsetmedexport4.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetmedexport4.N3Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t : string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetmedexport4.dxDBButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetmedexport4.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetmedexport4.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetmedexport4.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tsetmedexport4.Button1Click(Sender: TObject);
begin
...
procedure Tsetmedexport4.N5Click(Sender: TObject);
begin
if pubqry.active=true then pubqry.close;
pubqry.commandtext:='select bod_id from tb_bill where bod_type_id=2 and isnull(mate_id1,0)=0 and isnull(mate_id2,0)=0 and isnull(mate_id3,0)=0 and isnull(mate_id4,0)=0 and isnull(mate_id5,0)=0';
pubqry.open;
...
procedure Tsetmedexport4.dxEdit1Click(Sender: TObject);
begin
dxEdit1.SelectAll;
dxEdit1.SetFocus;
end;

procedure Tsetmedexport4.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetmedexport4.dxDBButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedexport4.dxDBButtonEdit2KeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#32) and ((Sender as TdxDBButtonEdit).Text<>'') then //
begin
...
procedure Tsetmedexport4.dxDBButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickstore do
begin
...
procedure Tsetmedexport4.dxDBButtonEdit4KeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#32) and ((Sender as TdxDBButtonEdit).Text<>'') then //
begin
