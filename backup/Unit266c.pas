...
var mi: integer;
begin
with dtl do if active then close;
with busiframe do
begin
    if active then
    begin
...
//    commandtext:=commandtext+' a.*,h.price10,broker='''',c.mate_name,agent=d.mate_name,m.med_code,m.material_code1,m.med_name,m.chm_name,m.specifi,med_unit=dbo.fn_obj_desc(m.unit_id),m.qtyperpack,'; //pdt_place=dbo.fn_mate_name(m.pdt_id),
    commandtext:=commandtext+' a.*,broker='''',c.mate_name,agent=d.mate_name,m.med_code,m.material_code,m.material_code1,m.med_name,m.chm_name,m.specifi,med_unit=dbo.fn_obj_desc(m.unit_id),m.qtyperpack,'; //pdt_place=dbo.fn_mate_name(m.pdt_id),
...
//    commandtext:=commandtext+' left join tb_staff b on a.broker_id=b.sta_id';
    commandtext:=commandtext+' left join (select mate_id,mate_name,prov_code,prov_name=BEZEI,city_code,city_name=CITY1 from tb_busimate x left join tb_district y on x.district=y.rec_id) c on a.mate_id=c.mate_id';
...
begin
    ...
    setprogress(1);
//导入 格式: 1商业公司,2业务负责人,3产品编码,4生效日期,5渠道编码,6子渠道编码,7比率,8销售单价,9考核单价,10应付金额/盒,11促销率,12促销费/盒,13明细业务负责人(选填), 14单据摘要, 15备注
    mi:=0;j:=2;  //第1行开始 每行一单
...
//导入 格式: 1商业公司,2业务负责人,3产品编码,4生效日期,5渠道编码,6子渠道编码,7比率,8销售单价,9考核单价,10应付金额/盒,11促销率,12促销费/盒,13明细业务负责人(选填), 14单据摘要, 15备注
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无业务负责人或数据无效'' from @tab a where f2='''' or not exists (select 1 from tb_busimate b where mate_type_id=4 and b.mate_name =a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无定价参考物料编码或数据无效'' from @tab a where f3='''' or not exists (select 1 from tb_medicine m where m.material_code1 =a.f3)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无启用日期或数据无效'' from @tab where f4='''' or try_cast(f4 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无渠道或数据无效'' from @tab a where a.f5='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.obj_code=a.f5)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无子渠道或数据无效'' from @tab a where a.f6='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.obj_code=a.f6)';
...
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无明细业务负责人或数据无效'' from @tab a where f13='''' or not exists (select 1 from tb_busimate b where mate_type_id=4 and b.mate_name =a.f13)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 明细业务负责人数据无效'' from @tab a where f13<>'''' and not exists (select 1 from tb_busimate b where mate_type_id=4 and b.mate_name =a.f13)';
    sql:=sql+' ) a order by info';
//导入 格式: 1商业公司,2业务负责人,3产品编码,4生效日期,5渠道编码,6子渠道编码,7税率,8销售单价,9考核单价,10应付金额/盒,11促销率,12促销费/盒,13明细业务负责人, 14单据摘要, 15备注
    with dm.pubqry do
    begin
        if active then close;
        commandtext:=sql;
edit1.text:=commandtext;        
        ...
        if active then close;
//导入 格式: 1商业公司,2业务负责人,3产品编码,4生效日期,5渠道编码,6子渠道编码,7比率,8销售单价,9考核单价,10应付金额/盒,11促销率,12促销费/盒, 13明细业务负责人, 14单据摘要, 15备注
        sql:=sql1;  //取前脚本
        ...
        sql:=sql+' insert into tb_busiframe3 (comp_id,channel_id,channel_dtl_id,mate_id,agent_id,med_id,valid_dt,creat_by,creat_dt)';
...
// 以下为 无主要数据 的 insert into tb_brokermed
        sql:=sql+' insert into tb_busiframe3 (comp_id,channel_id,channel_dtl_id,mate_id,agent_id,med_id,price1,price2,rate,rate1,bod_desc,valid_dt,creat_by,creat_dt)'; //remark, rate, amot,fee,rate1,
...
// 以下为 无明细数据 的 insert tb_busiframe3_dtl
        sql:=sql+' insert into tb_busiframe3_dtl (rela_id,agent_id,rate,amot,fee,fee_type_id,creat_by,creat_dt)';
        sql:=sql+' select rela_id=k.rec_id,agent_id=a.mate_id2,rate=cast(f7 as decimal(15,4)),amot=cast(f10 as decimal(15,4)),fee=cast(f12 as decimal(15,4)),';
...
begin
    ...
    commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,bod_desc,creat_by,creat_dt)';
...
//edit1.text:=commandtext;
    ...
    close;
    commandtext:='insert into tb_bill_dtl (bod_id,med_id)';  //,price,price1,qty,valid_dt,remark
...
edit2.text:=commandtext;
    ...
    if active then close;
    commandtext:='insert into tb_busiframe3_dtl (agent_id,mate_id,med_id,rela_id,fee_type_id,creat_by,creat_dt)';
