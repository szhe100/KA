...
//参照部分结束
    ...
    commandtext:=commandtext+' where a.bod_type_id=42 and a.bod_status_id=1 and a.busi_type in (1,2)'; //两票销售出库单 busi_type: 1销售,2退货,3调价 21.6.15应要求排除busi_type=3调价
...
//    if fdistltd then commandtext:=commandtext+' and exists (select 1 from tb_busimate s,tb_trustdistrict t where t.sta_id='+inttostr(curuserid)+' and a.dst_id=s.mate_id and dbo.fn_ischildnode(s.district,t.dist_id)=1)';
    ...
    commandtext:=commandtext+' recdt=cast(null as datetime),reccd='''',';
    commandtext:=commandtext+' cqty=0,';  //cqty 为厂家返入数量
...
//    commandtext:=commandtext+' left join (select b.med_id,amot5=sum(b.amot) from tb_bill a,tb_bill_dtl b where a.bod_type_id=49 and a.bod_status_id<>1 and a.bod_id=b.bod_id and b.type_id=43 group by b.med_id) i on a.bod_id=i.med_id';
    ...
    if RadioGroup3.ItemIndex=1 then commandtext:=commandtext+'  and a.bod_amot=a.ckd_amot'; //已完成付款
    if RadioGroup3.ItemIndex=2 then commandtext:=commandtext+'  and a.bod_amot<>isnull(a.ckd_amot,0)'; //未完成付款
    if RadioGroup3.ItemIndex=3 then commandtext:=commandtext+'  and (1=2)'; //两票其他支出记账单 无两票促销规则
        	...
        	or (fieldbyname('Camot4').asfloat<0) and (fieldbyname('ckd_amot3').asfloat<=fieldbyname('Camot4').asfloat));
{
	...
	fieldbyname('camot5').asfloat:=fieldbyname('amot2').asfloat-fieldbyname('ckd_amot').asfloat; //已审未付金额=已审核金额—已付款金额
	fieldbyname('camot6').asfloat:=fieldbyname('Camot4').asfloat+fieldbyname('Cfeeamot').asfloat-fieldbyname('amot1').asfloat; //未申请金额=应付金额+应付促销费—已申请金额
end;
end;
...
if mi=0 then raise Exception.Create('无可生成两票其他支出核销单记录(完成核销申请的不可再次核销)');
