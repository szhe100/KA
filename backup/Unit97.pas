...
type
  Tsetexpinvoice = class(TForm)
    ...
    Label29: TLabel;
    dxDBEdit2: TdxDBEdit;
    cdsbillmate_id4: TAutoIncField;
    cdsbillmate_id5: TAutoIncField;
    cdsbillmate_name4: TStringField;
    cdsbillmate_name5: TStringField;
    cdsbillbod_desc1: TStringField;
    cdsbillbod_desc2: TStringField;
    cdsbill_dtlbod_cd: TStringField;
    dxDBGrid2bod_cd: TdxDBGridColumn;
    cdsbill_dtlinvo_cd: TStringField;
    cdsbill_dtlinvo_dt: TDateTimeField;
    dxDBGrid2invo_cd: TdxDBGridColumn;
    dxDBGrid2invo_dt: TdxDBGridDateColumn;
    Bevel1: TBevel;
    ...
    cdsbill_dtlCqty1: TBCDField;
    cdsbill_dtlCqty2: TBCDField;
    dxDBGrid2Cqty1: TdxDBGridColumn;
    dxDBGrid2Cqty2: TdxDBGridColumn;
...
var
  setexpinvoice: Tsetexpinvoice;

implementation

uses datamodu, Unit7, Unit28b, Unit16c, Unit16a, Unit9, Unit16j, Unit16i;

{$R *.DFM}

procedure Tsetexpinvoice.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetexpinvoice.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetexpinvoice.FormActivate(Sender: TObject);
begin
//with dm.imp_type do
...
procedure Tsetexpinvoice.FormCloseQuery(Sender: TObject;
...
procedure Tsetexpinvoice.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetexpinvoice.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetexpinvoice.dxDBGrid2Enter(Sender: TObject);
begin
...
procedure Tsetexpinvoice.FormKeyPress(Sender: TObject; var Key: Char);
//var fapp: boolean;
begin
...
procedure Tsetexpinvoice.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetexpinvoice.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetexpinvoice.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:='select a.*,mate_name1=k.mate_name,mate_name2=c.mate_name,agent=f.mate_name,broker=g.zname,mate_name4=h.mate_name,mate_name5=i.mate_name,creater=d.zname,checker=e.zname,';
    commandtext:=commandtext+' c.zname1,c.address1,c.phone1,c.zname2,c.address2,c.phone2,cdistrict=dbo.fn_getdistrict(f.district),finished=dbo.fn_orderisfinished1(a.bod_id),'; //dbo.fn_orderisfinished(a.bod_id)';
    commandtext:=commandtext+' bod_cd1=dbo.fn_getbodcdbyorderid(a.order_id,45)';
//    commandtext:=commandtext+' cdistrict=dbo.fn_getdistrict(a.district_id)';
    ...
    commandtext:=commandtext+' left join tb_busimate h on a.mate_id4 =h.mate_id';
    commandtext:=commandtext+' left join tb_busimate i on a.mate_id5 =i.mate_id';
    commandtext:=commandtext+' where a.bod_type_id=46';
...
procedure Tsetexpinvoice.FormCreate(Sender: TObject);
begin
...
procedure Tsetexpinvoice.SpeedButton6Click(Sender: TObject);
var mid :integer;
begin
setprogress(1);
with dm.pubqry do
begin
    if active=true then close;
    commandtext:='insert into tb_bill (comp_id,bod_type_id,bod_status_id,creat_by,creat_dt,carry_dt)';    // cate_id 1:商业 2:分销 3:终端
    commandtext:=commandtext+' values ('+inttostr(compid)+',39,0,'+inttostr(curuserid)+',getdate(),cast(convert(char(10),getdate(),20) as datetime))';
...
procedure Tsetexpinvoice.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetexpinvoice.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
begin
    ...
    begin
        ...
        dxDBEdit2.enabled:=(active=true) and (recordcount>0) and (fieldbyname('creat_by').asinteger=curuserid);
        dxDBEdit3.enabled:=(active=true) and (recordcount>0) and (fieldbyname('creat_by').asinteger=curuserid);
...
//            and (fieldbyname('creat_by').asinteger=curuserid);
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active=true then close;
                commandtext:='select a.*,c.bod_cd,c.bod_desc,broker=d.zname,leader=e.zname,m.med_name,m.specifi,m.pdt_place,m.med_code,m.qtyperpack,med_unit=f.zdesc';
...
procedure Tsetexpinvoice.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
begin
    ...
    begin
       ...
       if focusedcolumn=9 then  // 输入完最后的字段的cursor控制
       begin
            fapp:=false;
            with cdsbill_dtl do
            begin
...
procedure Tsetexpinvoice.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetexpinvoice.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetexpinvoice.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
    if fieldbyname('bod_status_id').asinteger=1
        then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
    if curuserid<>fieldbyname('creat_by').asinteger then raise Exception.Create('非本人单据，请勿更改');
end;
end;

procedure Tsetexpinvoice.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetexpinvoice.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetexpinvoice.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
    then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
//if curuserid<>cdsbill.fieldbyname('creat_by').asinteger then raise Exception.Create('非本人单据，请勿更改');
end;

procedure Tsetexpinvoice.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
//        then raise Exception.Create('请输入对照单号');
    if fieldbyname('invo_cd').asstring='' then raise Exception.Create('请输入发票号码');
    if fieldbyname('invo_dt').asstring='' then raise Exception.Create('请输入开票日期');
//    if (fieldbyname('price').asfloat=0) or (fieldbyname('qty').asfloat=0)
//        then raise Exception.Create('货品单价或数量不可为零');
end;
end;

procedure Tsetexpinvoice.cdsbillBeforePost(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetexpinvoice.cdsbillReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetexpinvoice.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetexpinvoice.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetexpinvoice.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetexpinvoice.PageControl1Change(Sender: TObject);
begin
...
//                    raise Exception.Create('读取图片失败，可重新存入图片');
//            		MessageBox(0,pchar('读取图片失败，可重新存入图片'),'请注意',MB_OK+MB_ICONInformation);
{
                    if active=true then close;
                    commandtext:='select top 1 * from tb_image where type_id=1 and id='+cdsbill.fieldbyname('bod_id').asstring;
                    open;
...
procedure Tsetexpinvoice.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetexpinvoice.dxDBGrid2DblClick(Sender: TObject);
begin
{
...
procedure Tsetexpinvoice.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
...
edit1.text:=commandtext;            
            ...
            sb:=formatfloat('00000000',getbillid(compid,39));
...
procedure Tsetexpinvoice.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetexpinvoice.N3Click(Sender: TObject);
...
procedure Tsetexpinvoice.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetexpinvoice.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetexpinvoice.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tsetexpinvoice.dxDBButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetexpinvoice.dxDBButtonEdit1KeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#32) and ((Sender as TdxDBButtonEdit).Text<>'') then //
begin
...
procedure Tsetexpinvoice.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetexpinvoice.dxEdit1Click(Sender: TObject);
begin
dxEdit1.SelectAll;
dxEdit1.SetFocus;
end;

procedure Tsetexpinvoice.dxDBButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickbusimate8 do
begin
    if not (cdsbill.state in [dsinsert,dsedit]) then cdsbill.edit;
    if showmodal= mrOK then
    begin
        cdsbill.fieldbyname('mate_id5').asinteger:= query1.fieldbyname('mate_id').asinteger;
        cdsbill.fieldbyname('mate_name5').asstring:= query1.fieldbyname('mate_name').asstring;
    end
    else
    begin
        cdsbill.fieldbyname('mate_id5').asinteger:= 0;
        cdsbill.fieldbyname('mate_name5').asstring:= '';
    end;
end;
end;

procedure Tsetexpinvoice.SpeedButton2Click(Sender: TObject);
var s: string;
begin
with cdsbill do
begin
    ...
    if (fieldbyname('zname1').asstring='') or (fieldbyname('address1').asstring='') or (fieldbyname('phone1').asstring='')
        then raise Exception.Create('收货人资料(姓名地址电话)不完整，不可送审核');
    if (fieldbyname('zname2').asstring='') or (fieldbyname('address2').asstring='') or (fieldbyname('phone2').asstring='')
        then raise Exception.Create('收票人资料(姓名地址电话)不完整，不可送审核');
end;
with cdsbill_dtl do if (active=True) and (RecordCount=0) then raise Exception.Create('无明细数据，不可送审核');
with dm.pubqry do
begin
    if active=True then close;
    commandtext:='select top 5 m.med_code,m.material_code1,m.med_name,m.specifi,m.pdt_place,h.price,h.amot';
    commandtext:=commandtext+' from (select d.med_id,price=sum(b.amot),amot=sum(b.amot) from tb_bill a,tb_bill_dtl b,tb_bill_dtl d';
    commandtext:=commandtext+' where a.bod_type_id=46 and a.order_id='+cdsbill.fieldbyname('order_id').asstring+' and a.bod_id=b.bod_id and b.med_id=d.dtl_id';
    commandtext:=commandtext+' group by d.med_id) h,tb_medicine m';
//    commandtext:=commandtext+' group by d.med_id having sum(b.amot)>sum(b.price)) h,tb_medicine m';
    commandtext:=commandtext+' where h.med_id=m.med_id and h.price<h.amot';
    open;
    if recordcount>0 then
    begin
        s:='下列品种合计实开金额将超过应开金额，本单不可送审核'+#13+#10+'------------------------------------------------';
        first;
        while not eof do
        begin
            s:=s+#13+#10+fieldbyname('material_code1').asstring+' '+fieldbyname('med_name').asstring+' '+fieldbyname('specifi').asstring
            +' 应开金额:'+formatfloat('###,##0',fieldbyname('price').asfloat)+' 合计实开金额:'+formatfloat('###,##0',fieldbyname('amot').asfloat);
            next;
        end;
...
begin
    if active=true then close;
    commandtext:='update tb_bill set bod_status_id=2,receipt_by='+inttostr(curuserid)+',receipt_dt=getdate() where bod_id='+cdsbill.fieldbyname('bod_id').asstring;
    execute;
    close;
end;
...
procedure Tsetexpinvoice.dxDBGraphicEdit1AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetexpinvoice.dxDBGraphicEdit2AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetexpinvoice.dxDBGraphicEdit3AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetexpinvoice.dxDBGraphicEdit4AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetexpinvoice.dxDBGraphicEdit5AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetexpinvoice.dxDBGraphicEdit1Change(Sender: TObject);
begin
setupdatestatus;
//cdsbill.edit;
end;

procedure Tsetexpinvoice.dxDBGraphicEdit1GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass1').asstring));
end;

procedure Tsetexpinvoice.dxDBGraphicEdit2GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass2').asstring));
end;

procedure Tsetexpinvoice.dxDBGraphicEdit3GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass3').asstring));
end;

procedure Tsetexpinvoice.dxDBGraphicEdit4GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass4').asstring));
end;

procedure Tsetexpinvoice.dxDBGraphicEdit5GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass5').asstring));
end;

procedure Tsetexpinvoice.dxDBGraphicEdit1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
...
procedure Tsetexpinvoice.qryimageAfterPost(DataSet: TDataSet);
begin
qryimage.ApplyUpdates(-1);
end;

procedure Tsetexpinvoice.N2Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t,mcd: string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetexpinvoice.N4Click(Sender: TObject);
...
procedure Tsetexpinvoice.N5Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t,mcd: string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetexpinvoice.dxDBButtonEdit2Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetexpinvoice.dxDBButtonEdit2KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetexpinvoice.dxDBButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickbusimate7 do
begin
    if not (cdsbill.state in [dsinsert,dsedit]) then cdsbill.edit;
    if showmodal= mrOK then
    begin
        cdsbill.fieldbyname('mate_id4').asinteger:= query1.fieldbyname('mate_id').asinteger;
        cdsbill.fieldbyname('mate_name4').asstring:= query1.fieldbyname('mate_name').asstring;
    end
    else
    begin
        cdsbill.fieldbyname('mate_id4').asinteger:= 0;
        cdsbill.fieldbyname('mate_name4').asstring:= '';
    end;
end;
end;

procedure Tsetexpinvoice.MenuItem1Click(Sender: TObject);
var mi: integer;
begin
cdsbillBeforeEdit(nil);
...
begin
    ...
    commandtext:='insert into tb_bill_dtl (bod_id,med_id,price1,price2,amot,invo_cd,invo_dt)';  //price,  应开金额不需复制
    commandtext:=commandtext+' select top 1 bod_id,med_id,price1,price2,amot,invo_cd,invo_dt from tb_bill_dtl where dtl_id='+cdsbill_dtl.fieldbyname('dtl_id').asstring;
    commandtext:=commandtext+' select @@identity';
    open;
...
procedure Tsetexpinvoice.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
	if fieldbyname('price1').asfloat=0 then
    begin
    ...
    begin
        fieldbyname('cqty1').asfloat:=round(100*fieldbyname('price').asfloat/fieldbyname('price1').asfloat)/100;
        fieldbyname('cqty2').asfloat:=round(100*fieldbyname('amot').asfloat/fieldbyname('price1').asfloat)/100;
    end;
end;
end;

procedure Tsetexpinvoice.N6Click(Sender: TObject);
...
if dxDBGrid1.SelectedCount<2 then
begin
    if (cdsbill.fieldbyname('finished').asBoolean=False) then
    begin
        s:= s + #13#10+ cdsbill.fieldbyname('bod_cd').asstring+'单'; //+qry.fieldbyname('med_name').asstring;
        t:=cdsbill.fieldbyname('bod_id').asstring;
        mi:=1;
    end;
end
else
...
//    f2:= dxDBGrid1.ColumnByFieldName('med_name').Index;
    ...
    f4:= dxDBGrid1.ColumnByFieldName('bod_id').Index;
    f5:= dxDBGrid1.ColumnByFieldName('finished').Index;
    for i:= 0 to dxDBGrid1.SelectedCount - 1 do
    begin
        if (dxDBGrid1.SelectedNodes[i].values[f5]=False) then   // and (dxDBGrid1.SelectedNodes[i].values[f3]=2)
        begin
            if mi=0 then
            begin
...
//            s:= s + ' '+formatfloat('###,###,##0.00',dxDBGrid1.SelectedNodes[i].values[f5]);
            t:= t +','+ dxDBGrid1.SelectedNodes[i].Strings[f4];  // 记录 bod_id 字符串
...
}
{
    ...
    if active=True then close;
    commandtext:='select top 5 a.bod_cd,bod_cd1=c.bod_cd from tb_bill a,tb_bill c';
    commandtext:=commandtext+' where a.bod_type_id=46 and a.bod_id in ('+t+') and c.bod_type_id=45 and c.bod_status_id<>1 and a.bod_id=c.relative_id';
    open;
    if recordcount>0 then
    begin
//        s:='下列单据已生成了未完成审核的两票销售单，请先审核'+#13+#10+'------------------------------------------------';
...
begin
    ...
    begin
        ...
        if dm.pubqry.active=True then dm.pubqry.close;
        dm.pubqry.commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,cate_id,src_id,dst_id,agent_id,broker_id,district_id,order_id,relative_id,carry_dt,creat_by,creat_dt)';
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select top 1 '+inttostr(compid)+','''+mcd+''',45,0,cate_id,src_id,dst_id,agent_id,broker_id,district_id,order_id,bod_id,cast(convert(char(10),getdate(),20) as datetime),'+inttostr(curuserid)+',getdate()';
        dm.pubqry.commandtext:=dm.pubqry.commandtext+'  from tb_bill where bod_id='+fieldbyname('bod_id').asstring;
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select @@identity';
//edit1.text:=dm.pubqry.commandtext;
        dm.pubqry.open;
        mi:=dm.pubqry.fields[0].asinteger;
        dm.pubqry.close;
        dm.pubqry.commandtext:='insert into tb_bill_dtl (bod_id,med_id,price,amot,price1,price2,invo_cd,invo_dt)';
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select '+inttostr(mi)+',med_id,(amot-isnull(ckd_amot1,0)),(amot-isnull(ckd_amot1,0)),price1,price2,invo_cd,invo_dt';
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' from tb_bill_dtl where bod_id='+fieldbyname('bod_id').asstring;
    	dm.pubqry.commandtext:=dm.pubqry.commandtext+' update a set cate_id=c.cate_id from tb_bill a,tb_bill_dtl b,tb_bill c,tb_bill_dtl d';
    	dm.pubqry.commandtext:=dm.pubqry.commandtext+'  where a.bod_id='+inttostr(mi)+' and a.bod_id=b.bod_id and b.med_id=d.dtl_id and c.bod_id=d.bod_id';
//edit2.text:=dm.pubqry.commandtext;
        dm.pubqry.execute;
        next;
    end;
...
procedure Tsetexpinvoice.N8Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'列表1.xls', dxDBGrid1.SaveToXLS, self.tag)
end;

procedure Tsetexpinvoice.SpeedButton4Click(Sender: TObject);
begin
