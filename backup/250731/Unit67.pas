...
type
  Tsetlinkword = class(TForm)
    ...
    Label11: TLabel;
    link1material_code1: TStringField;
    Edit1: TEdit;
    Edit2: TEdit;
    Edit3: TEdit;
    link1material_code: TStringField;
...
begin
    ...
    begin
        ...
        commandtext:='select a.*,mate_name1=d.mate_name,m.med_code,m.material_code,m.material_code1,m.med_name,m.chm_name,m.specifi,m.pdt_place,qtyperpack,med_unit=dbo.fn_obj_desc(m.unit_id),med_type=dbo.fn_med_type(m.med_id),creater=c.zname';
        ...
        if Trim(dxEdit1.text)<>'' then commandtext:=commandtext+' and a.zdesc like ''%'+Trim(dxEdit1.text)+'%''';
...
begin
    ...
    begin
        ...
        if Trim(dxEdit2.text)<>'' then commandtext:=commandtext+' and a.zdesc like ''%'+Trim(dxEdit2.text)+'%''';
        if Trim(dxEdit3.text)<>'' then commandtext:=commandtext+' and a.zdesc like ''%'+Trim(dxEdit3.text)+'%''';
        ...
        if Trim(dxButtonEdit4.text)<>'' then commandtext:=commandtext+' and a.mate_id1='+inttostr(dxButtonEdit4.tag);
//edit1.text:=commandtext;
        try
            setprogress(1);
            open;
...
begin
    ...
    commandtext:='select top 1 1 from tb_linkword where type_id=1 and mate_id1='+inttostr(dxButtonEdit3.tag)+' and med_id='+inttostr(dxButtonEdit1.tag);
    commandtext:=commandtext+' and zdesc='''+Trim(dxEdit1.text)+'''';
    open;
    if recordcount>0 then raise Exception.Create('该商业公司、品种已有此流向词语对照记录，不可重复设置');
{
    if recordcount>0 then
    begin
        MessageBox(0,pchar('该商业公司、品种已有此流向词语对照记录，不可重复设置'),'请注意',MB_OK+MB_ICONError);
        abort;
    end;
}
end;
...
//    commandtext:='select top 1 1 from tb_linkword where comp_id='+inttostr(compid)+' and type_id=2';
    commandtext:='select top 1 1 from tb_linkword where type_id=2 and mate_id1='+inttostr(dxbuttonedit4.tag)+' and mate_id='+inttostr(dxbuttonedit2.tag)+' and zdesc='''+Trim(dxEdit2.text)+'''';
edit1.text:=commandtext;    
    open;
    if recordcount>0 then raise Exception.Create('该商业公司、客户已有此流向词语对照记录，不可重复设置');
    //MessageBox(0,pchar('该商业公司、客户已有此流向词语对照记录，不可重复设置'),'请注意',MB_OK+MB_ICONInformation);
end;
...
//从第2行开始导入,导入格式: 1商业公司, 2客户名称, 3流向词语
    ...
    sql:=sql+' select top 10 * from ( select top 5 info=''第''+line_no+''行 无商业公司'' from @tab a where f1='''' or not exists (select 1 from tb_busimate b where b.mate_type_id=2 and b.mate_name=a.f1)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无客户名称'' from @tab a where f2='''' or not exists (select 1 from tb_busimate b where b.mate_type_id=1 and b.mate_name=a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无流向词语'' from @tab where f3=''''';  // or try_cast(f4 as datetime) is null

//    sql:=sql+' union all select top 5 ''第''+line_no+''行 未取得商业公司'' from @tab a where not exists (select 1 from tb_busimate b where b.mate_type_id=2 and )';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行 未取得客户'' from @tab a where not exists (select 1 from tb_busimate b where b.mate_type_id=1 and b.mate_name=a.f2)';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行 商业公司及流向词语与已有记录重复'' from @tab a inner join tb_busimate b on b.mate_type_id=2 and b.mate_name=a.f1';
//    sql:=sql+'  where exists (select 1 from tb_linkword c where c.type_id=2 and c.mate_id1=b.mate_id and c.zdesc=a.f3)';
...
//从第2行开始导入,导入格式: 1商业公司, 2客户名称, 3流向词语
        ...
        sql:=sql+' update a set mate_id1=b.mate_id from @tab a,tb_busimate b where b.mate_type_id=2 and b.mate_name=a.f1';
        sql:=sql+' update a set mate_id =b.mate_id from @tab a,tb_busimate b where b.mate_type_id=1 and b.mate_name=a.f2';

        sql:=sql+' update a set zdesc=b.f3';
        sql:=sql+' from tb_linkword a,@tab b';
        sql:=sql+' where a.type_id=2 and a.mate_id=b.mate_id and a.mate_id1=b.mate_id1';
        sql:=sql+' set @cnt1=@@ROWCOUNT';

        sql:=sql+' insert into tb_linkword (type_id,comp_id,mate_id1,mate_id,zdesc,creat_by,creat_dt)';
        sql:=sql+'  select distinct type_id=2,comp_id='+inttostr(compid)+',mate_id1=a.mate_id1,mate_id=a.mate_id,zdesc=a.f3,creat_by='+inttostr(curuserid)+',creat_dt=getdate()';
        sql:=sql+' from @tab a';
        sql:=sql+' where not exists (select 1 from tb_linkword b where type_id=2 and a.mate_id=b.mate_id and a.mate_id1=b.mate_id1)';
...
//    if (m1>0) or (m2>0) then
    if m2=0 then MessageBox(0,pchar('未导入记录,请检查导入数据(重复数据不再导入)'),'请注意',MB_OK+MB_ICONInformation)
    else
    begin
...
//从第2行开始导入,导入格式: 1商业公司, 2关联编码, 3流向词语, 4单位系数
    sql1:=sql;  //导入数据无错误时，用sql1 插入数据
    sql:=sql+' select top 10 * from ( select top 5 info=''第''+line_no+''行 无商业公司或数据无效'' from @tab a where a.f1='''' or not exists (select 1 from tb_busimate b where b.mate_type_id=2 and a.f1=b.mate_name)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无定价参考物料编码或数据无效'' from @tab a where a.f2='''' or not exists (select 1 from tb_medicine b where b.material_code=a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无流向词语'' from @tab where f3='''''; // or try_cast(f4 as datetime) is null
//    sql:=sql+' union all select top 5 ''第''+line_no+''行 无单位系数或数据无效'' from @tab where f4='''' or try_cast(f4 as decimal(6,2)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 单位系数'' from @tab where f4<>'''' and try_cast(f4 as decimal(6,2)) is null';

//    sql:=sql+' union all select top 5 ''第''+line_no+''行 未取得商业公司'' from @tab a where not exists (select 1 from tb_busimate b where b.mate_type_id=2 and )';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行 未取得品种'' from @tab a where not exists (select 1 from tb_medicine b where b.med_code=a.f2)';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行 商业公司及流向词语与已有记录重复'' from @tab a inner join tb_busimate b on b.mate_type_id=2 and b.mate_name=a.f1';
//    sql:=sql+'  where exists (select 1 from tb_linkword c where c.type_id=1 and c.mate_id1=b.mate_id and c.zdesc=a.f3)';
...
//从第2行开始导入,导入格式: 1商业公司, 2关联编码, 3流向词语, 4单位系数
        ...
        sql:=sql+' declare @cnt1 int,@cnt2 int';
        sql:=sql+' update a set mate_id1=b.mate_id from @tab a,tb_busimate b where b.mate_type_id=2 and b.mate_name=a.f1';
        sql:=sql+' update a set med_id =b.med_id from @tab a,tb_medicine b where b.material_code=a.f2';

        sql:=sql+' update a set zdesc=b.f3,rate=case when b.f4='''' then 0 else cast(b.f4 as decimal(6,2)) end';
        sql:=sql+' from tb_linkword a,@tab b';
        sql:=sql+' where a.type_id=1 and a.mate_id1=b.mate_id1 and a.med_id=b.med_id';
        sql:=sql+' set @cnt1=@@ROWCOUNT';

        sql:=sql+' insert into tb_linkword (type_id,comp_id,mate_id1,med_id,zdesc,rate,creat_by,creat_dt)';
        sql:=sql+'  select distinct type_id=1,comp_id='+inttostr(compid)+',mate_id1=a.mate_id1,med_id=a.med_id,zdesc=a.f3,rate=case when f4='''' then 0 else cast(f4 as decimal(6,2)) end,creat_by='+inttostr(curuserid)+',creat_dt=getdate()';
        sql:=sql+' from @tab a';
        sql:=sql+' where not exists (select 1 from tb_linkword b where b.type_id=1 and a.mate_id1=b.mate_id1 and a.med_id=b.med_id)';
        sql:=sql+' set @cnt2=@@ROWCOUNT  select @cnt1,@cnt2';
edit2.text:=sql;
        try
            setprogress(1);
    ...
    SpeedButton3Click(nil);
//    if (m1>0) or (m2>0) then
    if (m1=0) and (m2=0) then MessageBox(0,pchar('未导入记录,请检查导入数据(重复数据不再导入)'),'请注意',MB_OK+MB_ICONInformation)
    else
    begin
//        MessageBox(0,pchar('已成功修改'+inttostr(m1)+'条记录, 新增'+inttostr(m2)+'条记录'),'请注意',MB_OK+MB_ICONInformation);
