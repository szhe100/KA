...
type
  Tpickagent = class(TForm)
    ...
    Edit1: TdxEdit;
    DBGrid1mate_code: TdxDBGridColumn;
...
begin
    ...
    begin
        ...
        commandtext:=commandtext+' mate_id,mate_code,mate_name,deputy,phone,address,broker_id,broker=dbo.fn_staff_name(broker_id),';
        commandtext:=commandtext+' district,cdistrict=isnull(BEZEI,'''')+'' ''+isnull(CITY1,''''),settle_type,stop_yn,qry_code from tb_busimate';
//        commandtext:=commandtext+' district,cdistrict=dbo.fn_getdistrict(district),settle_type,stop_yn,qry_code from tb_busimate';
        ...
        commandtext:=commandtext+' 	and (mate_code='''+sk+''' or mate_name like''%'+sk+'%'' or dbo.fn_getpy(mate_name) like''%'+sk+'%'')';
//        commandtext:=commandtext+' 	and (mate_name like''%'+sk+'%'' or qry_code like''%'+sk+'%'' or dbo.fn_getpy(mate_name) like''%'+sk+'%'')';
{
        ...
        if recordcount=0 then begin beep; raise Exception.Create('无所查询业务负责人'); end;
    end;
end;
end;
