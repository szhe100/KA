...
type
  Tpickbusimate = class(TForm)
    ...
    DBGrid1Lbroker: TdxDBGridMaskColumn;
    ...
    query1Lbroker: TStringField;
    query1broker_id: TIntegerField;
    query1broker_id1: TIntegerField;
    Edit1: TdxEdit;
...
procedure Tpickbusimate.Edit1KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
    fwid: boolean;
...
begin
    ...
    begin
        ...
        commandtext:='select top 100 pick_ord=cast(ROW_NUMBER() OVER(ORDER BY mate_name) as int),';
        commandtext:=commandtext+' mate_code,mate_code,mate_id,mate_name,deputy,phone,address,broker,broker_id,broker_id1,settle_type,stop_yn,qry_code';
        commandtext:=commandtext+' from tb_busimate where mate_type_id=1'; // and comp_id='+inttostr(compid);
        if fwid  // 宽字符串
...
//         else commandtext:=commandtext+' and qry_code like''%'+sk+'%''';
        open;
        if recordcount=0 then begin beep; raise Exception.Create('无所查询客户'); end;
    end;
end;
end;

procedure Tpickbusimate.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickbusimate.DBGrid1DblClick(Sender: TObject);
begin
   modalresult:=mrOK;
end;

procedure Tpickbusimate.wwDBNavigator1FirstClick(Sender: TObject);
begin
edit1.setfocus;
end;

procedure Tpickbusimate.FormCloseQuery(Sender: TObject;
...
procedure Tpickbusimate.FormActivate(Sender: TObject);
begin
edit1.setfocus;
edit1.selectall;
end;

procedure Tpickbusimate.BitBtn3Click(Sender: TObject);
begin
self.tag:=1;  // <Esc> (cancel) 退出标志
end;

procedure Tpickbusimate.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickbusimate.DSquery1DataChange(Sender: TObject; Field: TField);
begin
if activecontrol=dbgrid1 then
edit1.text:=query1.fieldbyname('pick_ord').asstring;
end;

procedure Tpickbusimate.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
