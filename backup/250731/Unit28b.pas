...
procedure Tpickbroker1.Edit1KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
    fwid: boolean;
...
begin
    ...
    begin
        ...
        commandtext:=commandtext+' sta_type_id,qry_code from tb_staff where sta_type_id=0 and broker=1';//业务设置中的商务人员   and comp_id='+inttostr(compid)+'
...
//        commandtext:=commandtext+' order by zname';
        open;
        if recordcount=0 then begin beep; raise Exception.Create('无所查询商务人员'); end;
    end;
end;
end;

procedure Tpickbroker1.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickbroker1.DBGrid1DblClick(Sender: TObject);
begin
   modalresult:=mrOK;
end;

procedure Tpickbroker1.wwDBNavigator1FirstClick(Sender: TObject);
begin
edit1.setfocus;
end;

procedure Tpickbroker1.FormCloseQuery(Sender: TObject;
...
procedure Tpickbroker1.FormActivate(Sender: TObject);
begin
edit1.setfocus;
edit1.selectall;
end;

procedure Tpickbroker1.BitBtn3Click(Sender: TObject);
begin
self.tag:=1;  // <Esc> (cancel) 退出标志
end;

procedure Tpickbroker1.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickbroker1.DSquery1DataChange(Sender: TObject; Field: TField);
begin
if activecontrol=dbgrid1 then
edit1.text:=query1.fieldbyname('pick_ord').asstring;
end;

procedure Tpickbroker1.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
