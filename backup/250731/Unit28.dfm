object pickbroker: Tpickbroker
  Left = 192
  Top = 121
  HorzScrollBar.Visible = False
  VertScrollBar.Visible = False
  AutoScroll = False
  BorderIcons = []
  Caption = #26597#35810#19994#21153#21592
  ClientHeight = 566
  ClientWidth = 766
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  OnActivate = FormActivate
  OnCloseQuery = FormCloseQuery
  OnKeyDown = Edit1KeyDown
  DesignSize = (
    766
    566)
  PixelsPerInch = 96
  TextHeight = 12
  object wwDBNavigator1: TwwDBNavigator
    Left = 193
    Top = 1
    Width = 96
    Height = 22
    AutosizeStyle = asSizeNavButtons
    DataSource = DSquery1
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    ParentShowHint = False
    object wwDBNavigator1First: TwwNavButton
      Left = 0
      Top = 0
      Width = 24
      Height = 22
      Hint = #39318#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      ParentShowHint = False
      ShowHint = True
      OnClick = wwDBNavigator1FirstClick
      Index = 0
      Style = nbsFirst
    end
    object wwDBNavigator1PriorPage: TwwNavButton
      Left = 24
      Top = 0
      Width = 24
      Height = 22
      Hint = #21069#19968#39029
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1PriorPage'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      ParentShowHint = False
      ShowHint = True
      OnClick = wwDBNavigator1FirstClick
      Index = 1
      Style = nbsPriorPage
    end
    object wwDBNavigator1NextPage: TwwNavButton
      Left = 48
      Top = 0
      Width = 24
      Height = 22
      Hint = #19979#19968#39029
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1NextPage'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      ParentShowHint = False
      ShowHint = True
      OnClick = wwDBNavigator1FirstClick
      Index = 2
      Style = nbsNextPage
    end
    object wwDBNavigator1Last: TwwNavButton
      Left = 72
      Top = 0
      Width = 24
      Height = 22
      Hint = #23614#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      ParentShowHint = False
      ShowHint = True
      OnClick = wwDBNavigator1FirstClick
      Index = 3
      Style = nbsLast
    end
  end
  object BitBtn2: TBitBtn
    Left = 701
    Top = 2
    Width = 22
    Height = 22
    Anchors = [akTop, akRight]
    Caption = ' '
    TabOrder = 1
    Kind = bkOK
    Spacing = 0
  end
  object BitBtn3: TBitBtn
    Left = 724
    Top = 2
    Width = 22
    Height = 22
    Anchors = [akTop, akRight]
    Caption = ' '
    TabOrder = 2
    OnClick = BitBtn3Click
    Kind = bkCancel
    Spacing = 0
  end
  object DBGrid1: TdxDBGrid
    Left = 0
    Top = 25
    Width = 766
    Height = 541
    Bands = <
      item
      end>
    DefaultLayout = True
    HeaderPanelRowCount = 1
    KeyField = 'sta_id'
    ShowSummaryFooter = True
    SummaryGroups = <>
    SummarySeparator = ', '
    Align = alBottom
    TabOrder = 3
    OnDblClick = DBGrid1DblClick
    OnKeyPress = DBGrid1KeyPress
    DataSource = DSquery1
    Filter.Active = True
    Filter.Criteria = {00000000}
    HighlightColor = clAqua
    HighlightTextColor = clBlack
    LookAndFeel = lfUltraFlat
    OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
    Anchors = [akLeft, akTop, akRight, akBottom]
    object DBGrid1pick_ord: TdxDBGridMaskColumn
      Alignment = taCenter
      Font.Charset = GB2312_CHARSET
      Font.Color = clMaroon
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 30
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pick_ord'
      SummaryFooterType = cstCount
      SummaryFooterField = 'sta_id'
      SummaryFooterFormat = '##,##0;; '
      DisableFilter = True
      SummaryType = cstCount
      SummaryField = 'sta_id'
      SummaryFormat = '##,##0;; '
    end
    object DBGrid1cdistrict: TdxDBGridColumn
      Width = 82
      BandIndex = 0
      RowIndex = 0
      FieldName = 'cdistrict'
    end
    object DBGrid1zname: TdxDBGridMaskColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 120
      BandIndex = 0
      RowIndex = 0
      FieldName = 'zname'
      SummaryFooterField = 'sta_id'
      SummaryFooterFormat = '###,##0'#20154';; '
      SummaryField = 'sta_id'
    end
    object DBGrid1sex: TdxDBGridColumn
      Alignment = taCenter
      Font.Charset = GB2312_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 30
      BandIndex = 0
      RowIndex = 0
      FieldName = 'sex'
      DisableFilter = True
    end
    object DBGrid1zpost: TdxDBGridColumn
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'zpost'
    end
    object DBGrid1degree: TdxDBGridColumn
      Width = 60
      BandIndex = 0
      RowIndex = 0
      FieldName = 'degree'
    end
    object DBGrid1title: TdxDBGridMaskColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 60
      BandIndex = 0
      RowIndex = 0
      FieldName = 'title'
    end
    object DBGrid1phone: TdxDBGridMaskColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 120
      BandIndex = 0
      RowIndex = 0
      FieldName = 'phone'
    end
    object DBGrid1residence: TdxDBGridMaskColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 100
      BandIndex = 0
      RowIndex = 0
      FieldName = 'residence'
    end
    object DBGrid1qry_code: TdxDBGridMaskColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clGreen
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'qry_code'
    end
  end
  object Edit1: TdxEdit
    Left = 2
    Top = 2
    Width = 120
    Hint = #21487#36755#20837#31616#30721#25110#22995#21517
    ImeMode = imClose
    ParentShowHint = False
    ShowHint = True
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 4
    OnClick = Edit1Click
    OnEnter = Edit1Click
    OnKeyDown = Edit1KeyDown
    OnKeyPress = Edit1KeyPress
  end
  object DSquery1: TDataSource
    DataSet = query1
    OnDataChange = DSquery1DataChange
    Left = 312
    Top = 232
  end
  object query1: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    ReadOnly = True
    RemoteServer = DM.RemoteServer
    Left = 281
    Top = 232
    object query1pick_ord: TIntegerField
      DisplayLabel = #36873#21495
      FieldName = 'pick_ord'
    end
    object query1sta_id: TAutoIncField
      FieldName = 'sta_id'
      ReadOnly = True
    end
    object query1zname: TStringField
      DisplayLabel = #22995#21517
      FieldName = 'zname'
    end
    object query1origin: TStringField
      DisplayLabel = #31821#36143
      FieldName = 'origin'
    end
    object query1degree: TStringField
      DisplayLabel = #23398#21382
      FieldName = 'degree'
    end
    object query1title: TStringField
      DisplayLabel = #32844#31216
      FieldName = 'title'
    end
    object query1zpost: TStringField
      DisplayLabel = #20219#32844
      FieldName = 'zpost'
    end
    object query1fax: TStringField
      FieldName = 'fax'
    end
    object query1phone: TStringField
      DisplayLabel = #30005#35805
      FieldName = 'phone'
      Size = 50
    end
    object query1residence: TStringField
      DisplayLabel = #20303#22336
      FieldName = 'residence'
      Size = 50
    end
    object query1sex: TBooleanField
      DisplayLabel = #24615#21035
      FieldName = 'sex'
      DisplayValues = #30007';'#22899
    end
    object query1qry_code: TStringField
      DisplayLabel = #31616#30721
      FieldName = 'qry_code'
    end
    object query1district: TIntegerField
      FieldName = 'district'
    end
    object query1cdistrict: TStringField
      DisplayLabel = #25152#23646#22320#21306
      FieldName = 'cdistrict'
      ReadOnly = True
      Size = 100
    end
    object query1sta_type_id: TIntegerField
      FieldName = 'sta_type_id'
      ReadOnly = True
    end
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=kingage;Data Source=wanjin-xinxi2'
    Parameters = <>
    SQL.Strings = (
      
        'select pick_ord,sta_id,zcode,zname,origin,degree,title,zpost,pho' +
        'ne,fax,residence,csex=case sex when 0 then '#39#30007#39' else '#39#22899#39' end,sta_' +
        'type_id,qry_code'
      ' from tb_staff')
    Left = 403
    Top = 6
    object ADOQuery1pick_ord: TIntegerField
      FieldName = 'pick_ord'
    end
    object ADOQuery1sta_id: TAutoIncField
      FieldName = 'sta_id'
      ReadOnly = True
    end
    object ADOQuery1zcode: TStringField
      FieldName = 'zcode'
      Size = 10
    end
    object ADOQuery1zname: TStringField
      FieldName = 'zname'
    end
    object ADOQuery1origin: TStringField
      FieldName = 'origin'
    end
    object ADOQuery1degree: TStringField
      FieldName = 'degree'
    end
    object ADOQuery1title: TStringField
      FieldName = 'title'
    end
    object ADOQuery1zpost: TStringField
      FieldName = 'zpost'
    end
    object ADOQuery1phone: TStringField
      FieldName = 'phone'
      Size = 50
    end
    object ADOQuery1fax: TStringField
      FieldName = 'fax'
    end
    object ADOQuery1residence: TStringField
      FieldName = 'residence'
      Size = 50
    end
    object ADOQuery1csex: TStringField
      FieldName = 'csex'
      ReadOnly = True
      Size = 2
    end
    object ADOQuery1sta_type_id: TIntegerField
      FieldName = 'sta_type_id'
      ReadOnly = True
    end
    object ADOQuery1qry_code: TStringField
      FieldName = 'qry_code'
    end
  end
end
