    ...
    sk:=trim(edit1.text);
{
    ...
    begin
        ...
        commandtext:='select top 200 pick_ord=cast(ROW_NUMBER() OVER(ORDER BY zname) as int),a.*,cdistrict=g.prov_name+'' ''+g.city_name'; //dbo.fn_getdistrict(district),';
{
        commandtext:='select top 200 pick_ord=cast(ROW_NUMBER() OVER(ORDER BY zname) as int),sta_type_id,qry_code';
        commandtext:=commandtext+' sta_id,zname,zcode,sex,origin,degree,title,zpost,phone,fax,residence,district,cdistrict=g.prov_name+'' ''+g.city_name'; //dbo.fn_getdistrict(district),';
}
        ...
        commandtext:=commandtext+' where sta_type_id=1'; // and comp_id='+inttostr(compid);
        if sk<>'' then commandtext:=commandtext+' and (zname like ''%'+sk+'%'' or qry_code like ''%'+sk+'%'')';
{
