...
procedure Tsetpromoapply.N1Click(Sender: TObject);
...
if mi=0 then raise Exception.Create('无可送审核单据(非制单状态不可送审核)');
with dm.pubqry do
begin
    if active then close;
    if sender= N1 then commandtext:='select top 1 1 from tb_bill where bod_id in ('+t+') and bod_status_id<>0';
//    if sender= N2 then commandtext:='select top 1 1 from tb_bill where bod_id in ('+t+') and bod_status_id<>1';
    open;
    if recordcount>0 then
    begin
        if sender= N1 then raise Exception.Create('所选单据中有非制单状态单据，请重新选择');
//        if sender= N2 then raise Exception.Create('所选单据中有未审核单据，请重新选择');
    end;
end;

if sender= N1 then if MessageBox(0,'确定所选单据送审核','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
setprogress(1);
with dm.pubqry do
begin
    if active then close;
