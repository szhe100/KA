...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1交货单号, 2订单编号, 3渠道, 4子渠道, 5业务负责人, 6供货单位, 7商业公司, 8销售人员, 9单据摘要, 10发运日期, 11物料编码, 12单价, 13数量, 14批号, 15有效期至
...
//从第2行开始导入，格式: 1交货单号, 2订单编号, 3渠道, 4子渠道, 5业务负责人, 6供货单位, 7商业公司, 8销售人员, 9单据摘要, 10发运日期, 11定价参考物料编码, 12单价, 13数量, 14批号, 15有效期至
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行无渠道或数据无效'' from @tab a where f3='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.zdesc=a.f3)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无子渠道或数据无效'' from @tab a where f4='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.zdesc=a.f4)';
