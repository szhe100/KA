...
{
    commandtext:=commandtext+' agent_code=a.ASSIGNED_BP,agent=case when i.rec_id>0 then i.agent else a.NAME_FIRST end,agent_id=c.mate_id,a.ASSIGNED_BP,';
    commandtext:=commandtext+' agent1=case when i.rec_id>0 then dbo.fn_mate_name(i.agent_id1) else a.NAME_FIRST end,';
    commandtext:=commandtext+' agent_code1=case when i.rec_id>0 then i.agent_code1 else a.ASSIGNED_BP end,';
}
//    commandtext:=commandtext+' broker_id=cast(ZSALESID_O as int),broker=ZSALESNAM_O,i.agent_code,i.agent,agent_id=c.mate_id,a.ASSIGNED_BP,agent1=dbo.fn_mate_name(i.agent_id1),stoppay=cast(0 as bit),';
    commandtext:=commandtext+' stoppay=cast(0 as bit), a.VTEXT,a.ZKDGRP,dist1=ZZREGION, dist2=ZCITYNAME, level1=ZBEZEI,';  //ZCITYNAME
//    commandtext:=commandtext+' a.VTEXT,a.ZKDGRP,dist1=dbo.fn_getdistrictname(c.district,1), dist2=dbo.fn_getdistrictname(c.district,2), level1=ZBEZEI,';
...
//    commandtext:=commandtext+' not_amot=cast((isnull(i.amot,0)+isnull(i.fee,0))*a.qty-isnull(e.amot,0) as decimal(15,2))'; //a.ckd_amot
    commandtext:=commandtext+' not_amot=cast((isnull(i.amot,0)+isnull(i.fee,0))*a.qty-isnull(a.ckd_amot,0) as decimal(15,2))';
//    commandtext:=commandtext+' ,amot6=cast(case when cast(a.KONV as decimal(15,4))=0 then 0 else cast(a.amot1/cast(a.KONV as decimal(15,4)) as decimal(15,2))*i.amot end as decimal(15,2))-isnull(a.ckd_amot,0)';
...
//    commandtext:=commandtext+' left join (select mate_id,mate_code,mate_name from tb_busimate where mate_type_id=2) b on a.KUNNR=b.mate_code'; //a.NAME1=b.mate_name';
...
//    commandtext:=commandtext+' left join (select a.mate_id,a.mate_code,mate_name from tb_busimate a inner join (select mate_code,mate_id=max(mate_id) from tb_busimate where mate_type_id=4 group by mate_code) b on a.mate_id=b.mate_id) c on c.mate_code=a.ASSIGNED_BP';
...
//    commandtext:=commandtext+' left join tb_busiframe3 i on i.rec_id=dbo.fn_getbusiframe3recid(a.VTWEG,a.KDGRP,b.mate_id,c.mate_id,a.MATNR,cast(a.KONV as decimal(15,4)),cast(ZPR03 as decimal(15,4)),a.carry_dt)';
...
//    commandtext:=commandtext+'  and a.price>cast(ZPR03 as decimal(15,4))'; //核销条件:原订单 销售单价 > 考核单价
...
//    if Trim(dxbuttonedit3.text)<>'' then commandtext:=commandtext+' and a.NAME_FIRST='''+Trim(dxbuttonedit3.text)+'''';
...
//    if Trim(dxbuttonedit3.text)<>'' then commandtext:=commandtext+' and a.NAME_FIRST=(select top 1 mate_name from tB_busimate where mate_id='+inttostr(dxbuttonedit3.tag)+')';
...
//    commandtext:=commandtext+' dist1=dbo.fn_getdistrictname(f.district,1),dist2=dbo.fn_getdistrictname(f.district,2),level1='''',';
...
//    commandtext:=commandtext+' ,amot6=cast(a.bod_amot as decimal(15,2))-isnull(z.ckd_amot,0)';
    ...
    // notice: union 得到的数据 Clientdataset 在post时，报read only 错误，所以要修改tb_bill.ckd_amot2字段，再inner join一下tb_bill
{
    ...
    commandtext:=commandtext+' z.ckd_amot,y.ckd_amot1,a.ckd_amot2,y.ckd_amot2a,h.ckd_amot3,';   //h.amot1,
    commandtext:=commandtext+' b.qty,b.price,b.amot,b.amot2,b.price1,x.price10,';
    ...
    commandtext:=commandtext+' left join (select ZREGIO,med_id,price10=price from tb_medprice where type_id=1) x on ZREGIO=dbo.fn_getdistrictlevel(a.district_id,1) and b.med_id=x.med_id';
    ...
    if RadioGroup3.ItemIndex=1 then commandtext:=commandtext+'  and z.ckd_amot=(isnull(c.amot,0)+isnull(c.fee,0))*b.qty'; //已完成收款
    if RadioGroup3.ItemIndex=2 then commandtext:=commandtext+'  and isnull(z.ckd_amot,0)<>(isnull(c.amot,0)+isnull(c.fee,0))*b.qty'; //未完成收款
    ...
    commandtext:=commandtext+' dist1=dbo.fn_getdistrictlevel (f.district,1),dist2=dbo.fn_getdistrictlevel (f.district,2),level1='''',';
    commandtext:=commandtext+' creater=d.zname,a.dst_id,e.mate_name,m.material_code,m.med_code,m.med_name,m.specifi,m.pdt_place,med_unit=dbo.fn_obj_desc(m.unit_id),m.type_id1,bat_cd='''',';
    commandtext:=commandtext+' z.ckd_amot,y.ckd_amot1,a.ckd_amot2,y.ckd_amot2a,h.ckd_amot3,';    //h.amot1,
    commandtext:=commandtext+' qty=0,price=0,amot=a.bod_amot,amot2=0.0000,price1=0,price10=0,';
    commandtext:=commandtext+' type_id=0,type_id2=0,rate=0,rate1=0,fee=0,amot1a=0,cprice1=0,cprice2=0,fee_type_id=0';
    commandtext:=commandtext+' ,not_amot=cast(a.bod_amot-isnull(z.ckd_amot,0) as decimal(15,2))';
    commandtext:=commandtext+' from tb_bill a';
	commandtext:=commandtext+' left join tb_bill_stadtl h on a.bod_id=h.bod_id';
...
begin
    ...
    if s1>'' then
    begin
        commandtext:='select distinct ltrim(str(year(a.carry_dt)))+ltrim(str(month(a.carry_dt))) from tb_bill ';
