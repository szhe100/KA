...
var
  setpromoapplyA2: TsetpromoapplyA2;
...
procedure TsetpromoapplyA2.setupdatestatus;    // 编辑状态
begin
speedbutton2.enabled:=false;    // examine disable
speedbutton5.enabled:=false;   // refresh disable
end;

procedure TsetpromoapplyA2.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure TsetpromoapplyA2.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure TsetpromoapplyA2.FormCloseQuery(Sender: TObject;
...
procedure TsetpromoapplyA2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TsetpromoapplyA2.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure TsetpromoapplyA2.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TsetpromoapplyA2.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=44 and a.bod_status_id in (1,2,3)';  // a.comp_id='+inttostr(compid)+' and
...
procedure TsetpromoapplyA2.FormCreate(Sender: TObject);
begin
...
procedure TsetpromoapplyA2.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
begin
    ...
    begin
        ...
        gifimage2.visible:=fieldbyname('bod_status_id').asinteger=3;  //待审核 图章显示
...
procedure TsetpromoapplyA2.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetpromoapplyA2.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetpromoapplyA2.SpeedButton2Click(Sender: TObject);
begin
...
procedure TsetpromoapplyA2.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure TsetpromoapplyA2.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure TsetpromoapplyA2.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure TsetpromoapplyA2.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure TsetpromoapplyA2.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure TsetpromoapplyA2.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure TsetpromoapplyA2.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure TsetpromoapplyA2.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure TsetpromoapplyA2.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TsetpromoapplyA2.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure TsetpromoapplyA2.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure TsetpromoapplyA2.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
{
...
procedure TsetpromoapplyA2.SpeedButton6Click(Sender: TObject);
begin
