object qryreport21: Tqryreport21
  Left = 164
  Top = 121
  AutoScroll = False
  Caption = #20379#24212#21830#36820#20837#36134#40836#25253#34920
  ClientHeight = 514
  ClientWidth = 1086
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnKeyPress = FormKeyPress
  DesignSize = (
    1086
    514)
  PixelsPerInch = 96
  TextHeight = 12
  object Label25: TLabel
    Left = 1065
    Top = 7
    Width = 7
    Height = 12
    Alignment = taRightJustify
    Anchors = [akTop, akRight]
    Font.Charset = GB2312_CHARSET
    Font.Color = clMaroon
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = [fsBold]
    ParentFont = False
  end
  object SpeedButton2: TSpeedButton
    Left = 904
    Top = 0
    Width = 25
    Height = 25
    Hint = #36716#20986#25253#34920
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton2Click
  end
  object SpeedButton5: TSpeedButton
    Left = 904
    Top = 38
    Width = 176
    Height = 25
    Hint = #21047#26032
    Anchors = [akTop, akRight]
    Caption = '&R '#21047#26032
    Glyph.Data = {
      
      3393333777333777337333333999993333333333377777333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Spacing = 10
    OnClick = SpeedButton5Click
  end
  object Shape6: TShape
    Left = 2
    Top = 21
    Width = 70
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label7: TLabel
    Left = 14
    Top = 27
    Width = 48
    Height = 12
    Caption = #21830#19994#20844#21496
    Transparent = True
  end
  object Label1: TLabel
    Left = 772
    Top = 48
    Width = 120
    Height = 12
    Anchors = [akTop, akRight]
    Caption = #27880': '#21452#20987#35760#24405#26597#30475#26126#32454
  end
  object Shape1: TShape
    Left = 2
    Top = 43
    Width = 70
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label2: TLabel
    Left = 9
    Top = 49
    Width = 60
    Height = 12
    Caption = #19994#21153#36127#36131#20154
    Transparent = True
  end
  object Shape2: TShape
    Left = 2
    Top = 0
    Width = 70
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label3: TLabel
    Left = 14
    Top = 6
    Width = 48
    Height = 12
    Caption = #20379#36135#21333#20301
    Transparent = True
  end
  object dxDBGrid1: TdxDBGrid
    Left = 0
    Top = 67
    Width = 1086
    Height = 447
    Bands = <
      item
        Caption = #20379#36135#21333#20301#21830#19994#20844#21496#19994#21153#36127#36131#20154#21697#31181
      end
      item
        Caption = '0-30'#22825
      end
      item
        Caption = '31-60'#22825
      end
      item
        Caption = '61-90'#22825
      end
      item
        Caption = '91-180'#22825
      end
      item
        Caption = '181-360'#22825
      end
      item
        Caption = '361+'#22825
      end>
    DefaultLayout = False
    HeaderPanelRowCount = 1
    KeyField = 'dst_id'
    ShowGroupPanel = True
    ShowSummaryFooter = True
    SummaryGroups = <
      item
        DefaultGroup = True
        SummaryItems = <
          item
            SummaryField = 'qty'
            SummaryFormat = '###,###,##0;; '
            SummaryType = cstSum
          end>
        Name = 'dxDBGrid1SummaryGroup2'
      end>
    SummarySeparator = ', '
    Align = alBottom
    TabOrder = 0
    OnDblClick = dxDBGrid1DblClick
    DataSource = DSdrawlog
    Filter.Active = True
    Filter.DropDownCount = 30
    Filter.Criteria = {00000000}
    HighlightColor = clAqua
    HighlightTextColor = clBlack
    LookAndFeel = lfUltraFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabThrough, edgoVertThrough]
    OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoRowSelect, edgoUseBitmap]
    ShowBands = True
    ShowRowFooter = True
    OnCustomDrawCell = dxDBGrid1CustomDrawCell
    OnCustomDrawFooter = dxDBGrid1CustomDrawFooter
    Anchors = [akLeft, akTop, akRight, akBottom]
    object dxDBGrid1mate_name1: TdxDBGridColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 180
      BandIndex = 0
      RowIndex = 0
      FieldName = 'mate_name1'
    end
    object dxDBGrid1mate_name: TdxDBGridMaskColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 180
      BandIndex = 0
      RowIndex = 0
      FieldName = 'mate_name'
      SummaryFooterType = cstCount
      SummaryFooterField = 'mate_id'
      SummaryFooterFormat = '###,##0;;'
      SummaryType = cstCount
      SummaryField = 'mate_id'
      SummaryFormat = '###,##0;;'
    end
    object dxDBGrid1Ccate: TdxDBGridColumn
      Alignment = taCenter
      Width = 45
      BandIndex = 0
      RowIndex = 0
      FieldName = 'Ccate'
    end
    object dxDBGrid1agent: TdxDBGridColumn
      Width = 81
      BandIndex = 0
      RowIndex = 0
      FieldName = 'agent'
    end
    object dxDBGrid1med_code: TdxDBGridColumn
      Alignment = taCenter
      Font.Charset = GB2312_CHARSET
      Font.Color = clRed
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_code'
    end
    object dxDBGrid1med_name: TdxDBGridColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_name'
    end
    object dxDBGrid1specifi: TdxDBGridColumn
      Width = 60
      BandIndex = 0
      RowIndex = 0
      FieldName = 'specifi'
    end
    object dxDBGrid1pdt_place: TdxDBGridColumn
      Width = 69
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pdt_place'
    end
    object dxDBGrid1med_unit: TdxDBGridColumn
      Alignment = taCenter
      Width = 30
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_unit'
      DisableFilter = True
    end
    object dxDBGrid1amot: TdxDBGridColumn
      Color = 16577777
      Width = 100
      BandIndex = 0
      RowIndex = 0
      FieldName = 'amot'
      SummaryFooterType = cstSum
      SummaryFooterField = 'amot'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'amot'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1amot1: TdxDBGridColumn
      Caption = #37329#39069
      Width = 100
      BandIndex = 1
      RowIndex = 0
      FieldName = 'amot1'
      SummaryFooterType = cstSum
      SummaryFooterField = 'amot1'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'amot1'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Crate1: TdxDBGridColumn
      Width = 40
      BandIndex = 1
      RowIndex = 0
      FieldName = 'Crate1'
      SummaryFooterType = cstAvg
      DisableFilter = True
      SummaryType = cstAvg
    end
    object dxDBGrid1amot2: TdxDBGridColumn
      Caption = #37329#39069
      Width = 100
      BandIndex = 2
      RowIndex = 0
      FieldName = 'amot2'
      SummaryFooterType = cstSum
      SummaryFooterField = 'amot2'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'amot2'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Crate2: TdxDBGridColumn
      Width = 40
      BandIndex = 2
      RowIndex = 0
      FieldName = 'Crate2'
      SummaryFooterType = cstAvg
      DisableFilter = True
      SummaryType = cstAvg
    end
    object dxDBGrid1amot3: TdxDBGridColumn
      Caption = #37329#39069
      Width = 100
      BandIndex = 3
      RowIndex = 0
      FieldName = 'amot3'
      SummaryFooterType = cstSum
      SummaryFooterField = 'amot3'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'amot3'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Crate3: TdxDBGridColumn
      Width = 40
      BandIndex = 3
      RowIndex = 0
      FieldName = 'Crate3'
      SummaryFooterType = cstAvg
      DisableFilter = True
      SummaryType = cstAvg
    end
    object dxDBGrid1amot4: TdxDBGridColumn
      Caption = #37329#39069
      Width = 100
      BandIndex = 4
      RowIndex = 0
      FieldName = 'amot4'
      SummaryFooterType = cstSum
      SummaryFooterField = 'amot4'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'amot4'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Crate4: TdxDBGridColumn
      Width = 40
      BandIndex = 4
      RowIndex = 0
      FieldName = 'Crate4'
      SummaryFooterType = cstAvg
      DisableFilter = True
      SummaryType = cstAvg
    end
    object dxDBGrid1amot5: TdxDBGridColumn
      Caption = #37329#39069
      Width = 100
      BandIndex = 5
      RowIndex = 0
      FieldName = 'amot5'
      SummaryFooterType = cstSum
      SummaryFooterField = 'amot5'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'amot5'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Crate5: TdxDBGridColumn
      Width = 40
      BandIndex = 5
      RowIndex = 0
      FieldName = 'Crate5'
      SummaryFooterType = cstAvg
      DisableFilter = True
      SummaryType = cstAvg
    end
    object dxDBGrid1amot6: TdxDBGridColumn
      Caption = #37329#39069
      Width = 100
      BandIndex = 6
      RowIndex = 0
      FieldName = 'amot6'
      SummaryFooterType = cstSum
      SummaryFooterField = 'amot6'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'amot6'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Crate6: TdxDBGridColumn
      Width = 40
      BandIndex = 6
      RowIndex = 0
      FieldName = 'Crate6'
      SummaryFooterType = cstAvg
      DisableFilter = True
      SummaryType = cstAvg
    end
  end
  object wwDBNavigator2: TwwDBNavigator
    Left = 929
    Top = 0
    Width = 150
    Height = 25
    DataSource = DSdrawlog
    Options = []
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    Anchors = [akTop, akRight]
    ParentShowHint = False
    object wwNavButton1: TwwNavButton
      Left = 0
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 0
      Style = nbsFirst
    end
    object wwNavButton2: TwwNavButton
      Left = 25
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Prior'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 1
      Style = nbsPrior
    end
    object wwNavButton3: TwwNavButton
      Left = 50
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Next'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 2
      Style = nbsNext
    end
    object wwNavButton4: TwwNavButton
      Left = 75
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 3
      Style = nbsLast
    end
    object wwNavButton5: TwwNavButton
      Left = 100
      Top = 0
      Width = 25
      Height = 25
      Hint = #35774#32622#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1SaveBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 4
      Style = nbsSaveBookmark
    end
    object wwNavButton6: TwwNavButton
      Left = 125
      Top = 0
      Width = 25
      Height = 25
      Hint = #36820#22238#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1RestoreBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 5
      Style = nbsRestoreBookmark
    end
  end
  object dxButtonEdit1: TdxButtonEdit
    Left = 74
    Top = 22
    Width = 200
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 2
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit1ButtonClick
    ExistButtons = True
  end
  object Edit1: TEdit
    Left = 485
    Top = 26
    Width = 121
    Height = 20
    TabOrder = 3
    Text = 'Edit1'
    Visible = False
  end
  object dxButtonEdit2: TdxButtonEdit
    Left = 74
    Top = 44
    Width = 200
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 4
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit2ButtonClick
    ExistButtons = True
  end
  object dxButtonEdit3: TdxButtonEdit
    Left = 74
    Top = 1
    Width = 200
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 5
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit1ButtonClick
    ExistButtons = True
  end
  object DSdrawlog: TDataSource
    DataSet = qry
    OnDataChange = DSdrawlogDataChange
    Left = 132
    Top = 167
  end
  object qry: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    OnCalcFields = qryCalcFields
    Left = 103
    Top = 167
    object qrycate_id: TWordField
      FieldName = 'cate_id'
    end
    object qryCcate: TStringField
      DisplayLabel = #31867#21035
      FieldKind = fkCalculated
      FieldName = 'Ccate'
      Size = 10
      Calculated = True
    end
    object qryagent_id: TAutoIncField
      FieldName = 'agent_id'
    end
    object qryagent: TStringField
      DisplayLabel = #19994#21153#36127#36131#20154
      FieldName = 'agent'
    end
    object qrysrc_id: TIntegerField
      FieldName = 'src_id'
    end
    object qrymate_name1: TStringField
      DisplayLabel = #20379#36135#21333#20301
      FieldName = 'mate_name1'
      Size = 100
    end
    object qryteam_id: TAutoIncField
      FieldName = 'dst_id'
      ReadOnly = True
    end
    object qryteam_name: TStringField
      DisplayLabel = #21830#19994#20844#21496
      FieldName = 'mate_name'
    end
    object qrycredit_day: TIntegerField
      DisplayLabel = #25480#20449#22825#25968
      FieldName = 'credit_day'
    end
    object qrymed_code: TStringField
      DisplayLabel = #32534#30721
      FieldName = 'med_code'
    end
    object qrymed_name: TStringField
      DisplayLabel = #21697#21517
      FieldName = 'med_name'
      Size = 50
    end
    object qryspecifi: TStringField
      DisplayLabel = #35268#26684
      FieldName = 'specifi'
      Size = 50
    end
    object qrypdt_place: TStringField
      DisplayLabel = #29983#20135#20225#19994
      FieldName = 'pdt_place'
      Size = 30
    end
    object qrymed_unit: TStringField
      DisplayLabel = #21333#20301
      FieldName = 'med_unit'
      Size = 30
    end
    object qryamot: TBCDField
      DisplayLabel = #21512#35745#37329#39069
      FieldName = 'amot'
      DisplayFormat = '###,###,##0.00;; '
      Precision = 15
      Size = 2
    end
    object qryamot1: TBCDField
      DisplayLabel = '0-30'#22825
      FieldName = 'amot1'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
    end
    object qryamot2: TBCDField
      DisplayLabel = '31-60'#22825
      FieldName = 'amot2'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
    end
    object qryamot3: TBCDField
      DisplayLabel = '61-90'#22825
      FieldName = 'amot3'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
    end
    object qryamot4: TBCDField
      DisplayLabel = '91-180'#22825
      FieldName = 'amot4'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
    end
    object qryamot5: TBCDField
      DisplayLabel = '181-360'#22825
      FieldName = 'amot5'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
    end
    object qryamot6: TBCDField
      DisplayLabel = '361+'#22825
      FieldName = 'amot6'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
    end
    object qryCrate1: TBCDField
      DisplayLabel = #21344#27604'%'
      FieldKind = fkCalculated
      FieldName = 'Crate1'
      DisplayFormat = '##,##0.##;; '
      Size = 2
      Calculated = True
    end
    object qryCrate2: TBCDField
      DisplayLabel = #21344#27604'%'
      FieldKind = fkCalculated
      FieldName = 'Crate2'
      DisplayFormat = '##,##0.##;; '
      Size = 2
      Calculated = True
    end
    object qryCrate3: TBCDField
      DisplayLabel = #21344#27604'%'
      FieldKind = fkCalculated
      FieldName = 'Crate3'
      DisplayFormat = '##,##0.##;; '
      Size = 2
      Calculated = True
    end
    object qryCrate4: TBCDField
      DisplayLabel = #21344#27604'%'
      FieldKind = fkCalculated
      FieldName = 'Crate4'
      DisplayFormat = '##,##0.##;; '
      Size = 2
      Calculated = True
    end
    object qryCrate5: TBCDField
      DisplayLabel = #21344#27604'%'
      FieldKind = fkCalculated
      FieldName = 'Crate5'
      DisplayFormat = '##,##0.##;; '
      Size = 2
      Calculated = True
    end
    object qryCrate6: TBCDField
      DisplayLabel = #21344#27604'%'
      FieldKind = fkCalculated
      FieldName = 'Crate6'
      DisplayFormat = '##,##0.##;; '
      Size = 2
      Calculated = True
    end
  end
  object StyleController: TdxEditStyleController
    BorderColor = 14065456
    BorderStyle = xbsSingle
    ButtonStyle = btsSimple
    Edges = [edgBottom]
    HotTrack = True
    Left = 72
    Top = 160
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingAGE'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'sp_qrybrokersale 0,0,'#39'2007-1-1'#39','#39'2008-12-20'#39)
    Left = 31
    Top = 155
    object ADOQuery1sta_id: TIntegerField
      FieldName = 'sta_id'
    end
    object ADOQuery1district: TStringField
      DisplayLabel = #19994#21153#21592#22320#21306
      FieldName = 'district'
      ReadOnly = True
      Size = 100
    end
    object ADOQuery1broker: TStringField
      DisplayLabel = #19994#21153#21592
      FieldName = 'broker'
    end
    object ADOQuery1ord: TLargeintField
      FieldName = 'ord'
      ReadOnly = True
    end
    object ADOQuery1profit: TBCDField
      DisplayLabel = #27611#21033#28070
      FieldName = 'profit'
      DisplayFormat = '###,###,##0.00;; '
      Precision = 16
      Size = 2
    end
    object ADOQuery1qty: TBCDField
      FieldName = 'qty'
      Precision = 15
      Size = 2
    end
    object ADOQuery1amot: TBCDField
      FieldName = 'amot'
      Precision = 15
      Size = 2
    end
    object ADOQuery1rate: TBCDField
      FieldName = 'rate'
      ReadOnly = True
      Precision = 32
      Size = 18
    end
  end
  object ImageList1: TImageList
    ShareImages = True
    Left = 198
    Top = 136
    Bitmap = {
      
      000000000000}
  end
end
