...
procedure Tsetbusiframe4.dxButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetbusiframe4.SpeedButton2Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag);
end;

procedure Tsetbusiframe4.DSbusiframeDataChange(Sender: TObject;
  Field: TField);
begin
with busiframe do
...
procedure Tsetbusiframe4.SpeedButton4Click(Sender: TObject);
var mi: integer;
begin
...
//edit5.Text:=commandtext;
    ...
    if active=true then close;
    commandtext:='select top 1 1 from tb_busiframe4'; // where broker_id='+inttostr(dxButtonEdit2.tag);
    ...
    if MessageBox(0,pchar('确定加入本记录'),'请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
{
    ...
    if active=true then close;
    commandtext:='insert into tb_busiframe4 (agent_id,mate_id,med_id,valid_dt,creat_by,creat_dt)'; //district_id,
...
procedure Tsetbusiframe4.FormCloseQuery(Sender: TObject;
...
procedure Tsetbusiframe4.busiframeBeforeDelete(DataSet: TDataSet);
begin
...
procedure Tsetbusiframe4.SpeedButton3Click(Sender: TObject);
var mi: integer;
begin
with busiframe do
begin
    ...
    commandtext:=commandtext+' a.*,broker='''',c.mate_name,agent=d.mate_name,m.med_code,m.med_name,m.chm_name,m.specifi,m.pdt_place,med_unit=dbo.fn_obj_desc(m.unit_id),m.qtyperpack,';
    ...
    commandtext:=commandtext+' from tb_busiframe4 a';
    ...
    begin
        ...
        commandtext:=commandtext+'  from tb_busiframe4 group by agent_id,mate_id,med_id) y';
...
procedure Tsetbusiframe4.busiframeBeforePost(DataSet: TDataSet);
begin
with busiframe do
begin
...
procedure Tsetbusiframe4.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetbusiframe4.FormActivate(Sender: TObject);
begin
{
with dm.district do
...
procedure Tsetbusiframe4.FormCreate(Sender: TObject);
begin
...
procedure Tsetbusiframe4.dxButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickbroker do
    if showmodal= mrOK then
...
procedure Tsetbusiframe4.dxLookupTreeView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if key=vk_delete then dxLookupTreeView1.text:='';
end;

procedure Tsetbusiframe4.dxButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetbusiframe4.dxButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
    if showmodal= mrOK then
...
procedure Tsetbusiframe4.busiframeAfterPost(DataSet: TDataSet);
begin
if busiframe.ChangeCount>0 then myupdaterefresh(busiframe,'tb_busiframe4','rec_id','');
end;

procedure Tsetbusiframe4.SpeedButton1Click(Sender: TObject);
begin
with dm.pubqry do
begin
    if active=True then close;
    commandtext:='update a set district_id=b.district from tb_busiframe4 a,tb_busimate b';
...
procedure Tsetbusiframe4.SpeedButton6Click(Sender: TObject);
var
    XL,Sheet: Variant;
...
//导入 格式: 1商业公司,2代理人,3产品编码,4生效日期,5(空),6授信天数,7税率,8销售单价,9结算单价,10应付金额/盒,11促销率,12促销费/盒,13备注,14合规类型)
            ...
            end;
{
            ...
            if (sheet.cells[j,6].text='') then serror:=serror+#13#10+'第'+inttostr(j)+'行 无授信天数'
            else
            begin
                try
                    id6:=strtoint(cleardeli(sheet.cells[j,6].text));
                except
                    serror:=serror+#13#10+'第'+inttostr(j)+'行 授信天数有误';
                end;
            end;
            ...
            begin
                try
                    price2:=strtofloat(cleardeli(sheet.cells[j,8].text));
                except
            ...
            end;
{
            ...
            begin
                if active=true then close;
                commandtext:='select top 1 1 from tb_busiframe4 where mate_id='+inttostr(id1)+' and agent_id='+inttostr(id2);
...
//导入 格式: 1商业公司,2代理人,3产品编码,4生效日期,5(空),6授信天数,7税率,8销售单价,9结算单价,10应付金额/盒,11促销率,12促销费/盒,13备注)
            if active=true then close;
            commandtext:='insert into tb_busiframe4 (mate_id,agent_id,med_id,valid_dt,type_id,type_id2,credit_day,rate,price1,price2,amot,rate1,fee,remark,creat_by,creat_dt)';
            commandtext:=commandtext+' values ('+inttostr(id1)+','+inttostr(id2)+','+inttostr(id3)+','''+datetimetostr(dt)+''',null,'+inttostr(id14)+','+inttostr(id6)+',';
//            commandtext:=commandtext+' values ('+inttostr(id1)+','+inttostr(id2)+','+inttostr(id3)+','''+datetimetostr(dt)+''','+inttostr(id5)+','+inttostr(id6)+',';
...
procedure Tsetbusiframe4.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetbusiframe4.N3Click(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure Tsetbusiframe4.N1Click(Sender: TObject);
begin
with dxdbgrid1 do Columns[GetAbsoluteColumnIndex(FocusedColumn)].Visible:=false;
end;

procedure Tsetbusiframe4.N2Click(Sender: TObject);
var i,mi: integer;
begin
with dxdbgrid1 do
begin
...
procedure Tsetbusiframe4.N6Click(Sender: TObject);
begin
...
procedure Tsetbusiframe4.N7Click(Sender: TObject);
...
begin
    if active=True then close;
    commandtext:='select top 3 b.mate_name,b.valid_dt from tb_settlelist a,(select b.district,b.mate_name,f.valid_dt from tb_busiframe4 f,tb_busimate b where f.rec_id in ('+t+') and f.mate_id=b.mate_id) b';
    ...
    if active=true then close;
    commandtext:='select top 1 bod_cd from tb_bill a,tb_bill_dtl b where a.bod_type_id=44 and bod_status_id<>1';
...
mcd:=formatfloat('00000000',getbillid(0,44)); //bod_type_id=44: &I 两票销售促销规则申请单
with dm.pubqry do
begin
    setprogress(1);
    if active=true then close;
    ...
    commandtext:=commandtext+' values ('+inttostr(compid)+','''+mcd+''',44,0,'+inttostr(curuserid)+',getdate())';
    commandtext:=commandtext+' select @@identity';
//edit1.text:=commandtext;
    open;
    ...
    commandtext:=commandtext+' select '+inttostr(mi)+',rec_id from tb_busiframe4 where rec_id in ('+t+')';
    execute;
    if active=True then close;
    setprogress(0);
    s:='已成功生成'+mcd+'号两票销售促销规则申请单';
    MessageBox(0,pchar(s),'请注意',MB_OK+MB_ICONInformation);
end;
end;

procedure Tsetbusiframe4.busiframeCalcFields(DataSet: TDataSet);
begin
with busiframe do
    fieldbyname('cfee').asfloat:=fieldbyname('price2').asfloat-fieldbyname('price1').asfloat-fieldbyname('amot').asfloat-fieldbyname('fee').asfloat;
...
procedure Tsetbusiframe4.busiframeBeforeEdit(DataSet: TDataSet);
begin
