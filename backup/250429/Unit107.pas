...
procedure TsetexpaycashA.setupdatestatus;    // 编辑状态
begin
...
procedure TsetexpaycashA.setunupdatestatus;  // 浏览状态
begin
...
procedure TsetexpaycashA.FormActivate(Sender: TObject);
begin
{
with homebank do
...
procedure TsetexpaycashA.FormCloseQuery(Sender: TObject;
...
procedure TsetexpaycashA.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TsetexpaycashA.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpaycashA.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TsetexpaycashA.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure TsetexpaycashA.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure TsetexpaycashA.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
procedure TsetexpaycashA.FormCreate(Sender: TObject);
begin
...
procedure TsetexpaycashA.SpeedButton7Click(Sender: TObject);
begin
...
procedure TsetexpaycashA.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure TsetexpaycashA.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpaycashA.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpaycashA.SpeedButton2Click(Sender: TObject);
//var s: string;
begin
{
with cdsbill do
...
procedure TsetexpaycashA.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure TsetexpaycashA.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure TsetexpaycashA.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure TsetexpaycashA.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
...
procedure TsetexpaycashA.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure TsetexpaycashA.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure TsetexpaycashA.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure TsetexpaycashA.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpaycashA.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TsetexpaycashA.N1Click(Sender: TObject);
...
procedure TsetexpaycashA.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure TsetexpaycashA.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure TsetexpaycashA.dxDBButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickbank do
begin
...
procedure TsetexpaycashA.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure TsetexpaycashA.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure TsetexpaycashA.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
