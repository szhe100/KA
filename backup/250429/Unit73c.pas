...
type
  Tsetbrokeraccount3 = class(TForm)
...
var
  setbrokeraccount3: Tsetbrokeraccount3;
...
procedure Tsetbrokeraccount3.setupdatestatus;    // 编辑状态
begin
speedbutton2.enabled:=false;    // examine disable
speedbutton5.enabled:=false;   // refresh disable
end;

procedure Tsetbrokeraccount3.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetbrokeraccount3.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsetbrokeraccount3.FormCloseQuery(Sender: TObject;
...
procedure Tsetbrokeraccount3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetbrokeraccount3.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetbrokeraccount3.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetbrokeraccount3.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetbrokeraccount3.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetbrokeraccount3.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=27 and a.bod_status_id in (1,4)';   // a.comp_id='+inttostr(compid)+' and
...
procedure Tsetbrokeraccount3.FormCreate(Sender: TObject);
begin
...
procedure Tsetbrokeraccount3.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetbrokeraccount3.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetbrokeraccount3.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetbrokeraccount3.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetbrokeraccount3.SpeedButton2Click(Sender: TObject);
begin
...
begin
    if active=true then close;
    commandtext:='sp_updatestock27 27,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsetbrokeraccount3.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetbrokeraccount3.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetbrokeraccount3.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetbrokeraccount3.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
...
procedure Tsetbrokeraccount3.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetbrokeraccount3.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetbrokeraccount3.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetbrokeraccount3.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetbrokeraccount3.dxDBGrid1CustomDrawCell(Sender: TObject;
...
begin
    if (ANode.Values[dxdbgrid1cbod_status.Index] ='财务审批') then Acolor := $00A3FEFC;
...
procedure Tsetbrokeraccount3.N1Click(Sender: TObject);
    ...
    try
        try
            ...
            begin
                ...
                begin
                    if active=true then close;
                    if sender= N1 then commandtext:='sp_updatestock27 27,'+dm.pubqry.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
                    if sender= N2 then commandtext:='sp_updatestock27 -27,'+dm.pubqry.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
                    execute;
                end;
                next;
            end;
...
procedure Tsetbrokeraccount3.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetbrokeraccount3.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetbrokeraccount3.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
{
with cdsbill_dtl do
begin
...
procedure Tsetbrokeraccount3.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsetbrokeraccount3.SpeedButton3Click(Sender: TObject);
begin
