...
///从第2行开始导入,导入格式:1发生日期, 2商业公司, 3代理人, 4金额, 5单据摘要, 6关联品种编码(可不填), 7渠道, 8子渠道
    setprogress(1);
    sbodcd:='';
    j:=2;  //第2行开始 每行一单
...
edit2.text:=commandtext;                
                open;
                if recordcount>0 then id2:=fieldbyname('mate_id').asinteger; // 商业公司
            end;
            ...
            begin
                ...
                open;
                if recordcount>0 then id3:=fieldbyname('mate_id').asinteger;  //代理人
            end;
///从第2行开始导入,导入格式:1发生日期, 2商业公司, 3代理人, 4金额, 5单据摘要, 6关联品种编码(可不填), 7渠道, 8子渠道
            ...
            begin
                ...
                open;
                if recordcount>0 then id6:=fieldbyname('med_id').asinteger; //药品
            end;
            ...
            begin
                ...
                open;
                if recordcount>0 then id7:=fieldbyname('obj_id').asinteger;  //渠道
            end;
            ...
            begin
                ...
                open;
                if recordcount>0 then id8:=fieldbyname('obj_id').asinteger;  //子渠道
            end;
            if (id2=0) or (id3=0) or (id7=0) or (id8=0) then
            begin
                sbodcd:=sbodcd+#13#10+'第'+inttostr(j)+'行:';
                if id2=0 then sbodcd:=sbodcd+' 未取得商业公司 ';
                if id3=0 then sbodcd:=sbodcd+' 未取得代理人 ';
                if id7=0 then sbodcd:=sbodcd+' 未取得渠道 ';
                if id8=0 then sbodcd:=sbodcd+' 未取得子渠道 ';
            end;
        end;
        j:=j+1;
    end;
