...
var
  setmedimport3: Tsetmedimport3;

implementation

uses datamodu, Unit7, Unit17, Unit28b, Unit16f, Unit16c, Unit16a;

{$R *.DFM}

procedure Tsetmedimport3.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetmedimport3.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetmedimport3.FormActivate(Sender: TObject);
begin
//with dm.imp_type do
...
procedure Tsetmedimport3.FormCloseQuery(Sender: TObject;
...
procedure Tsetmedimport3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetmedimport3.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedimport3.dxDBGrid2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
...
procedure Tsetmedimport3.dxDBGrid2Enter(Sender: TObject);
begin
...
procedure Tsetmedimport3.FormKeyPress(Sender: TObject; var Key: Char);
//var fapp: boolean;
begin
...
procedure Tsetmedimport3.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
if cdsbill.fieldbyname('dst_id').asinteger=0 then raise Exception.Create('请选择商业公司');
...
procedure Tsetmedimport3.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetmedimport3.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=1 and a.cate_id=4';     // cate_id 1:商业 2:分销 3:终端战略 4:终端普通 5:两票商业  a.comp_id='+inttostr(compid)+' and
...
procedure Tsetmedimport3.FormCreate(Sender: TObject);
begin
...
procedure Tsetmedimport3.SpeedButton6Click(Sender: TObject);
var mid :integer;
begin
...
begin
    ...
    commandtext:=commandtext+' values ('+inttostr(compid)+',1,0,4,1,'+inttostr(curuserid)+',getdate(),cast(convert(char(10),getdate(),20) as datetime))';
...
procedure Tsetmedimport3.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetmedimport3.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetmedimport3.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
procedure Tsetmedimport3.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedimport3.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedimport3.SpeedButton2Click(Sender: TObject);
begin
with cdsbill do
begin
...
procedure Tsetmedimport3.SpeedButton4Click(Sender: TObject);
begin
...
procedure Tsetmedimport3.SpeedButton3Click(Sender: TObject);
begin
with dm.pubqry do
begin
    if active=True then close;
...
procedure Tsetmedimport3.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport3.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport3.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetmedimport3.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
    then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
...
procedure Tsetmedimport3.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetmedimport3.cdsbillBeforePost(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport3.cdsbillReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedimport3.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedimport3.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetmedimport3.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetmedimport3.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedimport3.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetmedimport3.dxDBGrid2DblClick(Sender: TObject);
begin
{
if pickmedicine.showmodal= mrOK then
begin
...
procedure Tsetmedimport3.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
    SFileName,sbodcd,sb,s6,s11: string;
    J,mi,mid, id2,id3, id5,id6,id11: integer;
    dt: TDatetime;
begin
    ...
    screen.cursor:=crhourglass;
    j:=2; mi:=0;  //第2行开始 每行一单
//格式:1发生日期, 2供货单位id2, 3购货单位id3, 4单据摘要, 5入库类别id5, 6药品编码id6, 7销售单价, 8数量, 9批号, 10有效期, 11省份id11, 12城市
    while (sheet.cells[j,1].text<>'') and (mi<10) do
    begin
        id2:=0;id3:=0;id5:=0;id6:=0;id11:=0; s6:='';s11:='';
        with dm.pubqry do
        begin

//            if (sheet.cells[j,5].text='采购退货') or (sheet.cells[j,5].text='退货') then id5:=2 else id5:=1;
            if strtoint(cleardeli(sheet.cells[j,8].text))<0 then id5:=2 else id5:=1; //id11=1 销售，id11=2 退货

            if sheet.cells[j,2].text<>'' then
            begin
                if active=true then close;
                commandtext:='select top 1 mate_id,district from tb_busimate where mate_type_id=5 and mate_name ='''+Trim(sheet.cells[j,2].text)+'''';
                open;
                if recordcount>0 then
                begin
                    id2:=fieldbyname('mate_id').asinteger;  //商业公司
                    id11:=fieldbyname('district').asinteger;  //所在地区
                end;
            end;
            if sheet.cells[j,3].text<>'' then
            begin
                if active=true then close;
                commandtext:='select top 1 mate_id from tb_busimate where mate_type_id=5 and mate_name ='''+Trim(sheet.cells[j,3].text)+'''';
                open;
            ...
            if sheet.cells[j,12].text='' then s11:= Trim(sheet.cells[j,11].text) else s11:=Trim(sheet.cells[j,11].text)+' '+Trim(sheet.cells[j,12].text);
            if active=true then close;
            if (id11=0) and (Trim(s11)<>'') then
            begin
                commandtext:='select top 1 rec_id from tb_treedata where dbo.fn_getdistrict(rec_id)='''+s11+'''';
                open;
                if recordcount>0 then id11:=fieldbyname('rec_id').asinteger;  //所在地区
            end;
//格式:1发生日期, 2供货单位id2, 3购货单位id3, 4单据摘要, 5入库类别id5, 6药品编码id6, 7销售单价, 8数量, 9批号, 10有效期, 11省份id11, 12城市
            if (id2=0) or  (id3=0) or (id5=0) or (id6=0) then  // or (sheet.cells[j,10].text='')
            begin
                sbodcd:=sbodcd+#13#10+'第'+inttostr(j)+'行:';
                if id2=0 then sbodcd:=sbodcd+',未取得供货单位';
                if id3=0 then sbodcd:=sbodcd+',未取得购货单位';
                if id5=0 then sbodcd:=sbodcd+',未取得入库类别';
                if id6=0 then sbodcd:=sbodcd+',未取得药品';
                if id11=0 then sbodcd:=sbodcd+',未取得省份与城市';
                if Trim(sheet.cells[j,8].text)='' then sbodcd:=sbodcd+',无数量';
                mi:=mi+1;
            end;
        end;
        j:=j+1;
...
//格式:1发生日期, 2供货单位id2, 3购货单位id3, 4单据摘要, 5入库类别id5, 6药品编码id6, 7销售单价, 8数量, 9批号, 10有效期
            if active=true then close;
            commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,busi_type,creat_by,creat_dt,src_id,dst_id,broker_id,bod_desc,carry_dt,agent_id,cate_id,district_id)';  // ,district_id cate_id 1:商业 2:分销 3:终端
            commandtext:=commandtext+' values ('+inttostr(compid)+','''+sb+''',1,0,'+inttostr(id5)+','+inttostr(curuserid)+',getdate(),';
            commandtext:=commandtext+ inttostr(id2)+','+ inttostr(id3)+',null,'''+Trim(sheet.cells[j,4].text)+''','''+datetimetostr(dt)+''',null,4,'+inttostr(id11)+')';  //,'+inttostr(id6)+'
            commandtext:=commandtext+' select @@identity';
edit1.text:=commandtext;
            open;
            mid:=fields[0].asinteger;
            if active=true then close;
            commandtext:='insert into tb_bill_dtl (bod_id,med_id,price,qty,amot,bat_cd,valid_dt)';  //lead_id,sta_id, ,bod_cd1,bod_cd2
            commandtext:=commandtext+' values ('+inttostr(mid)+','+inttostr(id6)+',';
            if cleardeli(sheet.cells[j,7].text)='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,7].text)+',';
            if cleardeli(sheet.cells[j,8].text)='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,8].text)+',';
...
procedure Tsetmedimport3.N1Click(Sender: TObject);
...
procedure Tsetmedimport3.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetmedimport3.N3Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t : string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetmedimport3.dxDBButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickbroker1 do
begin
...
procedure Tsetmedimport3.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetmedimport3.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetmedimport3.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tsetmedimport3.dxDBButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetmedimport3.dxDBButtonEdit1KeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#32) and ((Sender as TdxDBButtonEdit).Text<>'') then //
begin
...
procedure Tsetmedimport3.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetmedimport3.dxEdit1Click(Sender: TObject);
begin
dxEdit1.SelectAll;
dxEdit1.SetFocus;
end;

procedure Tsetmedimport3.dxDBButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
begin
    ...
    if showmodal= mrOK then
    begin
        cdsbill.fieldbyname('src_id').asinteger:= query1.fieldbyname('sta_id').asinteger;
        cdsbill.fieldbyname('mate_name1').asstring:= query1.fieldbyname('zname').asstring;
    end
    else
    begin
...
procedure Tsetmedimport3.dxDBButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
begin
    ...
    if showmodal= mrOK then
    begin
        cdsbill.fieldbyname('dst_id').asinteger:= query1.fieldbyname('sta_id').asinteger;
        cdsbill.fieldbyname('mate_name2').asstring:= query1.fieldbyname('zname').asstring;
    end
    else
    begin
