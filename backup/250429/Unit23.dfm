object setreport16: Tsetreport16
  Left = 138
  Top = 146
  AutoScroll = False
  Caption = #37197#36865#21830#20837#24211#26126#32454#34920
  ClientHeight = 514
  ClientWidth = 1119
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  Visible = True
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  DesignSize = (
    1119
    514)
  PixelsPerInch = 96
  TextHeight = 12
  object Label25: TLabel
    Left = 1095
    Top = 9
    Width = 7
    Height = 12
    Alignment = taRightJustify
    Anchors = [akTop, akRight]
    Font.Charset = GB2312_CHARSET
    Font.Color = clMaroon
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = [fsBold]
    ParentFont = False
  end
  object SpeedButton2: TSpeedButton
    Left = 936
    Top = 2
    Width = 25
    Height = 25
    Hint = #36716#20986#25253#34920
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton2Click
  end
  object Shape4: TShape
    Left = 2
    Top = 1
    Width = 73
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label3: TLabel
    Left = 14
    Top = 7
    Width = 49
    Height = 14
    AutoSize = False
    Caption = #21457#29983#26085#26399
    Transparent = True
    WordWrap = True
  end
  object Shape2: TShape
    Left = 2
    Top = 23
    Width = 73
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label2: TLabel
    Left = 20
    Top = 29
    Width = 36
    Height = 12
    Caption = #37197#36865#21830
    Transparent = True
  end
  object Label1: TLabel
    Left = 182
    Top = 7
    Width = 12
    Height = 12
    Caption = #33267
  end
  object SpeedButton5: TSpeedButton
    Left = 936
    Top = 42
    Width = 175
    Height = 25
    Hint = #21047#26032
    Anchors = [akTop, akRight]
    Caption = '&R '#21047#26032
    Glyph.Data = {
      
      3393333777333777337333333999993333333333377777333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Spacing = 10
    OnClick = SpeedButton5Click
  end
  object Shape1: TShape
    Left = 2
    Top = 45
    Width = 73
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label4: TLabel
    Left = 20
    Top = 51
    Width = 36
    Height = 12
    Caption = #21697'  '#31181
    Transparent = True
  end
  object SpeedButton1: TSpeedButton
    Left = 865
    Top = 2
    Width = 25
    Height = 25
    Hint = #25171#21360#36755#20986
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      33333337FFFF7733333333300000033333333337777773333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Visible = False
    OnClick = SpeedButton1Click
  end
  object wwDBNavigator1: TwwDBNavigator
    Left = 961
    Top = 2
    Width = 150
    Height = 25
    DataSource = DSqry
    Options = []
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    Anchors = [akTop, akRight]
    ParentShowHint = False
    object wwDBNavigator1First: TwwNavButton
      Left = 0
      Top = 0
      Width = 25
      Height = 25
      Hint = #39318#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 0
      Style = nbsFirst
    end
    object wwDBNavigator1Prior: TwwNavButton
      Left = 25
      Top = 0
      Width = 25
      Height = 25
      Hint = #21069#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Prior'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 1
      Style = nbsPrior
    end
    object wwDBNavigator1Next: TwwNavButton
      Left = 50
      Top = 0
      Width = 25
      Height = 25
      Hint = #21518#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Next'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 2
      Style = nbsNext
    end
    object wwDBNavigator1Last: TwwNavButton
      Left = 75
      Top = 0
      Width = 25
      Height = 25
      Hint = #23614#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 3
      Style = nbsLast
    end
    object wwDBNavigator1SaveBookmark: TwwNavButton
      Left = 100
      Top = 0
      Width = 25
      Height = 25
      Hint = #35774#32622#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1SaveBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 4
      Style = nbsSaveBookmark
    end
    object wwDBNavigator1RestoreBookmark: TwwNavButton
      Left = 125
      Top = 0
      Width = 25
      Height = 25
      Hint = #36820#22238#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1RestoreBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 5
      Style = nbsRestoreBookmark
    end
  end
  object dxDBGrid1: TdxDBGrid
    Left = 0
    Top = 69
    Width = 1119
    Height = 445
    Bands = <
      item
      end>
    DefaultLayout = True
    HeaderPanelRowCount = 1
    KeyField = 'dtl_id'
    ShowGroupPanel = True
    ShowSummaryFooter = True
    SummaryGroups = <
      item
        DefaultGroup = True
        SummaryItems = <
          item
            ColumnName = 'dxDBGrid1med_code'
            SummaryField = 'dtl_id'
            SummaryFormat = '###,###,##0'#31508';; '
            SummaryType = cstCount
          end
          item
            ColumnName = 'dxDBGrid1amot'
            SummaryField = 'amot'
            SummaryFormat = '###,###,##0.00;; '
            SummaryType = cstSum
          end>
        Name = 'dxDBGrid1SummaryGroup2'
      end>
    SummarySeparator = ', '
    Align = alBottom
    PopupMenu = PopupMenu1
    TabOrder = 1
    DataSource = DSqry
    Filter.Active = True
    Filter.Criteria = {00000000}
    HighlightColor = clAqua
    HighlightTextColor = clBlack
    LookAndFeel = lfUltraFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabThrough, edgoVertThrough]
    OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
    ShowRowFooter = True
    Anchors = [akLeft, akTop, akRight, akBottom]
    object dxDBGrid1creat_dt: TdxDBGridDateColumn
      Alignment = taCenter
      DisableEditor = True
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'carry_dt'
      ShowButtonStyle = sbNone
    end
    object dxDBGrid1bod_cd: TdxDBGridColumn
      Alignment = taCenter
      Font.Charset = GB2312_CHARSET
      Font.Color = clPurple
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'bod_cd'
      DisableFilter = True
    end
    object dxDBGrid1cbusi_type: TdxDBGridColumn
      Alignment = taCenter
      Caption = #31867#21035
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'cbusi_type'
    end
    object dxDBGrid1supplier: TdxDBGridMaskColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'supplier'
    end
    object dxDBGrid1agent: TdxDBGridColumn
      Caption = #37197#36865#21830
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 100
      BandIndex = 0
      RowIndex = 0
      FieldName = 'agent'
    end
    object dxDBGrid1med_code: TdxDBGridMaskColumn
      Alignment = taCenter
      Font.Charset = GB2312_CHARSET
      Font.Color = clRed
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 60
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_code'
      SummaryFooterType = cstCount
      SummaryFooterFormat = '###,###'#31508
      SummaryField = 'dtl_id'
    end
    object dxDBGrid1med_name: TdxDBGridMaskColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 100
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_name'
    end
    object dxDBGrid1specifi: TdxDBGridMaskColumn
      Width = 100
      BandIndex = 0
      RowIndex = 0
      FieldName = 'specifi'
      DisableFilter = True
    end
    object dxDBGrid1med_unit: TdxDBGridMaskColumn
      Alignment = taCenter
      Width = 30
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_unit'
      DisableFilter = True
    end
    object dxDBGrid1pdt_place: TdxDBGridMaskColumn
      Caption = #29983#20135#20225#19994
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pdt_place'
      DisableFilter = True
    end
    object dxDBGrid1bat_cd: TdxDBGridColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clGreen
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'bat_cd'
    end
    object dxDBGrid1price: TdxDBGridMaskColumn
      Width = 60
      BandIndex = 0
      RowIndex = 0
      FieldName = 'price'
    end
    object dxDBGrid1qty: TdxDBGridMaskColumn
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'qty'
    end
    object dxDBGrid1amot: TdxDBGridMaskColumn
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'amot'
      SummaryFooterType = cstSum
      SummaryFooterField = 'amot'
      SummaryFooterFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1creater: TdxDBGridMaskColumn
      Width = 60
      BandIndex = 0
      RowIndex = 0
      FieldName = 'creater'
    end
    object dxDBGrid1checker: TdxDBGridMaskColumn
      Width = 60
      BandIndex = 0
      RowIndex = 0
      FieldName = 'checker'
    end
  end
  object dxDateEdit1: TdxDateEdit
    Left = 78
    Top = 3
    Width = 90
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 2
    StyleController = StyleController
    Date = -700000.000000000000000000
  end
  object dxDateEdit2: TdxDateEdit
    Left = 206
    Top = 3
    Width = 90
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 3
    StyleController = StyleController
    Date = -700000.000000000000000000
  end
  object dxButtonEdit1: TdxButtonEdit
    Left = 78
    Top = 24
    Width = 218
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 4
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit1ButtonClick
    ExistButtons = True
  end
  object dxButtonEdit2: TdxButtonEdit
    Left = 78
    Top = 45
    Width = 218
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 5
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit2ButtonClick
    ExistButtons = True
  end
  object DSqry: TDataSource
    DataSet = qry
    OnDataChange = DSqryDataChange
    Left = 411
    Top = 203
  end
  object qry: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    ReadOnly = True
    RemoteServer = DM.RemoteServer
    Left = 383
    Top = 203
    object qrydtl_id: TAutoIncField
      FieldName = 'dtl_id'
      ReadOnly = True
    end
    object qrydistrict: TStringField
      FieldName = 'district'
      ReadOnly = True
      Size = 100
    end
    object qryagent: TStringField
      DisplayLabel = #20195#29702#21830
      FieldName = 'agent'
    end
    object qrybod_cd: TStringField
      DisplayLabel = #21333#25454#21495
      FieldName = 'bod_cd'
    end
    object qrycbusi_type: TStringField
      DisplayLabel = #19994#21153#31867#21035
      FieldName = 'cbusi_type'
      ReadOnly = True
      Size = 10
    end
    object qrycarry_dt: TDateTimeField
      DisplayLabel = #26085#26399
      FieldName = 'carry_dt'
    end
    object qrycreat_dt: TDateTimeField
      DisplayLabel = #36141#36135#26085#26399
      FieldName = 'creat_dt'
      DisplayFormat = 'yy-mm-dd'
    end
    object qrysupplier: TStringField
      DisplayLabel = #20379#36135#21333#20301
      FieldName = 'supplier'
      ReadOnly = True
      Size = 100
    end
    object qrymed_code: TStringField
      DisplayLabel = #32534#30721
      FieldName = 'med_code'
    end
    object qrymed_name: TStringField
      DisplayLabel = #21697#21517
      FieldName = 'med_name'
      Size = 50
    end
    object qryspecifi: TStringField
      DisplayLabel = #35268#26684
      FieldName = 'specifi'
      Size = 50
    end
    object qrymed_unit: TStringField
      DisplayLabel = #21333#20301
      FieldName = 'med_unit'
      Size = 30
    end
    object qrypdt_place: TStringField
      DisplayLabel = #29983#20135#21378#21830'('#21407#20135#22320')'
      FieldName = 'pdt_place'
      Size = 30
    end
    object qrybat_cd: TStringField
      DisplayLabel = #25209#21495
      FieldName = 'bat_cd'
    end
    object qryprice: TBCDField
      DisplayLabel = #21333#20215
      FieldName = 'price'
      DisplayFormat = '###,##0.00;; '
      Precision = 15
      Size = 6
    end
    object qryqty: TBCDField
      DisplayLabel = #25968#37327
      FieldName = 'qty'
      DisplayFormat = '###,###,##0;; '
      Precision = 15
    end
    object qryamot: TBCDField
      DisplayLabel = #37329#39069
      FieldName = 'amot'
      DisplayFormat = '###,##0.00;; '
      Precision = 15
    end
    object qrycreater: TStringField
      DisplayLabel = #21046#21333
      FieldName = 'creater'
    end
    object qrychecker: TStringField
      DisplayLabel = #23457#26680
      FieldName = 'checker'
    end
  end
  object StyleController: TdxEditStyleController
    BorderColor = 14065456
    BorderStyle = xbsSingle
    ButtonStyle = btsSimple
    Edges = [edgBottom]
    HotTrack = True
    Left = 467
    Top = 28
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingAGE'
    Parameters = <>
    SQL.Strings = (
      
        'select dtl_id,a.bod_cd,a.carry_dt,a.bod_desc,e.med_code,e.med_na' +
        'me,e.specifi,agent=dbo.fn_mate_name(a.dst_id), supplier=dbo.fn_h' +
        'ome_name(a.src_id),cbusi_type=dbo.fn_obj_desc(a.busi_type), crea' +
        't_dt=cast(convert(char(10),a.creat_dt,102) as datetime), distric' +
        't=dbo.fn_getdistrictbymate_id(a.dst_id),agent=dbo.fn_mate_name(a' +
        '.dst_id), med_unit=dbo.fn_obj_desc(e.unit_id),e.pdt_place,b.bat_' +
        'cd,b.price,b.qty,b.amot,creater=dbo.fn_staff_name(a.creat_by),ch' +
        'ecker=dbo.fn_staff_name(a.check_by) from tb_bill a,tb_bill_dtl b' +
        ',tb_medicine e where a.bod_id=b.bod_id and a.bod_type_id=1 and a' +
        '.bod_status_id=1 and b.med_id=e.med_id  and cast(convert(char(10' +
        '),a.creat_dt,102) as datetime) between '#39'2004-8-6'#39' and '#39'2004-8-22' +
        #39'  order by dbo.fn_getdistrictbymate_id(a.dst_id),dbo.fn_mate_na' +
        'me(a.dst_id),a.creat_dt,b.dtl_id')
    Left = 431
    Top = 48
    object ADOQuery1dtl_id: TAutoIncField
      FieldName = 'dtl_id'
      ReadOnly = True
    end
    object ADOQuery1bod_cd: TStringField
      FieldName = 'bod_cd'
    end
    object ADOQuery1med_code: TStringField
      FieldName = 'med_code'
    end
    object ADOQuery1med_name: TStringField
      FieldName = 'med_name'
      Size = 50
    end
    object ADOQuery1specifi: TStringField
      FieldName = 'specifi'
      Size = 50
    end
    object ADOQuery1agent: TStringField
      FieldName = 'agent'
      ReadOnly = True
      Size = 50
    end
    object ADOQuery1supplier: TStringField
      FieldName = 'supplier'
      ReadOnly = True
      Size = 30
    end
    object ADOQuery1cbusi_type: TStringField
      FieldName = 'cbusi_type'
      ReadOnly = True
      Size = 10
    end
    object ADOQuery1creat_dt: TDateTimeField
      FieldName = 'creat_dt'
      ReadOnly = True
    end
    object ADOQuery1med_unit: TStringField
      FieldName = 'med_unit'
      ReadOnly = True
      Size = 10
    end
    object ADOQuery1pdt_place: TStringField
      FieldName = 'pdt_place'
      Size = 30
    end
    object ADOQuery1bat_cd: TStringField
      FieldName = 'bat_cd'
      Size = 30
    end
    object ADOQuery1price: TBCDField
      FieldName = 'price'
      Precision = 15
      Size = 6
    end
    object ADOQuery1qty: TBCDField
      FieldName = 'qty'
      Precision = 15
    end
    object ADOQuery1amot: TBCDField
      FieldName = 'amot'
      Precision = 15
    end
    object ADOQuery1creater: TStringField
      FieldName = 'creater'
      ReadOnly = True
      Size = 10
    end
    object ADOQuery1checker: TStringField
      FieldName = 'checker'
      ReadOnly = True
      Size = 10
    end
    object ADOQuery1carry_dt: TDateTimeField
      FieldName = 'carry_dt'
    end
    object ADOQuery1bod_desc: TStringField
      FieldName = 'bod_desc'
      Size = 30
    end
    object ADOQuery1district: TStringField
      FieldName = 'district'
      ReadOnly = True
      Size = 100
    end
  end
  object PopupMenu1: TPopupMenu
    Left = 133
    Top = 174
    object N3: TMenuItem
      Caption = #26597#25214'...'
      OnClick = N3Click
    end
    object N4: TMenuItem
      Caption = '-'
    end
    object N1: TMenuItem
      Caption = #38544#34255#26412#21015
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #26174#31034#25152#26377#21015
      OnClick = N2Click
    end
    object N5: TMenuItem
      Caption = '-'
    end
    object N6: TMenuItem
      Caption = #26684#24335#22797#21407
      OnClick = N6Click
    end
  end
end
