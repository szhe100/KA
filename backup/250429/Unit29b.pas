        ...
        if Trim(dxLookupTreeView1.text)<>'' then commandtext:=commandtext+' and dbo.fn_treeischild(e.district,'+DM.district.fieldbyname('rec_id').asstring+')>0 ';
{
        ...
        commandtext:=commandtext+' a.med_id,m.med_code,m.med_name,m.specifi,m.pdt_place,med_unit=dbo.fn_obj_desc(m.unit_id),m.type_id1,';
        commandtext:=commandtext+' bat_cd='''',amot=cast(a.bod_amot as decimal(15,2)),qty=0,rate=0,y.ckd_amot,j.amot1,j.amot2,h.ckd_amot3';
...
s:=#13#10+ '--------------------------------------------';
s1:=''; s3:=''; mamt:=0; mi:=0;
//bodid:=query1.fieldbyname('bod_id').asinteger;
j := dxDBGrid2.ColumnByFieldName('broker').Index;
k := dxDBGrid2.ColumnByFieldName('bod_cd').Index;
p := dxDBGrid2.ColumnByFieldName('amot').Index; //金额
//q := dxDBGrid2.ColumnByFieldName('bod_id').Index;
...
//    s3:=s3+' update tb_bill set ckd_amot3='+floattostr(dxDBGrid2.SelectedNodes[i].values[p])+' where bod_id='+dxDBGrid2.SelectedNodes[i].Strings[q];
    mi := mi+1;
end;
//s:=s+ #13#10+'------------------------------------';
    ...
    end;
{
    ...
    while not eof do
    begin
    	query1.locate('bod_id',fieldbyname('bod_id').asinteger,[]);
    	refreshmyrecord(query1,dm.Refreshcds,0,'a.bod_id', fieldbyname('bod_id').asinteger);
        next;
    end;
}
end;
//query1.locate('bod_id',bodid,[]);
setprogress(0);
//s:='所选单据已成功完成合规资料';
//s:='已成功完成合规资料,可刷新查看结果';
