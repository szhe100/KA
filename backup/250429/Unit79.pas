...
type
  Tsetcontractapply = class(TForm)
    ...
    dxDBGrid2contact1: TdxDBGridColumn;
    dxDBGrid2fax1: TdxDBGridColumn;
...
var
  setcontractapply: Tsetcontractapply;
...
procedure Tsetcontractapply.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetcontractapply.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetcontractapply.FormActivate(Sender: TObject);
begin
with dm.district do
...
procedure Tsetcontractapply.FormCloseQuery(Sender: TObject;
...
procedure Tsetcontractapply.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetcontractapply.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetcontractapply.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetcontractapply.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetcontractapply.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetcontractapply.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=28'; // and a.bod_status_id in (1,5)';  // a.comp_id='+inttostr(compid)+' and
...
procedure Tsetcontractapply.FormCreate(Sender: TObject);
begin
...
procedure Tsetcontractapply.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
//                commandtext:=commandtext+' left join tb_busiframe e on a.med_id=e.med_id and dbo.fn_treeischild(a.district_id,e.district_id)=1';
                commandtext:=commandtext+' left join tb_busiframe2 e on e.rec_id=dbo.fn_getbusiframerecid1(a.broker_id,a.med_id,a.sign_dt)';
...
procedure Tsetcontractapply.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetcontractapply.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetcontractapply.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetcontractapply.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetcontractapply.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetcontractapply.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetcontractapply.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetcontractapply.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetcontractapply.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetcontractapply.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetcontractapply.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetcontractapply.N1Click(Sender: TObject);
...
procedure Tsetcontractapply.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetcontractapply.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetcontractapply.SpeedButton3Click(Sender: TObject);
begin
...
begin
    if active=true then close;
    commandtext:='sp_updatestock28 -28,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsetcontractapply.SpeedButton2Click(Sender: TObject);
begin
...
procedure Tsetcontractapply.dxEdit2KeyDown(Sender: TObject;
...
procedure Tsetcontractapply.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetcontractapply.SpeedButton7Click(Sender: TObject);
begin
