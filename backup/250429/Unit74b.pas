...
type
  Tsetreport18 = class(TForm)
    ...
    dxDBGrid1stock_dt: TdxDBGridDateColumn;
...
procedure Tsetreport18.dxButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetreport18.SpeedButton2Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag)
end;

procedure Tsetreport18.DSqryDataChange(Sender: TObject;
  Field: TField);
begin
with qry do
begin
...
procedure Tsetreport18.SpeedButton4Click(Sender: TObject);
var mi: integer;
begin
...
procedure Tsetreport18.qryBeforeDelete(DataSet: TDataSet);
begin
with dm.pubqry do
begin
    if active=True then close;
...
procedure Tsetreport18.qryAfterPost(DataSet: TDataSet);
begin
if qry.ChangeCount>0 then myupdaterefresh(qry,'tb_report18','rec_id','');
end;

procedure Tsetreport18.SpeedButton3Click(Sender: TObject);
var mi: integer;
begin
with qry do
begin
...
procedure Tsetreport18.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetreport18.medataReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetreport18.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetreport18.FormCloseQuery(Sender: TObject;
...
procedure Tsetreport18.FormCreate(Sender: TObject);
begin
...
procedure Tsetreport18.N3Click(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure Tsetreport18.N1Click(Sender: TObject);
begin
with dxdbgrid1 do Columns[GetAbsoluteColumnIndex(FocusedColumn)].Visible:=false;
end;

procedure Tsetreport18.N2Click(Sender: TObject);
var i,mi: integer;
begin
setprogress(1);
with dxdbgrid1 do
...
procedure Tsetreport18.N6Click(Sender: TObject);
begin
if FileExists(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini')
    then dxDBGrid1.LoadFromIniFile(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini');//加载
end;

procedure Tsetreport18.FormActivate(Sender: TObject);
begin
with dm.district do
...
procedure Tsetreport18.dxLookupTreeView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if key=vk_delete then dxLookupTreeView1.text:='';
end;

procedure Tsetreport18.qryCalcFields(DataSet: TDataSet);
begin
with qry do
begin
...
procedure Tsetreport18.qryBeforePost(DataSet: TDataSet);
begin
with qry do
begin
...
procedure Tsetreport18.SpeedButton6Click(Sender: TObject);
var
    XL,Sheet: Variant;
...
procedure Tsetreport18.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
...
procedure Tsetreport18.dxButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with picklevel3 do
begin
...
procedure Tsetreport18.dxButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetreport18.qrydtlCalcFields(DataSet: TDataSet);
begin
with qrydtl do
begin
    fieldbyname('cqty').asfloat:=
...
procedure Tsetreport18.qrydtlAfterPost(DataSet: TDataSet);
begin
if qrydtl.ChangeCount>0 then myupdaterefresh(qrydtl,'tb_medindexdtl','dtl_id','');
end;

procedure Tsetreport18.dxButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickmanager do
begin
...
procedure Tsetreport18.qrydtlBeforePost(DataSet: TDataSet);
begin
with qrydtl do
begin
    if state=dsinsert then
...
procedure Tsetreport18.qrydtlBeforeDelete(DataSet: TDataSet);
begin
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetreport18.wwDBNavigator1PostClick(Sender: TObject);
begin
if (qrydtl.State in [dsinsert,dsedit]) then qrydtl.post;
end;

procedure Tsetreport18.DSqrydtlStateChange(Sender: TObject);
begin
if qrydtl.state in [dsinsert,dsedit] then qry.edit;
end;

procedure Tsetreport18.wwDBNavigator1CancelClick(Sender: TObject);
begin
if (qrydtl.State in [dsinsert,dsedit]) then qrydtl.cancelUpdates;
end;

procedure Tsetreport18.dxDBGrid2Enter(Sender: TObject);
begin
with qrydtl do
