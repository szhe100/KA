object qryclassexpense: Tqryclassexpense
  Left = 156
  Top = 91
  AutoScroll = False
  Caption = #26597#35810#36153#29992#21457#29983
  ClientHeight = 478
  ClientWidth = 984
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  Visible = True
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  DesignSize = (
    984
    478)
  PixelsPerInch = 96
  TextHeight = 12
  object SpeedButton2: TSpeedButton
    Left = 805
    Top = 2
    Width = 25
    Height = 25
    Hint = #36716#20986#25253#34920
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton2Click
  end
  object Shape4: TShape
    Left = 563
    Top = 7
    Width = 73
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
    Visible = False
  end
  object Label3: TLabel
    Left = 576
    Top = 13
    Width = 49
    Height = 14
    AutoSize = False
    Caption = #26085#26399#33539#22260
    Transparent = True
    Visible = False
    WordWrap = True
  end
  object Label1: TLabel
    Left = 172
    Top = 7
    Width = 12
    Height = 12
    Caption = #33267
  end
  object SpeedButton5: TSpeedButton
    Left = 805
    Top = 27
    Width = 175
    Height = 25
    Hint = #21047#26032
    Anchors = [akTop, akRight]
    Caption = '&R '#21047#26032
    Glyph.Data = {
      
      3393333777333777337333333999993333333333377777333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Spacing = 10
    OnClick = SpeedButton5Click
  end
  object wwDBNavigator1: TwwDBNavigator
    Left = 830
    Top = 2
    Width = 150
    Height = 25
    DataSource = DSqry
    Options = []
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    Anchors = [akTop, akRight]
    ParentShowHint = False
    object wwDBNavigator1First: TwwNavButton
      Left = 0
      Top = 0
      Width = 25
      Height = 25
      Hint = #39318#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 0
      Style = nbsFirst
    end
    object wwDBNavigator1Prior: TwwNavButton
      Left = 25
      Top = 0
      Width = 25
      Height = 25
      Hint = #21069#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Prior'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 1
      Style = nbsPrior
    end
    object wwDBNavigator1Next: TwwNavButton
      Left = 50
      Top = 0
      Width = 25
      Height = 25
      Hint = #21518#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Next'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 2
      Style = nbsNext
    end
    object wwDBNavigator1Last: TwwNavButton
      Left = 75
      Top = 0
      Width = 25
      Height = 25
      Hint = #23614#35760#24405
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 3
      Style = nbsLast
    end
    object wwDBNavigator1SaveBookmark: TwwNavButton
      Left = 100
      Top = 0
      Width = 25
      Height = 25
      Hint = #35774#32622#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1SaveBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 4
      Style = nbsSaveBookmark
    end
    object wwDBNavigator1RestoreBookmark: TwwNavButton
      Left = 125
      Top = 0
      Width = 25
      Height = 25
      Hint = #36820#22238#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1RestoreBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 5
      Style = nbsRestoreBookmark
    end
  end
  object dxDBGrid1: TdxDBGrid
    Left = 0
    Top = 54
    Width = 984
    Height = 424
    Bands = <
      item
      end>
    DefaultLayout = True
    HeaderPanelRowCount = 1
    KeyField = 'dtl_id'
    ShowGroupPanel = True
    ShowSummaryFooter = True
    SummaryGroups = <
      item
        DefaultGroup = True
        SummaryItems = <
          item
            SummaryField = 'dtl_id'
            SummaryFormat = '###,###,##0'#31508';; '
            SummaryType = cstCount
          end
          item
            SummaryField = 'amot'
            SummaryFormat = '###,###,##0.00;; '
            SummaryType = cstSum
          end>
        Name = 'dxDBGrid1SummaryGroup2'
      end>
    SummarySeparator = ', '
    Align = alBottom
    TabOrder = 1
    DataSource = DSqry
    Filter.Active = True
    Filter.DropDownCount = 30
    Filter.Criteria = {00000000}
    HighlightColor = clAqua
    HighlightTextColor = clBlack
    LookAndFeel = lfUltraFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSelect, edgoMultiSort, edgoTabThrough, edgoVertThrough]
    OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoUseBitmap]
    ShowRowFooter = True
    Anchors = [akLeft, akTop, akRight, akBottom]
    object dxDBGrid1creat_dt: TdxDBGridColumn
      Alignment = taCenter
      Caption = #21046#21333#26085#26399
      Width = 69
      BandIndex = 0
      RowIndex = 0
      FieldName = 'creat_dt'
      SummaryFooterType = cstCount
      SummaryFooterField = 'dtl_id'
      SummaryFooterFormat = '###,##0'#31508';; '
      SummaryType = cstCount
      SummaryField = 'dtl_id'
      SummaryFormat = '###,##0'#31508';; '
    end
    object dxDBGrid1carry_dt: TdxDBGridColumn
      Alignment = taCenter
      Width = 69
      BandIndex = 0
      RowIndex = 0
      FieldName = 'carry_dt'
    end
    object dxDBGrid1bod_cd: TdxDBGridMaskColumn
      Alignment = taCenter
      Font.Charset = GB2312_CHARSET
      Font.Color = clPurple
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'bod_cd'
    end
    object dxDBGrid1acc_cd: TdxDBGridMaskColumn
      Width = 69
      BandIndex = 0
      RowIndex = 0
      FieldName = 'acc_cd'
    end
    object dxDBGrid1ptitle: TdxDBGridMaskColumn
      Width = 90
      BandIndex = 0
      RowIndex = 0
      FieldName = 'ptitle'
    end
    object dxDBGrid1acc_title: TdxDBGridMaskColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 90
      BandIndex = 0
      RowIndex = 0
      FieldName = 'acc_title'
    end
    object dxDBGrid1zdesc: TdxDBGridMaskColumn
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'zdesc'
      DisableFilter = True
    end
    object dxDBGrid1comptitle: TdxDBGridMaskColumn
      Width = 120
      BandIndex = 0
      RowIndex = 0
      FieldName = 'comptitle'
    end
    object dxDBGrid1expensesup: TdxDBGridMaskColumn
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'expensesup'
    end
    object dxDBGrid1cost: TdxDBGridMaskColumn
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'cost'
      SummaryFooterType = cstSum
      SummaryFooterField = 'cost'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'cost'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1price: TdxDBGridMaskColumn
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'price'
      SummaryFooterType = cstSum
      SummaryFooterField = 'price'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'price'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1creater: TdxDBGridMaskColumn
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'creater'
    end
    object dxDBGrid1checker: TdxDBGridMaskColumn
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'checker'
    end
  end
  object dxDateEdit1: TdxDateEdit
    Left = 81
    Top = 3
    Width = 90
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 2
    StyleController = StyleController
    Date = -700000.000000000000000000
  end
  object dxDateEdit2: TdxDateEdit
    Left = 184
    Top = 3
    Width = 90
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 3
    StyleController = StyleController
    Date = -700000.000000000000000000
  end
  object Edit1: TEdit
    Left = 364
    Top = 13
    Width = 121
    Height = 20
    TabOrder = 4
    Text = 'Edit1'
    Visible = False
  end
  object dxPickEdit1: TdxPickEdit
    Left = 3
    Top = 2
    Width = 73
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 5
    ReadOnly = False
    Text = #21457#29983#26085#26399
    Items.Strings = (
      #21457#29983#26085#26399
      #21046#21333#26085#26399)
    StoredValues = 64
  end
  object DSqry: TDataSource
    DataSet = qry
    OnDataChange = DSqryDataChange
    Left = 262
    Top = 266
  end
  object qry: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    ReadOnly = True
    RemoteServer = DM.RemoteServer
    Left = 234
    Top = 266
    object qrydtl_id: TAutoIncField
      FieldName = 'dtl_id'
      ReadOnly = True
    end
    object qrybod_id: TAutoIncField
      FieldName = 'bod_id'
      ReadOnly = True
    end
    object qrybod_cd: TStringField
      DisplayLabel = #20973#35777#21333#21495
      FieldName = 'bod_cd'
    end
    object qrycreat_dt: TDateTimeField
      DisplayLabel = #20973#35777#26085#26399
      FieldName = 'creat_dt'
      DisplayFormat = 'yyyy-mm-dd'
    end
    object qrycheck_dt: TDateTimeField
      DisplayLabel = #26085#26399
      FieldName = 'check_dt'
      ReadOnly = True
      DisplayFormat = 'yyyy-mm-dd'
    end
    object qrycarry_dt: TDateTimeField
      DisplayLabel = #21457#29983#26085#26399
      FieldName = 'carry_dt'
      DisplayFormat = 'yyyy-mm-dd'
    end
    object qryzdesc: TStringField
      DisplayLabel = #25688#35201
      FieldName = 'zdesc'
      ReadOnly = True
      Size = 101
    end
    object qrymed_id: TIntegerField
      FieldName = 'med_id'
    end
    object qryacc_cd: TStringField
      DisplayLabel = #31185#30446#32534#30721
      FieldName = 'acc_cd'
      Size = 10
    end
    object qryacc_title: TStringField
      DisplayLabel = #21457#29983#31185#30446
      FieldName = 'acc_title'
      Size = 50
    end
    object qryLtitle: TStringField
      FieldName = 'Ltitle'
      ReadOnly = True
      Size = 100
    end
    object qryptitle: TStringField
      DisplayLabel = #19978#32423#31185#30446
      FieldName = 'ptitle'
      ReadOnly = True
      Size = 100
    end
    object qrycomptitle: TStringField
      DisplayLabel = #36153#29992#20154
      FieldName = 'comptitle'
      Size = 50
    end
    object qryexpensesup: TStringField
      DisplayLabel = #36153#29992#20379#26041
      FieldName = 'expensesup'
      Size = 100
    end
    object qrycost: TBCDField
      DisplayLabel = #20511#26041#21457#29983
      FieldName = 'cost'
      DisplayFormat = '###,###,##0.00;; '
      Precision = 15
    end
    object qryprice: TBCDField
      DisplayLabel = #36151#26041#21457#29983
      FieldName = 'price'
      DisplayFormat = '###,###,##0.00;; '
      Precision = 15
      Size = 6
    end
    object qrycreater: TStringField
      DisplayLabel = #21046#21333
      FieldName = 'creater'
      ReadOnly = True
      Size = 100
    end
    object qrychecker: TStringField
      DisplayLabel = #23457#26680
      FieldName = 'checker'
      ReadOnly = True
      Size = 100
    end
  end
  object StyleController: TdxEditStyleController
    BorderColor = 14065456
    BorderStyle = xbsSingle
    ButtonStyle = btsSimple
    Edges = [edgBottom]
    HotTrack = True
    Left = 467
    Top = 28
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=Kingdom'
    Parameters = <>
    SQL.Strings = (
      
        'select top 1 a.bod_id,a.bod_cd,check_dt=cast(convert(char(10),a.' +
        'check_dt,20) as datetime),'
      '  zdesc=rtrim(isnull(a.bod_desc,'#39' '#39'))+'#39' '#39'+isnull(b.zdesc,'#39' '#39'),'
      
        '  b.dtl_id,b.med_id,c.acc_cd,acc_title,Ltitle=dbo.fn_getacctitle' +
        '0(b.med_id),'
      
        '  ptitle=dbo.fn_getacctitle0(c.parent_id),comptitle=t.title,expe' +
        'nsesup=m.mate_name,'
      
        '  b.cost,b.price,creater=dbo.fn_staff_name(a.creat_by),checker=d' +
        'bo.fn_staff_name(a.check_by),bal=0'
      
        ' from tb_bill a,tb_bill_dtl b,tb_accounts c,tb_busimate m,tb_com' +
        'ptree t'
      
        ' where a.bod_id=b.bod_id and a.bod_type_id=25 and a.bod_status_i' +
        'd=1 and b.med_id=c.acc_id'
      '  and b.rela_id*=t.rec_id and b.rela_value*=m.mate_id'
      '  and a.comp_id=2'
      '  and a.check_dt>'#39'2007-1-1'#39
      
        '  and med_id in (select acc_id from tb_accounts where comp_id=2 ' +
        'and (dbo.fn_accischild(acc_id,250)=1 or dbo.fn_accischild(acc_id' +
        ',251)=1 or dbo.fn_accischild(acc_id,252)=1))')
    Left = 148
    Top = 150
    object ADOQuery1bod_id: TAutoIncField
      FieldName = 'bod_id'
      ReadOnly = True
    end
    object ADOQuery1bod_cd: TStringField
      DisplayLabel = #20973#35777#21333#21495
      FieldName = 'bod_cd'
    end
    object ADOQuery1check_dt: TDateTimeField
      DisplayLabel = #26085#26399
      FieldName = 'check_dt'
      ReadOnly = True
    end
    object ADOQuery1zdesc: TStringField
      DisplayLabel = #25688#35201
      FieldName = 'zdesc'
      ReadOnly = True
      Size = 101
    end
    object ADOQuery1dtl_id: TAutoIncField
      FieldName = 'dtl_id'
      ReadOnly = True
    end
    object ADOQuery1med_id: TIntegerField
      FieldName = 'med_id'
    end
    object ADOQuery1acc_cd: TStringField
      DisplayLabel = #31185#30446#32534#30721
      FieldName = 'acc_cd'
      Size = 10
    end
    object ADOQuery1acc_title: TStringField
      DisplayLabel = #21457#29983#31185#30446
      FieldName = 'acc_title'
      Size = 50
    end
    object ADOQuery1Ltitle: TStringField
      FieldName = 'Ltitle'
      ReadOnly = True
      Size = 100
    end
    object ADOQuery1ptitle: TStringField
      DisplayLabel = #19978#32423#31185#30446
      FieldName = 'ptitle'
      ReadOnly = True
      Size = 100
    end
    object ADOQuery1comptitle: TStringField
      DisplayLabel = #36153#29992#20154
      FieldName = 'comptitle'
      Size = 50
    end
    object ADOQuery1expensesup: TStringField
      DisplayLabel = #36153#29992#20379#26041
      FieldName = 'expensesup'
      Size = 100
    end
    object ADOQuery1cost: TBCDField
      DisplayLabel = #20511#26041#21457#29983
      FieldName = 'cost'
      Precision = 15
    end
    object ADOQuery1price: TBCDField
      DisplayLabel = #36151#26041#21457#29983
      FieldName = 'price'
      Precision = 15
      Size = 6
    end
    object ADOQuery1creater: TStringField
      DisplayLabel = #21046#21333
      FieldName = 'creater'
      ReadOnly = True
      Size = 100
    end
    object ADOQuery1checker: TStringField
      DisplayLabel = #23457#26680
      FieldName = 'checker'
      ReadOnly = True
      Size = 100
    end
  end
end
