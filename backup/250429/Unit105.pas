...
type
  TsetexpayA = class(TForm)
    ...
    bill_dtlbod_type_id: TIntegerField;
    dxDBGrid2: TdxDBGrid;
    ...
    dxDBGrid3: TdxDBGrid;
    dxDBGrid3carry_dt: TdxDBGridColumn;
    dxDBGrid3bod_cd1: TdxDBGridColumn;
    ...
    dxDBGrid3amot: TdxDBGridMaskColumn;
    bill_dtltype_id2: TIntegerField;
    bill_dtlctype2: TStringField;
    bill_dtl1type_id2: TIntegerField;
    bill_dtl1ctype2: TStringField;
    ...
    procedure bill_dtl1AfterPost(DataSet: TDataSet);
...
procedure TsetexpayA.setupdatestatus;    // 编辑状态
begin
...
procedure TsetexpayA.setunupdatestatus;  // 浏览状态
begin
...
procedure TsetexpayA.FormActivate(Sender: TObject);
begin
with homebank do
begin
...
procedure TsetexpayA.FormCloseQuery(Sender: TObject;
...
procedure TsetexpayA.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TsetexpayA.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpayA.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TsetexpayA.SpeedButton8Click(Sender: TObject);
begin
...
begin
    ...
    refreshmyrecord(bill,dm.Refreshcds,0,'bod_id',fieldbyname('bod_id').asinteger)
end;
with bill_dtl do
begin
...
//    if state = dsinsert then cancel;
    if state in [dsinsert,dsedit] then post;
    if ChangeCount>0 then myupdaterefresh(bill_dtl1,'tb_bill_dtl','dtl_id','');
end;
setunupdatestatus;
setprogress(0);
end;

procedure TsetexpayA.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
setprogress(1);
...
procedure TsetexpayA.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
with bill_dtl do
...
begin
    ...
    commandtext:=commandtext+' a.*,mate_name1=dbo.fn_mate_name(a.src_id),m.mate_name,agent=c.mate_name,c.stoppay,deputy=c.receiver,c.acnt_bank,c.bank_account,creater=d.zname,checker=e.zname,';
    ...
    commandtext:=commandtext+' where bod_type_id=49';
...
procedure TsetexpayA.FormCreate(Sender: TObject);
begin
...
procedure TsetexpayA.SpeedButton7Click(Sender: TObject);
begin
...
procedure TsetexpayA.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
//        dxDBDateEdit1.enabled:=(recordcount>0) and (fieldbyname('creat_by').asinteger=curuserid);
    ...
    begin
        ...
        begin
            ...
            begin
                if active=true then close;
                commandtext:='select b.bod_id,b.dtl_id,b.med_id,b.price,b.rela_value,b.amot,c.carry_dt,c.bod_cd,o.type_id2,';
    			...
    			commandtext:=commandtext+' left join tb_busiframe4 o on o.rec_id=dbo.fn_getbusiframerecid6(c.agent_id,c.dst_id,d.med_id,c.carry_dt)'; // --a.agent_id=o.agent_id and a.dst_id=o.mate_id and b.med_id=o.med_id';
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                commandtext:=commandtext+' left join tb_bill c on b.med_id=c.bod_id';
                commandtext:=commandtext+' left join tb_medicine m on c.med_id=m.med_id';
    			commandtext:=commandtext+' left join tb_busiframe4 o on o.rec_id=dbo.fn_getbusiframerecid6(c.agent_id,c.dst_id,c.med_id,c.carry_dt)'; // --a.agent_id=o.agent_id and a.dst_id=o.mate_id and b.med_id=o.med_id';
                commandtext:=commandtext+' where b.bod_id='+bill.fieldbyname('bod_id').asstring+' and b.type_id=43';
edit1.text:=commandtext;
                try
                    setprogress(1);
...
procedure TsetexpayA.DSbillStateChange(Sender: TObject);
begin
with bill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpayA.billBeforeEdit(DataSet: TDataSet);
begin
with bill do
begin
...
procedure TsetexpayA.billCalcFields(DataSet: TDataSet);
begin
with bill do
...
procedure TsetexpayA.billReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure TsetexpayA.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpayA.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TsetexpayA.dxDBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
if key in ['a'..'z'] then key:=chr(ord(key)-32);   // turn lower charactor to upper
end;

procedure TsetexpayA.SpeedButton1Click(Sender: TObject);
begin
...
procedure TsetexpayA.SpeedButton2Click(Sender: TObject);
var s: string;
begin
with bill do
begin
...
//        abort;
    ...
    if active=True then close;
    commandtext:=' select top 5 a.bod_cd from tb_bill a';
    ...
    commandtext:=commandtext+'  and d.med_id=a.bod_id and d.amot>a.bod_amot';
    open;
    if recordcount>0 then
    begin
    ...
    commandtext:=commandtext+' left join (select b.med_id,amot=sum(b.amot) from tb_bill a,tb_bill_dtl b where a.bod_type_id=37 and (a.bod_status_id=1 or a.bod_id='+bill.fieldbyname('bod_id').asstring+') and a.bod_id=b.bod_id and b.type_id=1 group by b.med_id) d on b.dtl_id=d.med_id';
    ...
    commandtext:=commandtext+'  and b.dtl_id=d.med_id and d.amot>b.qty*(isnull(c.amot,0)+isnull(c.fee,0))';
    edit1.text:=commandtext;
    open;
    if recordcount>0 then
    begin
...
procedure TsetexpayA.DSbill_dtlStateChange(Sender: TObject);
begin
with bill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpayA.bill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
showmessage(dataset.name+':'+e.message);
...
procedure TsetexpayA.bill_dtlAfterPost(DataSet: TDataSet);
begin
if bill_dtl.ChangeCount>0 then myupdaterefresh(bill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure TsetexpayA.dxDBGrid2CustomDrawCell(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
  AColumn: TdxTreeListColumn; ASelected, AFocused, ANewItemRow: Boolean;
  var AText: String; var AColor: TColor; AFont: TFont;
  var AAlignment: TAlignment; var ADone: Boolean);
...
procedure TsetexpayA.bill_dtlBeforePost(DataSet: TDataSet);
begin
with bill_dtl do
begin
...
procedure TsetexpayA.bill_dtl1AfterPost(DataSet: TDataSet);
begin
if bill_dtl1.ChangeCount>0 then myupdaterefresh(bill_dtl1,'tb_bill_dtl','dtl_id','');
end;

procedure TsetexpayA.bill_dtlCalcFields(DataSet: TDataSet);
begin
with bill_dtl do
begin
