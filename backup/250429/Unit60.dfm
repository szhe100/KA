object setvisitlog: Tsetvisitlog
  Left = 182
  Top = 118
  AutoScroll = False
  Caption = #24847#21521#19994#21153#20154#21592#25308#35775#35760#24405
  ClientHeight = 496
  ClientWidth = 830
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  OldCreateOrder = False
  Position = poScreenCenter
  Visible = True
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  DesignSize = (
    830
    496)
  PixelsPerInch = 96
  TextHeight = 12
  object Shape4: TShape
    Left = 3
    Top = 3
    Width = 60
    Height = 19
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label2: TLabel
    Left = 2
    Top = 32
    Width = 60
    Height = 12
    Caption = #19994#21153#21592#22995#21517
  end
  object SpeedButton2: TSpeedButton
    Left = 599
    Top = 0
    Width = 25
    Height = 25
    Hint = #36716#20986#25253#34920
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton2Click
  end
  object SpeedButton3: TSpeedButton
    Left = 599
    Top = 25
    Width = 226
    Height = 25
    Hint = #21047#26032
    Anchors = [akTop, akRight]
    Flat = True
    Glyph.Data = {
      
      3393333777333777337333333999993333333333377777333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton3Click
  end
  object Label3: TLabel
    Left = 146
    Top = 7
    Width = 12
    Height = 12
    Caption = #33267
    Transparent = True
  end
  object Label1: TLabel
    Left = 10
    Top = 7
    Width = 52
    Height = 14
    AutoSize = False
    Caption = #26085#26399#33539#22260
    Transparent = True
    WordWrap = True
  end
  object dxDBGrid1: TdxDBGrid
    Left = 0
    Top = 51
    Width = 830
    Height = 445
    Bands = <
      item
      end>
    DefaultLayout = True
    HeaderPanelRowCount = 1
    KeyField = 'rec_id'
    ShowGroupPanel = True
    ShowSummaryFooter = True
    SummaryGroups = <>
    SummarySeparator = ', '
    Align = alBottom
    PopupMenu = PopupMenu1
    TabOrder = 1
    OnClick = dxDBGrid1Enter
    OnEnter = dxDBGrid1Enter
    DataSource = DSqry
    Filter.Active = True
    Filter.DropDownCount = 30
    Filter.Criteria = {00000000}
    HideSelectionColor = clAqua
    HideSelectionTextColor = clBlack
    HighlightColor = clAqua
    HighlightTextColor = clBlack
    LookAndFeel = lfUltraFlat
    OptionsBehavior = [edgoAnsiSort, edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabThrough, edgoVertThrough]
    OptionsDB = [edgoCanAppend, edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
    Anchors = [akLeft, akTop, akRight, akBottom]
    object dxDBGrid1occur_dt: TdxDBGridDateColumn
      Alignment = taCenter
      Color = 12910588
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'occur_dt'
      SummaryFooterType = cstCount
      SummaryFooterField = 'rec_id'
      SummaryType = cstCount
      SummaryField = 'rec_id'
      SummaryFormat = '###;; '
    end
    object dxDBGrid1broker: TdxDBGridMaskColumn
      Color = 12910588
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'broker'
    end
    object dxDBGrid1district: TdxDBGridMaskColumn
      Color = 12910588
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'district'
    end
    object dxDBGrid1hospital: TdxDBGridColumn
      Color = 12910588
      Width = 100
      BandIndex = 0
      RowIndex = 0
      FieldName = 'hospital'
    end
    object dxDBGrid1medicine: TdxDBGridColumn
      Color = 12910588
      Width = 100
      BandIndex = 0
      RowIndex = 0
      FieldName = 'medicine'
      DisableFilter = True
    end
    object dxDBGrid1zcontent: TdxDBGridMaskColumn
      Color = 12910588
      Width = 180
      BandIndex = 0
      RowIndex = 0
      FieldName = 'zcontent'
      DisableFilter = True
    end
    object dxDBGrid1evaluate: TdxDBGridMaskColumn
      Color = 12910588
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'evaluate'
    end
    object dxDBGrid1creat_dt: TdxDBGridDateColumn
      DisableEditor = True
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'creat_dt'
    end
    object dxDBGrid1creater: TdxDBGridMaskColumn
      DisableEditor = True
      Width = 57
      BandIndex = 0
      RowIndex = 0
      FieldName = 'creater'
    end
  end
  object dxEdit1: TdxEdit
    Left = 65
    Top = 28
    Width = 80
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    TabOrder = 0
    OnKeyDown = dxEdit1KeyDown
  end
  object wwDBNavigator1: TwwDBNavigator
    Left = 624
    Top = 0
    Width = 200
    Height = 25
    DataSource = DSqry
    Options = []
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    Anchors = [akTop, akRight]
    ParentShowHint = False
    object wwDBNavigator1First: TwwNavButton
      Left = 0
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 0
      Style = nbsFirst
    end
    object wwDBNavigator1Prior: TwwNavButton
      Left = 25
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Prior'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 1
      Style = nbsPrior
    end
    object wwDBNavigator1Next: TwwNavButton
      Left = 50
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Next'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 2
      Style = nbsNext
    end
    object wwDBNavigator1Last: TwwNavButton
      Left = 75
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 3
      Style = nbsLast
    end
    object wwDBNavigator1Insert: TwwNavButton
      Left = 100
      Top = 0
      Width = 25
      Height = 25
      Hint = 'Insert new record'
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Insert'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 4
      Style = nbsInsert
    end
    object wwDBNavigator1Delete: TwwNavButton
      Left = 125
      Top = 0
      Width = 25
      Height = 25
      Hint = #21024#38500
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Delete'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 5
      Style = nbsDelete
    end
    object wwDBNavigator1Post: TwwNavButton
      Left = 150
      Top = 0
      Width = 25
      Height = 25
      Hint = 'Post changes of current record'
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Post'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 6
      Style = nbsPost
    end
    object wwDBNavigator1Cancel: TwwNavButton
      Left = 175
      Top = 0
      Width = 25
      Height = 25
      Hint = 'Cancel changes made to current record'
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Cancel'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 7
      Style = nbsCancel
    end
  end
  object dxDateEdit1: TdxDateEdit
    Left = 65
    Top = 3
    Width = 80
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 3
    Date = -700000.000000000000000000
  end
  object dxDateEdit2: TdxDateEdit
    Left = 159
    Top = 3
    Width = 80
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 4
    Date = -700000.000000000000000000
  end
  object qry: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    AfterInsert = qryAfterInsert
    BeforeEdit = qryBeforeEdit
    BeforePost = qryBeforePost
    AfterPost = qryAfterPost
    BeforeDelete = qryBeforeDelete
    AfterDelete = qryAfterPost
    Left = 81
    Top = 134
    object qryrec_id: TAutoIncField
      FieldName = 'rec_id'
      ReadOnly = True
    end
    object qrycomp_id: TWordField
      FieldName = 'comp_id'
    end
    object qryoccur_dt: TDateTimeField
      DisplayLabel = #25308#35775#26085#26399
      FieldName = 'occur_dt'
    end
    object qrybroker: TStringField
      DisplayLabel = #19994#21153#21592
      FieldName = 'broker'
    end
    object qrydistrict: TStringField
      DisplayLabel = #22320#21306
      FieldName = 'district'
    end
    object qryzcontent: TStringField
      DisplayLabel = #25308#35775#20869#23481
      FieldName = 'zcontent'
      Size = 50
    end
    object qryevaluate: TStringField
      DisplayLabel = #35780#20215
      FieldName = 'evaluate'
    end
    object qryhospital: TStringField
      DisplayLabel = #21307#38498
      FieldName = 'hospital'
    end
    object qrymedicine: TStringField
      DisplayLabel = #21697#31181
      FieldName = 'medicine'
      Size = 50
    end
    object qrycreat_by: TIntegerField
      FieldName = 'creat_by'
    end
    object qrycreat_dt: TDateTimeField
      DisplayLabel = #35760#24405#26102#38388
      FieldName = 'creat_dt'
    end
    object qrycreater: TStringField
      DisplayLabel = #35760#24405#20154
      FieldName = 'creater'
    end
  end
  object DSqry: TDataSource
    DataSet = qry
    OnDataChange = DSqryDataChange
    Left = 111
    Top = 134
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingageYPH;Data Source=.\heyang'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select a.*,creater=b.zname'
      ' from tb_visitlog a,tb_staff b'
      ' where a.creat_by*=b.sta_id')
    Left = 468
    Top = 281
    object ADOQuery1rec_id: TAutoIncField
      FieldName = 'rec_id'
      ReadOnly = True
    end
    object ADOQuery1comp_id: TWordField
      FieldName = 'comp_id'
    end
    object ADOQuery1occur_dt: TDateTimeField
      FieldName = 'occur_dt'
    end
    object ADOQuery1broker: TStringField
      FieldName = 'broker'
    end
    object ADOQuery1district: TStringField
      FieldName = 'district'
    end
    object ADOQuery1zcontent: TStringField
      FieldName = 'zcontent'
      Size = 50
    end
    object ADOQuery1evaluate: TStringField
      FieldName = 'evaluate'
    end
    object ADOQuery1creat_by: TIntegerField
      FieldName = 'creat_by'
    end
    object ADOQuery1creat_dt: TDateTimeField
      FieldName = 'creat_dt'
    end
    object ADOQuery1creater: TStringField
      FieldName = 'creater'
    end
    object ADOQuery1hospital: TStringField
      FieldName = 'hospital'
    end
    object ADOQuery1medicine: TStringField
      FieldName = 'medicine'
      Size = 50
    end
  end
  object PopupMenu1: TPopupMenu
    Left = 133
    Top = 175
    object N3: TMenuItem
      Caption = #26597#25214'...'
      OnClick = N3Click
    end
    object N4: TMenuItem
      Caption = '-'
    end
    object N1: TMenuItem
      Caption = #38544#34255#26412#21015
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #26174#31034#25152#26377#21015
      OnClick = N2Click
    end
    object N5: TMenuItem
      Caption = '-'
    end
    object N6: TMenuItem
      Caption = #26684#24335#22797#21407
      OnClick = N6Click
    end
  end
end
