...
procedure Tsetbusiframe2.SpeedButton2Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag);
end;

procedure Tsetbusiframe2.DSbusiframeDataChange(Sender: TObject;
  Field: TField);
begin
with busiframe do
begin
...
begin
    ...
    commandtext:=commandtext+' from tb_busiframe2_dtl a';
...
procedure Tsetbusiframe2.SpeedButton4Click(Sender: TObject);
var mi,ni: integer;
begin
...
begin
    if active then close;
    commandtext:='insert into tb_busiframe2 (comp_id,district_id,med_id,creat_by,creat_dt) values';
...
begin
    ...
    if active then close;
    commandtext:='select top 1 rec_id from tb_busiframe2 where f1='+inttostr(RadioGroup1.itemindex+1)+' and med_id='+inttostr(dxButtonEdit1.tag);
    commandtext:=commandtext+' and district_id='+dm.district.fieldbyname('rec_id').asstring;
    open;
    if recordcount=0 then
    begin
        if active then close;
        commandtext:='insert into tb_busiframe2 (comp_id,f1,med_id,district_id,creat_by,creat_dt)';
    ...
    if active then close;
    commandtext:='insert into tb_busiframe2_dtl (rela_id,level_id1,level_id2,level_id3,sta_id,med_id,district_id,valid_dt,creat_by,creat_dt)';
...
procedure Tsetbusiframe2.FormCloseQuery(Sender: TObject;
...
procedure Tsetbusiframe2.busiframeBeforeDelete(DataSet: TDataSet);
begin
with busiframe do
begin
...
begin
    if active then close;
    commandtext:='select top 1 1 from tb_busiframe2_dtl where rela_id='+busiframe.fieldbyname('rec_id').asstring;
    open;
...
procedure Tsetbusiframe2.SpeedButton3Click(Sender: TObject);
var mi: integer;
begin
with busiframe do
begin
...
//    commandtext:=commandtext+'	dist3=dbo.fn_getdistrictlevel (a.district_id,3)';
    commandtext:=commandtext+' from tb_busiframe2 a';
//    commandtext:=commandtext+' left join tb_object b on a.level_id1=b.obj_id';
//    commandtext:=commandtext+' left join tb_object c on a.level_id2=c.obj_id';
//    commandtext:=commandtext+' left join tb_object d on a.level_id3=d.obj_id';
    commandtext:=commandtext+' left join tb_staff e on a.sta_id=e.sta_id';
...
procedure Tsetbusiframe2.busiframeBeforePost(DataSet: TDataSet);
begin
with busiframe do
begin
...
procedure Tsetbusiframe2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetbusiframe2.FormActivate(Sender: TObject);
begin
{
with dm.level1 do
...
procedure Tsetbusiframe2.busiframeAfterDelete(DataSet: TDataSet);
begin
if busiframe.ChangeCount>0 then myupdaterefresh(busiframe,'tb_busiframe2','rec_id','');
end;

procedure Tsetbusiframe2.FormCreate(Sender: TObject);
begin
...
procedure Tsetbusiframe2.dxLookupTreeView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if key=vk_delete then dxLookupTreeView1.text:='';
end;

procedure Tsetbusiframe2.PageControl1Change(Sender: TObject);
begin
...
procedure Tsetbusiframe2.dxDBTreeList1GetStateIndex(Sender: TObject;
  Node: TdxTreeListNode; var Index: Integer);
begin
...
procedure Tsetbusiframe2.btnFullCollapseClick(Sender: TObject);
begin
  dxDBTreeList1.FullCollapse;
end;

procedure Tsetbusiframe2.btnFullExpandClick(Sender: TObject);
begin
  dxDBTreeList1.FullExpand;
end;

procedure Tsetbusiframe2.SpeedButton1Click(Sender: TObject);
var mi: integer;
begin
if pagecontrol1.activepage=tabsheet2 then
begin
    ...
    begin
        ...
        commandtext:=commandtext+' from tb_busiframe2 a,tb_object b';
        ...
        commandtext:=commandtext+' from tb_busiframe2 a,tb_staff b';
        ...
        commandtext:=commandtext+' from tb_busiframe2 a where a.comp_id='+inttostr(compid);
        if fdistltd then commandtext:=commandtext+' and exists (select 1 from tb_trustdistrict t where t.sta_id='+inttostr(curuserid)+' and dbo.fn_ischildnode(a.district_id,t.dist_id)=1)';
        ...
        commandtext:=commandtext+' from tb_busiframe2 a,tb_medicine m';
...
procedure Tsetbusiframe2.dxButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetbusiframe2.SpeedButton6Click(Sender: TObject);
var
    XL,Sheet: Variant;
...
{
            if active then close;
            commandtext:='if not exists (select 1 from tb_busiframe2 where comp_id='+inttostr(compid);
            ...
            commandtext:=commandtext+' insert into tb_busiframe2 (comp_id,level_id1,level_id2,level_id3,district_id,sta_id,med_id,valid_dt,creat_by,creat_dt)';
            ...
            if active then close;
            commandtext:='select top 1 rec_id from tb_busiframe2 where f1='+inttostr(id8)+' and med_id='+inttostr(id6);
            commandtext:=commandtext+' and district_id='+inttostr(id4);
            open;
            if recordcount=0 then
            begin
...
{
            if active then close;
            commandtext:='insert into tb_busiframe2_dtl (rela_id,level_id1,level_id2,level_id3,sta_id,med_id,district_id,valid_dt,creat_by,creat_dt)'; //comp_id,
...
procedure Tsetbusiframe2.SpeedButton5Click(Sender: TObject);
var mi: integer;
begin
{
with busiframe do
...
begin
    if active then close;
    commandtext:='update tb_busiframe2 set modify_by='+inttostr(curuserid)+',modify_dt=getdate()';
...
procedure Tsetbusiframe2.busiframeBeforeEdit(DataSet: TDataSet);
begin
//if busiframe.fieldbyname('creat_by').asinteger<>curuserid then raise Exception.Create('非本人设置的业务结构，不可修改');
end;

procedure Tsetbusiframe2.dxButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
{
with pickmanager do
...
procedure Tsetbusiframe2.dxButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
{
with picklevel3 do
...
procedure Tsetbusiframe2.dxButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
{
with picklevel2 do
...
procedure Tsetbusiframe2.N3Click(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure Tsetbusiframe2.N1Click(Sender: TObject);
begin
with dxdbgrid1 do Columns[GetAbsoluteColumnIndex(FocusedColumn)].Visible:=false;
end;

procedure Tsetbusiframe2.N2Click(Sender: TObject);
var i,mi: integer;
begin
setprogress(1);
with dxdbgrid1 do
...
procedure Tsetbusiframe2.N6Click(Sender: TObject);
begin
if FileExists(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini')
    then dxDBGrid1.LoadFromIniFile(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini');//加载
end;

procedure Tsetbusiframe2.N10Click(Sender: TObject);
...
begin
    if active then close;
    commandtext:='select top 1 1 from tb_busiframe2 where rec_id in ('+t+') and creat_by<>'+inttostr(curuserid);
    open;
    if recordcount>0 then
    begin
...
begin
    if active then close;
    commandtext:='delete from tb_busiframe2 where rec_id in ('+t+')';
    execute;
end;
SpeedButton3Click(nil);  //刷新
end;

procedure Tsetbusiframe2.dxDBGrid1Enter(Sender: TObject);
begin
wwDBNavigator1.datasource:=(sender as TdxDbgrid).datasource;
end;

procedure Tsetbusiframe2.dxDBGrid2level2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
{
with picklevel2 do
...
procedure Tsetbusiframe2.dxDBGrid2level3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
{
with picklevel3 do
...
procedure Tsetbusiframe2.dxDBGrid2leaderButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
{
with pickmanager do
...
procedure Tsetbusiframe2.MenuItem1Click(Sender: TObject);
begin
{
with busiframe_dtl do
begin
...
procedure Tsetbusiframe2.MenuItem2Click(Sender: TObject);
//var mi: integer;
begin
{
with busiframe_dtl do
...
begin
    if active then close;
    commandtext:='insert into tb_busiframe2_dtl (rela_id,level_id1,level_id2,level_id3,sta_id,district_id,med_id,valid_dt,creat_by,creat_dt)';
    ...
    commandtext:=commandtext+'   from tb_busiframe2_dtl where rec_id='+inttostr(frecid);
    commandtext:=commandtext+' select @@identity';
    try
        setprogress(1);
        open;
        ...
        //因trigger同步主表tb_busiframe2,故刷新busiframe
    finally
        setprogress(0);
    end;
end;
...
procedure Tsetbusiframe2.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetbusiframe2.busiframeCalcFields(DataSet: TDataSet);
begin
with busiframe do
begin
...
procedure Tsetbusiframe2.dxDBGrid1brokerButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickstaff do
begin
