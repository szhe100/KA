...
var
  setmedexport6: Tsetmedexport6;
  ptype: string[20];
  pcent: real;

implementation

uses datamodu, Unit14, Unit18, Unit16, Unit16c; //Unit16a, 

{$R *.DFM}

procedure Tsetmedexport6.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetmedexport6.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetmedexport6.FormActivate(Sender: TObject);
begin
if cdsbill.active=false then SpeedButton5Click(nil);
end;

procedure Tsetmedexport6.FormCloseQuery(Sender: TObject;
...
procedure Tsetmedexport6.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetmedexport6.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedexport6.dxDBGrid2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
...
procedure Tsetmedexport6.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
exit;
...
procedure Tsetmedexport6.dxDBGrid2Enter(Sender: TObject);
begin
...
procedure Tsetmedexport6.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetmedexport6.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetmedexport6.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetmedexport6.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=2 and a.cate_id=5';   // a.comp_id='+inttostr(compid)+' and
...
procedure Tsetmedexport6.FormCreate(Sender: TObject);
begin
...
procedure Tsetmedexport6.SpeedButton6Click(Sender: TObject);
var mid : integer;
begin
...
begin
    ...
    commandtext:=commandtext+' values ('+inttostr(compid)+',2,0,5,1,'+inttostr(curuserid)+',getdate(),cast(convert(char(10),getdate(),20) as datetime))';
...
procedure Tsetmedexport6.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetmedexport6.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetmedexport6.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
procedure Tsetmedexport6.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedexport6.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedexport6.SpeedButton2Click(Sender: TObject);
begin
with cdsbill do
begin
...
procedure Tsetmedexport6.SpeedButton4Click(Sender: TObject);
begin
...
procedure Tsetmedexport6.SpeedButton3Click(Sender: TObject);
var s: string;
begin
with dm.pubqry do
begin
...
procedure Tsetmedexport6.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedexport6.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedexport6.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetmedexport6.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
    then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
...
procedure Tsetmedexport6.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetmedexport6.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedexport6.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetmedexport6.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetmedexport6.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedexport6.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetmedexport6.dxDBGrid2DblClick(Sender: TObject);
var key:char;
begin
exit;
key:=#32;
...
procedure Tsetmedexport6.Edit2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
    fwid: boolean; //  东方语言 宽字符 (2bytes)
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                fmateid:=pickcustomer.query1.FieldByName('mate_id').asinteger;
//                fmatetypeid:=pickcustomer.query1.FieldByName('mate_type_id').asinteger;
            end;
        end;
    end;
end;
...
procedure Tsetmedexport6.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
    SFileName,sbodcd,sb: string;
    J,mi,mid,id1,id2,id3,id4,id10,id11,qty: integer;
    rate: single;
    dt: TDatetime;
begin
    ...
    sbodcd:='';
    j:=2; mi:=0;  //第2行开始 每行一单
//格式:1发生日期dt, 2医院id2, 3本公司产品编码id3, 4数量, 5批号, 6单据摘要
//格式:1发生日期dt、2配送企业、3医院名称、4产品编码、5采购数量、6配送数量、7入库数量、8批号
    while (sheet.cells[j,1].text<>'') and (mi<10) do
    begin
        id1:=0;id2:=0;id3:=0;id4:=0;id10:=0;
...
{       配送企业 非本系统内商业公司,不需检查
            ...
            begin
                ...
                open;
                if recordcount>0 then id2:=fieldbyname('mate_id').asinteger else id2:=0; //供货公司(商业公司)
            end;
}
            if sheet.cells[j,3].text<>'' then
            begin
                if active=true then close;
                commandtext:='select top 1 mate_id,district from tb_busimate where mate_type_id=1 and isnull(stop_yn,0)=0 and mate_name ='''+Trim(sheet.cells[j,3].text)+'''';
                open;
                if recordcount>0 then
                begin
                    id3:=fieldbyname('mate_id').asinteger;
                    id10:=fieldbyname('district').asinteger;
                end 
                else
                begin
                    if active=True then close;
                    ...
                    commandtext:=commandtext+' and zdesc='''+Trim(sheet.cells[j,3].text)+'''';
                    open;
                    if recordcount>0 then
                    begin
                        id3:=fieldbyname('mate_id').asinteger; //客户/医院
                        id10:=fieldbyname('district').asinteger;
                    end;
                end;
            end;
...
{
            if active=true then close;
            commandtext:='select top 1 sta_id from tb_busiframe where district_id='+inttostr(id10)+' and med_id='+inttostr(id4);
...
//格式:1发生日期dt、2配送企业、3医院名称、4产品编码、5采购数量、6配送数量、7入库数量、8批号
            ...
            if (id3=0) or (id4=0) or (length(sheet.cells[j,2].text)>100) then
            begin
                sbodcd:=sbodcd+#13#10+'第'+inttostr(j)+'行:';
//                if id1=0 then sbodcd:=sbodcd+',未取得区域经理';
//                if id2=0 then sbodcd:=sbodcd+',未取得配送企业';
                if id3=0 then sbodcd:=sbodcd+',未取得医院或已停用';
                if id4=0 then sbodcd:=sbodcd+',未取得药品';
//                if sheet.cells[j,4].text='' then sbodcd:=sbodcd+',无数量';
                if length(sheet.cells[j,2].text)>100 then sbodcd:=sbodcd+',配送企业过长(应小于100字符)';
                mi:=mi+1;
            end;
        end;
        inc(j);
...
{       配送企业 非本系统内商业公司,不需检查
            ...
            begin
                ...
                begin
                    if active=True then close;
                    commandtext:='select top 1 b.mate_id,b.district from tb_linkword a,tb_busimate b';
                    commandtext:=commandtext+' where a.type_id=2 and a.mate_id=b.mate_id and zdesc='''+Trim(sheet.cells[j,3].text)+'''';
                    open;
                    if recordcount>0 then
                    begin
                        id3:=fieldbyname('mate_id').asinteger; //客户/医院
...
//格式:1发生日期dt、2配送企业、3医院名称、4产品编码、5采购数量、6配送数量、7入库数量、8批号
{
            ...
            if strtoint(cleardeli(sheet.cells[j,7].text))<0 then id11:=2 else id11:=1; //id11=1 销售，id11=2 退货

            sb:=formatfloat('00000000',getbillid(compid,2));
            sbodcd:=sbodcd+#13#10+sb;

            if active=true then close;
            commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,cate_id,busi_type,creat_by,creat_dt,bod_desc1,src_id,dst_id,broker_id,district_id,carry_dt)';
            commandtext:=commandtext+' values ('+inttostr(compid)+','''+sb+''',2,0,5,'+inttostr(id11)+','+inttostr(curuserid)+',getdate(),';
            commandtext:=commandtext+' '''+sheet.cells[j,2].text+''',0,'+inttostr(id3)+',0,'+inttostr(id10)+','''+datetimetostr(dt)+''')';
//            commandtext:=commandtext+' 0,'+inttostr(id3)+','+inttostr(id1)+','+inttostr(id10)+','''+datetimetostr(dt)+''')';
            commandtext:=commandtext+' select @@identity';
            open;
            mid:=fields[0].asinteger;
//格式:1发生日期dt、2配送企业、3医院名称、4产品编码、5采购数量、6配送数量、7入库数量、8批号
            if active=true then close;
            commandtext:='insert into tb_bill_dtl (bod_id,med_id,qty,qty1,qty2,sta_id,bat_cd)';   //,valid_dt
            commandtext:=commandtext+' values ('+inttostr(mid)+','+inttostr(id4)+',';
{
            try
                qty:=strtoint(sheet.cells[j,7].text);
            except
                qty:=0;
            end;
            commandtext:=commandtext+cleardeli(sheet.cells[j,7].text); //inttostr(qty)+',';
            try
                qty:=strtoint(sheet.cells[j,5].text);
            except
                qty:=0;
            end;
            commandtext:=commandtext+inttostr(qty)+',';
            try
                qty:=strtoint(sheet.cells[j,6].text);
            except
                qty:=0;
            end;
            commandtext:=commandtext+inttostr(qty)+',';
            ...
            commandtext:=commandtext+inttostr(id1)+',';  //区域经理
...
procedure Tsetmedexport6.N1Click(Sender: TObject);
...
procedure Tsetmedexport6.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetmedexport6.N3Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t : string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetmedexport6.dxDBButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetmedexport6.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetmedexport6.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetmedexport6.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tsetmedexport6.Button1Click(Sender: TObject);
begin
...
procedure Tsetmedexport6.N5Click(Sender: TObject);
begin
if pubqry.active=true then pubqry.close;
pubqry.commandtext:='select bod_id from tb_bill where bod_type_id=2 and isnull(mate_id1,0)=0 and isnull(mate_id2,0)=0 and isnull(mate_id3,0)=0 and isnull(mate_id4,0)=0 and isnull(mate_id5,0)=0';
pubqry.open;
...
procedure Tsetmedexport6.dxEdit1Click(Sender: TObject);
begin
dxEdit1.SelectAll;
dxEdit1.SetFocus;
end;

procedure Tsetmedexport6.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetmedexport6.Edit2Click(Sender: TObject);
begin
