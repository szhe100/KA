...
type
  Tsetbrokermedtl = class(TForm)
    ...
    brokermedLpricetype: TStringField;
    Edit1: TEdit;
    Label16: TLabel;
    Label17: TLabel;
    Label18: TLabel;
    ...
    dxDBGrid1Lpricetype: TdxDBGridLookupColumn;
    ...
    dxDBGrid3stop_yn: TdxDBGridCheckColumn;
    dxDBGrid3stop_dt: TdxDBGridColumn;
    ...
    dxDBGrid3trusted: TdxDBGridCheckColumn;
    ...
    dxDBGrid3Lpricetype: TdxDBGridColumn;
    ...
    brokermedLtype2: TStringField;
    qrytype_id2: TIntegerField;
    qryctype2: TStringField;
    brokermedctype2: TStringField;
    dxDBGrid3ctype2: TdxDBGridColumn;
...
uses datamodu, Unit7, Unit28, Unit28a, Unit16;
...
begin
    ...
    begin
        ...
        commandtext:='select a.*,leader=dbo.fn_staff_name(a.lead_id),leader1=dbo.fn_staff_name(k.sta_id),b.med_code,b.med_name,b.material_code,b.specifi,b.pdt_place,med_unit=c.zdesc,b.qtyperpack,b.qtyperbox,';
        ...
        commandtext:=commandtext+' b.chm_name,med_type=dbo.fn_med_type(b.med_id),broker=d.zname,e.mate_name,e.trusted,creater=g.zname,e.district,dist_name=dbo.fn_getdistrict(e.district),';
        commandtext:=commandtext+' f1=dbo.fn_getsysrulef1(a.mate_id,a.med_id,a.valid_dt),f2=dbo.fn_getsysrulef1a(a.mate_id,a.med_id)';
        commandtext:=commandtext+' ,mate_name1=h.mate_name,a.channel_id,channel=dbo.fn_obj_desc(a.channel_id),a.channel_dtl_id,channel_dtl=dbo.fn_obj_desc(a.channel_dtl_id)';
...
//                commandtext:=commandtext+' and dbo.fn_treeischild(e.district,'+dm.district.fieldbyname('rec_id').asstring+')>0 '
        		commandtext:=commandtext+' and e.district in (select rec_id from tb_treedata where rec_id='+dm.district.fieldbyname('rec_id').asstring+' or parent= '+dm.district.fieldbyname('rec_id').asstring+')';
            end;
        end;
...
begin
    ...
    begin
        ...
        commandtext:='select h.rate,a.*,leader=dbo.fn_staff_name(a.lead_id),leader1=dbo.fn_staff_name(k.sta_id),b.med_code,b.med_name,b.specifi,b.pdt_place,med_unit=c.zdesc,b.qtyperpack,b.qtyperbox,';
        ...
        commandtext:=commandtext+' sta_id1=h.sta_id,broker1=dbo.fn_staff_name(h.sta_id),h.fee_type_id,feetype=dbo.fn_obj_desc(h.fee_type_id),rate1=h.rate';
        commandtext:=commandtext+' from tb_brokermed a';
        commandtext:=commandtext+' inner join tb_medicine b on a.med_id=b.med_id';
        commandtext:=commandtext+' left join tb_object c on b.unit_id=c.obj_id';
        commandtext:=commandtext+' left join tb_staff d on a.sta_id=d.sta_id';
        commandtext:=commandtext+' left join tb_busimate e on a.mate_id=e.mate_id';
        commandtext:=commandtext+' left join tb_staff g on a.creat_by=g.sta_id';
        commandtext:=commandtext+' inner join tb_brokermedtl h on a.rec_id=h.rela_id';
...
begin
    ...
    if fieldbyname('stop_yn').asboolean=True then MessageBox(0,pchar('该医院为托管医院'),'请注意',MB_OK+MB_ICONInformation);
{
...
//    commandtext:='select dbo.fn_getbidprice('+inttostr(dxbuttonedit2.tag)+','+inttostr(dxbuttonedit1.tag)+')';
    ...
    if active then close;
    commandtext:='insert into tb_brokermed (valid,comp_id,sta_id,mate_id,med_id,price1,price_type_id,creat_by,creat_dt) values'; // lead_id, price1:中标价
    commandtext:=commandtext+' (0,'+inttostr(compid)+','+inttostr(dxbuttonedit4.tag)+',';  //+inttostr(dxbuttonedit3.tag)+','
    commandtext:=commandtext +inttostr(dxbuttonedit2.tag)+','+inttostr(dxbuttonedit1.tag)+',';
...
//edit5.Text:=commandtext;
    ...
    begin
        first;
        s:='下列医院所在区域启用日期年月已结账，不可申请'+ #13#10;
        s:=s+'-----------------------------------------------------';  //+ #13#10
...
//edit1.text:=commandtext;
    ...
    commandtext:=commandtext+' from tb_brokermed where rec_id in ('+t+')';
    execute;
    if active then close;
    setprogress(0);
...
procedure Tsetbrokermedtl.N9Click(Sender: TObject);
var i,mi,l,m,n,k,o,p,q: integer;
    s,t: string;
    mcd: string[20];
begin
if (brokermed.active=false) or (brokermed.recordcount=0) then exit;
...
for i:= 0 to dxDBGrid1.SelectedCount - 1 do
begin
//    if dxDBGrid1.SelectedNodes[i].Values[l]=0 then raise Exception.Create('所选记录中有未设置费用记录，不可启用');
//    if dxDBGrid1.SelectedNodes[i].Values[k]<>curuserid then raise Exception.Create('所选记录中有非本人设置规则，不可启用');
    if dxDBGrid1.SelectedNodes[i].Values[m]=False then raise Exception.Create('所选记录中有未启用规则，不可停用');
    s:= s + #13#10+ dxDBGrid1.SelectedNodes[i].Strings[o]+' '+dxDBGrid1.SelectedNodes[i].Strings[p]+' '+dxDBGrid1.SelectedNodes[i].Strings[q];
    t:= t +','+ dxDBGrid1.SelectedNodes[i].Strings[n];  // 记录 rec_id 字符串
    mi := mi+1;
end;
...
begin
    if active then close;
    commandtext:='select top 1 bod_cd from tb_bill a,tb_bill_dtl b where a.bod_type_id=21 and bod_status_id<>1';
    commandtext:=commandtext+' and a.bod_id=b.bod_id and b.med_id in ('+t+')';
    open;
    if recordcount>0 then raise Exception.Create('所选规则已生成'+fieldbyname('bod_cd').asstring+'号停用单尚未完成审批,不可再次生成');
end;

if MessageBox(0,Pchar('确定由下列规则生成停用单:'+ #13#10+s),'请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;

mcd:=formatfloat('00000000',getbillid(0,21)); //bod_type_id=21: 业务员规则停用单
with dm.pubqry do
begin
    setprogress(1);
    if active then close;
    ...
    commandtext:=commandtext+' values ('+inttostr(compid)+','''+mcd+''',21,0,'+inttostr(curuserid)+',getdate())';  //cast(convert(char(10),getdate(),20) as datetime),
    commandtext:=commandtext+' select @@identity';
//edit1.text:=commandtext;
    open;
    mi:=fields[0].asinteger;
    close;
    commandtext:='insert into tb_bill_dtl (bod_id,med_id,price,price1,valid_dt)'; //
    commandtext:=commandtext+' select '+inttostr(mi)+',rec_id,rate,price1,valid_dt=DATEADD(mm, DATEDIFF(mm,0,getdate()), -1)'; //,valid_dt
    commandtext:=commandtext+' from tb_brokermed where rec_id in ('+t+')';
    execute;
    if active then close;
    setprogress(0);
    s:='已成功生成'+mcd+'号业务员规则停用单';
    MessageBox(0,pchar(s),'请注意',MB_OK+MB_ICONInformation);
end;
end;
...
begin
if brokermed.fieldbyname('valid').asboolean=True then raise Exception.Create('本规则已启用，不可更改');
end;
...
procedure Tsetbrokermedtl.N11Click(Sender: TObject);
var i,mi,l,m,n,k,o,p,q,i1,i2,i3,i4,i5,m1,m2,m3: integer;
    s,t: string;
    mcd: string[20];
begin
if (brokermed.active=false) or (brokermed.recordcount=0) then exit;
//  生成申请单时，应限制同设置人、同品种、同地区(地区不可包含)
s:='确定由下列规则生成调整单(中标价按所选类别调整):'+ #13#10+'------------------------------------';
t:='';
mi:=0;
l := dxDBGrid1.ColumnByFieldName('rate').Index;
k := dxDBGrid1.ColumnByFieldName('creat_by').Index;
...
begin
    if (dxDBGrid1.SelectedNodes[i].Values[i3]>0) and (dxDBGrid1.SelectedNodes[i].Values[i4]+dxDBGrid1.SelectedNodes[i].Values[i5]>dxDBGrid1.SelectedNodes[i].Values[i3]) then
    begin
        if isaccounter(curuserid)=False then raise Exception.Create('所选记录中有费用与托管费之和超过费用政策，不可调整');
    end;
    if i=0 then
    begin
        m1:=dxDBGrid1.SelectedNodes[i].Values[i1];
//        m2:=dxDBGrid1.SelectedNodes[i].Values[i2];
    ...
    begin
        if m1<>dxDBGrid1.SelectedNodes[i].Values[i1] then raise Exception.Create('同一调整单中，品种应相同');
        if m2<>dxDBGrid1.SelectedNodes[i].Values[i2] then raise Exception.Create('同一调整单中，区域应相同');
    end;
//    if (isaccounter(curuserid)=False) and (dxDBGrid1.SelectedNodes[i].Values[k]<>curuserid) then raise Exception.Create('所选记录中有非本人设置规则，不可启用');
    if dxDBGrid1.SelectedNodes[i].Values[m]=False then raise Exception.Create('所选记录中有未启用规则，不需调整');
    s:= s + #13#10+ dxDBGrid1.SelectedNodes[i].Strings[o]+' '+dxDBGrid1.SelectedNodes[i].Strings[p]+' '+dxDBGrid1.SelectedNodes[i].Strings[q];
    t:= t +','+ dxDBGrid1.SelectedNodes[i].Strings[n];  // 记录 rec_id 字符串
    mi := mi+1;
end;
...
begin
    if active then close;
    commandtext:='select top 1 bod_cd from tb_bill a,tb_bill_dtl b where a.bod_type_id=23 and bod_status_id<>1';
    commandtext:=commandtext+' and a.bod_id=b.bod_id and b.med_id in ('+t+')';
    open;
    if recordcount>0 then raise Exception.Create('所选规则已生成'+fieldbyname('bod_cd').asstring+'号调整单尚未完成审批,不可再次生成');
end;

if MessageBox(0,Pchar(s),'请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;

mcd:=formatfloat('00000000',getbillid(0,23)); //bod_type_id=23: 业务员费用调整申请单
with dm.pubqry do
begin
    setprogress(1);
    if active then close;

    commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,busi_type,creat_by,creat_dt)';
    commandtext:=commandtext+' values ('+inttostr(compid)+','''+mcd+''',23,0,'+inttostr(RadioGroup2.ItemIndex)+','+inttostr(curuserid)+',getdate())';  //cast(convert(char(10),getdate(),20) as datetime),
    commandtext:=commandtext+' select @@identity';
//edit1.text:=commandtext;
    open;
    mi:=fields[0].asinteger;
    close;
    commandtext:='insert into tb_bill_dtl (bod_id,med_id,lead_id,sta_id,price,cost,price1,valid_dt)';
    commandtext:=commandtext+' select '+inttostr(mi)+',rec_id,lead_id,sta_id,rate,'; //price1,price1,valid_dt';
    case RadioGroup2.ItemIndex of
        0:  commandtext:=commandtext+' price1,price1,';
        1:  commandtext:=commandtext+' price4,price4,';
        2:  commandtext:=commandtext+' price5,price5,';
        3:  commandtext:=commandtext+' price6,price6,';
        4:  commandtext:=commandtext+' price7,price7,';
    end;
    commandtext:=commandtext+' valid_dt from tb_brokermed where rec_id in ('+t+')';
    execute;
    if active then close;
    setprogress(0);
    s:='已成功生成'+mcd+'号业务员费用调整单';
    MessageBox(0,pchar(s),'请注意',MB_OK+MB_ICONInformation);
end;
end;
...
begin
    case RadioGroup2.ItemIndex of
        ...
        1:  begin
                ...
                if (fieldbyname('price4').asfloat=0)
                    then fieldbyname('crate1').asfloat:=0
                    else fieldbyname('crate1').asfloat:=100*fieldbyname('rate1').asfloat/fieldbyname('price4').asfloat;
            end;
        2:  begin
                if (fieldbyname('price5').asfloat=0)
                    then fieldbyname('crate').asfloat:=0
                ...
                if (fieldbyname('price5').asfloat=0)
                    then fieldbyname('crate1').asfloat:=0
                    else fieldbyname('crate1').asfloat:=100*fieldbyname('rate1').asfloat/fieldbyname('price5').asfloat;
            end;
        3:  begin
                if (fieldbyname('price6').asfloat=0)
                    then fieldbyname('crate').asfloat:=0
                ...
                if (fieldbyname('price6').asfloat=0)
                    then fieldbyname('crate1').asfloat:=0
                    else fieldbyname('crate1').asfloat:=100*fieldbyname('rate1').asfloat/fieldbyname('price6').asfloat;
            end;
        4:  begin
                if (fieldbyname('price7').asfloat=0)
                    then fieldbyname('crate').asfloat:=0
                ...
                if (fieldbyname('price7').asfloat=0)
                    then fieldbyname('crate1').asfloat:=0
                    else fieldbyname('crate1').asfloat:=100*fieldbyname('rate1').asfloat/fieldbyname('price7').asfloat;
            end;
    end;
...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1申请摘要, 2渠道, 3子渠道, 4业务员, 5商业公司, 6医院, 7药品编码, 8明细业务员, 9费用金额, 10中标价, 11医院托管结算率, 12启用日期
    mi:=0;j:=2;  //第1行开始 每行一单
    ...
    sql:=sql+' channel_id int,k.channel_dtl_id int,sta_id int,sta_id1 int,mate_id int,mate_id1 int,med_id int)';
...
//从第2行开始导入，格式: 1申请摘要, 2渠道, 3子渠道, 4业务员, 5商业公司, 6医院, 7药品编码, 8明细业务员, 9费用金额, 10中标价, 11医院托管结算率, 12启用日期
    sql:=sql+'select top 5 * from ( select top 5 info=''第''+line_no+''行无渠道'' from @tab a where f2='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.zname=a.f2)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无子渠道'' from @tab where f3='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.zname=a.f3)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无业务员或数据无效'' from @tab a where f4='''' or not exists (select 1 from tb_staff b where sta_type_id=1 and b.zname =a.f4)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无商业公司或数据无效'' from @tab a where f5='''' or not exists (select 1 from tb_busimate b where mate_type_id=2 and b.mate_name =a.f5)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无医院或数据无效'' from @tab a where f6='''' or not exists (select 1 from tb_busimate b where mate_type_id=1 and b.mate_name =a.f6)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无药品编码或数据无效'' from @tab a where f7='''' or not exists (select 1 from tb_medicine m where m.med_name =a.f7)';
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行无费用金额或数据无效'' from @tab a where f9='''' or try_cast(f9 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无中标价或数据无效'' from @tab where f10='''' or try_cast(f10 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无医院托管结算率或数据无效'' from @tab where f11='''' or try_cast(f11 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无启用日期或数据无效'' from @tab where f12='''' or try_cast(f2 as datetime) is null';
    sql:=sql+' ) a';
//从第2行开始导入，格式: 1申请摘要, 2渠道, 3子渠道, 4业务员, 5商业公司, 6医院, 7药品编码, 8明细业务员, 9费用金额, 10中标价, 11医院托管结算率, 12启用日期
    with dm.pubqry do
    begin
        ...
        sql:=sql+' update a set channel_id=b.obj_id,channel_dtl_id=c.obj_id,sta_id=d.sta_id,sta_id1=g.sta_id, mate_id1=e.mate_id,mate_id=f.mate_id,med_id=m.med_id';
        sql:=sql+' from @tab a';
        sql:=sql+' left join tb_object b on b.obj_type_id=11 and b.zdesc=a.f2';
        sql:=sql+' left join tb_object c on b.obj_type_id=12 and c.zdesc=a.f3';
        sql:=sql+' left join tb_staff d on d.sta_type_id=1 and d.zname =a.f4';
        sql:=sql+' left join tb_staff g on g.sta_type_id=1 and d.zname =a.f8';
        sql:=sql+' left join tb_busimate e on e.mate_type_id=2 and e.mate_name =a.f5';
        sql:=sql+' left join tb_busimate f on d.mate_type_id=1 and f.mate_name =a.f6';
        sql:=sql+' left join tb_medicine m on m.med_name =a.f7';

//从第2行开始导入，格式: 1申请摘要, 2渠道, 3子渠道, 4业务员, 5商业公司, 6医院, 7药品编码, 8明细业务员, 9费用金额, 10中标价, 11医院托管结算率, 12启用日期
// 以下为 无主要数据 的 insert into tb_brokermed
        sql:=sql+' insert into tb_brokermed (comp_id,bod_desc,channel_id,channel_dtl_id,sta_id,mate_id1,mate_id,med_id,rate,price1,f1,valid_dt,creat_by,creat_dt)';
        sql:=sql+'  select comp_id='+inttostr(compid)+',bod_desc=f1,channel_id=a.channel_id,channel_dtl_id=a.channel_dtl_id,a.sta_id,';
        sql:=sql+'      mate_id1=a.mate_id1,a.mate_id,a.med_id,rate=cast(f9 as decimal(15,4)),price1=cast(f10 as decimal(15,4)),f1=cast(f11 as decimal(15,4)),valid_dt=cast(f12 as datetime),'; //,price1,price_type_id,type_id2,valid_dt,creat_by,creat_dt    //type_id=4,channel_id=b.obj_id,channel_dtl_id=c.obj_id,district_id=d.rec_id,'; //type_id=4 设置分销标准政策
        sql:=sql+'      creat_by='+inttostr(curuserid)+',creat_dt=getdate()';
        sql:=sql+' from @tab a';
{
        sql:=sql+'  select comp_id='+inttostr(compid)+',bod_desc=f1,channel_id=b.obj_id,channel_dtl_id=c.obj_id,d.sta_id,';
        sql:=sql+'      mate_id1=e.mate_id,f.mate_id,m.med_id,rate=cast(f9 as decimal(15,4)),price1=cast(f10 as decimal(15,4)),f1=cast(f11 as decimal(15,4)),valid_dt=cast(f12 as datetime),'; //,price1,price_type_id,type_id2,valid_dt,creat_by,creat_dt    //type_id=4,channel_id=b.obj_id,channel_dtl_id=c.obj_id,district_id=d.rec_id,'; //type_id=4 设置分销标准政策
        sql:=sql+'      creat_by='+inttostr(curuserid)+',creat_dt=getdate()';
        sql:=sql+' from @tab a';
        sql:=sql+' left join tb_object b on b.obj_type_id=11 and b.zdesc=a.f2';
        sql:=sql+' left join tb_object c on b.obj_type_id=12 and c.zdesc=a.f3';
        ...
        sql:=sql+' left join tb_busimate f on d.mate_type_id=1 and f.mate_name =a.f6';
        sql:=sql+' left join tb_medicine m on m.med_name =a.f7';
        sql:=sql+' where not exists (select 1 from tb_brokermed k where k.channel_id=a.obj_id and k.channel_dtl_id=c.obj_id and k.sta_id=d.sta_id and k.mate_id1=e.mate_id and k.mate_id=f.mate_id and k.med_id=m.med_id and k.valid_dt=cast(f12 as datetime)';
}
        sql:=sql+' where not exists (select 1 from tb_brokermed k where k.channel_id=a.channel_id and k.channel_dtl_id=a.channel_dtl_id and k.sta_id=a.sta_id and k.mate_id1=a.mate_id1 and k.mate_id=a.mate_id and k.med_id=a.med_id and k.valid_dt=cast(f12 as datetime))';

// 以下为 有主要数据 的 update tb_brokermed 价格类数据
        sql:=sql+' update set rate=cast(f9 as decimal(15,4)),price1=cast(f10 as decimal(15,4)),f1=cast(f11 as decimal(15,4))';
        sql:=sql+' from @tab a inner join tb_brokermed k on k.channel_id=a.channel_id and k.channel_dtl_id=a.channel_dtl_id and k.sta_id=a.sta_id and k.mate_id1=a.mate_id1 and k.mate_id=a.mate_id and k.med_id=a.med_id and k.valid_dt=cast(f12 as datetime)';
{
        sql:=sql+' update a set rate=cast(f9 as decimal(15,4)),price1=cast(f10 as decimal(15,4)),f1=cast(f11 as decimal(15,4))';
        sql:=sql+' from @tab a';
        sql:=sql+' left join tb_object b on b.obj_type_id=11 and b.zdesc=a.f2';
        sql:=sql+' left join tb_object c on b.obj_type_id=12 and c.zdesc=a.f3';
        sql:=sql+' left join tb_staff d on d.sta_type_id=1 and d.zname =a.f4';
        ...
        sql:=sql+' left join tb_busimate f on d.mate_type_id=1 and f.mate_name =a.f6';
        sql:=sql+' left join tb_medicine m on m.med_name =a.f7';
        sql:=sql+' where exists (select 1 from tb_brokermed k where k.channel_id=b.obj_id and k.channel_dtl_id=c.obj_id and k.sta_id=d.sta_id and k.mate_id1=e.mate_id and k.mate_id=f.mate_id and k.med_id=m.med_id and k.valid_dt=cast(f12 as datetime)';
}

// 以下为 无明细数据 的 insert tb_brokermedtl
        sql:=sql+' insert into tb_brokermedtl (rela_id,sta_id,fee_type_id,rate,creat_by,creat_dt)';
        sql:=sql+' select rela_id=k.rec_id,sta_id=k.sta_id1,fee_type_id=185,rate=cast(f9 as decimal(15,4)),creat_by='+inttostr(curuserid)+',creat_dt=getdate() from @tab a';
        sql:=sql+' inner join tb_brokermed k on k.channel_id=a.channel_id and k.channel_dtl_id=a.channel_dtl_id and k.sta_id=a.sta_id and k.mate_id1=a.mate_id1 and k.mate_id=a.mate_id and k.med_id=a.med_id and k.valid_dt=cast(f12 as datetime)';
        sql:=sql+'  where not exists (select 1 from tb_brokermedtl b where k.rec_id=b.rela_id and k.sta_id1=b.sta_id';

// 以下为 有明细数据 的 update tb_brokermedtl 价格类数据
        sql:=sql+' update b set rate=cast(f9 as decimal(15,4)),modify_by='+inttostr(curuserid)+',modiby_dt=getdate()';
        sql:=sql+' select * from @tab a ';
        sql:=sql+' inner join tb_brokermed k on k.channel_id=a.channel_id and k.channel_dtl_id=a.channel_dtl_id and k.sta_id=a.sta_id and k.mate_id1=a.mate_id1 and k.mate_id=a.mate_id and k.med_id=a.med_id and k.valid_dt=cast(f12 as datetime)';
        sql:=sql+' inner join tb_brokermedtl b on a.rec_id=b.rela_id';
