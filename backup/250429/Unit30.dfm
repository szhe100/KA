object setreport05: Tsetreport05
  Left = 178
  Top = 122
  AutoScroll = False
  Caption = #25237#36164#27454#27719#24635#34920
  ClientHeight = 514
  ClientWidth = 980
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  DesignSize = (
    980
    514)
  PixelsPerInch = 96
  TextHeight = 12
  object Shape3: TShape
    Left = 489
    Top = 2
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
    Visible = False
  end
  object Shape2: TShape
    Left = 2
    Top = 48
    Width = 73
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
    Visible = False
  end
  object Shape6: TShape
    Left = 2
    Top = 25
    Width = 73
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label25: TLabel
    Left = 959
    Top = 9
    Width = 7
    Height = 12
    Alignment = taRightJustify
    Anchors = [akTop, akRight]
    Font.Charset = GB2312_CHARSET
    Font.Color = clMaroon
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = [fsBold]
    ParentFont = False
  end
  object SpeedButton2: TSpeedButton
    Left = 798
    Top = 2
    Width = 25
    Height = 25
    Hint = #36716#20986#25253#34920
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton2Click
  end
  object Shape4: TShape
    Left = 2
    Top = 2
    Width = 73
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label3: TLabel
    Left = 14
    Top = 8
    Width = 49
    Height = 14
    AutoSize = False
    Caption = #21457#29983#26085#26399
    Transparent = True
    WordWrap = True
  end
  object Label1: TLabel
    Left = 176
    Top = 8
    Width = 12
    Height = 12
    Caption = #33267
  end
  object SpeedButton5: TSpeedButton
    Left = 746
    Top = 45
    Width = 227
    Height = 25
    Hint = #21047#26032
    Anchors = [akTop, akRight]
    Caption = '&R '#21047#26032
    Glyph.Data = {
      
      3393333777333777337333333999993333333333377777333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Spacing = 10
    OnClick = SpeedButton5Click
  end
  object SpeedButton1: TSpeedButton
    Left = 772
    Top = 2
    Width = 25
    Height = 25
    Hint = #25171#21360#36755#20986#27719#24635#34920
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      33333337FFFF7733333333300000033333333337777773333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    OnClick = SpeedButton1Click
  end
  object SpeedButton3: TSpeedButton
    Left = 747
    Top = 2
    Width = 25
    Height = 25
    Hint = #25171#21360#36755#20986#31616#34920
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      33333337FFFF7733333333300000033333333337777773333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    OnClick = SpeedButton3Click
  end
  object Shape1: TShape
    Left = 168
    Top = 25
    Width = 46
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label2: TLabel
    Left = 8
    Top = 30
    Width = 60
    Height = 12
    Caption = #19994#21153#21592#22320#21306
    Transparent = True
  end
  object Label4: TLabel
    Left = 20
    Top = 54
    Width = 36
    Height = 12
    Caption = #21697'  '#31181
    Transparent = True
    Visible = False
  end
  object Label5: TLabel
    Left = 496
    Top = 8
    Width = 48
    Height = 12
    Caption = #32463#33829#31867#21035
    Transparent = True
    Visible = False
  end
  object Label7: TLabel
    Left = 174
    Top = 30
    Width = 36
    Height = 12
    Caption = #19994#21153#21592
    Transparent = True
  end
  object dxDateEdit1: TdxDateEdit
    Left = 76
    Top = 4
    Width = 90
    Style.BorderStyle = xbsSingle
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 1
    StyleController = StyleController
    Date = -700000.000000000000000000
  end
  object dxDateEdit2: TdxDateEdit
    Left = 196
    Top = 4
    Width = 90
    Style.BorderStyle = xbsSingle
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 2
    StyleController = StyleController
    Date = -700000.000000000000000000
  end
  object wwDBNavigator2: TwwDBNavigator
    Left = 823
    Top = 2
    Width = 150
    Height = 25
    DataSource = DSqry
    Options = []
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    Anchors = [akTop, akRight]
    ParentShowHint = False
    object wwNavButton1: TwwNavButton
      Left = 0
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 0
      Style = nbsFirst
    end
    object wwNavButton2: TwwNavButton
      Left = 25
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Prior'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 1
      Style = nbsPrior
    end
    object wwNavButton3: TwwNavButton
      Left = 50
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Next'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 2
      Style = nbsNext
    end
    object wwNavButton4: TwwNavButton
      Left = 75
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 3
      Style = nbsLast
    end
    object wwNavButton5: TwwNavButton
      Left = 100
      Top = 0
      Width = 25
      Height = 25
      Hint = #35774#32622#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1SaveBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 4
      Style = nbsSaveBookmark
    end
    object wwNavButton6: TwwNavButton
      Left = 125
      Top = 0
      Width = 25
      Height = 25
      Hint = #36820#22238#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1RestoreBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 5
      Style = nbsRestoreBookmark
    end
  end
  object dxButtonEdit4: TdxButtonEdit
    Left = 215
    Top = 26
    Width = 71
    Style.BorderStyle = xbsSingle
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 0
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit4ButtonClick
    ExistButtons = True
  end
  object dxLookupEdit1: TdxLookupEdit
    Left = 550
    Top = 3
    Width = 74
    Font.Charset = GB2312_CHARSET
    Font.Color = clMaroon
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Style.BorderStyle = xbsSingle
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 4
    Visible = False
    StyleController = StyleController
    DropDownRows = 20
    ClearKey = 46
    ListFieldName = 'dist_name'
    CanDeleteText = True
    KeyFieldName = 'dist_id'
    ListSource = DM.DSedistrict
    LookupKeyValue = Null
  end
  object Edit1: TEdit
    Left = 603
    Top = 43
    Width = 121
    Height = 20
    TabOrder = 5
    Text = 'Edit1'
    Visible = False
  end
  object dxLookupTreeView1: TdxLookupTreeView
    Left = 76
    Top = 27
    Width = 91
    Height = 20
    CanSelectParents = True
    DropDownRows = 30
    ParentColor = False
    TabOrder = 6
    TabStop = True
    TreeViewColor = clWindow
    TreeViewCursor = crDefault
    TreeViewFont.Charset = DEFAULT_CHARSET
    TreeViewFont.Color = clWindowText
    TreeViewFont.Height = -11
    TreeViewFont.Name = 'MS Sans Serif'
    TreeViewFont.Style = []
    TreeViewImages = ImageList
    TreeViewIndent = 19
    TreeViewReadOnly = True
    TreeViewShowButtons = True
    TreeViewShowHint = False
    TreeViewShowLines = True
    TreeViewShowRoot = True
    TreeViewSortType = stNone
    OnKeyDown = dxLookupTreeView1KeyDown
    DividedChar = '.'
    ListSource = DM.DSdistrict
    KeyField = 'rec_id'
    ListField = 'zdesc'
    Options = [trCanDBNavigate, trSmartRecordCopy, trCheckHasChildren]
    ParentField = 'parent'
    TextStyle = tvtsShort
    Alignment = taLeftJustify
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 79
    Width = 980
    Height = 435
    ActivePage = TabSheet1
    Align = alBottom
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 7
    OnChange = PageControl1Change
    object TabSheet1: TTabSheet
      Caption = #19994#21153#21592#27719#24635
      object dxDBGrid1: TdxDBGrid
        Left = 0
        Top = 0
        Width = 972
        Height = 408
        Bands = <
          item
          end>
        DefaultLayout = True
        HeaderPanelRowCount = 1
        KeyField = 'broker_id'
        ShowGroupPanel = True
        ShowSummaryFooter = True
        SummaryGroups = <
          item
            DefaultGroup = True
            SummaryItems = <
              item
                SummaryFormat = '###,###'#20154';; '
                SummaryType = cstCount
              end
              item
                ColumnName = 'dxDBGrid1bal0'
                SummaryField = 'bal0'
                SummaryFormat = '###,###,##0.00;; '
                SummaryType = cstSum
              end
              item
                SummaryField = 'drawamot'
                SummaryFormat = '###,###,##0.00;; '
                SummaryType = cstSum
              end
              item
                ColumnName = 'dxDBGrid1rec_amot'
                SummaryField = 'rec'
                SummaryFormat = '###,###,##0.00;; '
                SummaryType = cstSum
              end
              item
                ColumnName = 'dxDBGrid1pay_amot'
                SummaryField = 'pay'
                SummaryFormat = '###,###,##0.00;; '
                SummaryType = cstSum
              end
              item
                ColumnName = 'dxDBGrid1bal'
                SummaryField = 'bal1'
                SummaryFormat = '###,###,##0.00;; '
                SummaryType = cstSum
              end>
            Name = 'dxDBGrid1SummaryGroup2'
          end>
        SummarySeparator = ', '
        Align = alClient
        PopupMenu = PopupMenu1
        TabOrder = 0
        DataSource = DSqry
        Filter.Active = True
        Filter.DropDownCount = 30
        Filter.Criteria = {00000000}
        HighlightColor = clAqua
        HighlightTextColor = clBlack
        LookAndFeel = lfUltraFlat
        OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabThrough, edgoVertThrough]
        OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
        OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
        ShowRowFooter = True
        object dxDBGrid1cdistrict: TdxDBGridColumn
          Font.Charset = GB2312_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          Width = 85
          BandIndex = 0
          RowIndex = 0
          FieldName = 'cdistrict'
        end
        object dxDBGrid1broker: TdxDBGridColumn
          Font.Charset = GB2312_CHARSET
          Font.Color = clNavy
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          Width = 100
          BandIndex = 0
          RowIndex = 0
          FieldName = 'broker'
          SummaryFooterType = cstCount
          SummaryFooterFormat = '###,##0'#20154';; '
          SummaryType = cstCount
          SummaryFormat = '###,##0'#20154';; '
        end
        object dxDBGrid1bal0: TdxDBGridColumn
          Caption = #24212#20184#26399#21021#37329#39069
          Color = 16316664
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Width = 100
          BandIndex = 0
          RowIndex = 0
          FieldName = 'bal0'
          SummaryFooterType = cstSum
          SummaryFooterField = 'bal0'
          SummaryFooterFormat = '###,###,##0.00;; '
          SummaryType = cstSum
          SummaryField = 'bal0'
          SummaryGroupName = '###,###,##0.00;; '
        end
        object dxDBGrid1drawamot: TdxDBGridColumn
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Width = 100
          BandIndex = 0
          RowIndex = 0
          FieldName = 'drawamot'
          SummaryFooterType = cstSum
          SummaryFooterField = 'drawamot'
          SummaryFooterFormat = '###,###,##0.00;; '
          SummaryType = cstSum
          SummaryField = 'drawamot'
          SummaryFormat = '###,###,##0.00;; '
        end
        object dxDBGrid1rec_amot: TdxDBGridColumn
          Font.Charset = ANSI_CHARSET
          Font.Color = clGreen
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Width = 100
          BandIndex = 0
          RowIndex = 0
          FieldName = 'rec_amot'
          SummaryFooterType = cstSum
          SummaryFooterField = 'rec_amot'
          SummaryFooterFormat = '###,###,##0.00;; '
          SummaryType = cstSum
          SummaryField = 'rec_amot'
          SummaryFormat = '###,###,##0.00;; '
        end
        object dxDBGrid1pay_amot: TdxDBGridColumn
          Font.Charset = ANSI_CHARSET
          Font.Color = clRed
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Width = 100
          BandIndex = 0
          RowIndex = 0
          FieldName = 'pay_amot'
          SummaryFooterType = cstSum
          SummaryFooterField = 'pay_amot'
          SummaryFooterFormat = '###,###,##0.00;; '
          SummaryType = cstSum
          SummaryField = 'pay_amot'
          SummaryFormat = '###,###,##0.00;; '
        end
        object dxDBGrid1bal: TdxDBGridColumn
          Color = 16316664
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Width = 100
          BandIndex = 0
          RowIndex = 0
          FieldName = 'bal'
          SummaryFooterType = cstSum
          SummaryFooterField = 'bal'
          SummaryFooterFormat = '###,###,##0.00;; '
          SummaryType = cstSum
          SummaryField = 'bal'
          SummaryFormat = '###,###,##0.00;; '
        end
      end
    end
    object TabSheet2: TTabSheet
      Caption = #21697#31181#27719#24635
      ImageIndex = 1
      object dxDBGrid2: TdxDBGrid
        Left = 0
        Top = 0
        Width = 972
        Height = 408
        Bands = <
          item
          end>
        DefaultLayout = True
        HeaderPanelRowCount = 1
        KeyField = 'med_id'
        ShowGroupPanel = True
        ShowSummaryFooter = True
        SummaryGroups = <
          item
            DefaultGroup = True
            SummaryItems = <
              item
                ColumnName = 'dxDBGrid1broker'
                SummaryFormat = '###,###'#20154';; '
                SummaryType = cstCount
              end
              item
                ColumnName = 'dxDBGrid1bal0'
                SummaryField = 'bal0'
                SummaryFormat = '###,###,##0.00;; '
                SummaryType = cstSum
              end
              item
                ColumnName = 'dxDBGrid1drawamot'
                SummaryField = 'drawamot'
                SummaryFormat = '###,###,##0.00;; '
                SummaryType = cstSum
              end
              item
                ColumnName = 'dxDBGrid1rec'
                SummaryField = 'rec'
                SummaryFormat = '###,###,##0.00;; '
                SummaryType = cstSum
              end
              item
                ColumnName = 'dxDBGrid1pay'
                SummaryField = 'pay'
                SummaryFormat = '###,###,##0.00;; '
                SummaryType = cstSum
              end
              item
                ColumnName = 'dxDBGrid1bal1'
                SummaryField = 'bal1'
                SummaryFormat = '###,###,##0.00;; '
                SummaryType = cstSum
              end>
            Name = 'dxDBGrid1SummaryGroup2'
          end>
        SummarySeparator = ', '
        Align = alClient
        PopupMenu = PopupMenu1
        TabOrder = 0
        DataSource = DSqry1
        Filter.Active = True
        Filter.DropDownCount = 30
        Filter.Criteria = {00000000}
        HighlightColor = clAqua
        HighlightTextColor = clBlack
        LookAndFeel = lfUltraFlat
        OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabThrough, edgoVertThrough]
        OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
        OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
        ShowRowFooter = True
        OnCustomDrawCell = dxDBGrid2CustomDrawCell
        object dxDBGrid2med_code: TdxDBGridColumn
          Alignment = taCenter
          Font.Charset = GB2312_CHARSET
          Font.Color = clRed
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          Width = 55
          BandIndex = 0
          RowIndex = 0
          FieldName = 'med_code'
          SummaryFooterType = cstCount
          SummaryFooterFormat = '###,##0'#20010';; '
          SummaryType = cstCount
          SummaryFormat = '###,##0'#20010';; '
        end
        object dxDBGrid2med_name: TdxDBGridColumn
          Font.Charset = GB2312_CHARSET
          Font.Color = clNavy
          Font.Height = -12
          Font.Name = #23435#20307
          Font.Style = []
          Width = 120
          BandIndex = 0
          RowIndex = 0
          FieldName = 'med_name'
        end
        object dxDBGrid2specifi: TdxDBGridColumn
          Width = 100
          BandIndex = 0
          RowIndex = 0
          FieldName = 'specifi'
        end
        object dxDBGrid2pdt_place: TdxDBGridColumn
          Width = 100
          BandIndex = 0
          RowIndex = 0
          FieldName = 'pdt_place'
        end
        object dxDBGrid2Ctypeid1: TdxDBGridColumn
          Alignment = taCenter
          Caption = #25152#23646
          Width = 45
          BandIndex = 0
          RowIndex = 0
          FieldName = 'Ctypeid1'
        end
        object dxDBGrid2bal0: TdxDBGridColumn
          Color = 16316664
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Width = 110
          BandIndex = 0
          RowIndex = 0
          FieldName = 'bal0'
          SummaryFooterType = cstSum
          SummaryFooterField = 'bal0'
          SummaryFooterFormat = '###,###,##0.00;; '
          SummaryType = cstSum
          SummaryField = 'bal0'
          SummaryFormat = '###,###,##0.00;; '
        end
        object dxDBGrid2drawamot: TdxDBGridColumn
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Width = 100
          BandIndex = 0
          RowIndex = 0
          FieldName = 'drawamot'
          SummaryFooterType = cstSum
          SummaryFooterField = 'drawamot'
          SummaryFooterFormat = '###,###,##0.00;; '
          SummaryType = cstSum
          SummaryField = 'drawamot'
          SummaryFormat = '###,###,##0.00;; '
        end
        object dxDBGrid2rec_amot: TdxDBGridColumn
          Font.Charset = ANSI_CHARSET
          Font.Color = clGreen
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Width = 100
          BandIndex = 0
          RowIndex = 0
          FieldName = 'rec_amot'
          SummaryFooterType = cstSum
          SummaryFooterField = 'rec_amot'
          SummaryFooterFormat = '###,###,##0.00;; '
          SummaryType = cstSum
          SummaryField = 'rec_amot'
          SummaryFormat = '###,###,##0.00;; '
        end
        object dxDBGrid2pay_amot: TdxDBGridColumn
          Font.Charset = ANSI_CHARSET
          Font.Color = clRed
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Width = 100
          BandIndex = 0
          RowIndex = 0
          FieldName = 'pay_amot'
          SummaryFooterType = cstSum
          SummaryFooterField = 'pay_amot'
          SummaryFooterFormat = '###,###,##0.00;; '
          SummaryType = cstSum
          SummaryField = 'pay_amot'
          SummaryFormat = '###,###,##0.00;; '
        end
        object dxDBGrid2notamot: TdxDBGridColumn
          Color = 16316664
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Width = 110
          BandIndex = 0
          RowIndex = 0
          FieldName = 'Cbal'
          SummaryFooterType = cstSum
          SummaryFooterField = 'notamot'
          SummaryFooterFormat = '###,###,##0.00;; '
          SummaryType = cstSum
          SummaryField = 'notamot'
          SummaryFormat = '###,###,##0.00;; '
        end
      end
    end
  end
  object dxCheckEdit1: TdxCheckEdit
    Left = 423
    Top = 47
    Width = 109
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.Edges = []
    TabOrder = 8
    Visible = False
    Caption = #21253#21547#24050#23436#25104#20184#27454
    State = cbsChecked
  end
  object dxButtonEdit2: TdxButtonEdit
    Left = 76
    Top = 49
    Width = 210
    Style.BorderColor = 12615680
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 9
    Visible = False
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit2ButtonClick
    ExistButtons = True
  end
  object RadioGroup3: TRadioGroup
    Left = 322
    Top = -2
    Width = 75
    Height = 72
    ItemIndex = 0
    Items.Strings = (
      #25152#26377
      #20195#29702
      #33258#26377)
    TabOrder = 10
  end
  object DSqry: TDataSource
    DataSet = qry
    OnDataChange = DSqryDataChange
    Left = 50
    Top = 211
  end
  object qry: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    ReadOnly = True
    RemoteServer = DM.RemoteServer
    OnCalcFields = qryCalcFields
    Left = 21
    Top = 211
    object qryIntegerField: TIntegerField
      FieldName = 'broker_id'
    end
    object qrycdistrict: TStringField
      DisplayLabel = #25152#23646#22320#21306
      FieldName = 'cdistrict'
      Size = 100
    end
    object qrybroker: TStringField
      DisplayLabel = #19994#21153#21592
      FieldName = 'broker'
      Size = 100
    end
    object qryreceiver: TStringField
      DisplayLabel = #25910#27454#20154
      FieldName = 'receiver'
    end
    object qrybank: TStringField
      DisplayLabel = #24320#25143#38134#34892
      FieldName = 'bank'
      Size = 100
    end
    object qrydepo_acco: TStringField
      DisplayLabel = #38134#34892#24080#25143
      FieldName = 'depo_acco'
    end
    object qrybal0: TBCDField
      DisplayLabel = #24212#20184#26399#21021#25968
      FieldName = 'bal0'
      ReadOnly = True
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Precision = 32
      Size = 2
    end
    object qrydrawamot: TBCDField
      DisplayLabel = #25237#36164#27454#37329#39069
      FieldName = 'drawamot'
      ReadOnly = True
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Precision = 32
      Size = 2
    end
    object qryrec: TBCDField
      DisplayLabel = #25910#27454
      FieldName = 'rec_amot'
      ReadOnly = True
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Precision = 32
      Size = 2
    end
    object qrypay: TBCDField
      DisplayLabel = #20184#27454
      FieldName = 'pay_amot'
      ReadOnly = True
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Precision = 32
      Size = 2
    end
    object qryCbal: TBCDField
      DisplayLabel = #24212#20184#20313#39069
      FieldKind = fkCalculated
      FieldName = 'Cbal'
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Size = 2
      Calculated = True
    end
    object qrybal: TBCDField
      DisplayLabel = #24212#20184#20313#39069
      FieldName = 'bal'
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Size = 2
    end
  end
  object StyleController: TdxEditStyleController
    BorderColor = 14065456
    BorderStyle = xbsSingle
    ButtonStyle = btsSimple
    Edges = [edgBottom]
    HotTrack = True
    Left = 321
    Top = 201
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingAGE'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'sp_brokerexgbal '#39'2004-9-1'#39','#39'2004-11-8'#39',0')
    Left = 371
    Top = 34
    object ADOQuery1broker_id: TIntegerField
      FieldName = 'broker_id'
    end
    object ADOQuery1cdistrict: TStringField
      FieldName = 'cdistrict'
      Size = 100
    end
    object ADOQuery1broker: TStringField
      FieldName = 'broker'
      Size = 100
    end
    object ADOQuery1depo_acco: TStringField
      FieldName = 'depo_acco'
    end
    object ADOQuery1bank: TStringField
      FieldName = 'bank'
      Size = 100
    end
    object ADOQuery1bal0: TBCDField
      FieldName = 'bal0'
      ReadOnly = True
      Precision = 32
      Size = 8
    end
    object ADOQuery1drawamot: TBCDField
      FieldName = 'drawamot'
      ReadOnly = True
      Precision = 32
      Size = 8
    end
    object ADOQuery1rec: TBCDField
      FieldName = 'rec'
      ReadOnly = True
      Precision = 32
    end
    object ADOQuery1pay: TBCDField
      FieldName = 'pay'
      ReadOnly = True
      Precision = 32
    end
    object ADOQuery1bal1: TBCDField
      FieldName = 'bal1'
      ReadOnly = True
      Precision = 32
      Size = 8
    end
  end
  object PopupMenu1: TPopupMenu
    Left = 133
    Top = 174
    object N3: TMenuItem
      Caption = #26597#25214'...'
      OnClick = N3Click
    end
    object N4: TMenuItem
      Caption = '-'
    end
    object N1: TMenuItem
      Caption = #38544#34255#26412#21015
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #26174#31034#25152#26377#21015
      OnClick = N2Click
    end
    object N5: TMenuItem
      Caption = '-'
    end
    object N6: TMenuItem
      Caption = #26684#24335#22797#21407
      OnClick = N6Click
    end
  end
  object ImageList: TImageList
    Left = 267
    Top = 71
    Bitmap = {
      
      000000000000}
  end
  object qry1: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    ReadOnly = True
    RemoteServer = DM.RemoteServer
    OnCalcFields = qry1CalcFields
    Left = 99
    Top = 211
    object StringField1: TStringField
      DisplayLabel = #25152#23646#22320#21306
      FieldName = 'cdistrict'
      Size = 100
    end
    object qry1med_id: TIntegerField
      FieldName = 'med_id'
    end
    object qry1med_code: TStringField
      DisplayLabel = #32534#30721
      FieldName = 'med_code'
    end
    object qry1med_name: TStringField
      DisplayLabel = #21697#21517
      FieldName = 'med_name'
      Size = 50
    end
    object qry1specifi: TStringField
      DisplayLabel = #35268#26684
      FieldName = 'specifi'
      Size = 50
    end
    object qry1pdt_place: TStringField
      DisplayLabel = #29983#20135#20225#19994
      FieldName = 'pdt_place'
      Size = 30
    end
    object qry1med_unit: TStringField
      DisplayLabel = #21333#20301
      FieldName = 'med_unit'
      Size = 30
    end
    object BCDField1: TBCDField
      DisplayLabel = #24212#20184#26399#21021#37329#39069
      FieldName = 'bal0'
      ReadOnly = True
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Precision = 32
      Size = 2
    end
    object BCDField2: TBCDField
      DisplayLabel = #25237#36164#27454#37329#39069
      FieldName = 'drawamot'
      ReadOnly = True
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Precision = 32
      Size = 2
    end
    object qry1rec_amot: TBCDField
      DisplayLabel = #25910#27454
      FieldName = 'rec_amot'
      ReadOnly = True
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Precision = 32
      Size = 2
    end
    object qry1pay_amot: TBCDField
      DisplayLabel = #20184#27454
      FieldName = 'pay_amot'
      ReadOnly = True
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Precision = 32
      Size = 2
    end
    object qry1type_id1: TWordField
      FieldName = 'type_id1'
    end
    object qry1Ctypeid1: TStringField
      DisplayLabel = #32463#33829#31867#21035
      FieldKind = fkCalculated
      FieldName = 'Ctypeid1'
      Calculated = True
    end
    object qry1Cbal: TBCDField
      DisplayLabel = #24212#20184#20313#39069
      FieldKind = fkCalculated
      FieldName = 'Cbal'
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Size = 2
      Calculated = True
    end
    object qry1Cbal0: TBCDField
      DisplayLabel = #24212#20184#26399#21021#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Cbal0'
      DisplayFormat = '###,###,##0.00;; '
      EditFormat = '########0.00;; '
      Size = 2
      Calculated = True
    end
  end
  object DSqry1: TDataSource
    DataSet = qry1
    OnDataChange = DSqry1DataChange
    Left = 128
    Top = 211
  end
end
