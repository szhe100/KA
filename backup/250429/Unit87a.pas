...
procedure Tsetbusiframe7.dxButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetbusiframe7.SpeedButton2Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag);
end;

procedure Tsetbusiframe7.DSbusiframeDataChange(Sender: TObject;
  Field: TField);
begin
with busiframe do
...
procedure Tsetbusiframe7.SpeedButton4Click(Sender: TObject);
var mi: integer;
begin
...
begin
    ...
    commandtext:='select top 1 1 from tb_busiframe7'; // where broker_id='+inttostr(dxButtonEdit2.tag);
    ...
    if MessageBox(0,pchar('确定加入本记录'),'请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
{
    ...
    if active=true then close;
    commandtext:='insert into tb_busiframe7 (med_id,valid_dt,creat_by,creat_dt)'; //district_id,
...
procedure Tsetbusiframe7.FormCloseQuery(Sender: TObject;
...
procedure Tsetbusiframe7.busiframeBeforeDelete(DataSet: TDataSet);
begin
...
procedure Tsetbusiframe7.SpeedButton3Click(Sender: TObject);
var mi: integer;
begin
with busiframe do
begin
    ...
    commandtext:=commandtext+' from tb_busiframe7 a';
...
//        if Trim(dxButtonEdit4.text)<>'' then commandtext:=commandtext+' and a.mate_id='+inttostr(dxButtonEdit4.Tag);
    ...
    begin
        ...
        commandtext:=commandtext+'  from tb_busiframe7 group by med_id) y';
...
procedure Tsetbusiframe7.busiframeBeforePost(DataSet: TDataSet);
begin
with busiframe do
begin
    ...
    begin
		if fieldbyname('price').asfloat=0 then raise Exception.Create('请输入采购结算价');
...
procedure Tsetbusiframe7.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetbusiframe7.FormActivate(Sender: TObject);
begin
{
with dm.district do
...
procedure Tsetbusiframe7.FormCreate(Sender: TObject);
begin
...
procedure Tsetbusiframe7.dxButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetbusiframe7.dxButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetbusiframe7.busiframeAfterPost(DataSet: TDataSet);
begin
if busiframe.ChangeCount>0 then myupdaterefresh(busiframe,'tb_busiframe7','rec_id','');
end;

procedure Tsetbusiframe7.SpeedButton6Click(Sender: TObject);
var
    XL,Sheet: Variant;
    ...
    dt: TDatetime;
    price: real;
//    rate,price,amot,rate1,fee: real;
begin
...
//从第2行开始导入，导入格式:1产品编码、2采购结算价、3生效日期
    ...
    begin
        ...
        begin
            ...
            if (sheet.cells[j,2].text='') then serror:=serror+#13#10+'第'+inttostr(j)+'行 无采购结算价'
            else
            begin
                try
                    price:=strtofloat(cleardeli(sheet.cells[j,2].text));
                except
                    serror:=serror+#13#10+'第'+inttostr(j)+'行 采购结算价有误';
                end;
            end;
            if sheet.cells[j,3].text='' then serror:=serror+#13#10+'第'+inttostr(j)+'行 无生效日期'
            else
            begin
                try
                    dt:=strtodatetime(sheet.cells[j,3].text);
                except
...
//从第2行开始导入，导入格式:1产品编码、2采购结算价、3生效日期
            ...
            begin
                if active=true then close;
                commandtext:='select top 1 1 from tb_busiframe7 where med_id='+inttostr(id1)+' and valid_dt='''+datetimetostr(dt)+'''';
                open;
...
//从第2行开始导入，导入格式:1产品编码、2采购结算价、3生效日期
            if active=true then close;
            commandtext:='insert into tb_busiframe7 (med_id,price,valid_dt,creat_by,creat_dt)';
            commandtext:=commandtext+' values ('+inttostr(id1)+','+floattostr(price)+','''+datetimetostr(dt)+''',';
...
procedure Tsetbusiframe7.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetbusiframe7.N3Click(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure Tsetbusiframe7.N1Click(Sender: TObject);
begin
with dxdbgrid1 do Columns[GetAbsoluteColumnIndex(FocusedColumn)].Visible:=false;
end;

procedure Tsetbusiframe7.N2Click(Sender: TObject);
var i,mi: integer;
begin
with dxdbgrid1 do
begin
...
procedure Tsetbusiframe7.N6Click(Sender: TObject);
begin
...
procedure Tsetbusiframe7.N7Click(Sender: TObject);
...
begin
    if active=true then close;
    commandtext:='select top 1 bod_cd from tb_bill a,tb_bill_dtl b where a.bod_type_id=51 and bod_status_id<>1';
...
mcd:=formatfloat('00000000',getbillid(0,51)); //bod_type_id=51: 价目表申请单
with dm.pubqry do
begin
    setprogress(1);
    if active=true then close;
    ...
    commandtext:=commandtext+' values ('+inttostr(compid)+','''+mcd+''',51,0,'+inttostr(curuserid)+',getdate())';
    commandtext:=commandtext+' select @@identity';
//edit1.text:=commandtext;
    open;
    ...
    commandtext:=commandtext+' from tb_busiframe7 where rec_id in ('+t+')';
    execute;
    if active=True then close;
    setprogress(0);
    s:='已成功生成'+mcd+'号申请单';
...
procedure Tsetbusiframe7.busiframeBeforeEdit(DataSet: TDataSet);
begin
