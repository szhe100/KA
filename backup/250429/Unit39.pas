...
procedure Treppaybill.Setfpriwhole(fp: boolean);
begin
end;

procedure Treppaybill.QRBand5BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
qrlabel5.caption:=saydigit(cdsbill.fieldbyname('bod_amot').asfloat)+formatfloat('(##,###,##0.00)',cdsbill.fieldbyname('bod_amot').asfloat);
end;

procedure Treppaybill.DetailBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
qrlabel5.caption:=saydigit(cdsbill.fieldbyname('bod_amot').asfloat)+formatfloat('(##,###,##0.00)',cdsbill.fieldbyname('bod_amot').asfloat);
end;

procedure Treppaybill.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
if ent_title<>'' then qrlabel20.caption:=ent_title;
end;

procedure Treppaybill.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
action:=cafree;
end;

procedure Treppaybill.PageHeaderBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
qrlabel28.caption:=FormatdateTime('''付款日期:''yyyy''年''mm''月''dd''日''',cdsbill.fieldbyname('carry_dt').asdatetime);
end;
