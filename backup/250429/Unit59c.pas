...
begin
if (sysrule.fieldbyname('valid').asboolean=True) or (sysrule.fieldbyname('stop_yn').asboolean=True)
...
begin
    ...
    commandtext:='select a.rec_id,a.type_id,a.comp_id,a.med_id,a.mate_id,b.mate_name,b.rate,a.mate_id1,mate_name1=e.mate_name,m.med_code,m.med_name,m.chm_name,m.specifi,m.pdt_place,';
...
procedure Tsettrustrule.N1Click(Sender: TObject);
...
procedure Tsettrustrule.dxDBGrid1med_codeButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickmedicine do
begin
    ...
    end;
{
    ...
    if active=true then close;
//    commandtext:='select top 1 1 from tb_sysrule where rec_id in ('+t+') and (isnull(f4,0)=0 and isnull(f8,0)=0)';
...
{
    commandtext:=commandtext+' from tb_sysrule s,tb_medata a,tb_busimate b,tb_medicine m';
    commandtext:=commandtext+' where s.rec_id in ('+t+') and a.med_id=s.med_id and a.type_id=1 and s.med_id=m.med_id';
    commandtext:=commandtext+'  and b.mate_id=s.mate_id1 and dbo.fn_treeischild(b.district,a.id1)=1 ';
    commandtext:=commandtext+'  and (isnull(a.price4,0)=0 and isnull(a.price8,0)=0)';
edit1.text:=commandtext;
}
    open;
...
var
    ...
    J,mi,id1,id2,id3,id4,districtid : integer;
    rate5,rate6: real;
    price,price1,rate: real;
...
begin
    ...
    sbodcd:='';
    j:=2; mi:=0; //第2行开始 每行一单
//格式:1启用日,2商业公司,3医院,4药品编码,5医院托管结算率,6商业托管结算率,7医院标准政策,8医院执行日期,9商业标准政策,10商业执行日期,11商业通知政策,12文件执行日期,13商业通知日期
