...
begin
    ...
    serror:='';
    j:=2; mi:=0; //第2行开始 每行一单
//导入格式: 1省份,2城市,3医院名称,4是否目标医院,5药品编码,6月目标量,7月潜力量,8是否限量,9限量金额
    ...
    begin
        ...
        begin
            ...
            if sheet.cells[j,4].text='是' then id4:=1 else id4:=0;
            ...
            begin
                if active=true then close;
                commandtext:='select top 1 sta_id from tb_staff where comp_id='+inttostr(compid)+' and sta_type_id=0 and manager=1 and zname ='''+Trim(sheet.cells[j,5].text)+'''';  //区域经理
                open;
...
//导入格式: 1省份,2城市,3医院名称,4是否目标医院,5药品编码,6月目标量,7月潜力量,8是否限量,9限量金额
            ...
            begin
                ...
                if id3=0 then serror:=serror+',未取得医院';
                if id5=0 then serror:=serror+',未取得药品';
                mi:=mi+1;
            end;
        end;
        inc(j);
...
//导入格式: 1省份,2城市,3医院名称,4是否目标医院,5药品编码,6月目标量,7月潜力量,8是否限量,9限量金额
            ...
            begin
                ...
                if (sheet.cells[j,7].text='是') or (sheet.cells[j,7].text='限量')
