...
begin
    ...
    commandtext:=commandtext+'	b.district,cdistrict=dbo.fn_getdistrict(b.district),med_unit=dbo.fn_obj_desc(m.unit_id),med_type=dbo.fn_med_type(m.med_id),m.qtyperpack,import_log='''',';
...
begin
    ...
    setprogress(1);
    m1:=0; m2:=0; j:=2;  
//从第2行开始导入, 导入格式: 1商业公司, 2品种编码, 3商业级别, 4启用日期
...
//从第2行开始导入, 导入格式: 1商业公司, 2品种编码, 3商业级别, 4启用日期
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行未取得商业公司'' from @tab a where not exists (select 1 from tb_busimate b where b.mate_type_id=2 and b.mate_name=a.f1)';
...
//type_id,comp_id,mate_id,med_id,level_id,valid_dt,creat_by,creat_dt
        ...
        sql:=sql1+' insert into tb_sysrule (type_id,comp_id,mate_id,med_id,level_id,valid_dt,creat_by,creat_dt,import_log)';
        sql:=sql+'  select type_id=2,comp_id='+inttostr(compid)+',a.mate_id,a.med_id,level_id=cast(b.f3 as int),valid_dt=cast(f4 as datetime),creat_by='+inttostr(curuserid)+',creat_dt=getdate(),';
        sql:=sql+'   import_log=isnull(import_log,'''')+cast(getdate() as varchar(20))+''导入'''; // type_id=2 设置商业级别
        sql:=sql+' from @tab a';
