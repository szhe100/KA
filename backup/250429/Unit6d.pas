...
//从第2行开始导入，格式: 1渠道,2子渠道,3业务省份,4业务城市,5物料编号,6中标价/开票价,7金额,8定价单位,9有效从,10有效到
    ...
    sql:=sql+' select top 5 * from ( select top 5 info=''第''+line_no+''行无渠道'' from @tab where f1=''''';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无子渠道'' from @tab where f2=''''';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无业务省份城市'' from @tab where f3='''' and f4=''''';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无业务城市'' from @tab where f4=''''';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无物料编号'' from @tab where f5=''''';
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行无有效从或数据无效'' from @tab where f9='''' or try_cast(f9 as datetime) is null';
...
//rec_id,comp_id,type_id,channel_id,channel_dtl_id,cust_group_id,district_id,med_id,price,price_unit,price1,valid_from,valid_to,creat_by,creat_dt
        ...
        sql:=sql+'  select comp_id='+inttostr(compid)+',type_id=3,channel_id=b.obj_id,channel_dtl_id=c.obj_id,district_id=d.rec_id,'; //type_id=3 设置学术标准政策
        ...
        sql:=sql+' left join tb_object c on b.obj_type_id=12 and b.zdesc= a.f2';
