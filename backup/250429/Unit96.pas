...
type
  Tsetreceive2 = class(TForm)
    ...
    N6: TMenuItem;
    dmSave1xlsMicrosoftExcel40Worksheetxlsxlsselfcaption1xlsdxDBGrid1SaveToXLSselftag1: TMenuItem;
    dxDBGrid1Ccate: TdxDBGridColumn;
    dxDBMemo1: TdxDBMemo;
    Label11: TLabel;
    cdsbillzmemo: TMemoField;
    cdsbillfpic: TBooleanField;
    dxDBGrid1fpic: TdxDBGridCheckColumn;
    ...
    procedure dmSave1xlsMicrosoftExcel40Worksheetxlsxlsselfcaption1xlsdxDBGrid1SaveToXLSselftag1Click(
      Sender: TObject);
  private
...
var
  setreceive2: Tsetreceive2;

implementation

uses datamodu, Unit28b, Unit16c, Unit16a, Unit9; //, Unit7

{$R *.DFM}

procedure Tsetreceive2.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetreceive2.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetreceive2.FormActivate(Sender: TObject);
begin
//with dm.imp_type do
...
procedure Tsetreceive2.FormCloseQuery(Sender: TObject;
...
procedure Tsetreceive2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetreceive2.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetreceive2.dxDBGrid2Enter(Sender: TObject);
begin
{
...
procedure Tsetreceive2.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetreceive2.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetreceive2.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetreceive2.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
//    commandtext:='select h.carry_dt,a.*,mate_name1=k.mate_name,mate_name2=c.mate_name,agent=f.mate_name,broker=g.zname,creater=d.zname,checker=e.zname,';
    commandtext:=commandtext+' c.zname1,c.address1,c.phone1,c.zname2,c.address2,c.phone2,cdistrict=dbo.fn_getdistrict(f.district),finished=cast(0 as bit),';
    commandtext:=commandtext+' fpic=cast(case when exists (select top 1 1 from tb_image where type_id=1 and id=a.bod_id and fpic=1) then 1 else 0 end as bit)';
//    commandtext:=commandtext+' c.zname1,c.address1,c.phone1,c.zname2,c.address2,c.phone2,cdistrict=dbo.fn_getdistrict(f.district),finished=dbo.fn_orderisfinished(a.bod_id)';
...
//    commandtext:=commandtext+' left join (select a.bod_id,carry_dt=max(b.valid_dt) from tb_bill a,tb_bill_dtl b where a.bod_type_id=45 and a.bod_id=b.bod_id group by a.bod_id) h on a.bod_id=h.bod_id';
    commandtext:=commandtext+' where a.bod_type_id=45';
...
procedure Tsetreceive2.FormCreate(Sender: TObject);
begin
...
procedure Tsetreceive2.SpeedButton6Click(Sender: TObject);
var mid :integer;
begin
setprogress(1);
with dm.pubqry do
...
procedure Tsetreceive2.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetreceive2.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
//        dxDBLookupEdit3.enabled:=(active=true) and (recordcount>0) and (fieldbyname('creat_by').asinteger=curuserid);
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active=true then close;
                commandtext:='select c.bod_desc,c.bod_cd,a.*,broker=d.zname,leader=e.zname,m.med_name,m.specifi,m.pdt_place,m.med_code,m.qtyperpack,med_unit=f.zdesc';
...
procedure Tsetreceive2.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
procedure Tsetreceive2.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetreceive2.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetreceive2.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetreceive2.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetreceive2.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetreceive2.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
    then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
...
procedure Tsetreceive2.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
    if fieldbyname('amot').asfloat=0
        then raise Exception.Create('实收金额不可为零');
...
procedure Tsetreceive2.cdsbillBeforePost(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetreceive2.cdsbillReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetreceive2.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetreceive2.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetreceive2.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetreceive2.PageControl1Change(Sender: TObject);
begin
...
procedure Tsetreceive2.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetreceive2.dxDBGrid2DblClick(Sender: TObject);
begin
{
...
procedure Tsetreceive2.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
...
procedure Tsetreceive2.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetreceive2.N3Click(Sender: TObject);
...
procedure Tsetreceive2.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetreceive2.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetreceive2.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tsetreceive2.dxDBButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetreceive2.dxDBButtonEdit1KeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#32) and ((Sender as TdxDBButtonEdit).Text<>'') then //
begin
...
procedure Tsetreceive2.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetreceive2.dxEdit1Click(Sender: TObject);
begin
dxEdit1.SelectAll;
dxEdit1.SetFocus;
end;

procedure Tsetreceive2.dxDBButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetreceive2.dxDBButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetreceive2.SpeedButton2Click(Sender: TObject);
begin
with cdsbill do
begin
    if fieldbyname('src_id').asinteger=0 then raise Exception.Create('请选择供货单位');
    ...
    if fieldbyname('carry_dt').asstring='' then raise Exception.Create('请输入发生日期');
...
begin
    if active=true then close;
    commandtext:='select top 1 1 from tb_bill_dtl where bod_id='+cdsbill.fieldbyname('bod_id').asstring+' and valid_dt is null'; 
    open;
    if recordcount>0 then raise Exception.Create('请填写明细表中发生日期');
end;
}
if MessageBox(0,'确定本单送采购部审核','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
setprogress(1);
SpeedButton2.enabled:=false; // 避免连续按两次
with dm.pubqry do
begin
    if active=true then close;
    commandtext:='update tb_bill set bod_status_id=2,storage_dt=getdate() where bod_id='+cdsbill.fieldbyname('bod_id').asstring;
    execute;
    close;
end;
...
procedure Tsetreceive2.dxDBGraphicEdit1AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetreceive2.dxDBGraphicEdit2AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetreceive2.dxDBGraphicEdit3AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetreceive2.dxDBGraphicEdit4AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetreceive2.dxDBGraphicEdit5AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetreceive2.dxDBGraphicEdit1Change(Sender: TObject);
begin
setupdatestatus;
//cdsbill.edit;
end;

procedure Tsetreceive2.dxDBGraphicEdit1GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass1').asstring));
end;

procedure Tsetreceive2.dxDBGraphicEdit2GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass2').asstring));
end;

procedure Tsetreceive2.dxDBGraphicEdit3GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass3').asstring));
end;

procedure Tsetreceive2.dxDBGraphicEdit4GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass4').asstring));
end;

procedure Tsetreceive2.dxDBGraphicEdit5GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass5').asstring));
end;

procedure Tsetreceive2.dxDBGraphicEdit1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
...
procedure Tsetreceive2.qryimageAfterPost(DataSet: TDataSet);
begin
qryimage.ApplyUpdates(-1);
end;

procedure Tsetreceive2.N2Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t,mcd: string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetreceive2.N4Click(Sender: TObject);
...
procedure Tsetreceive2.N5Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t,mcd: string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetreceive2.dmSave1xlsMicrosoftExcel40Worksheetxlsxlsselfcaption1xlsdxDBGrid1SaveToXLSselftag1Click(
  Sender: TObject);
begin
