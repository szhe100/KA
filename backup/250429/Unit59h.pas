...
var
  Sethospotcmed: TSethospotcmed;

implementation

uses datamodu, Unit7, Unit16g, Unit28a;

{$R *.DFM}

procedure TSethospotcmed.dxButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure TSethospotcmed.SpeedButton2Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag)
end;

procedure TSethospotcmed.DSsysruleDataChange(Sender: TObject;
  Field: TField);
begin
speedbutton2.enabled:=(sysrule.active=true) and (sysrule.recordcount>0);
end;

procedure TSethospotcmed.sysruleBeforeDelete(DataSet: TDataSet);
begin
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure TSethospotcmed.sysruleAfterPost(DataSet: TDataSet);
begin
if sysrule.ChangeCount>0 then myupdaterefresh(sysrule,'tb_sysrule','rec_id','');
end;

procedure TSethospotcmed.SpeedButton3Click(Sender: TObject);
var mi: integer;
begin
with sysrule do
begin
    ...
    commandtext:=commandtext+'	where a.type_id=9'; // and a.comp_id='+inttostr(compid);
...
procedure TSethospotcmed.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TSethospotcmed.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TSethospotcmed.dxButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickhospital do
begin
    if showmodal= mrOK then
    begin
...
procedure TSethospotcmed.dxDBGrid1lead_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickstaff do
begin
...
procedure TSethospotcmed.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
dxDBGrid1.SaveToIniFile(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid1.ini');//保存
end;

procedure TSethospotcmed.FormCreate(Sender: TObject);
begin
...
procedure TSethospotcmed.N3Click(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure TSethospotcmed.N1Click(Sender: TObject);
begin
with dxdbgrid1 do Columns[GetAbsoluteColumnIndex(FocusedColumn)].Visible:=false;
end;

procedure TSethospotcmed.N2Click(Sender: TObject);
var i,mi: integer;
begin
setprogress(1);
with dxdbgrid1 do
...
procedure TSethospotcmed.N6Click(Sender: TObject);
begin
if FileExists(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini')
    then dxDBGrid1.LoadFromIniFile(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini');//加载
end;

procedure TSethospotcmed.dxDBGrid1Enter(Sender: TObject);
begin
with sysrule do
    if (active=true) and (recordcount=0) then append;
end;

procedure TSethospotcmed.sysruleBeforePost(DataSet: TDataSet);
begin
with sysrule do
...
begin
    ...
    commandtext:='select top 1 1 from tb_sysrule where type_id=9'; // and comp_id='+inttostr(compid);
...
//edit1.text:=commandtext;
    open;
    if recordcount>0 then raise Exception.Create('已有此医院品种的记录，不可再次加入');
end;
with sysrule do
begin
    ...
    begin
        ...
        fieldbyname('type_id').asinteger:=9;
...
begin
    if active=true then close;
    commandtext:='select top 1 1 from tb_sysrule where type_id=9 and mate_id='+inttostr(dxButtonEdit2.tag);
    commandtext:=commandtext+' and med_id='+inttostr(dxButtonEdit1.tag);
    open;
    if recordcount>0 then raise Exception.Create('已有该医院品种记录，不可重复设置');
    if MessageBox(0,pchar('确定加入本医院品种记录'),'请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;

    if active=true then close;
    commandtext:='insert into tb_sysrule (type_id,comp_id,med_id,mate_id,creat_by,creat_dt)';
    commandtext:=commandtext+' values (9,'+inttostr(compid)+','+inttostr(dxButtonEdit1.tag)+','+inttostr(dxButtonEdit2.tag)+',';
...
procedure TSethospotcmed.SpeedButton6Click(Sender: TObject);
var
    XL,Sheet: Variant;
...
//导入格式:1医院名称，2药品编码
    ...
    begin
        ...
        with dm.pubqry do
        begin
            if sheet.cells[j,1].text='' then serror:=serror+#13#10+'第'+inttostr(j)+'行 无医院名称'
            else
            begin
                if active=true then close;
                commandtext:='select top 1 mate_id from tb_busimate where mate_type_id=1 and (busi_type_id not in (7,8) or busi_type_id is null) and mate_name='''+Trim(sheet.cells[j,1].text)+'''';
                open;
                if recordcount>0 then id1:=fieldbyname('mate_id').asinteger else serror:=serror+#13#10+'第'+inttostr(j)+'行 医院名称有误';
            end;
            ...
            if active=true then close;
            commandtext:='select top 1 1 from tb_sysrule where type_id=9 and mate_id='+inttostr(id1)+' and med_id='+inttostr(id2);
            open;
            if recordcount>0 then serror:=serror+#13#10+'第'+inttostr(j)+'行 已有该医院品种记录，不可重复导入';
        end;
        inc(j);
    end;
    setprogress(0);
            ...
            end;
{
            ...
            commandtext:=commandtext+' values (9,'+inttostr(compid)+','+inttostr(id1)+','+inttostr(id2)+',';
            commandtext:=commandtext+inttostr(curuserid)+',getdate())';
            commandtext:=commandtext+'select @@identity';
//showmessage(inttostr(j)+': '+commandtext);
            open;
        end;
        inc(j);
    end;
...
procedure TSethospotcmed.sysrulestop_dtChange(Sender: TField);
begin
with sysrule do
begin
...
procedure TSethospotcmed.sysruleBeforeEdit(DataSet: TDataSet);
begin
