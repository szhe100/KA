object setreport29: Tsetreport29
  Left = 94
  Top = 112
  AutoScroll = False
  Caption = #20379#24212#21830#27969#21521#36820#20837#26680#38144'$'
  ClientHeight = 556
  ClientWidth = 1197
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  DesignSize = (
    1197
    556)
  PixelsPerInch = 96
  TextHeight = 12
  object Shape7: TShape
    Left = 669
    Top = 2
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
    Visible = False
  end
  object Label25: TLabel
    Left = 1176
    Top = 9
    Width = 7
    Height = 12
    Alignment = taRightJustify
    Anchors = [akTop, akRight]
    Font.Charset = GB2312_CHARSET
    Font.Color = clMaroon
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = [fsBold]
    ParentFont = False
  end
  object SpeedButton2: TSpeedButton
    Left = 1015
    Top = 2
    Width = 25
    Height = 25
    Hint = #23548#20986#25253#34920
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton2Click
  end
  object Shape2: TShape
    Left = 2
    Top = 45
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label2: TLabel
    Left = 14
    Top = 50
    Width = 36
    Height = 12
    Caption = #21307'  '#38498
    Transparent = True
  end
  object SpeedButton5: TSpeedButton
    Left = 1015
    Top = 63
    Width = 175
    Height = 25
    Hint = #21047#26032
    Anchors = [akTop, akRight]
    Caption = '&R '#21047#26032
    Glyph.Data = {
      
      3393333777333777337333333999993333333333377777333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Spacing = 10
    OnClick = SpeedButton5Click
  end
  object Shape1: TShape
    Left = 2
    Top = 67
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label4: TLabel
    Left = 14
    Top = 73
    Width = 36
    Height = 12
    Caption = #21697'  '#31181
    Transparent = True
  end
  object Shape3: TShape
    Left = 294
    Top = 34
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label6: TLabel
    Left = 306
    Top = 40
    Width = 36
    Height = 12
    Caption = #21333#25454#21495
    Transparent = True
  end
  object Shape5: TShape
    Left = 2
    Top = 23
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label5: TLabel
    Left = 9
    Top = 29
    Width = 48
    Height = 12
    Caption = #20379#36135#21333#20301
    Transparent = True
  end
  object Label8: TLabel
    Left = 681
    Top = 7
    Width = 36
    Height = 12
    Caption = #21306'  '#22495
    Font.Charset = GB2312_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Transparent = True
    Visible = False
  end
  object Label13: TLabel
    Left = 1017
    Top = 47
    Width = 174
    Height = 12
    Anchors = [akTop, akRight]
    Caption = #27880':'#26412#26597#35810#21463#25480#26435#22320#21306#12289#21697#31181#38480#21046
    Font.Charset = GB2312_CHARSET
    Font.Color = clBlack
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
  end
  object Shape4: TShape
    Left = 2
    Top = 1
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label3: TLabel
    Left = 9
    Top = 7
    Width = 49
    Height = 14
    AutoSize = False
    Caption = #21457#29983#26085#26399
    Transparent = True
    WordWrap = True
  end
  object Label1: TLabel
    Left = 168
    Top = 7
    Width = 12
    Height = 12
    Caption = #33267
  end
  object Shape6: TShape
    Left = 294
    Top = 3
    Width = 60
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label7: TLabel
    Left = 306
    Top = 9
    Width = 36
    Height = 12
    Caption = #19994#21153#21592
    Transparent = True
  end
  object Label9: TLabel
    Left = 810
    Top = 31
    Width = 198
    Height = 12
    Caption = #27880':'#25353#21333#21495#26597#25214#26102#65292#21487#19981#35843#25972#26085#26399#33539#22260
  end
  object Shape8: TShape
    Left = 294
    Top = 66
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label10: TLabel
    Left = 300
    Top = 72
    Width = 48
    Height = 12
    Caption = #21697#21517#21253#21547
    Transparent = True
  end
  object RadioGroup1: TRadioGroup
    Left = 467
    Top = 5
    Width = 80
    Height = 80
    Caption = #22266#23450#24212#25910
    ItemIndex = 0
    Items.Strings = (
      #25152#26377
      #24050#23436#25104
      #26410#23436#25104)
    TabOrder = 7
  end
  object dxButtonEdit1: TdxButtonEdit
    Left = 64
    Top = 46
    Width = 218
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 1
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit1ButtonClick
    ExistButtons = True
  end
  object wwDBNavigator2: TwwDBNavigator
    Left = 1040
    Top = 2
    Width = 150
    Height = 25
    DataSource = DSqry
    Options = []
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    Anchors = [akTop, akRight]
    ParentShowHint = False
    object wwNavButton1: TwwNavButton
      Left = 0
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 0
      Style = nbsFirst
    end
    object wwNavButton2: TwwNavButton
      Left = 25
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Prior'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 1
      Style = nbsPrior
    end
    object wwNavButton3: TwwNavButton
      Left = 50
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Next'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 2
      Style = nbsNext
    end
    object wwNavButton4: TwwNavButton
      Left = 75
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 3
      Style = nbsLast
    end
    object wwNavButton5: TwwNavButton
      Left = 100
      Top = 0
      Width = 25
      Height = 25
      Hint = #35774#32622#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1SaveBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 4
      Style = nbsSaveBookmark
    end
    object wwNavButton6: TwwNavButton
      Left = 125
      Top = 0
      Width = 25
      Height = 25
      Hint = #36820#22238#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1RestoreBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 5
      Style = nbsRestoreBookmark
    end
  end
  object dxButtonEdit2: TdxButtonEdit
    Left = 64
    Top = 68
    Width = 218
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 2
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit2ButtonClick
    ExistButtons = True
  end
  object dxEdit1: TdxEdit
    Left = 356
    Top = 35
    Width = 102
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 3
    OnClick = dxEdit1Click
    OnEnter = dxEdit1Click
    OnKeyDown = dxEdit1KeyDown
    StyleController = StyleController
  end
  object dxButtonEdit3: TdxButtonEdit
    Left = 64
    Top = 25
    Width = 218
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 0
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit3ButtonClick
    ExistButtons = True
  end
  object dxDBGrid1: TdxDBGrid
    Left = 0
    Top = 91
    Width = 1197
    Height = 465
    Bands = <
      item
        Caption = #21333#25454#21450#21697#31181
      end
      item
        Caption = #27604#29575#21450#36820#20837
      end
      item
        Caption = #22266#23450#24212#25910
      end
      item
        Caption = #24212#25910#20108#27425#36820#27454
      end
      item
        Caption = #36798#25104#22870
      end>
    DefaultLayout = False
    HeaderPanelRowCount = 1
    KeyField = 'dtl_id'
    ShowSummaryFooter = True
    SummaryGroups = <
      item
        DefaultGroup = True
        SummaryItems = <
          item
            ColumnName = 'dxDBGrid1med_code'
            SummaryField = 'dtl_id'
            SummaryFormat = '###,##0'#31508';; '
            SummaryType = cstCount
          end
          item
            ColumnName = 'dxDBGrid1rate'
            SummaryField = 'amot'
            SummaryFormat = '###,###,##0.00;; '
            SummaryType = cstSum
          end
          item
            SummaryField = 'cost'
            SummaryFormat = '###,###,##0.00;; '
            SummaryType = cstSum
          end
          item
            SummaryField = 'profit'
            SummaryFormat = '###,###,##0.00;; '
            SummaryType = cstSum
          end
          item
            SummaryField = 'amot'
            SummaryFormat = '###,###,##0.00;; '
            SummaryType = cstSum
          end>
        Name = 'dxDBGrid1SummaryGroup2'
      end>
    SummarySeparator = ', '
    Align = alBottom
    PopupMenu = PopupMenu1
    TabOrder = 4
    OnDblClick = dxDBGrid1DblClick
    DataSource = DSqry
    Filter.Active = True
    Filter.DropDownCount = 30
    Filter.Criteria = {00000000}
    HighlightColor = clAqua
    HighlightTextColor = clBlack
    LookAndFeel = lfUltraFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSelect, edgoMultiSort, edgoTabThrough, edgoVertThrough]
    OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
    ShowBands = True
    ShowRowFooter = True
    OnCustomDrawCell = dxDBGrid1CustomDrawCell
    OnCustomDrawFooter = dxDBGrid1CustomDrawFooter
    Anchors = [akLeft, akTop, akRight, akBottom]
    object dxDBGrid1carry_dt: TdxDBGridColumn
      Alignment = taCenter
      Width = 69
      BandIndex = 0
      RowIndex = 0
      FieldName = 'carry_dt'
    end
    object dxDBGrid1Cbusitype: TdxDBGridColumn
      Alignment = taCenter
      Width = 45
      BandIndex = 0
      RowIndex = 0
      FieldName = 'Cbusitype'
    end
    object dxDBGrid1Cbusitype1: TdxDBGridColumn
      Alignment = taCenter
      Width = 69
      BandIndex = 0
      RowIndex = 0
      FieldName = 'Cbusitype1'
    end
    object dxDBGrid1bod_cd: TdxDBGridColumn
      Alignment = taCenter
      Caption = #21830#19994#38144#21806#21333
      Font.Charset = GB2312_CHARSET
      Font.Color = clPurple
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 67
      BandIndex = 0
      RowIndex = 0
      FieldName = 'bod_cd'
      SummaryFooterType = cstCount
      SummaryFooterField = 'dtl_id'
      SummaryFooterFormat = '###,##0;; '
      DisableFilter = True
      SummaryType = cstCount
      SummaryField = 'dtl_id'
      SummaryFormat = '###,##0;; '
    end
    object dxDBGrid1mate_name: TdxDBGridColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 120
      BandIndex = 0
      RowIndex = 0
      FieldName = 'mate_name'
    end
    object dxDBGrid1broker: TdxDBGridColumn
      Width = 57
      BandIndex = 0
      RowIndex = 0
      FieldName = 'broker'
    end
    object dxDBGrid1customer: TdxDBGridColumn
      Caption = #23458#25143'('#21307#38498')'
      Width = 150
      BandIndex = 0
      RowIndex = 0
      FieldName = 'customer'
    end
    object dxDBGrid1bod_desc: TdxDBGridColumn
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'bod_desc'
    end
    object dxDBGrid1med_code: TdxDBGridColumn
      Alignment = taCenter
      Font.Charset = GB2312_CHARSET
      Font.Color = clRed
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_code'
    end
    object dxDBGrid1med_name: TdxDBGridColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 120
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_name'
    end
    object dxDBGrid1specifi: TdxDBGridColumn
      Width = 90
      BandIndex = 0
      RowIndex = 0
      FieldName = 'specifi'
    end
    object dxDBGrid1pdt_place: TdxDBGridColumn
      Width = 69
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pdt_place'
    end
    object dxDBGrid1med_unit: TdxDBGridColumn
      Alignment = taCenter
      Width = 30
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_unit'
      DisableFilter = True
    end
    object dxDBGrid1bat_cd: TdxDBGridColumn
      Font.Charset = GB2312_CHARSET
      Font.Color = clGreen
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'bat_cd'
    end
    object dxDBGrid1qty: TdxDBGridColumn
      Caption = #25968#37327
      Font.Charset = GB2312_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'qty'
      SummaryFooterType = cstSum
      SummaryFooterField = 'qty'
      SummaryFooterFormat = '###,###,##0;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'qty'
      SummaryFormat = '###,###,##0;; '
    end
    object dxDBGrid1price3: TdxDBGridColumn
      Width = 60
      BandIndex = 0
      RowIndex = 0
      FieldName = 'price3'
      DisableFilter = True
    end
    object dxDBGrid1Camot3: TdxDBGridColumn
      Width = 90
      BandIndex = 0
      RowIndex = 0
      FieldName = 'Camot3'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Camot3'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Camot3'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1rate: TdxDBGridColumn
      Alignment = taCenter
      Caption = #27604#29575'%'
      Width = 52
      BandIndex = 1
      RowIndex = 0
      FieldName = 'rate'
      SummaryFooterFormat = '###,###,##0.00;; '
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Camot1: TdxDBGridColumn
      Width = 80
      BandIndex = 1
      RowIndex = 0
      FieldName = 'Camot1'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Camot1'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Camot1'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Camot1a: TdxDBGridColumn
      Width = 80
      BandIndex = 1
      RowIndex = 0
      FieldName = 'Camot1a'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Camot1a'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Camot1a'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Cnotamot1: TdxDBGridColumn
      Color = 16577777
      Width = 80
      BandIndex = 1
      RowIndex = 0
      FieldName = 'Cnotamot1'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Cnotamot1'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Cnotamot1'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Cf1: TdxDBGridCheckColumn
      Caption = #23436#25104
      Width = 45
      BandIndex = 1
      RowIndex = 0
      FieldName = 'Cf1'
      ValueChecked = 'True'
      ValueUnchecked = 'False'
    end
    object dxDBGrid1Camot2: TdxDBGridColumn
      Width = 80
      BandIndex = 2
      RowIndex = 0
      FieldName = 'Camot2'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Camot2'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Camot2'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1amot2a: TdxDBGridColumn
      Width = 80
      BandIndex = 2
      RowIndex = 0
      FieldName = 'amot2a'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1amot2: TdxDBGridColumn
      Width = 80
      BandIndex = 2
      RowIndex = 0
      FieldName = 'amot2'
      SummaryFooterType = cstSum
      SummaryFooterField = 'amot2'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'amot2'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Cnotamot2: TdxDBGridColumn
      Color = 16577777
      Width = 80
      BandIndex = 2
      RowIndex = 0
      FieldName = 'Cnotamot2'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Cnotamot2'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Cnotamot2'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1rate1: TdxDBGridColumn
      Alignment = taCenter
      Caption = #25176#31649#29575'%'
      Width = 63
      BandIndex = 3
      RowIndex = 0
      FieldName = 'rate1'
    end
    object dxDBGrid1Cfee: TdxDBGridColumn
      Width = 75
      BandIndex = 3
      RowIndex = 0
      FieldName = 'Cfee'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Cfee'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Cfee'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Camot4: TdxDBGridColumn
      Width = 80
      BandIndex = 3
      RowIndex = 0
      FieldName = 'Camot4'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Camot4'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Camot4'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1amot4: TdxDBGridColumn
      Width = 80
      BandIndex = 3
      RowIndex = 0
      FieldName = 'amot4'
      SummaryFooterType = cstSum
      SummaryFooterField = 'amot4'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'amot4'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Cnotamot4: TdxDBGridColumn
      Color = 16577777
      Width = 80
      BandIndex = 3
      RowIndex = 0
      FieldName = 'Cnotamot4'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Cnotamot4'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Cnotamot4'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1bonus: TdxDBGridColumn
      Width = 80
      BandIndex = 4
      RowIndex = 0
      FieldName = 'bonus'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Camot5: TdxDBGridColumn
      Width = 80
      BandIndex = 4
      RowIndex = 0
      FieldName = 'Camot5'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Camot5'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Camot5'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1amot5: TdxDBGridColumn
      Width = 80
      BandIndex = 4
      RowIndex = 0
      FieldName = 'amot5'
      SummaryFooterType = cstSum
      SummaryFooterField = 'amot5'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'amot5'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1Cnotamot5: TdxDBGridColumn
      Color = 16577777
      Width = 80
      BandIndex = 4
      RowIndex = 0
      FieldName = 'Cnotamot5'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Cnotamot5'
      SummaryFooterFormat = '###,###,##0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Cnotamot5'
      SummaryFormat = '###,###,##0.00;; '
    end
    object dxDBGrid1src_id: TdxDBGridColumn
      Visible = False
      BandIndex = 0
      RowIndex = 0
      FieldName = 'src_id'
    end
    object dxDBGrid1dst_id: TdxDBGridColumn
      Visible = False
      BandIndex = 0
      RowIndex = 0
      FieldName = 'dst_id'
    end
    object dxDBGrid1agent_id: TdxDBGridColumn
      Visible = False
      BandIndex = 0
      RowIndex = 0
      FieldName = 'agent_id'
    end
    object dxDBGrid1sta_id: TdxDBGridColumn
      Visible = False
      BandIndex = 0
      RowIndex = 0
      FieldName = 'sta_id'
    end
    object dxDBGrid1dtl_id: TdxDBGridColumn
      Visible = False
      BandIndex = 0
      RowIndex = 0
      FieldName = 'dtl_id'
    end
  end
  object dxLookupTreeView1: TdxLookupTreeView
    Left = 731
    Top = 3
    Width = 102
    Height = 20
    CanSelectParents = True
    DropDownRows = 30
    ParentColor = False
    TabOrder = 5
    TabStop = True
    TreeViewColor = clWindow
    TreeViewCursor = crDefault
    TreeViewFont.Charset = DEFAULT_CHARSET
    TreeViewFont.Color = clWindowText
    TreeViewFont.Height = -11
    TreeViewFont.Name = 'MS Sans Serif'
    TreeViewFont.Style = []
    TreeViewImages = ImageList
    TreeViewIndent = 19
    TreeViewReadOnly = True
    TreeViewShowButtons = True
    TreeViewShowHint = False
    TreeViewShowLines = True
    TreeViewShowRoot = True
    TreeViewSortType = stNone
    Visible = False
    OnKeyDown = dxLookupTreeView1KeyDown
    DividedChar = '.'
    ListSource = DM.DSdistrict
    KeyField = 'rec_id'
    ListField = 'zdesc'
    Options = [trCanDBNavigate, trSmartRecordCopy, trCheckHasChildren]
    ParentField = 'parent'
    TextStyle = tvtsShort
    Alignment = taLeftJustify
  end
  object Edit1: TEdit
    Left = 710
    Top = 28
    Width = 121
    Height = 20
    Color = 16638206
    TabOrder = 8
    Text = 'Edit1'
    Visible = False
  end
  object dxDateEdit1: TdxDateEdit
    Left = 64
    Top = 3
    Width = 90
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 9
    StyleController = StyleController
    Date = -700000.000000000000000000
  end
  object dxDateEdit2: TdxDateEdit
    Left = 192
    Top = 3
    Width = 90
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 10
    StyleController = StyleController
    Date = -700000.000000000000000000
  end
  object Edit2: TEdit
    Left = 710
    Top = 47
    Width = 121
    Height = 20
    TabOrder = 11
    Text = 'Edit2'
    Visible = False
  end
  object RadioGroup2: TRadioGroup
    Left = 542
    Top = 5
    Width = 90
    Height = 80
    Caption = #24212#25910#20108#27425#36820#27454
    ItemIndex = 0
    Items.Strings = (
      #25152#26377
      #24050#23436#25104
      #26410#23436#25104)
    TabOrder = 12
  end
  object RadioGroup3: TRadioGroup
    Left = 626
    Top = 5
    Width = 80
    Height = 80
    Caption = #36798#25104#22870
    ItemIndex = 0
    Items.Strings = (
      #25152#26377
      #24050#23436#25104
      #26410#23436#25104)
    TabOrder = 13
  end
  object dxButtonEdit4: TdxButtonEdit
    Left = 356
    Top = 4
    Width = 102
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 14
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit4ButtonClick
    ExistButtons = True
  end
  object dxEdit2: TdxEdit
    Left = 356
    Top = 67
    Width = 102
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 15
    OnClick = dxEdit2Click
    OnEnter = dxEdit2Click
    OnKeyDown = dxEdit2KeyDown
    StyleController = StyleController
  end
  object DSqry: TDataSource
    DataSet = qry
    OnDataChange = DSqryDataChange
    Left = 114
    Top = 205
  end
  object qry: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    ReadOnly = True
    RemoteServer = DM.RemoteServer
    OnCalcFields = qryCalcFields
    Left = 83
    Top = 205
    object qrydtl_id: TAutoIncField
      FieldName = 'dtl_id'
      ReadOnly = True
    end
    object qrybod_id: TIntegerField
      FieldName = 'bod_id'
    end
    object qrybod_cd: TStringField
      DisplayLabel = #21333#25454#21495
      FieldName = 'bod_cd'
    end
    object qrykind_id: TWordField
      FieldName = 'kind_id'
    end
    object qrycate_id: TWordField
      FieldName = 'cate_id'
    end
    object qrysrc_id: TIntegerField
      FieldName = 'src_id'
    end
    object qrydst_id: TIntegerField
      FieldName = 'dst_id'
    end
    object qrymate_name: TStringField
      DisplayLabel = #20379#36135#21333#20301
      FieldName = 'mate_name'
      Size = 100
    end
    object qrycustomer: TStringField
      DisplayLabel = #21307#38498
      FieldName = 'customer'
      Size = 100
    end
    object qrysta_id: TAutoIncField
      FieldName = 'sta_id'
    end
    object qrybroker: TStringField
      DisplayLabel = #19994#21153#21592
      FieldName = 'broker'
    end
    object qryagent_id: TAutoIncField
      FieldName = 'agent_id'
    end
    object qryagent: TStringField
      DisplayLabel = #20195#29702#20154
      FieldName = 'agent'
    end
    object qrybusi_type: TIntegerField
      FieldName = 'busi_type'
    end
    object qryCbusitype: TStringField
      DisplayLabel = #31867#21035
      FieldKind = fkCalculated
      FieldName = 'Cbusitype'
      Calculated = True
    end
    object qrybod_desc: TStringField
      DisplayLabel = #21333#25454#25688#35201
      FieldName = 'bod_desc'
      Size = 30
    end
    object qrymed_code: TStringField
      DisplayLabel = #32534#30721
      FieldName = 'med_code'
    end
    object qrymed_name: TStringField
      DisplayLabel = #21697#21517
      FieldName = 'med_name'
      Size = 50
    end
    object qryspecifi: TStringField
      DisplayLabel = #35268#26684
      FieldName = 'specifi'
      Size = 50
    end
    object qrypdt_place: TStringField
      DisplayLabel = #29983#20135#20225#19994
      FieldName = 'pdt_place'
      Size = 30
    end
    object qrymed_unit: TStringField
      DisplayLabel = #21333#20301
      FieldName = 'med_unit'
      Size = 30
    end
    object qrycarry_dt: TDateTimeField
      DisplayLabel = #21457#29983#26085#26399
      FieldName = 'carry_dt'
      DisplayFormat = 'yyyy-mm-dd'
    end
    object qrycreat_dt: TDateTimeField
      DisplayLabel = #26085#26399
      FieldName = 'creat_dt'
      ReadOnly = True
      DisplayFormat = 'yy-mm-dd'
    end
    object qryprice: TBCDField
      DisplayLabel = #21333#20215
      FieldName = 'price'
      ReadOnly = True
      DisplayFormat = '###,###,##0.00##;; '
      Precision = 26
      Size = 6
    end
    object qryqty: TBCDField
      DisplayLabel = #20986#24211#25968#37327
      FieldName = 'qty'
      ReadOnly = True
      DisplayFormat = '###,###,###;; '
      Precision = 26
    end
    object qryamot: TBCDField
      DisplayLabel = #37329#39069
      FieldName = 'amot'
      ReadOnly = True
      DisplayFormat = '###,###,##0.00;; '
      Precision = 26
      Size = 2
    end
    object qryprice3: TBCDField
      DisplayLabel = #21830#19994#20215
      FieldName = 'price3'
      DisplayFormat = '###,###,##0.00;; '
    end
    object qryCamot3: TFloatField
      DisplayLabel = #21830#19994#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Camot3'
      DisplayFormat = '###,###,##0.00;; '
      Calculated = True
    end
    object qrybat_cd: TStringField
      DisplayLabel = #25209#21495
      FieldName = 'bat_cd'
      Size = 30
    end
    object qryamot2a: TBCDField
      DisplayLabel = #22266#23450#37329#39069'/'#30418
      FieldName = 'amot2a'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
    end
    object qryrate: TBCDField
      DisplayLabel = #27604#29575
      FieldName = 'rate'
      DisplayFormat = '###,##0.##;; '
      EditFormat = '#####0.##;; '
      Precision = 10
      Size = 2
    end
    object qrybonus: TBCDField
      DisplayLabel = #36798#25104#22870'/'#30418
      FieldName = 'bonus'
      DisplayFormat = '###,##0.00##;; '
      EditFormat = '#####0.00;; '
      Precision = 10
      Size = 2
    end
    object qryCbusitype1: TStringField
      DisplayLabel = #19994#21153#31867#21035
      FieldKind = fkCalculated
      FieldName = 'Cbusitype1'
      Size = 10
      Calculated = True
    end
    object qryrate1: TBCDField
      DisplayLabel = #25176#31649#29575
      FieldName = 'rate1'
      DisplayFormat = '###,##0.##;; '
      EditFormat = '#####0.##;; '
      Size = 2
    end
    object qryCamot1: TBCDField
      DisplayLabel = #24212#25910#36820#20837#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Camot1'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
      Calculated = True
    end
    object qryCamot1a: TBCDField
      DisplayLabel = #23454#25910#36820#20837#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Camot1a'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
      Calculated = True
    end
    object qryCfee: TBCDField
      DisplayLabel = #25176#31649#36153
      FieldKind = fkCalculated
      FieldName = 'Cfee'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
      Calculated = True
    end
    object qryCamot2: TBCDField
      DisplayLabel = #24212#25910#22266#23450#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Camot2'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
      Calculated = True
    end
    object qryCamot4: TBCDField
      DisplayLabel = #24212#25910#20108#27425#36820#27454
      FieldKind = fkCalculated
      FieldName = 'Camot4'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
      Calculated = True
    end
    object qryCamot5: TBCDField
      DisplayLabel = #24212#25910#36798#25104#22870
      FieldKind = fkCalculated
      FieldName = 'Camot5'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
      Calculated = True
    end
    object qryamot2: TBCDField
      DisplayLabel = #24050#25910#22266#23450#37329#39069
      FieldName = 'amot2'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
    end
    object qryamot4: TBCDField
      DisplayLabel = #24050#25910#20108#27425#36820#27454
      FieldName = 'amot4'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
    end
    object qryamot5: TBCDField
      DisplayLabel = #24050#25910#36798#25104#22870
      FieldName = 'amot5'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
    end
    object qryCnotamot1: TBCDField
      DisplayLabel = #26410#25910#36820#20837#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Cnotamot1'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
      Calculated = True
    end
    object qryCnotamot2: TBCDField
      DisplayLabel = #26410#25910#22266#23450#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Cnotamot2'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
      Calculated = True
    end
    object qryCnotamot4: TBCDField
      DisplayLabel = #26410#25910#20108#27425#36820#27454
      FieldKind = fkCalculated
      FieldName = 'Cnotamot4'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
      Calculated = True
    end
    object qryCnotamot5: TBCDField
      DisplayLabel = #26410#25910#36798#25104#22870
      FieldKind = fkCalculated
      FieldName = 'Cnotamot5'
      DisplayFormat = '###,###,##0.00;; '
      Size = 2
      Calculated = True
    end
    object qryCf1: TBooleanField
      FieldKind = fkCalculated
      FieldName = 'Cf1'
      Calculated = True
    end
  end
  object StyleController: TdxEditStyleController
    BorderColor = 14065456
    BorderStyle = xbsSingle
    ButtonStyle = btsSimple
    Edges = [edgBottom]
    HotTrack = True
    Left = 321
    Top = 201
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingageYPH;Data Source=.'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select top 1 mate_id1=mate_id,mate_id2=mate_id,mate_id3=mate_id,' +
        'mate_id4=mate_id,mate_id5=mate_id,'
      
        ' mate_name1=mate_name,mate_name2=mate_name,mate_name3=mate_name,' +
        'mate_name4=mate_name,mate_name5=mate_name'
      ' from tb_busimate')
    Left = 415
    Top = 189
    object ADOQuery1mate_id1: TAutoIncField
      FieldName = 'mate_id1'
      ReadOnly = True
    end
    object ADOQuery1mate_id2: TAutoIncField
      FieldName = 'mate_id2'
      ReadOnly = True
    end
    object ADOQuery1mate_id3: TAutoIncField
      FieldName = 'mate_id3'
      ReadOnly = True
    end
    object ADOQuery1mate_id4: TAutoIncField
      FieldName = 'mate_id4'
      ReadOnly = True
    end
    object ADOQuery1mate_id5: TAutoIncField
      FieldName = 'mate_id5'
      ReadOnly = True
    end
    object ADOQuery1mate_name1: TStringField
      FieldName = 'mate_name1'
      Size = 100
    end
    object ADOQuery1mate_name2: TStringField
      FieldName = 'mate_name2'
      Size = 100
    end
    object ADOQuery1mate_name3: TStringField
      FieldName = 'mate_name3'
      Size = 100
    end
    object ADOQuery1mate_name4: TStringField
      FieldName = 'mate_name4'
      Size = 100
    end
    object ADOQuery1mate_name5: TStringField
      FieldName = 'mate_name5'
      Size = 100
    end
  end
  object PopupMenu1: TPopupMenu
    Left = 179
    Top = 205
    object N7: TMenuItem
      Caption = #29983#25104#22266#23450#24212#25910#25910#27454#21333
      OnClick = N7Click
    end
    object N8: TMenuItem
      Caption = #29983#25104#24212#25910#20108#27425#25910#27454#21333
      OnClick = N8Click
    end
    object N10: TMenuItem
      Caption = #29983#25104#36798#25104#22870#25910#27454#21333
      OnClick = N10Click
    end
    object N9: TMenuItem
      Caption = '-'
    end
    object N3: TMenuItem
      Caption = #26597#25214'...'
      OnClick = N3Click
    end
    object N4: TMenuItem
      Caption = '-'
    end
    object N1: TMenuItem
      Caption = #38544#34255#26412#21015
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #26174#31034#25152#26377#21015
      OnClick = N2Click
    end
    object N5: TMenuItem
      Caption = '-'
    end
    object N6: TMenuItem
      Caption = #26684#24335#22797#21407
      OnClick = N6Click
    end
  end
  object ImageList: TImageList
    Left = 472
    Top = 71
    Bitmap = {
      
      000000000000}
  end
  object pubqry: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    Left = 308
    Top = 277
  end
end
