object setdrawrate: Tsetdrawrate
  Left = 97
  Top = 105
  AutoScroll = False
  Caption = #25237#36164#27454#35745#31639#34920
  ClientHeight = 514
  ClientWidth = 914
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  DesignSize = (
    914
    514)
  PixelsPerInch = 96
  TextHeight = 12
  object Shape1: TShape
    Left = 121
    Top = 4
    Width = 60
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label25: TLabel
    Left = 893
    Top = 7
    Width = 7
    Height = 12
    Alignment = taRightJustify
    Anchors = [akTop, akRight]
    Font.Charset = GB2312_CHARSET
    Font.Color = clMaroon
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = [fsBold]
    ParentFont = False
  end
  object SpeedButton2: TSpeedButton
    Left = 732
    Top = 0
    Width = 25
    Height = 25
    Hint = #36716#20986#25253#34920
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton2Click
  end
  object Shape4: TShape
    Left = 1
    Top = 4
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label3: TLabel
    Left = 15
    Top = 10
    Width = 34
    Height = 14
    AutoSize = False
    Caption = #24180' '#24230
    Transparent = True
    WordWrap = True
  end
  object SpeedButton5: TSpeedButton
    Left = 656
    Top = 28
    Width = 251
    Height = 25
    Hint = #21047#26032
    Anchors = [akTop, akRight]
    Caption = '&R '#21047#26032
    Glyph.Data = {
      
      3393333777333777337333333999993333333333377777333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Spacing = 10
    OnClick = SpeedButton5Click
  end
  object SpeedButton1: TSpeedButton
    Left = 706
    Top = 0
    Width = 25
    Height = 25
    Hint = #25171#21360#36755#20986
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      33333337FFFF7733333333300000033333333337777773333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
  end
  object Shape3: TShape
    Left = 1
    Top = 34
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label1: TLabel
    Left = 6
    Top = 40
    Width = 51
    Height = 14
    AutoSize = False
    Caption = #21457#29983#26376#20221
    Transparent = True
    WordWrap = True
  end
  object SpeedButton3: TSpeedButton
    Left = 385
    Top = 0
    Width = 137
    Height = 25
    Hint = #35745#31639#25237#36164#27454
    Caption = #35745#31639#25237#36164#27454
    Glyph.Data = {
      
      03333373FFFFFFFF733333700000000073333337777777773333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    OnClick = SpeedButton3Click
  end
  object SpeedButton4: TSpeedButton
    Left = 681
    Top = 0
    Width = 25
    Height = 25
    Hint = #20445#23384#24405#20837
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      5990555555555555577755555555555555555555555555555555}
    Layout = blGlyphTop
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Spacing = 2
    OnClick = SpeedButton4Click
  end
  object SpeedButton6: TSpeedButton
    Left = 656
    Top = 0
    Width = 25
    Height = 25
    Hint = #25918#24323#24405#20837
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333333333333333333333333333333333333333333333333}
    Layout = blGlyphTop
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Spacing = 2
    OnClick = SpeedButton6Click
  end
  object Label2: TLabel
    Left = 136
    Top = 10
    Width = 30
    Height = 12
    Caption = #22320' '#21306
    Transparent = True
  end
  object SpeedButton7: TSpeedButton
    Left = 385
    Top = 28
    Width = 137
    Height = 25
    Hint = #35745#31639#25237#36164#27454
    Caption = #35745#31639#24403#21069#35760#24405#25237#36164#27454
    Enabled = False
    Glyph.Data = {
      
      3000333333333333377733333333333333333333333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    OnClick = SpeedButton7Click
  end
  object dxDBGrid1: TdxDBGrid
    Left = 0
    Top = 61
    Width = 914
    Height = 453
    Bands = <
      item
      end>
    DefaultLayout = True
    HeaderPanelRowCount = 1
    KeyField = 'rec_id'
    ShowGroupPanel = True
    ShowSummaryFooter = True
    SummaryGroups = <
      item
        DefaultGroup = True
        SummaryItems = <
          item
            ColumnName = 'dxDBGrid1qty'
            SummaryField = 'qty'
            SummaryFormat = '###,###,##0;; '
            SummaryType = cstSum
          end>
        Name = 'dxDBGrid1SummaryGroup2'
      end>
    SummarySeparator = ', '
    Align = alBottom
    PopupMenu = PopupMenu1
    TabOrder = 0
    DataSource = DSdrawlog
    Filter.Active = True
    Filter.DropDownCount = 30
    Filter.Criteria = {00000000}
    HighlightColor = clAqua
    HighlightTextColor = clBlack
    LookAndFeel = lfUltraFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabThrough, edgoVertThrough]
    OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
    ShowRowFooter = True
    OnCustomDrawCell = dxDBGrid1CustomDrawCell
    Anchors = [akLeft, akTop, akRight, akBottom]
    object dxDBGrid1district: TdxDBGridColumn
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 85
      BandIndex = 0
      RowIndex = 0
      FieldName = 'district'
      SummaryFooterType = cstCount
      SummaryFooterFormat = '###,##0'#31508';; '
      SummaryField = 'rec_id'
    end
    object dxDBGrid1broker: TdxDBGridMaskColumn
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clPurple
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 57
      BandIndex = 0
      RowIndex = 0
      FieldName = 'broker'
    end
    object dxDBGrid1customer: TdxDBGridMaskColumn
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 120
      BandIndex = 0
      RowIndex = 0
      FieldName = 'customer'
    end
    object dxDBGrid1med_code: TdxDBGridMaskColumn
      Alignment = taCenter
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clRed
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_code'
    end
    object dxDBGrid1med_name: TdxDBGridMaskColumn
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 120
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_name'
    end
    object dxDBGrid1chm_name: TdxDBGridColumn
      DisableEditor = True
      Width = 65
      BandIndex = 0
      RowIndex = 0
      FieldName = 'chm_name'
    end
    object dxDBGrid1specifi: TdxDBGridMaskColumn
      DisableDragging = True
      DisableEditor = True
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'specifi'
      DisableFilter = True
    end
    object dxDBGrid1pdt_place: TdxDBGridMaskColumn
      DisableEditor = True
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pdt_place'
    end
    object dxDBGrid1med_unit: TdxDBGridMaskColumn
      Alignment = taCenter
      DisableDragging = True
      DisableEditor = True
      Width = 30
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_unit'
      DisableFilter = True
    end
    object dxDBGrid1qty: TdxDBGridMaskColumn
      Color = 15132390
      DisableDragging = True
      DisableEditor = True
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'qty'
      SummaryFooterType = cstSum
      SummaryFooterField = 'qty'
      SummaryFooterFormat = '###,###,##0;; '
      DisableFilter = True
      SummaryField = 'qty'
    end
    object dxDBGrid1rate: TdxDBGridMaskColumn
      DisableDragging = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'rate'
      DisableFilter = True
    end
    object dxDBGrid1Camot: TdxDBGridColumn
      DisableEditor = True
      Width = 90
      BandIndex = 0
      RowIndex = 0
      FieldName = 'Camot'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Camot'
      SummaryFooterFormat = '###,###,#0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Camot'
      SummaryFormat = '###,###,#0.00;; '
    end
  end
  object wwDBNavigator2: TwwDBNavigator
    Left = 757
    Top = 0
    Width = 150
    Height = 25
    DataSource = DSdrawlog
    Options = []
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    Anchors = [akTop, akRight]
    ParentShowHint = False
    object wwNavButton1: TwwNavButton
      Left = 0
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 0
      Style = nbsFirst
    end
    object wwNavButton2: TwwNavButton
      Left = 25
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Prior'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 1
      Style = nbsPrior
    end
    object wwNavButton3: TwwNavButton
      Left = 50
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Next'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 2
      Style = nbsNext
    end
    object wwNavButton4: TwwNavButton
      Left = 75
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 3
      Style = nbsLast
    end
    object wwNavButton5: TwwNavButton
      Left = 100
      Top = 0
      Width = 25
      Height = 25
      Hint = #35774#32622#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1SaveBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 4
      Style = nbsSaveBookmark
    end
    object wwNavButton6: TwwNavButton
      Left = 125
      Top = 0
      Width = 25
      Height = 25
      Hint = #36820#22238#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1RestoreBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 5
      Style = nbsRestoreBookmark
    end
  end
  object dxSpinEdit1: TdxSpinEdit
    Left = 65
    Top = 7
    Width = 50
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 2
    MaxValue = 3000.000000000000000000
    MinValue = 1900.000000000000000000
    Value = 2004.000000000000000000
    StoredValues = 48
  end
  object dxSpinEdit2: TdxSpinEdit
    Left = 65
    Top = 35
    Width = 50
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 3
    MaxValue = 12.000000000000000000
    MinValue = 1.000000000000000000
    Value = 9.000000000000000000
    StoredValues = 48
  end
  object CheckBox1: TdxCheckEdit
    Left = 121
    Top = 36
    Width = 132
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    Style.Edges = []
    TabOrder = 4
    Visible = False
    Caption = #20165#35745#31639#26080#25237#36164#27454#35760#24405
  end
  object dxLookupTreeView1: TdxLookupTreeView
    Left = 182
    Top = 6
    Width = 120
    Height = 20
    CanSelectParents = True
    DropDownRows = 30
    ParentColor = False
    TabOrder = 5
    TabStop = True
    TreeViewColor = clWindow
    TreeViewCursor = crDefault
    TreeViewFont.Charset = DEFAULT_CHARSET
    TreeViewFont.Color = clWindowText
    TreeViewFont.Height = -11
    TreeViewFont.Name = 'MS Sans Serif'
    TreeViewFont.Style = []
    TreeViewImages = ImageList
    TreeViewIndent = 19
    TreeViewReadOnly = True
    TreeViewShowButtons = True
    TreeViewShowHint = False
    TreeViewShowLines = True
    TreeViewShowRoot = True
    TreeViewSortType = stNone
    OnKeyDown = dxLookupTreeView1KeyDown
    DividedChar = '.'
    ListSource = DM.DSdistrict
    KeyField = 'rec_id'
    ListField = 'zdesc'
    Options = [trCanDBNavigate, trSmartRecordCopy, trCheckHasChildren]
    ParentField = 'parent'
    TextStyle = tvtsShort
    Alignment = taLeftJustify
  end
  object DSdrawlog: TDataSource
    DataSet = drawlog
    OnDataChange = DSdrawlogDataChange
    Left = 381
    Top = 208
  end
  object drawlog: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdrawlog'
    RemoteServer = DM.RemoteServer
    BeforeEdit = drawlogBeforeEdit
    AfterPost = drawlogAfterPost
    OnCalcFields = drawlogCalcFields
    Left = 352
    Top = 208
    object drawlogrec_id: TAutoIncField
      FieldName = 'rec_id'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      ReadOnly = True
    end
    object drawlogsta_id: TIntegerField
      FieldName = 'sta_id'
    end
    object drawlogmate_id: TIntegerField
      FieldName = 'mate_id'
    end
    object drawlogmed_id: TIntegerField
      FieldName = 'med_id'
    end
    object drawlogfyear: TIntegerField
      FieldName = 'fyear'
    end
    object drawlogfmonth: TWordField
      FieldName = 'fmonth'
    end
    object drawlogqty: TBCDField
      DisplayLabel = #38144#21806#25968#37327
      FieldName = 'qty'
      DisplayFormat = '###,###,##0;; '
      Precision = 15
    end
    object drawlograte: TBCDField
      DisplayLabel = #25237#36164#27454
      FieldName = 'rate'
      DisplayFormat = '###,###,#0.00;; '
      Precision = 15
    end
    object drawlogbroker: TStringField
      DisplayLabel = #19994#21153#21592
      FieldName = 'broker'
    end
    object drawlogdist_id: TIntegerField
      FieldName = 'dist_id'
    end
    object drawlogdistrict: TStringField
      DisplayLabel = #22320#21306
      FieldName = 'district'
      ReadOnly = True
      Size = 100
    end
    object drawlogcustomer: TStringField
      DisplayLabel = #23458#25143
      FieldName = 'customer'
      Size = 100
    end
    object drawlogmed_code: TStringField
      DisplayLabel = #32534#30721
      FieldName = 'med_code'
    end
    object drawlogmed_name: TStringField
      DisplayLabel = #36890#29992#21517
      FieldName = 'med_name'
      Size = 50
    end
    object drawlogchm_name: TStringField
      DisplayLabel = #21830#21697#21517
      FieldName = 'chm_name'
      Size = 50
    end
    object drawlogspecifi: TStringField
      DisplayLabel = #35268#26684
      FieldName = 'specifi'
      Size = 50
    end
    object drawlogmed_unit: TStringField
      DisplayLabel = #21333#20301
      FieldName = 'med_unit'
      Size = 30
    end
    object drawlogpdt_place: TStringField
      DisplayLabel = #29983#20135#20225#19994
      FieldName = 'pdt_place'
      Size = 30
    end
    object drawlogqtyperpack: TBCDField
      DisplayLabel = #21253#35013#35268#26684
      FieldName = 'qtyperpack'
      DisplayFormat = '###,##0;; '
      Precision = 18
      Size = 0
    end
    object drawlogCamot: TFloatField
      DisplayLabel = #25237#36164#27454#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Camot'
      DisplayFormat = '###,###,#0.00;; '
      Calculated = True
    end
  end
  object StyleController: TdxEditStyleController
    BorderColor = 14065456
    BorderStyle = xbsSingle
    ButtonStyle = btsSimple
    Edges = [edgBottom]
    HotTrack = True
    Left = 321
    Top = 201
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingAGE'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select a.*,broker=d.zname,customer=e.mate_name,dist_id=e.distric' +
        't,district=dbo.fn_getdistrict(e.district),b.med_code,b.med_name,' +
        'b.chm_name,b.specifi,med_unit=c.zdesc,b.pdt_place,b.qtyperpack'
      
        ' from tb_drawlog a,tb_medicine b,tb_object c,tb_staff d,tb_busim' +
        'ate e'
      
        ' where a.med_id=b.med_id and b.unit_id*=c.obj_id and a.sta_id*=d' +
        '.sta_id and a.mate_id*=e.mate_id'
      '  and fyear=2004 and fmonth=8')
    Left = 509
    Top = 52
    object ADOQuery1rec_id: TAutoIncField
      FieldName = 'rec_id'
      ReadOnly = True
    end
    object ADOQuery1sta_id: TIntegerField
      FieldName = 'sta_id'
    end
    object ADOQuery1mate_id: TIntegerField
      FieldName = 'mate_id'
    end
    object ADOQuery1med_id: TIntegerField
      FieldName = 'med_id'
    end
    object ADOQuery1fyear: TIntegerField
      FieldName = 'fyear'
    end
    object ADOQuery1fmonth: TWordField
      FieldName = 'fmonth'
    end
    object ADOQuery1qty: TBCDField
      FieldName = 'qty'
      Precision = 15
    end
    object ADOQuery1rate: TBCDField
      FieldName = 'rate'
      Precision = 15
    end
    object ADOQuery1broker: TStringField
      FieldName = 'broker'
    end
    object ADOQuery1customer: TStringField
      FieldName = 'customer'
      Size = 100
    end
    object ADOQuery1dist_id: TIntegerField
      FieldName = 'dist_id'
    end
    object ADOQuery1district: TStringField
      FieldName = 'district'
      ReadOnly = True
      Size = 100
    end
    object ADOQuery1med_code: TStringField
      FieldName = 'med_code'
    end
    object ADOQuery1med_name: TStringField
      FieldName = 'med_name'
      Size = 50
    end
    object ADOQuery1chm_name: TStringField
      FieldName = 'chm_name'
      Size = 50
    end
    object ADOQuery1specifi: TStringField
      FieldName = 'specifi'
      Size = 50
    end
    object ADOQuery1med_unit: TStringField
      FieldName = 'med_unit'
      Size = 30
    end
    object ADOQuery1pdt_place: TStringField
      FieldName = 'pdt_place'
      Size = 30
    end
    object ADOQuery1qtyperpack: TBCDField
      FieldName = 'qtyperpack'
      Precision = 18
      Size = 0
    end
  end
  object pickdist: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubquery'
    RemoteServer = DM.RemoteServer
    Left = 589
    Top = 430
  end
  object ImageList: TImageList
    Left = 327
    Top = 30
    Bitmap = {
      
      000000000000}
  end
  object PopupMenu1: TPopupMenu
    Left = 133
    Top = 175
    object N3: TMenuItem
      Caption = #26597#25214'...'
      OnClick = N3Click
    end
    object N4: TMenuItem
      Caption = '-'
    end
    object N1: TMenuItem
      Caption = #38544#34255#26412#21015
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #26174#31034#25152#26377#21015
      OnClick = N2Click
    end
    object N5: TMenuItem
      Caption = '-'
    end
    object N6: TMenuItem
      Caption = #26684#24335#22797#21407
      OnClick = N6Click
    end
  end
end
