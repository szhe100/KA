object setdrawrate1: Tsetdrawrate1
  Left = 89
  Top = 93
  AutoScroll = False
  Caption = #25237#36164#27454#35745#31639#34920
  ClientHeight = 514
  ClientWidth = 1174
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  DesignSize = (
    1174
    514)
  PixelsPerInch = 96
  TextHeight = 12
  object Shape1: TShape
    Left = 246
    Top = 1
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object SpeedButton2: TSpeedButton
    Left = 991
    Top = 0
    Width = 25
    Height = 25
    Hint = #36716#20986#25253#34920
    Anchors = [akTop, akRight]
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton2Click
  end
  object Shape4: TShape
    Left = 1
    Top = 1
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label3: TLabel
    Left = 13
    Top = 7
    Width = 41
    Height = 14
    AutoSize = False
    Caption = #24180'  '#24230
    Transparent = True
    WordWrap = True
  end
  object SpeedButton5: TSpeedButton
    Left = 991
    Top = 86
    Width = 175
    Height = 25
    Hint = #21047#26032
    Anchors = [akTop, akRight]
    Caption = '&R '#21047#26032
    Glyph.Data = {
      
      3393333777333777337333333999993333333333377777333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Spacing = 10
    OnClick = SpeedButton5Click
  end
  object Shape3: TShape
    Left = 127
    Top = 2
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label1: TLabel
    Left = 143
    Top = 7
    Width = 33
    Height = 14
    AutoSize = False
    Caption = #26376' '#20221
    Transparent = True
    WordWrap = True
  end
  object SpeedButton3: TSpeedButton
    Left = 542
    Top = 4
    Width = 137
    Height = 25
    Hint = #35745#31639#25237#36164#27454
    Caption = #35745#31639#25237#36164#27454
    Glyph.Data = {
      
      03333373FFFFFFFF733333700000000073333337777777773333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Visible = False
    OnClick = SpeedButton3Click
  end
  object Label2: TLabel
    Left = 261
    Top = 6
    Width = 30
    Height = 12
    Caption = #22320' '#21306
    Transparent = True
  end
  object SpeedButton7: TSpeedButton
    Left = 542
    Top = 32
    Width = 137
    Height = 25
    Hint = #35745#31639#25237#36164#27454
    Caption = #35745#31639#24403#21069#35760#24405#25237#36164#27454
    Enabled = False
    Glyph.Data = {
      
      3000333333333333377733333333333333333333333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Visible = False
    OnClick = SpeedButton7Click
  end
  object Shape6: TShape
    Left = 1
    Top = 46
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Shape2: TShape
    Left = 1
    Top = 68
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label4: TLabel
    Left = 13
    Top = 73
    Width = 36
    Height = 12
    Caption = #21307'  '#38498
    Transparent = True
  end
  object Label7: TLabel
    Left = 13
    Top = 52
    Width = 36
    Height = 12
    Caption = #19994#21153#21592
    Transparent = True
  end
  object Shape5: TShape
    Left = 1
    Top = 90
    Width = 60
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label5: TLabel
    Left = 13
    Top = 96
    Width = 36
    Height = 12
    Caption = #21697'  '#31181
    Transparent = True
  end
  object Shape7: TShape
    Left = 1
    Top = 23
    Width = 60
    Height = 23
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label6: TLabel
    Left = 13
    Top = 29
    Width = 36
    Height = 12
    Caption = #37197#36865#21830
    Transparent = True
  end
  object Label8: TLabel
    Left = 249
    Top = 95
    Width = 498
    Height = 12
    Caption = #27880#65306'<Ctrl>+'#40736#26631#28857#20987#21487#22810#36873';<Shift>+'#40736#26631#28857#20987#21487#25104#27573#36873#25321';'#21491#20987#21487#37325#26032#35745#31639#25152#36873#21333#25454#30340#25237#36164#27454
    Font.Charset = GB2312_CHARSET
    Font.Color = clNavy
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
  end
  object dxDBGrid1: TdxDBGrid
    Left = 0
    Top = 115
    Width = 1174
    Height = 399
    Bands = <
      item
      end>
    DefaultLayout = True
    HeaderPanelRowCount = 1
    KeyField = 'dtl_id'
    ShowGroupPanel = True
    ShowSummaryFooter = True
    SummaryGroups = <
      item
        DefaultGroup = True
        SummaryItems = <
          item
            ColumnName = 'dxDBGrid1qty'
            SummaryField = 'qty'
            SummaryFormat = '###,###,##0;; '
            SummaryType = cstSum
          end>
        Name = 'dxDBGrid1SummaryGroup2'
      end>
    SummarySeparator = ', '
    Align = alBottom
    PopupMenu = PopupMenu1
    TabOrder = 0
    DataSource = DSdrawlog
    Filter.Active = True
    Filter.DropDownCount = 30
    Filter.Criteria = {00000000}
    HighlightColor = clAqua
    HighlightTextColor = clBlack
    LookAndFeel = lfUltraFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSelect, edgoMultiSort, edgoTabThrough, edgoVertThrough]
    OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
    ShowRowFooter = True
    OnCustomDrawCell = dxDBGrid1CustomDrawCell
    Anchors = [akLeft, akTop, akRight, akBottom]
    object dxDBGrid1carry_dt: TdxDBGridColumn
      Alignment = taCenter
      DisableEditor = True
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'carry_dt'
    end
    object dxDBGrid1bod_cd: TdxDBGridColumn
      Alignment = taCenter
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clPurple
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'bod_cd'
      DisableFilter = True
    end
    object dxDBGrid1agent: TdxDBGridColumn
      DisableEditor = True
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'agent'
    end
    object dxDBGrid1broker: TdxDBGridMaskColumn
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 57
      BandIndex = 0
      RowIndex = 0
      FieldName = 'broker'
    end
    object dxDBGrid1district: TdxDBGridColumn
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 85
      BandIndex = 0
      RowIndex = 0
      FieldName = 'district'
      SummaryFooterType = cstCount
      SummaryFooterFormat = '###,##0'#31508';; '
      SummaryField = 'rec_id'
    end
    object dxDBGrid1customer: TdxDBGridMaskColumn
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 150
      BandIndex = 0
      RowIndex = 0
      FieldName = 'customer'
    end
    object dxDBGrid1med_code: TdxDBGridMaskColumn
      Alignment = taCenter
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clRed
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 60
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_code'
    end
    object dxDBGrid1med_name: TdxDBGridMaskColumn
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 120
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_name'
    end
    object dxDBGrid1chm_name: TdxDBGridColumn
      DisableEditor = True
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'chm_name'
    end
    object dxDBGrid1specifi: TdxDBGridMaskColumn
      DisableDragging = True
      DisableEditor = True
      Width = 90
      BandIndex = 0
      RowIndex = 0
      FieldName = 'specifi'
      DisableFilter = True
    end
    object dxDBGrid1pdt_place: TdxDBGridMaskColumn
      DisableEditor = True
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pdt_place'
    end
    object dxDBGrid1med_unit: TdxDBGridMaskColumn
      Alignment = taCenter
      DisableDragging = True
      DisableEditor = True
      Width = 30
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_unit'
      DisableFilter = True
    end
    object dxDBGrid1qty: TdxDBGridMaskColumn
      Color = 15132390
      DisableDragging = True
      DisableEditor = True
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'qty'
      SummaryFooterType = cstSum
      SummaryFooterField = 'qty'
      SummaryFooterFormat = '###,###,##0;; '
      DisableFilter = True
      SummaryField = 'qty'
    end
    object dxDBGrid1dtlrate: TdxDBGridMaskColumn
      DisableDragging = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 55
      BandIndex = 0
      RowIndex = 0
      FieldName = 'dtlrate'
      DisableFilter = True
    end
    object dxDBGrid1Camot: TdxDBGridColumn
      DisableEditor = True
      Width = 90
      BandIndex = 0
      RowIndex = 0
      FieldName = 'Camot'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Camot'
      SummaryFooterFormat = '###,###,#0.00;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Camot'
      SummaryFormat = '###,###,#0.00;; '
    end
    object dxDBGrid1bod_id: TdxDBGridColumn
      Visible = False
      BandIndex = 0
      RowIndex = 0
      FieldName = 'bod_id'
    end
  end
  object dxSpinEdit1: TdxSpinEdit
    Left = 63
    Top = 4
    Width = 50
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 1
    MaxValue = 3000.000000000000000000
    MinValue = 1900.000000000000000000
    Value = 2004.000000000000000000
    StoredValues = 48
  end
  object dxSpinEdit2: TdxSpinEdit
    Left = 191
    Top = 3
    Width = 40
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 2
    MaxValue = 12.000000000000000000
    MinValue = 1.000000000000000000
    Value = 9.000000000000000000
    StoredValues = 48
  end
  object CheckBox1: TdxCheckEdit
    Left = 360
    Top = 31
    Width = 132
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    Style.Edges = []
    TabOrder = 3
    Visible = False
    Caption = #20165#35745#31639#26080#25237#36164#27454#35760#24405
  end
  object dxLookupTreeView1: TdxLookupTreeView
    Left = 307
    Top = 3
    Width = 120
    Height = 20
    CanSelectParents = True
    DropDownRows = 30
    ParentColor = False
    TabOrder = 4
    TabStop = True
    TreeViewColor = clWindow
    TreeViewCursor = crDefault
    TreeViewFont.Charset = DEFAULT_CHARSET
    TreeViewFont.Color = clWindowText
    TreeViewFont.Height = -11
    TreeViewFont.Name = 'MS Sans Serif'
    TreeViewFont.Style = []
    TreeViewImages = ImageList
    TreeViewIndent = 19
    TreeViewReadOnly = True
    TreeViewShowButtons = True
    TreeViewShowHint = False
    TreeViewShowLines = True
    TreeViewShowRoot = True
    TreeViewSortType = stNone
    OnKeyDown = dxLookupTreeView1KeyDown
    DividedChar = '.'
    ListSource = DM.DSdistrict
    KeyField = 'rec_id'
    ListField = 'zdesc'
    Options = [trCanDBNavigate, trSmartRecordCopy, trCheckHasChildren]
    ParentField = 'parent'
    TextStyle = tvtsShort
    Alignment = taLeftJustify
  end
  object wwDBNavigator1: TwwDBNavigator
    Left = 1016
    Top = 0
    Width = 100
    Height = 25
    DataSource = DSdrawlog
    Options = []
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    Anchors = [akTop, akRight]
    ParentShowHint = False
    object wwDBNavigator1First: TwwNavButton
      Left = 0
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 0
      Style = nbsFirst
    end
    object wwDBNavigator1Prior: TwwNavButton
      Left = 25
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Prior'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 1
      Style = nbsPrior
    end
    object wwDBNavigator1Next: TwwNavButton
      Left = 50
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Next'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 2
      Style = nbsNext
    end
    object wwDBNavigator1Last: TwwNavButton
      Left = 75
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 3
      Style = nbsLast
    end
  end
  object dxButtonEdit1: TdxButtonEdit
    Left = 63
    Top = 69
    Width = 167
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 6
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit1ButtonClick
    ExistButtons = True
  end
  object dxButtonEdit2: TdxButtonEdit
    Left = 63
    Top = 91
    Width = 167
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 7
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit2ButtonClick
    ExistButtons = True
  end
  object dxButtonEdit4: TdxButtonEdit
    Left = 63
    Top = 47
    Width = 167
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 8
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit4ButtonClick
    ExistButtons = True
  end
  object dxButtonEdit3: TdxButtonEdit
    Left = 63
    Top = 25
    Width = 167
    Style.ButtonTransparence = ebtAlways
    Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
    TabOrder = 9
    StyleController = StyleController
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit3ButtonClick
    ExistButtons = True
  end
  object Edit1: TEdit
    Left = 712
    Top = 47
    Width = 121
    Height = 20
    TabOrder = 10
    Text = 'Edit1'
    Visible = False
  end
  object DSdrawlog: TDataSource
    DataSet = drawlog
    OnDataChange = DSdrawlogDataChange
    Left = 381
    Top = 208
  end
  object drawlog: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    BeforeEdit = drawlogBeforeEdit
    AfterPost = drawlogAfterPost
    OnCalcFields = drawlogCalcFields
    Left = 351
    Top = 209
    object drawlogdtl_id: TAutoIncField
      FieldName = 'dtl_id'
      ReadOnly = True
    end
    object drawlogbod_id: TIntegerField
      FieldName = 'bod_id'
    end
    object drawlogbod_cd: TStringField
      DisplayLabel = #21333#25454#21495
      FieldName = 'bod_cd'
    end
    object drawlogagent: TStringField
      DisplayLabel = #37197#36865#21830
      FieldName = 'agent'
      ReadOnly = True
      Size = 100
    end
    object drawlogcarry_dt: TDateTimeField
      DisplayLabel = #26085#26399
      FieldName = 'carry_dt'
      DisplayFormat = 'yy-mm-dd'
    end
    object drawlogsta_id: TIntegerField
      FieldName = 'sta_id'
    end
    object drawlogmate_id: TIntegerField
      FieldName = 'mate_id'
    end
    object drawlogmed_id: TIntegerField
      FieldName = 'med_id'
    end
    object drawlogqty: TBCDField
      DisplayLabel = #38144#21806#25968#37327
      FieldName = 'qty'
      DisplayFormat = '###,###,##0;; '
      Precision = 15
    end
    object drawlogdtlrate: TBCDField
      DisplayLabel = #25237#36164#27454
      FieldName = 'dtlrate'
      DisplayFormat = '###,###,#0.00;; '
      Precision = 15
    end
    object drawlogbroker: TStringField
      DisplayLabel = #19994#21153#21592
      FieldName = 'broker'
    end
    object drawlogdist_id: TIntegerField
      FieldName = 'dist_id'
    end
    object drawlogdistrict: TStringField
      DisplayLabel = #22320#21306
      FieldName = 'district'
      ReadOnly = True
      Size = 100
    end
    object drawlogcustomer: TStringField
      DisplayLabel = #21307#38498
      FieldName = 'customer'
      Size = 100
    end
    object drawlogmed_code: TStringField
      DisplayLabel = #32534#30721
      FieldName = 'med_code'
    end
    object drawlogmed_name: TStringField
      DisplayLabel = #36890#29992#21517
      FieldName = 'med_name'
      Size = 50
    end
    object drawlogchm_name: TStringField
      DisplayLabel = #21830#21697#21517
      FieldName = 'chm_name'
      Size = 50
    end
    object drawlogspecifi: TStringField
      DisplayLabel = #35268#26684
      FieldName = 'specifi'
      Size = 50
    end
    object drawlogmed_unit: TStringField
      DisplayLabel = #21333#20301
      FieldName = 'med_unit'
      Size = 30
    end
    object drawlogpdt_place: TStringField
      DisplayLabel = #29983#20135#20225#19994
      FieldName = 'pdt_place'
      Size = 30
    end
    object drawlogqtyperpack: TBCDField
      DisplayLabel = #21253#35013#35268#26684
      FieldName = 'qtyperpack'
      DisplayFormat = '###,##0;; '
      Precision = 18
      Size = 0
    end
    object drawlogCamot: TFloatField
      DisplayLabel = #25237#36164#27454#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Camot'
      DisplayFormat = '###,###,#0.00;; '
      Calculated = True
    end
  end
  object StyleController: TdxEditStyleController
    BorderColor = 14065456
    BorderStyle = xbsSingle
    ButtonStyle = btsSimple
    Edges = [edgBottom]
    HotTrack = True
    Left = 321
    Top = 201
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingAGE'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select top 10 b.dtl_id,a.bod_cd,a.carry_dt,b.sta_id,mate_id=a.ds' +
        't_id,b.med_id,broker=dbo.fn_staff_name(b.sta_id),customer=e.mate' +
        '_name,'
      #9'dist_id=e.district,district=dbo.fn_getreetitle(e.district),'
      #9'b.qty,b.dtlrate,'
      
        #9'm.med_code,m.med_name,m.chm_name,m.specifi,med_unit=dbo.fn_obj_' +
        'desc(m.unit_id),m.pdt_place,m.qtyperpack'
      ' from tb_bill a,tb_bill_dtl b,tb_medicine m,tb_busimate e'
      
        ' where a.bod_id=b.bod_id and b.med_id=m.med_id and a.dst_id=e.ma' +
        'te_id')
    Left = 510
    Top = 197
    object ADOQuery1dtl_id: TAutoIncField
      FieldName = 'dtl_id'
      ReadOnly = True
    end
    object ADOQuery1bod_cd: TStringField
      DisplayLabel = #21333#25454#21495
      FieldName = 'bod_cd'
    end
    object ADOQuery1carry_dt: TDateTimeField
      DisplayLabel = #26085#26399
      FieldName = 'carry_dt'
      DisplayFormat = 'yyyy-mm-dd'
    end
    object ADOQuery1sta_id: TIntegerField
      FieldName = 'sta_id'
    end
    object ADOQuery1mate_id: TIntegerField
      FieldName = 'mate_id'
    end
    object ADOQuery1med_id: TIntegerField
      FieldName = 'med_id'
    end
    object ADOQuery1qty: TBCDField
      DisplayLabel = #38144#21806#25968#37327
      FieldName = 'qty'
      DisplayFormat = '###,###,##0;; '
      Precision = 15
    end
    object ADOQuery1dtlrate: TBCDField
      FieldName = 'dtlrate'
      Precision = 15
    end
    object ADOQuery1broker: TStringField
      DisplayLabel = #19994#21153#21592
      FieldName = 'broker'
    end
    object ADOQuery1dist_id: TIntegerField
      FieldName = 'dist_id'
    end
    object ADOQuery1district: TStringField
      DisplayLabel = #22320#21306
      FieldName = 'district'
      ReadOnly = True
      Size = 100
    end
    object ADOQuery1customer: TStringField
      DisplayLabel = #23458#25143
      FieldName = 'customer'
      Size = 100
    end
    object ADOQuery1med_code: TStringField
      DisplayLabel = #32534#30721
      FieldName = 'med_code'
    end
    object ADOQuery1med_name: TStringField
      DisplayLabel = #36890#29992#21517
      FieldName = 'med_name'
      Size = 50
    end
    object ADOQuery1chm_name: TStringField
      DisplayLabel = #21830#21697#21517
      FieldName = 'chm_name'
      Size = 50
    end
    object ADOQuery1specifi: TStringField
      DisplayLabel = #35268#26684
      FieldName = 'specifi'
      Size = 50
    end
    object ADOQuery1med_unit: TStringField
      DisplayLabel = #21333#20301
      FieldName = 'med_unit'
      Size = 30
    end
    object ADOQuery1pdt_place: TStringField
      DisplayLabel = #29983#20135#20225#19994
      FieldName = 'pdt_place'
      Size = 30
    end
    object ADOQuery1qtyperpack: TBCDField
      DisplayLabel = #21253#35013#35268#26684
      FieldName = 'qtyperpack'
      DisplayFormat = '###,##0;; '
      Precision = 18
      Size = 0
    end
    object ADOQuery1Camot: TFloatField
      DisplayLabel = #25237#36164#27454#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Camot'
      DisplayFormat = '###,###,#0.00;; '
      Calculated = True
    end
  end
  object pickdist: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubquery'
    RemoteServer = DM.RemoteServer
    Left = 504
    Top = 58
  end
  object ImageList: TImageList
    Left = 327
    Top = 27
    Bitmap = {
      
      000000000000}
  end
  object PopupMenu1: TPopupMenu
    Left = 133
    Top = 175
    object N7: TMenuItem
      Caption = #37325#26032#35745#31639#25237#36164#27454
      OnClick = N7Click
    end
    object N8: TMenuItem
      Caption = '-'
    end
    object N3: TMenuItem
      Caption = #26597#25214'...'
      OnClick = N3Click
    end
    object N4: TMenuItem
      Caption = '-'
    end
    object N1: TMenuItem
      Caption = #38544#34255#26412#21015
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #26174#31034#25152#26377#21015
      OnClick = N2Click
    end
    object N5: TMenuItem
      Caption = '-'
    end
    object N6: TMenuItem
      Caption = #26684#24335#22797#21407
      OnClick = N6Click
    end
  end
  object pubqry: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubquery'
    Left = 632
    Top = 291
  end
end
