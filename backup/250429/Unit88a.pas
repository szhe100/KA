...
type
  Tsetbusipriceapply1 = class(TForm)
...
var
  setbusipriceapply1: Tsetbusipriceapply1;
...
procedure Tsetbusipriceapply1.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetbusipriceapply1.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetbusipriceapply1.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsetbusipriceapply1.FormCloseQuery(Sender: TObject;
...
procedure Tsetbusipriceapply1.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetbusipriceapply1.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetbusipriceapply1.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetbusipriceapply1.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=38 and a.bod_status_id in (1,2)';
...
procedure Tsetbusipriceapply1.FormCreate(Sender: TObject);
begin
...
procedure Tsetbusipriceapply1.DSbillDataChange(Sender: TObject; Field: TField);
begin
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                ...
                commandtext:=commandtext+' from tb_bill_dtl b,tb_busiframe5 c,tb_medicine m';
...
procedure Tsetbusipriceapply1.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetbusipriceapply1.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetbusipriceapply1.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetbusipriceapply1.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetbusipriceapply1.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetbusipriceapply1.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetbusipriceapply1.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetbusipriceapply1.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetbusipriceapply1.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetbusipriceapply1.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetbusipriceapply1.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetbusipriceapply1.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetbusipriceapply1.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetbusipriceapply1.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetbusipriceapply1.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsetbusipriceapply1.SpeedButton2Click(Sender: TObject);
begin
...
begin
    if active=true then close;
    commandtext:='sp_updatestock38 38,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsetbusipriceapply1.SpeedButton3Click(Sender: TObject);
begin
