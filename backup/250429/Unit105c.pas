...
type
  TsetexpayA3 = class(TForm)
    ...
    dxDBGrid2ctype2: TdxDBGridColumn;
...
var
  setexpayA3: TsetexpayA3;
  fsendback: Boolean;

implementation

uses datamodu, Unit51, Unit15, Unit16;

{$R *.DFM}

procedure TsetexpayA3.setupdatestatus;    // 编辑状态
begin
speedbutton2.enabled:=false;   // refresh disable
speedbutton5.enabled:=false;   // refresh disable
end;

procedure TsetexpayA3.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure TsetexpayA3.FormActivate(Sender: TObject);
begin
with homebank do
begin
...
procedure TsetexpayA3.FormCloseQuery(Sender: TObject;
...
procedure TsetexpayA3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TsetexpayA3.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpayA3.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TsetexpayA3.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
with bill_dtl do
...
begin
    ...
    close;
    commandtext:='select top 10000 a.*,mate_name1=dbo.fn_mate_name(a.src_id),check_dt1=cast(convert(char(10),a.check_dt,20) as datetime),m.mate_name,agent=c.mate_name,deputy=c.receiver,c.acnt_bank,c.bank_account,creater=d.zname,checker=e.zname,';
    commandtext:=commandtext+' f.bank_name,broker=g.zname,casher=dbo.fn_staff_name(a.cash_by),carryer=dbo.fn_staff_name(a.carry_by),carry_dt1=j.carry_dt,';
    ...
    commandtext:=commandtext+' left join (select b.med_id,a.carry_dt from tb_bill a,tb_bill_dtl b where a.bod_type_id=50 and a.bod_status_id=1 and a.bod_id=b.bod_id) j on a.bod_id =j.med_id';
//    commandtext:=commandtext+' left join (select a.bod_cd,b.med_id,bank_id1=a.bank_id,a.carry_dt from tb_bill a,tb_bill_dtl b where a.bod_type_id=30 and a.bod_status_id=1 and a.bod_id=b.bod_id) j on a.bod_id =j.med_id';
    commandtext:=commandtext+' where a.bod_type_id=49 and a.bod_status_id in (1,5)';
...
procedure TsetexpayA3.FormCreate(Sender: TObject);
begin
...
procedure TsetexpayA3.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
//            and (datetostr(fieldbyname('check_dt').asdatetime)=datetostr(dt0)); // 审核当天可反审
    ...
    begin
        ...
        begin
            ...
            begin
                if active=true then close;
//                commandtext:='select * from vi_boddtlmed1 where bod_id='+bill.fieldbyname('bod_id').asstring; //等效于以下注释部分
...
{
                commandtext:='select b.bod_id,b.dtl_id,b.med_id,b.price,b.rela_value,d.qty,b.amot,c.carry_dt,c.bod_cd,';
                ...
                commandtext:=commandtext+' union all select b.bod_id,b.dtl_id,b.med_id,b.price,b.rela_value,b.qty,b.amot,c.carry_dt,c.bod_cd,';
                commandtext:=commandtext+' m.med_name,m.med_code,m.specifi,m.pdt_place,med_unit=dbo.fn_obj_desc(m.unit_id),c.bod_type_id,b.bod_cd1';
                commandtext:=commandtext+' from tb_bill_dtl b';
...
procedure TsetexpayA3.DSbillStateChange(Sender: TObject);
begin
with bill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpayA3.billBeforeEdit(DataSet: TDataSet);
begin
with bill do
begin
...
procedure TsetexpayA3.billCalcFields(DataSet: TDataSet);
begin
with bill do
...
procedure TsetexpayA3.billReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure TsetexpayA3.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure TsetexpayA3.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure TsetexpayA3.dxDBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
if key in ['a'..'z'] then key:=chr(ord(key)-32);   // turn lower charactor to upper
end;

procedure TsetexpayA3.SpeedButton1Click(Sender: TObject);
begin
with dm.pubqry do
begin
    if active=true then close;
...
procedure TsetexpayA3.SpeedButton2Click(Sender: TObject);
var s: string;
begin
with dm.pubqry do
begin
...
begin
    if active=true then close;
    commandtext:='sp_updatestock49 49,'+bill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    try
        execute;
    except
...
procedure TsetexpayA3.SpeedButton3Click(Sender: TObject);
begin
with dm.pubqry do
begin
    if active=True then close;
    commandtext:='select top 1 bod_cd from tb_bill a,tb_bill_dtl b where a.bod_type_id=50 and a.bod_id=b.bod_id and b.med_id='+bill.fieldbyname('bod_id').asstring;
...
procedure TsetexpayA3.DSbill_dtlStateChange(Sender: TObject);
begin
with bill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure TsetexpayA3.bill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
showmessage(dataset.name+':'+e.message);
...
procedure TsetexpayA3.hello1Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag);
end;

procedure TsetexpayA3.dxDBGrid2CustomDrawCell(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
  AColumn: TdxTreeListColumn; ASelected, AFocused, ANewItemRow: Boolean;
  var AText: String; var AColor: TColor; AFont: TFont;
  var AAlignment: TAlignment; var ADone: Boolean);
...
procedure TsetexpayA3.bill_dtlBeforePost(DataSet: TDataSet);
begin
with bill_dtl do
begin
...
procedure TsetexpayA3.Button1Click(Sender: TObject);
begin
