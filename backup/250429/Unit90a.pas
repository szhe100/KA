...
var
  setmedimport5a: Tsetmedimport5a;
...
procedure Tsetmedimport5a.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetmedimport5a.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetmedimport5a.FormActivate(Sender: TObject);
begin
if (cdsbill.active) and (fbodid>0) then cdsbill.Active:=False;
if cdsbill.active=false then SpeedButton5Click(nil);
end;

procedure Tsetmedimport5a.FormCloseQuery(Sender: TObject;
...
with qryimage do if active then close;
end;

procedure Tsetmedimport5a.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetmedimport5a.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedimport5a.dxDBGrid2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
...
procedure Tsetmedimport5a.dxDBGrid2Enter(Sender: TObject);
begin
...
procedure Tsetmedimport5a.FormKeyPress(Sender: TObject; var Key: Char);
//var fapp: boolean;
begin
...
procedure Tsetmedimport5a.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=41 and a.bod_status_id in (1,2)';
...
procedure Tsetmedimport5a.FormCreate(Sender: TObject);
begin
...
procedure Tsetmedimport5a.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
begin
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active then close;
                commandtext:='select f.amot1,a.*,broker=d.zname,leader=e.zname,b.med_name,b.specifi,b.pdt_place,b.med_code,b.qtyperpack,med_unit=c.zdesc,f.amot1';
                ...
                commandtext:=commandtext+' left join (select b.med_id,amot1=cast(sum(amot) as decimal(15,2)) from tb_bill a,tb_bill_dtl b';
                commandtext:=commandtext+'      where a.bod_type_id=40 and a.bod_status_id=1 and a.bod_id=b.bod_id and a.order_id='+cdsbill.fieldbyname('bod_id').asstring+' group by b.med_id) f on a.dtl_id =f.med_id';
...
procedure Tsetmedimport5a.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
procedure Tsetmedimport5a.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedimport5a.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedimport5a.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport5a.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport5a.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetmedimport5a.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetmedimport5a.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetmedimport5a.cdsbillBeforePost(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport5a.cdsbillReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedimport5a.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedimport5a.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetmedimport5a.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetmedimport5a.PageControl1Change(Sender: TObject);
begin
...
procedure Tsetmedimport5a.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetmedimport5a.N1Click(Sender: TObject);
    ...
    begin
{
        ...
        open;
        if recordcount>0 then raise Exception.Create('所选单据已生成了'+fieldbyname('bod_cd').asstring+'号商业采购收款单，不可反审');
        if active then close;
...
procedure Tsetmedimport5a.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetmedimport5a.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetmedimport5a.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetmedimport5a.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tsetmedimport5a.dxDBButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetmedimport5a.dxDBButtonEdit1KeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#32) and ((Sender as TdxDBButtonEdit).Text<>'') then //
begin
...
procedure Tsetmedimport5a.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetmedimport5a.dxEdit1Click(Sender: TObject);
begin
dxEdit1.SelectAll;
dxEdit1.SetFocus;
end;

procedure Tsetmedimport5a.dxDBButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedimport5a.dxDBButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedimport5a.dxDBGraphicEdit1AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport5a.dxDBGraphicEdit2AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport5a.dxDBGraphicEdit3AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport5a.dxDBGraphicEdit4AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport5a.dxDBGraphicEdit5AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport5a.dxDBGraphicEdit1Change(Sender: TObject);
begin
setupdatestatus;
//cdsbill.edit;
end;

procedure Tsetmedimport5a.dxDBGraphicEdit1GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass1').asstring));
end;

procedure Tsetmedimport5a.dxDBGraphicEdit2GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass2').asstring));
end;

procedure Tsetmedimport5a.dxDBGraphicEdit3GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass3').asstring));
end;

procedure Tsetmedimport5a.dxDBGraphicEdit4GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass4').asstring));
end;

procedure Tsetmedimport5a.dxDBGraphicEdit5GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass5').asstring));
end;

procedure Tsetmedimport5a.dxDBGraphicEdit1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
...
procedure Tsetmedimport5a.qryimageAfterPost(DataSet: TDataSet);
begin
qryimage.ApplyUpdates(-1);
end;

procedure Tsetmedimport5a.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsetmedimport5a.SpeedButton2Click(Sender: TObject);
begin
...
begin
    if active then close;
    commandtext:='sp_updatestock41 41,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsetmedimport5a.SpeedButton3Click(Sender: TObject);
var s: string;
begin
with dm.pubqry do
begin
    ...
    open;
    if recordcount>0 then raise Exception.Create('本单已生成了'+fieldbyname('bod_cd').asstring+'号商业采购收款单，不可反审');
    if active then close;
    commandtext:='select top 5 bod_cd from tb_bill where bod_type_id=42 and busi_type in (1,2) and order_id='+cdsbill.fieldbyname('bod_id').asstring;
    open;
    if recordcount>0 then
    begin
...
procedure Tsetmedimport5a.SpeedButton1Click(Sender: TObject);
var mcd: string;
    mi: integer;
begin
//注意判断 已收完单据不可再收款
...
begin
    if active then close;
    commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,order_id,src_id,dst_id,broker_id,agent_id,district_id,busi_type,allo_desc,bod_desc,creat_by,creat_dt)';
    commandtext:=commandtext+' select top 1 '+inttostr(compid)+','''+mcd+''',40,0,bod_id,src_id,dst_id,broker_id,agent_id,district_id,busi_type,allo_desc,''由'+cdsbill.fieldbyname('bod_cd').asstring+'号分销采购计划单生成'','+inttostr(curuserid)+',getdate()';
    ...
    close;
{
    ...
    commandtext:=commandtext+' select '+inttostr(mi)+',dtl_id,price2*qty,price2*qty';
    commandtext:=commandtext+' from tb_bill_dtl where bod_id='+cdsbill.fieldbyname('bod_id').asstring;
}
    commandtext:='insert into tb_bill_dtl (bod_id,med_id,qty,price,amot)';
    commandtext:=commandtext+' select '+inttostr(mi)+',d.dtl_id,d.qty,d.price2*d.qty-isnull(c.amot1,0),price2*qty-isnull(c.amot1,0)';
    commandtext:=commandtext+' from tb_bill_dtl d left join (select b.med_id,amot1=sum(b.amot) from tb_bill a,tb_bill_dtl b where bod_type_id=40 and bod_status_id=1 and a.order_id='+cdsbill.fieldbyname('bod_id').asstring+' and a.bod_id=b.bod_id group by b.med_id) c';
    commandtext:=commandtext+' on d.dtl_id=c.med_id where d.bod_id='+cdsbill.fieldbyname('bod_id').asstring+' and d.price2*d.qty>isnull(c.amot1,0)';
edit1.text:=commandtext;    
    execute;
    setprogress(0);
...
procedure Tsetmedimport5a.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('camot').asfloat:=fieldbyname('price1').asfloat*fieldbyname('qty').asfloat;
end;

procedure Tsetmedimport5a.N4Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t,mcd: string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
begin
{
    ...
    if recordcount>0 then
    begin
        MessageBox(0,pchar('所选单据中不得有已完成销售的单据'),'请注意',MB_OK+MB_IconError);
        abort;
    end;
    if active then close;
    ...
    begin
        ...
        while not eof do
        begin
            s:=s+#13+#10+'商业采购计划单:'+fieldbyname('bod_cd').asstring+' 两票销售单:'+fieldbyname('bod_cd1').asstring;
            next;
        end;
        MessageBox(0, pchar(s),'请注意',MB_OK+MB_ICONInformation);
        abort;
...
//    showmessage(inttostr(recordcount));
    ...
    begin
        ...
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select top 1 comp_id,'''+mcd+''',42,0,cate_id=1,1,src_id,dst_id,agent_id,broker_id,district_id,bod_id,''由''+bod_cd+''号商业采购计划单生成'','+inttostr(curuserid)+',getdate()';
...
//edit1.text:=dm.pubqry.commandtext;
        ...
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select '+inttostr(mi)+',med_id,price,price1,price2,qty,qty,qty,dtl_id';
...
procedure Tsetmedimport5a.N5Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t,mcd: string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
//    commandtext:=commandtext+' and dbo.fn_orderisfinished(bod_id)=1';
    ...
    if recordcount>0 then
    begin
        MessageBox(0,pchar('所选单据中不得有无销售数的单据'),'请注意',MB_OK+MB_IconError);
        abort;
    end;
    if active then close;
    commandtext:='select top 5 a.bod_cd,bod_cd1=c.bod_cd from tb_bill a,tb_bill c';
    ...
    begin
        ...
        while not eof do
        begin
            s:=s+#13+#10+'商业采购计划单:'+fieldbyname('bod_cd').asstring+' 两票销售/退货单:'+fieldbyname('bod_cd1').asstring;
            next;
        end;
        MessageBox(0, pchar(s),'请注意',MB_OK+MB_ICONInformation);
        abort;
...
begin
    ...
    begin
        ...
        if dm.pubqry.active then dm.pubqry.close;
        dm.pubqry.commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,cate_id,busi_type,src_id,dst_id,agent_id,broker_id,district_id,order_id,bod_desc,creat_by,creat_dt)';
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select top 1 '+inttostr(compid)+','''+mcd+''',42,0,cate_id=1,2,src_id,dst_id,agent_id,broker_id,district_id,bod_id,''由''+bod_cd+''号商业采购计划单生成'','+inttostr(curuserid)+',getdate()';
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' from tb_bill where bod_id='+fieldbyname('bod_id').asstring;
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select @@identity';
//edit1.text:=dm.pubqry.commandtext;
        dm.pubqry.open;
...
procedure Tsetmedimport5a.N7Click(Sender: TObject);
begin
