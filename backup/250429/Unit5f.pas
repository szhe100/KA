...
begin
    ...
    setprogress(1);
    m1:=0; m2:=0; j:=2;
//从第2行开始导入，格式: 1业务负责人姓名,2省份,3城市,4推广服务商名称,5纳税类型,6准入状态,7使用状态,8类型,9确认时间
...
//从第2行开始导入，格式: 1推广服务商名称, 2纳税类型, 3准入状态, 4使用状态, 5确认时间
        ...
        sql:=sql+'  status_id=case f3 when ''准入'' then 2 when ''终止'' then 2 end,';
        sql:=sql+'  status_id1=case f4 when ''正常'' then 1 when ''暂停'' then 2 when ''终止'' then 3 end,';
        sql:=sql+'  confirm_dt=f5,creat_by='+inttostr(curuserid)+',creat_dt=getdate()'; //mate_type_id=10 推广服务商
        sql:=sql+' from @tab a';
