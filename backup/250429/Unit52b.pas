...
type
  Tsetreport12 = class(TForm)
    ...
    dxDBGrid1Ctaskamot: TdxDBGridColumn;
...
procedure Tsetreport12.dxButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetreport12.SpeedButton2Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag)
end;

procedure Tsetreport12.DSqryDataChange(Sender: TObject;
  Field: TField);
begin
with qry do
begin
    speedbutton2.enabled:=(active) and (recordcount>0);
    speedbutton5.enabled:=(active) and (recordcount>0) and (fieldbyname('creat_by').asinteger=curuserid);
...
procedure Tsetreport12.qryBeforeDelete(DataSet: TDataSet);
begin
...
procedure Tsetreport12.qryAfterPost(DataSet: TDataSet);
begin
if qry.ChangeCount>0 then myupdaterefresh(qry,'tb_report01','rec_id','');
end;

procedure Tsetreport12.SpeedButton3Click(Sender: TObject);
var mi: integer;
begin
with qry do
begin
...
procedure Tsetreport12.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetreport12.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetreport12.FormCloseQuery(Sender: TObject;
...
procedure Tsetreport12.FormCreate(Sender: TObject);
begin
...
procedure Tsetreport12.N3Click(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure Tsetreport12.N1Click(Sender: TObject);
begin
with dxdbgrid1 do Columns[GetAbsoluteColumnIndex(FocusedColumn)].Visible:=false;
end;

procedure Tsetreport12.N2Click(Sender: TObject);
var i,mi: integer;
begin
setprogress(1);
with dxdbgrid1 do
...
procedure Tsetreport12.N6Click(Sender: TObject);
begin
if FileExists(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini')
    then dxDBGrid1.LoadFromIniFile(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini');//加载
end;

procedure Tsetreport12.FormActivate(Sender: TObject);
begin
with dm.district do
...
begin
    if active=False then
    begin
        commandtext:='select obj_id=0,zdesc='''' union select obj_id=1,zdesc=''GD'' union select obj_id=2,zdesc=''GZ'' union select obj_id=3,zdesc=''HC'' union select obj_id=4,zdesc=''ZR''';
        open;
    end;
end;
with type3 do
begin
    if active=False then
    begin
        commandtext:='select obj_id=0,zdesc='''' union select obj_id=1,zdesc=''医院代理'' union select obj_id=2,zdesc=''区域代理'' union select obj_id=3,zdesc=''省区代理'' union select obj_id=4,zdesc=''战略合作''';
        open;
    end;
end;
end;

procedure Tsetreport12.dxLookupTreeView1CloseUp(Sender: TObject;
  Accept: Boolean);
begin
speedbutton4.Enabled:=(dxbuttonedit1.tag<>0) and (dxLookupTreeView1.text<>'');
end;

procedure Tsetreport12.dxLookupTreeView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if key=vk_delete then dxLookupTreeView1.text:='';
end;

procedure Tsetreport12.qryBeforePost(DataSet: TDataSet);
begin
with dm.pubqry do
begin
    if active then close;
    commandtext:='select top 1 1 from tb_report01 where med_id='+ inttostr(qry.fieldbyname('med_id').asinteger);
...
procedure Tsetreport12.SpeedButton5Click(Sender: TObject);
begin
with qry do
begin
...
procedure Tsetreport12.dxButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetreport12.dxButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetreport12.dxButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
    if showmodal= mrOK then
...
procedure Tsetreport12.Button1Click(Sender: TObject);
begin
with dm.pubqry do
begin
    if active then close;
...
procedure Tsetreport12.qryBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetreport12.N7Click(Sender: TObject);
begin
with qry do
begin
...
procedure Tsetreport12.N8Click(Sender: TObject);
var mi: integer;
begin
with qry do
begin
...
procedure Tsetreport12.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetreport12.SpeedButton6Click(Sender: TObject);
var
    XL,Sheet: Variant;
    SFileName: string;
    mi,J,id2,id4,id5,id6,id8,id9,id10,id11,id33,id34,id35: integer;
    sbodcd: string;
    dt: TDatetime;
begin
...
//  33类型(新签、续签),34合同类型(医院代理、区域代理、省区代理、战略合作),35签约人,36结算价,37流程编号

    while (sheet.cells[j,1].text<>'') and (mi<10) do
    begin
        id4:=0;id5:=0;id6:=0;id8:=0;id9:=0;id10:=0;id11:=0;id33:=0;id34:=0;
        with dm.pubqry do
        begin
            if active then close;
...
//从第2行开始导入，导入格式: 1配送商编码,2配送商名称
    ...
    begin
        id2:=0;id4:=0;id5:=0;id6:=0;id8:=0;id9:=0;id10:=0;id11:=0;id33:=0;id34:=0;id35:=0;
        with dm.pubqry do
        begin
            ...
            else if Trim(sheet.cells[j,2].text)='ZR' then id2:=4
            else id2:=0;
//  33类型(新签、续签),34合同类型(医院代理、区域代理、省区代理、战略合作),35签约人,36结算价
            if sheet.cells[j,33].text='' then id33:=0
            else if Trim(sheet.cells[j,33].text)='新签' then id33:=1
            else if Trim(sheet.cells[j,33].text)='续签' then id33:=2
            else id33:=0;
            if sheet.cells[j,34].text='' then id34:=0
            else if Trim(sheet.cells[j,34].text)='医院代理' then id34:=1
            else if Trim(sheet.cells[j,34].text)='区域代理' then id34:=2
            else if Trim(sheet.cells[j,34].text)='省区代理' then id34:=3
            else if Trim(sheet.cells[j,34].text)='战略合作' then id34:=4
            else id34:=0;
            ...
            if sheet.cells[j,35].text<>'' then
            begin
                if active then close;
                commandtext:='select top 1 sta_id from tb_staff where comp_id='+inttostr(compid)+' and sta_type_id=0 and manager=1 and zname='''+Trim(sheet.cells[j,35].text)+'''';
                open;
                if recordcount>0 then id35:=fieldbyname('sta_id').asinteger; //区域经理
            end;
            if sheet.cells[j,6].text<>'' then
            begin
                if active then close;
...
//1年度,4事业部id4,5区域经理id5,6省份id6,7城市,8业务员id8,9客户id9,11药品编码id11
{
            ...
            if active then close;
            commandtext:=' if exists (select top 1 rec_id from tb_report01 where fyear='+sheet.cells[j,1].text;
            commandtext:=commandtext+' and level_id1='+inttostr(id4)+' and lead_id='+inttostr(id5)+' and district_id='+inttostr(id6);
            commandtext:=commandtext+' and broker_id='+inttostr(id8)+' and mate_id='+inttostr(id9)+' and med_id='+inttostr(id11)+')';
            commandtext:=commandtext+' update tb_report01 set type_id1='+inttostr(id33)+',type_id2='+inttostr(id2)+',type_id3='+inttostr(id34)+',';
            ...
            if dt=0 then commandtext:=commandtext+'sign_dt=null,' else commandtext:=commandtext+'sign_dt='''+datetimetostr(dt)+''',';

            commandtext:=commandtext+' sign_by='+inttostr(id35)+',';
//            price1,price,amot1,f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,qty1,qty2,fmonth,dt1,dt2,appoint,insure_amot,flow_cd';
            if sheet.cells[j,36].text='' then commandtext:=commandtext+'price1=0,' else commandtext:=commandtext+'price1='+cleardeli(sheet.cells[j,36].text)+',';
            if sheet.cells[j,12].text='' then commandtext:=commandtext+'price=0,' else commandtext:=commandtext+'price='+cleardeli(sheet.cells[j,12].text)+',';
            if sheet.cells[j,13].text='' then commandtext:=commandtext+'amot1=0,' else commandtext:=commandtext+'amot1='+cleardeli(sheet.cells[j,13].text)+',';
            if sheet.cells[j,14].text='' then commandtext:=commandtext+'f1=0,' else commandtext:=commandtext+'f1='+cleardeli(sheet.cells[j,14].text)+',';
            if sheet.cells[j,15].text='' then commandtext:=commandtext+'f2=0,' else commandtext:=commandtext+'f2='+cleardeli(sheet.cells[j,14].text)+',';
            if sheet.cells[j,16].text='' then commandtext:=commandtext+'f3=0,' else commandtext:=commandtext+'f3='+cleardeli(sheet.cells[j,14].text)+',';
            if sheet.cells[j,17].text='' then commandtext:=commandtext+'f4=0,' else commandtext:=commandtext+'f4='+cleardeli(sheet.cells[j,14].text)+',';
            if sheet.cells[j,18].text='' then commandtext:=commandtext+'f5=0,' else commandtext:=commandtext+'f5='+cleardeli(sheet.cells[j,14].text)+',';
            if sheet.cells[j,19].text='' then commandtext:=commandtext+'f6=0,' else commandtext:=commandtext+'f6='+cleardeli(sheet.cells[j,14].text)+',';
            if sheet.cells[j,20].text='' then commandtext:=commandtext+'f7=0,' else commandtext:=commandtext+'f7='+cleardeli(sheet.cells[j,14].text)+',';
            if sheet.cells[j,21].text='' then commandtext:=commandtext+'f8=0,' else commandtext:=commandtext+'f8='+cleardeli(sheet.cells[j,14].text)+',';
            if sheet.cells[j,22].text='' then commandtext:=commandtext+'f9=0,' else commandtext:=commandtext+'f9='+cleardeli(sheet.cells[j,14].text)+',';
            if sheet.cells[j,23].text='' then commandtext:=commandtext+'f10=0,' else commandtext:=commandtext+'f10='+cleardeli(sheet.cells[j,14].text)+',';
            if sheet.cells[j,24].text='' then commandtext:=commandtext+'f11=0,' else commandtext:=commandtext+'f11='+cleardeli(sheet.cells[j,14].text)+',';
            if sheet.cells[j,25].text='' then commandtext:=commandtext+'f12=0,' else commandtext:=commandtext+'f12='+cleardeli(sheet.cells[j,14].text)+',';

            if sheet.cells[j,26].text='' then commandtext:=commandtext+'qty1=0,' else commandtext:=commandtext+'qty1='+cleardeli(sheet.cells[j,26].text)+',';
            if sheet.cells[j,27].text='' then commandtext:=commandtext+'qty2=0,' else commandtext:=commandtext+'qty2='+cleardeli(sheet.cells[j,27].text)+',';
            commandtext:=commandtext+'fmonth='''+Trim(sheet.cells[j,28].text)+''','; //预计开发月份
            ...
            if dt=0 then commandtext:=commandtext+'dt1=null,' else commandtext:=commandtext+'dt1='''+datetimetostr(dt)+''',';
            ...
            if dt=0 then commandtext:=commandtext+'dt2=null,' else commandtext:=commandtext+'dt2='''+datetimetostr(dt)+''',';
            commandtext:=commandtext+'appoint='''+Trim(sheet.cells[j,31].text)+''','; //其他约定
            if sheet.cells[j,32].text='' then commandtext:=commandtext+'insure_amot=0,' else commandtext:=commandtext+'insure_amot='+cleardeli(sheet.cells[j,32].text)+',';
            commandtext:=commandtext+'flow_cd='''+Trim(sheet.cells[j,37].text)+'''';
            commandtext:=commandtext+' where fyear='+sheet.cells[j,1].text;
            commandtext:=commandtext+'  and level_id1='+inttostr(id4)+' and lead_id='+inttostr(id5)+' and district_id='+inttostr(id6);
            commandtext:=commandtext+'  and broker_id='+inttostr(id8)+' and mate_id='+inttostr(id9)+' and med_id='+inttostr(id11);
            commandtext:=commandtext+' else insert into tb_report01 (fyear,type_id1,type_id2,type_id3,sign_dt,sign_by,level_id1,lead_id,district_id,broker_id,mate_id,mate_id1,med_id,';
            commandtext:=commandtext+' price1,price,amot1,f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,qty1,qty2,';
            commandtext:=commandtext+' fmonth,dt1,dt2,appoint,insure_amot,flow_cd,creat_by,creat_dt)';
            if sheet.cells[j,1].text='' then commandtext:=commandtext+'values (null,' else commandtext:=commandtext+' values ('+sheet.cells[j,1].text+',';
            commandtext:=commandtext+inttostr(id33)+',';
            commandtext:=commandtext+inttostr(id2)+',';
            commandtext:=commandtext+inttostr(id34)+',';
            if sheet.cells[j,3].text='' then dt:=0 else
            try
                dt:=strtodatetime(Trim(sheet.cells[j,3].text));
            except
            ...
            commandtext:=commandtext+inttostr(id35)+',';
            ...
            commandtext:=commandtext+inttostr(id11)+','; //药品med_id

            if sheet.cells[j,36].text='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,36].text)+',';
            ...
            if dt=0 then commandtext:=commandtext+'null,' else commandtext:=commandtext+''''+datetimetostr(dt)+''',';
            commandtext:=commandtext+''''+Trim(sheet.cells[j,31].text)+''','; //其他约定
            if sheet.cells[j,32].text='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,32].text)+',';
            commandtext:=commandtext+''''+Trim(sheet.cells[j,37].text)+''','+inttostr(curuserid)+',getdate())';
edit1.text:=commandtext;
            commandtext:=commandtext+' select @@identity';
            open;
            mi:=fields[0].asinteger;
...
procedure Tsetreport12.qrydtlAfterPost(DataSet: TDataSet);
begin
if qrydtl.ChangeCount>0 then myupdaterefresh(qrydtl,'tb_report01dtl','dtl_id','');
end;

procedure Tsetreport12.qrydtlBeforeDelete(DataSet: TDataSet);
begin
if qry.fieldbyname('valid').asboolean=True then raise Exception.Create('本记录已生效，不可删除');
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetreport12.qrydtlBeforePost(DataSet: TDataSet);
begin
with qrydtl do
begin
    if state=dsinsert then
...
procedure Tsetreport12.dxDBGrid2mate_name1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickcustomer do
    if showmodal= mrOK then
...
procedure Tsetreport12.DSqrydtlStateChange(Sender: TObject);
begin
if qrydtl.state in [dsinsert,dsedit] then qry.edit;
end;

procedure Tsetreport12.wwDBNavigator1PostClick(Sender: TObject);
begin
...
procedure Tsetreport12.wwDBNavigator1CancelClick(Sender: TObject);
begin
if (qrydtl.State in [dsinsert,dsedit]) then qrydtl.cancelUpdates;
end;

procedure Tsetreport12.dxDBGrid2Enter(Sender: TObject);
begin
with qrydtl do
    if recordcount=0 then append;
end;

procedure Tsetreport12.dxButtonEdit5ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickmanager do
    if showmodal= mrOK then
...
procedure Tsetreport12.dxDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetreport12.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetreport12.MenuItem9Click(Sender: TObject);
begin
...
procedure Tsetreport12.dxDBGrid1signerButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickmanager do
begin
...
procedure Tsetreport12.dxButtonEdit6ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with picklevel3 do
    if showmodal= mrOK then
...
procedure Tsetreport12.N10Click(Sender: TObject);
...
procedure Tsetreport12.qryAfterInsert(DataSet: TDataSet);
begin
with qry do
begin
...
procedure Tsetreport12.N11Click(Sender: TObject);
var i,mi,l,m,n,k,o,p,q,i1,i2,i3,i4,i5,i6,m1,m2,m3 : integer;
    s,t: string;
    mcd: string[20];
begin
...
procedure Tsetreport12.dxButtonEdit7ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with picklevel2 do
    if showmodal= mrOK then
...
procedure Tsetreport12.qryCalcFields(DataSet: TDataSet);
begin
with qry do
begin
    fieldbyname('Ctaskamot').asfloat:=fieldbyname('f1').asfloat+fieldbyname('f2').asfloat+fieldbyname('f3').asfloat+fieldbyname('f4').asfloat+fieldbyname('f5').asfloat+fieldbyname('f6').asfloat
        +fieldbyname('f7').asfloat+fieldbyname('f8').asfloat+fieldbyname('f9').asfloat+fieldbyname('f10').asfloat+fieldbyname('f11').asfloat+fieldbyname('f12').asfloat;
end;
end;
