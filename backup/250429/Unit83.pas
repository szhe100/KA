...
var
  setagentaccount: Tsetagentaccount;
...
procedure Tsetagentaccount.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetagentaccount.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetagentaccount.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsetagentaccount.FormCloseQuery(Sender: TObject;
...
procedure Tsetagentaccount.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetagentaccount.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetagentaccount.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetagentaccount.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetagentaccount.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetagentaccount.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=14 and busi_type=1';  // a.comp_id='+inttostr(compid)+' and
...
procedure Tsetagentaccount.FormCreate(Sender: TObject);
begin
N2.Visible:=False;
...
procedure Tsetagentaccount.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetagentaccount.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetagentaccount.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetagentaccount.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetagentaccount.SpeedButton2Click(Sender: TObject);
//var s: string;
begin
{
with cdsbill do
...
//    commandtext:='update tb_bill set bod_status_id=2 where bod_id='+cdsbill.fieldbyname('bod_id').asstring;
    commandtext:='update tb_bill set bod_status_id=3 where bod_id='+cdsbill.fieldbyname('bod_id').asstring; // 2020.7.27 取消业务审核界面，直接送财务复核
...
procedure Tsetagentaccount.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetagentaccount.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetagentaccount.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetagentaccount.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
...
procedure Tsetagentaccount.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetagentaccount.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetagentaccount.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetagentaccount.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetagentaccount.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetagentaccount.N1Click(Sender: TObject);
...
procedure Tsetagentaccount.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetagentaccount.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetagentaccount.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
