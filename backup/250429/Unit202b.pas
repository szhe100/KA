...
begin
    ...
    begin
        ...
        begin
            commandtext:='select b.dtl_id,cate_id=isnull(a.cate_id,1),a.bod_type_id,a.bod_cd,a.bod_status_id,d.med_code,d.med_name,d.specifi,d.pdt_place,d.qtyperpack,d.type_id1,j.mate_name,';
        //    commandtext:=commandtext+' cdistrict=dbo.fn_getdistrict(j.district),';
            commandtext:=commandtext+' a.platform_id,cate_id1=dbo.fn_getmatemedcateid(a.comp_id,a.src_id,b.med_id),';
            commandtext:=commandtext+' cdistrict='''',a.channel_id,channel=dbo.fn_obj_desc(a.channel_id),a.channel_dtl_id,channel_dtl=dbo.fn_obj_desc(a.channel_dtl_id),b.bod_cd2,';
            ...
            commandtext:=commandtext+' j.kind_id,j.busi_type_id,leader=p.zname,p.zcode,';
        ...
        //    commandtext:=commandtext+' price1=dbo.fn_getprice2a(j.district,b.med_id,a.carry_dt),b.price3,';
        ...
        //	commandtext:=commandtext+' left join tb_busiframe k on b.busiframe_id=k.rec_id'; //=dbo.fn_getbusiframerecid5(j.district,b.med_id,a.dst_id,a.carry_dt)';
        ...
        begin
            ...
            commandtext:=commandtext+' select b.dtl_id,cate_id=isnull(a.cate_id,1),a.bod_type_id,a.bod_cd,a.bod_status_id,d.med_code,d.med_name,d.specifi,d.pdt_place,d.qtyperpack,d.type_id1,j.mate_name,';
...
//            commandtext:=commandtext+' dist2=dbo.fn_getdistrictlevel (j.district,2),';  // a.district_id
            commandtext:=commandtext+' dist1=dbo.fn_getdistrictlevel (case a.cate_id when 1 then e.district else a.district_id end,1),';
            commandtext:=commandtext+' dist2=dbo.fn_getdistrictlevel (case a.cate_id when 1 then e.district else a.district_id end,2),';
...
//            commandtext:=commandtext+' amot=cast(b.price1*b.qty as decimal(15,2)),';
    ...
    //        if fdistltd then commandtext:=commandtext+' and exists (select 1 from tb_trustdistrict t where t.sta_id='+inttostr(curuserid)+' and dbo.fn_ischildnode(e.district,t.dist_id)=1)';  //依据业务负责人地区 以限制地区
        ...
        begin
            if commandtext<>'' then commandtext:=commandtext+' union all';
            commandtext:=commandtext+' select b.dtl_id,a.cate_id,a.bod_type_id,a.bod_cd,a.bod_status_id,d.med_code,d.med_name,d.specifi,d.pdt_place,d.qtyperpack,d.type_id1,j.mate_name,';
            commandtext:=commandtext+' a.platform_id,cate_id1=0,'; //dbo.fn_getmatemedcateid(a.comp_id,a.src_id,b.med_id),';
            commandtext:=commandtext+' cdistrict=dbo.fn_getdistrict(p.district),a.channel_id,channel=dbo.fn_obj_desc(a.channel_id),a.channel_dtl_id,channel_dtl=dbo.fn_obj_desc(a.channel_dtl_id),bod_cd2='''',';
    ...
    //        commandtext:=commandtext+' dist2=dbo.fn_getdistrictlevel (p.district,2),';
            commandtext:=commandtext+' dist1=dbo.fn_getdistrictlevel (case a.cate_id when 1 then p.district else a.district_id end,1),';
            commandtext:=commandtext+' dist2=dbo.fn_getdistrictlevel (case a.cate_id when 1 then p.district else a.district_id end,2),';
...
begin
    ...
    begin
        ...
        begin
        	fn:=False;
            commandtext:=commandtext+' select b.med_id,d.med_code,d.med_name,d.specifi,d.pdt_place,med_unit=dbo.fn_obj_desc(d.unit_id),';
            commandtext:=commandtext+' b.price,price1=0.0000,price2=case a.cate_id when 2 then b.price else dbo.fn_getprice2(a.dst_id,b.med_id,a.carry_dt) end,';
...
//            commandtext:=commandtext+' b.price3,b.qty,amot=cast(b.price1*b.qty as decimal(15,2))';
            commandtext:=commandtext+' from tb_bill a';
            commandtext:=commandtext+' inner join tb_bill_dtl b on a.bod_id=b.bod_id';
            commandtext:=commandtext+' left join tb_staff c on b.sta_id=c.sta_id';
            commandtext:=commandtext+' inner join tb_medicine d on b.med_id=d.med_id';
...
//            if fdistltd then commandtext:=commandtext+' and exists (select 1 from tb_trustdistrict s,tb_treedata t where s.sta_id='+inttostr(curuserid)+' and (s.dist_id=t.rec_id or s.dist_id=t.parent) and e.district=t.rec_id)';
    ...
    //        if fdistltd then commandtext:=commandtext+'  and exists (select 1 from tb_trustdistrict s,tb_treedata t where s.sta_id='+inttostr(curuserid)+' and (s.dist_id=t.rec_id or s.dist_id=t.parent) and h.district=t.rec_id)';
        ...
        commandtext:=commandtext+'  ) h group by med_id,med_code,med_name,specifi,pdt_place,med_unit';
