object setreport02: Tsetreport02
  Left = 140
  Top = 135
  AutoScroll = False
  Caption = #38144#21806#39044#27979#34920
  ClientHeight = 496
  ClientWidth = 1180
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  Icon.Data = {
    
    0000870100009F810000FFC10000FFF30000FFFF0000}
  OldCreateOrder = False
  Position = poScreenCenter
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  DesignSize = (
    1180
    496)
  PixelsPerInch = 96
  TextHeight = 12
  object Shape1: TShape
    Left = 1
    Top = 59
    Width = 45
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object SpeedButton4: TSpeedButton
    Left = 303
    Top = 33
    Width = 25
    Height = 25
    Hint = #21152#20837#21306#22495#21697#31181#35760#24405
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      33333333337F7F33333333333300033333333333337773333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Visible = False
  end
  object SpeedButton2: TSpeedButton
    Left = 203
    Top = 0
    Width = 25
    Height = 25
    Hint = #36716#20986#25253#34920
    Enabled = False
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton2Click
  end
  object Label1: TLabel
    Left = 11
    Top = 64
    Width = 24
    Height = 12
    Caption = #21697#31181
    Transparent = True
  end
  object SpeedButton3: TSpeedButton
    Left = 1008
    Top = 73
    Width = 152
    Height = 25
    Hint = #21047#26032
    Anchors = [akTop, akRight]
    Flat = True
    Glyph.Data = {
      
      3393333777333777337333333999993333333333377777333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = SpeedButton3Click
  end
  object Shape2: TShape
    Left = 1
    Top = 81
    Width = 45
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label3: TLabel
    Left = 11
    Top = 86
    Width = 24
    Height = 12
    Caption = #22320#21306
    Transparent = True
  end
  object Label19: TLabel
    Left = 177
    Top = 38
    Width = 24
    Height = 12
    Caption = #21253#21547
  end
  object SpeedButton6: TSpeedButton
    Left = 303
    Top = 0
    Width = 110
    Height = 25
    Hint = #20174#31532'2'#34892#24320#22987#23548#20837#65292#23548#20837#26684#24335': '#22320#21306','#33647#21697#32534#30721','#20013#26631','#36153#29992#25919#31574
    Caption = #23548#20837'&Excel'
    Flat = True
    Glyph.Data = {
      
      3333333773333333333333303333333333333337333333333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Visible = False
    OnClick = SpeedButton6Click
  end
  object Label13: TLabel
    Left = 827
    Top = 81
    Width = 174
    Height = 12
    Anchors = [akTop, akRight]
    Caption = #27880':'#26412#26597#35810#21463#25480#26435#22320#21306#12289#21697#31181#38480#21046
    Font.Charset = GB2312_CHARSET
    Font.Color = clBlack
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Visible = False
  end
  object SpeedButton5: TSpeedButton
    Left = 473
    Top = 76
    Width = 121
    Height = 25
    Hint = #20174#19994#21153#32467#26500#20013#23548#20837
    Caption = #20174#19994#21153#32467#26500#20013#23548#20837
    Flat = True
    Glyph.Data = {
      
      33333333337F7F33333333333300033333333333337773333333}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    Spacing = 0
    OnClick = SpeedButton5Click
  end
  object Shape4: TShape
    Left = 322
    Top = 78
    Width = 35
    Height = 22
    Brush.Color = 14789952
    Pen.Color = 11764252
    Shape = stRoundRect
  end
  object Label2: TLabel
    Left = 328
    Top = 83
    Width = 26
    Height = 15
    AutoSize = False
    Caption = #24180#26376
    Transparent = True
    WordWrap = True
  end
  object Label4: TLabel
    Left = 409
    Top = 81
    Width = 12
    Height = 12
    Caption = #24180
  end
  object Label5: TLabel
    Left = 457
    Top = 81
    Width = 12
    Height = 12
    Caption = #26376
  end
  object dxDBGrid1: TdxDBGrid
    Left = 0
    Top = 104
    Width = 1180
    Height = 392
    Bands = <
      item
      end>
    DefaultLayout = True
    HeaderPanelRowCount = 1
    KeyField = 'rec_id'
    ShowGroupPanel = True
    ShowSummaryFooter = True
    SummaryGroups = <>
    SummarySeparator = ', '
    Align = alBottom
    PopupMenu = PopupMenu1
    TabOrder = 2
    DataSource = DSbrokermed
    Filter.Active = True
    Filter.DropDownCount = 30
    Filter.Criteria = {00000000}
    HideSelectionColor = clAqua
    HideSelectionTextColor = clBlack
    HighlightColor = clAqua
    HighlightTextColor = clBlack
    LookAndFeel = lfUltraFlat
    OptionsBehavior = [edgoAnsiSort, edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabThrough, edgoVertThrough]
    OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanNavigation, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoInvertSelect, edgoUseBitmap]
    Anchors = [akLeft, akTop, akRight, akBottom]
    object dxDBGrid1Cyearmonth: TdxDBGridColumn
      Alignment = taCenter
      DisableEditor = True
      Width = 69
      BandIndex = 0
      RowIndex = 0
      FieldName = 'Cyearmonth'
      SummaryFooterType = cstCount
      SummaryFooterField = 'rec_id'
      SummaryFooterFormat = '###,##0'#31508';; '
      SummaryType = cstCount
      SummaryField = 'rec_id'
      SummaryFormat = '###,##0'#31508';; '
    end
    object dxDBGrid1level1: TdxDBGridColumn
      DisableEditor = True
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'level1'
    end
    object dxDBGrid1level2: TdxDBGridColumn
      DisableEditor = True
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'level2'
    end
    object dxDBGrid1level3: TdxDBGridColumn
      DisableEditor = True
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'level3'
    end
    object dxDBGrid1leader: TdxDBGridColumn
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 69
      BandIndex = 0
      RowIndex = 0
      FieldName = 'leader'
    end
    object dxDBGrid1dist1: TdxDBGridColumn
      DisableEditor = True
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'dist1'
    end
    object dxDBGrid1dist2: TdxDBGridColumn
      DisableEditor = True
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'dist2'
    end
    object dxDBGrid1med_code: TdxDBGridColumn
      Alignment = taCenter
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clRed
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_code'
    end
    object dxDBGrid1med_name: TdxDBGridColumn
      DisableEditor = True
      Font.Charset = GB2312_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = #23435#20307
      Font.Style = []
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_name'
    end
    object dxDBGrid1specifi: TdxDBGridColumn
      DisableEditor = True
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'specifi'
    end
    object dxDBGrid1pdt_place: TdxDBGridColumn
      DisableEditor = True
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pdt_place'
    end
    object dxDBGrid1med_unit: TdxDBGridColumn
      Alignment = taCenter
      DisableEditor = True
      Width = 30
      BandIndex = 0
      RowIndex = 0
      FieldName = 'med_unit'
      DisableFilter = True
    end
    object dxDBGrid1price: TdxDBGridColumn
      DisableEditor = True
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'price'
      DisableFilter = True
    end
    object dxDBGrid1qty2: TdxDBGridColumn
      DisableEditor = True
      Width = 78
      BandIndex = 0
      RowIndex = 0
      FieldName = 'qty2'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryFormat = '###,###,##0;; '
    end
    object dxDBGrid1Camot2: TdxDBGridColumn
      DisableEditor = True
      BandIndex = 0
      RowIndex = 0
      FieldName = 'Camot2'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Camot2'
      SummaryFooterFormat = '###,###,##0;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Camot2'
      SummaryFormat = '###,###,##0;; '
    end
    object dxDBGrid1qty1: TdxDBGridColumn
      Color = 13303293
      Width = 78
      BandIndex = 0
      RowIndex = 0
      FieldName = 'qty1'
      SummaryFooterType = cstSum
      SummaryFooterField = 'qty1'
      SummaryFooterFormat = '###,###,##0;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'qty1'
      SummaryFormat = '###,###,##0;; '
    end
    object dxDBGrid1Camot1: TdxDBGridColumn
      DisableEditor = True
      Width = 78
      BandIndex = 0
      RowIndex = 0
      FieldName = 'Camot1'
      SummaryFooterType = cstSum
      SummaryFooterField = 'Camot1'
      SummaryFooterFormat = '###,###,##0;; '
      DisableFilter = True
      SummaryType = cstSum
      SummaryField = 'Camot1'
      SummaryFormat = '###,###,##0;; '
    end
    object dxDBGrid1creater: TdxDBGridMaskColumn
      DisableEditor = True
      TabStop = False
      Width = 45
      BandIndex = 0
      RowIndex = 0
      FieldName = 'creater'
      DisableFilter = True
    end
    object dxDBGrid1creat_dt: TdxDBGridDateColumn
      DisableEditor = True
      TabStop = False
      Width = 65
      BandIndex = 0
      RowIndex = 0
      FieldName = 'creat_dt'
      DisableFilter = True
    end
    object dxDBGrid1modifier: TdxDBGridColumn
      DisableEditor = True
      Width = 45
      BandIndex = 0
      RowIndex = 0
      FieldName = 'modifier'
      DisableFilter = True
    end
    object dxDBGrid1modify_dt: TdxDBGridColumn
      DisableEditor = True
      Width = 65
      BandIndex = 0
      RowIndex = 0
      FieldName = 'modify_dt'
      DisableFilter = True
    end
  end
  object wwDBNavigator1: TwwDBNavigator
    Left = 3
    Top = 0
    Width = 200
    Height = 25
    DataSource = DSbrokermed
    Options = []
    ShowHint = True
    RepeatInterval.InitialDelay = 500
    RepeatInterval.Interval = 100
    ParentShowHint = False
    object wwDBNavigator1First: TwwNavButton
      Left = 0
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1First'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 0
      Style = nbsFirst
    end
    object wwDBNavigator1Prior: TwwNavButton
      Left = 25
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21069
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Prior'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 1
      Style = nbsPrior
    end
    object wwDBNavigator1Next: TwwNavButton
      Left = 50
      Top = 0
      Width = 25
      Height = 25
      Hint = #21521#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Next'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 2
      Style = nbsNext
    end
    object wwDBNavigator1Last: TwwNavButton
      Left = 75
      Top = 0
      Width = 25
      Height = 25
      Hint = #26368#21518
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Last'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 3
      Style = nbsLast
    end
    object wwDBNavigator1Post: TwwNavButton
      Left = 100
      Top = 0
      Width = 25
      Height = 25
      Hint = 'Post changes of current record'
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Post'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 4
      Style = nbsPost
    end
    object wwDBNavigator1Cancel: TwwNavButton
      Left = 125
      Top = 0
      Width = 25
      Height = 25
      Hint = 'Cancel changes made to current record'
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1Cancel'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 5
      Style = nbsCancel
    end
    object wwDBNavigator1SaveBookmark: TwwNavButton
      Left = 150
      Top = 0
      Width = 25
      Height = 25
      Hint = #35774#32622#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1SaveBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 6
      Style = nbsSaveBookmark
    end
    object wwDBNavigator1RestoreBookmark: TwwNavButton
      Left = 175
      Top = 0
      Width = 25
      Height = 25
      Hint = #36820#22238#20070#31614
      ImageIndex = -1
      NumGlyphs = 2
      Spacing = 4
      Transparent = False
      Caption = 'wwDBNavigator1RestoreBookmark'
      Enabled = False
      DisabledTextColors.ShadeColor = clGray
      DisabledTextColors.HighlightColor = clBtnHighlight
      Index = 7
      Style = nbsRestoreBookmark
    end
  end
  object dxButtonEdit1: TdxButtonEdit
    Left = 50
    Top = 60
    Width = 200
    Font.Charset = GB2312_CHARSET
    Font.Color = clNavy
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 0
    Buttons = <
      item
        Default = True
      end>
    OnButtonClick = dxButtonEdit1ButtonClick
    ExistButtons = True
  end
  object dxLookupTreeView1: TdxLookupTreeView
    Left = 51
    Top = 81
    Width = 198
    Height = 20
    CanSelectParents = True
    DropDownRows = 30
    ParentColor = False
    TabOrder = 3
    TabStop = True
    TreeViewColor = clWindow
    TreeViewCursor = crDefault
    TreeViewFont.Charset = DEFAULT_CHARSET
    TreeViewFont.Color = clWindowText
    TreeViewFont.Height = -11
    TreeViewFont.Name = 'MS Sans Serif'
    TreeViewFont.Style = []
    TreeViewImages = ImageList
    TreeViewIndent = 19
    TreeViewReadOnly = True
    TreeViewShowButtons = True
    TreeViewShowHint = False
    TreeViewShowLines = True
    TreeViewShowRoot = True
    TreeViewSortType = stNone
    OnKeyDown = dxLookupTreeView1KeyDown
    DividedChar = '.'
    ListSource = DM.DSdistrict
    KeyField = 'rec_id'
    ListField = 'zdesc'
    Options = [trCanDBNavigate, trSmartRecordCopy, trCheckHasChildren]
    ParentField = 'parent'
    TextStyle = tvtsShort
    Alignment = taLeftJustify
  end
  object RadioGroup1: TRadioGroup
    Left = 1
    Top = 25
    Width = 175
    Height = 33
    Columns = 4
    ItemIndex = 0
    Items.Strings = (
      #31616#30721
      #21697#21517
      #21378#23478
      #32534#30721)
    TabOrder = 4
  end
  object dxEdit1: TdxEdit
    Left = 198
    Top = 34
    Width = 56
    Style.BorderColor = 12615680
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = bts3D
    Style.ButtonTransparence = ebtNone
    Style.HotTrack = False
    Style.Shadow = False
    TabOrder = 5
    OnKeyDown = dxEdit1KeyDown
  end
  object dxCheckEdit1: TdxCheckEdit
    Left = 1006
    Top = 4
    Width = 97
    Font.Charset = GB2312_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.Edges = []
    TabOrder = 6
    Visible = False
    Anchors = [akTop, akRight]
    Caption = #25353#27492#26465#20214#26597#35810
  end
  object Edit1: TEdit
    Left = 403
    Top = 28
    Width = 121
    Height = 20
    TabOrder = 7
    Text = 'Edit1'
    Visible = False
  end
  object dxSpinEdit2: TdxSpinEdit
    Left = 420
    Top = 78
    Width = 36
    Style.BorderColor = 14065456
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 8
    MaxValue = 12.000000000000000000
    MinValue = 1.000000000000000000
    Value = 1.000000000000000000
    StoredValues = 48
  end
  object dxSpinEdit1: TdxSpinEdit
    Left = 357
    Top = 78
    Width = 50
    Style.BorderColor = 14065456
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtAlways
    TabOrder = 9
    MaxValue = 2020.000000000000000000
    MinValue = 2000.000000000000000000
    Value = 2000.000000000000000000
    StoredValues = 48
  end
  object report02: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pvdpubqry'
    RemoteServer = DM.RemoteServer
    BeforePost = report02BeforePost
    AfterPost = report02AfterPost
    BeforeDelete = report02BeforeDelete
    AfterDelete = report02AfterPost
    OnCalcFields = report02CalcFields
    Left = 86
    Top = 229
    object report02rec_id: TIntegerField
      FieldName = 'rec_id'
    end
    object report02fyear: TIntegerField
      FieldName = 'fyear'
    end
    object report02fmonth: TWordField
      FieldName = 'fmonth'
    end
    object report02district_id: TIntegerField
      FieldName = 'district_id'
    end
    object report02dist1: TStringField
      DisplayLabel = #30465#20221
      FieldName = 'dist1'
      ReadOnly = True
      Size = 100
    end
    object report02dist2: TStringField
      DisplayLabel = #22478#24066
      FieldName = 'dist2'
      ReadOnly = True
      Size = 100
    end
    object report02med_id: TIntegerField
      FieldName = 'med_id'
    end
    object report02level1: TStringField
      DisplayLabel = #20107#19994#37096
      FieldName = 'level1'
      ReadOnly = True
      Size = 100
    end
    object report02level2: TStringField
      DisplayLabel = #22823#21306
      FieldName = 'level2'
      ReadOnly = True
      Size = 50
    end
    object report02level3: TStringField
      DisplayLabel = #23567#21306
      FieldName = 'level3'
      Size = 30
    end
    object report02leader: TStringField
      DisplayLabel = #21306#22495#32463#29702
      FieldName = 'leader'
      ReadOnly = True
      Size = 100
    end
    object report02qty1: TBCDField
      DisplayLabel = #26412#26376#39044#27979#25968#37327
      FieldName = 'qty1'
      DisplayFormat = '###,###,##0;; '
      Precision = 10
      Size = 0
    end
    object report02Camot1: TFloatField
      DisplayLabel = #26412#26376#39044#27979#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Camot1'
      DisplayFormat = '###,###,##0.00;; '
      Calculated = True
    end
    object report02qty2: TBCDField
      DisplayLabel = #26412#26376#21512#21516#25968#37327
      FieldName = 'qty2'
      DisplayFormat = '###,###,##0;; '
      Precision = 15
      Size = 2
    end
    object report02Camot2: TFloatField
      DisplayLabel = #26412#26376#21512#21516#37329#39069
      FieldKind = fkCalculated
      FieldName = 'Camot2'
      DisplayFormat = '###,###,##0.00;; '
      Calculated = True
    end
    object report02med_code: TStringField
      DisplayLabel = #32534#30721
      FieldName = 'med_code'
    end
    object report02med_name: TStringField
      DisplayLabel = #21697#21517
      FieldName = 'med_name'
      Size = 50
    end
    object report02specifi: TStringField
      DisplayLabel = #35268#26684
      FieldName = 'specifi'
      Size = 50
    end
    object report02pdt_place: TStringField
      DisplayLabel = #29983#20135#20225#19994
      FieldName = 'pdt_place'
      Size = 30
    end
    object report02med_unit: TStringField
      DisplayLabel = #21333#20301
      FieldName = 'med_unit'
      Size = 30
    end
    object report02price: TBCDField
      DisplayLabel = #25307#21830#20215
      FieldName = 'price'
      DisplayFormat = '###,###,##0.00##;; '
      Precision = 10
    end
    object report02zdesc: TStringField
      FieldName = 'zdesc'
      Size = 100
    end
    object report02creat_dt: TDateTimeField
      DisplayLabel = #35774#32622#26102#38388
      FieldName = 'creat_dt'
    end
    object report02creat_by: TIntegerField
      FieldName = 'creat_by'
    end
    object report02creater: TStringField
      DisplayLabel = #35774#32622#20154
      FieldName = 'creater'
    end
    object report02modify_dt: TDateTimeField
      DisplayLabel = #20462#25913#26102#38388
      FieldName = 'modify_dt'
    end
    object report02modify_by: TIntegerField
      FieldName = 'modify_by'
    end
    object report02modifier: TStringField
      DisplayLabel = #20462#25913#20154
      FieldName = 'modifier'
      Size = 100
    end
    object report02Cyearmonth: TStringField
      DisplayLabel = #39044#27979#24180#26376
      FieldKind = fkCalculated
      FieldName = 'Cyearmonth'
      Calculated = True
    end
  end
  object DSbrokermed: TDataSource
    DataSet = report02
    OnDataChange = DSbrokermedDataChange
    Left = 116
    Top = 230
  end
  object PopupMenu1: TPopupMenu
    Left = 133
    Top = 175
    object N3: TMenuItem
      Caption = #26597#25214'...'
      OnClick = N3Click
    end
    object N4: TMenuItem
      Caption = '-'
    end
    object N1: TMenuItem
      Caption = #38544#34255#26412#21015
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #26174#31034#25152#26377#21015
      OnClick = N2Click
    end
    object N5: TMenuItem
      Caption = '-'
    end
    object N6: TMenuItem
      Caption = #26684#24335#22797#21407
      OnClick = N6Click
    end
  end
  object ImageList: TImageList
    Left = 551
    Top = 52
    Bitmap = {
      
      000000000000}
  end
  object ADOQuery1: TADOQuery
    ConnectionString = 
      'Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;Initi' +
      'al Catalog=KingageYPH;Data Source=.'
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select top 1 qty2=f1 from tb_report01')
    Left = 308
    Top = 227
    object ADOQuery1qty2: TBCDField
      FieldName = 'qty2'
      Precision = 15
      Size = 2
    end
  end
end
