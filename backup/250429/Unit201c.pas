...
uses
  ...
  wwSpeedButton, wwDBNavigator, wwclearpanel, Buttons, dxdbtrel, ImgList;

type
  Tqryclassimp3 = class(TForm)
...
var
  qryclassimp3: Tqryclassimp3;

implementation

uses datamodu, Unit7, Unit16a, Unit16c, Main, Unit16f, Unit9;

{$R *.DFM}

procedure Tqryclassimp3.FormCloseQuery(Sender: TObject;
...
procedure Tqryclassimp3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tqryclassimp3.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tqryclassimp3.DSqryDataChange(Sender: TObject;
  Field: TField);
begin
speedbutton2.enabled:=(qry.active=true) and (qry.recordcount>0);
end;

procedure Tqryclassimp3.FormCreate(Sender: TObject);
begin
...
procedure Tqryclassimp3.SpeedButton2Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag)
end;

procedure Tqryclassimp3.SpeedButton5Click(Sender: TObject);
var si: integer;
    ss: string[12];
begin
...
begin
    ...
    commandtext:='select dtl_id,a.bod_id,a.bod_cd,e.med_code,e.med_name,e.specifi,mate_name1=g.mate_name,mate_name2=c.mate_name,a.agent_id,agent=d.mate_name,';
    ...
    commandtext:=commandtext+' a.cate_id,a.busi_type,b.lead_id,leader=dbo.fn_staff_name(b.lead_id),b.bod_cd1,';
    commandtext:=commandtext+' creat_dt=cast(convert(char(10),a.creat_dt,20) as datetime),a.carry_dt,';
    commandtext:=commandtext+' price1=dbo.fn_getbidprice9(a.dst_id,b.med_id,a.carry_dt),broker=f.zname,';
    commandtext:=commandtext+' med_unit=dbo.fn_obj_desc(e.unit_id),e.pdt_place,b.bat_cd,b.price,b.qty,b.amot,creater=dbo.fn_staff_name(a.creat_by),checker=dbo.fn_staff_name(a.check_by)';
    ...
    commandtext:=commandtext+' where a.bod_type_id=1 and a.cate_id=4';  // a.comp_id='+inttostr(compid)+' and
...
//        if Trim(dxLookupTreeView1.text)<>'' then commandtext:=commandtext+' and dbo.fn_treeischild(c.district,'+dm.district.fieldbyname('rec_id').asstring+')>0 '
        if Trim(dxLookupTreeView1.text)<>'' then commandtext:=commandtext+' and exists (select 1 from tb_treedata t where c.district=t.rec_id and (t.rec_id='+dm.district.fieldbyname('rec_id').asstring+' or t.parent='+dm.district.fieldbyname('rec_id').asstring+'))'; // dbo.fn_treeischild(c.district,'+dm.district.fieldbyname('rec_id').asstring+')>0 '
    end;
...
procedure Tqryclassimp3.dxButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tqryclassimp3.dxButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tqryclassimp3.N3Click(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure Tqryclassimp3.N1Click(Sender: TObject);
begin
with dxdbgrid1 do Columns[GetAbsoluteColumnIndex(FocusedColumn)].Visible:=false;
end;

procedure Tqryclassimp3.N2Click(Sender: TObject);
var i,mi: integer;
begin
setprogress(1);
with dxdbgrid1 do
...
procedure Tqryclassimp3.N6Click(Sender: TObject);
begin
if FileExists(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini')
    then dxDBGrid1.LoadFromIniFile(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini');//加载
end;

procedure Tqryclassimp3.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tqryclassimp3.qryCalcFields(DataSet: TDataSet);
begin
with qry do
begin
    ...
    end;
{
    ...
    fieldbyname('Ccate_id').asstring:='普通KA';
...
procedure Tqryclassimp3.dxButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tqryclassimp3.dxDBGrid1DblClick(Sender: TObject);
begin
...
procedure Tqryclassimp3.dxButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
//with pickbusimate5 do
...
procedure Tqryclassimp3.FormActivate(Sender: TObject);
begin
with dm.district do
...
procedure Tqryclassimp3.dxLookupTreeView1KeyDown(Sender: TObject;
