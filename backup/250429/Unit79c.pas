...
type
  Tsetcontractapply3 = class(TForm)
...
var
  setcontractapply3: Tsetcontractapply3;
...
procedure Tsetcontractapply3.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetcontractapply3.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetcontractapply3.FormActivate(Sender: TObject);
begin
with dm.district do
...
procedure Tsetcontractapply3.FormCloseQuery(Sender: TObject;
...
procedure Tsetcontractapply3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetcontractapply3.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetcontractapply3.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetcontractapply3.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetcontractapply3.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetcontractapply3.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=34'; // and a.bod_status_id in (1,5)';  // a.comp_id='+inttostr(compid)+' and
...
procedure Tsetcontractapply3.FormCreate(Sender: TObject);
begin
...
procedure Tsetcontractapply3.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetcontractapply3.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetcontractapply3.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetcontractapply3.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetcontractapply3.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetcontractapply3.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetcontractapply3.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetcontractapply3.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetcontractapply3.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetcontractapply3.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetcontractapply3.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetcontractapply3.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetcontractapply3.N1Click(Sender: TObject);
...
procedure Tsetcontractapply3.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetcontractapply3.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetcontractapply3.SpeedButton3Click(Sender: TObject);
begin
...
begin
    if active=true then close;
    commandtext:='sp_updatestock34 -34,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsetcontractapply3.SpeedButton2Click(Sender: TObject);
begin
...
procedure Tsetcontractapply3.dxEdit2KeyDown(Sender: TObject;
...
procedure Tsetcontractapply3.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetcontractapply3.SpeedButton7Click(Sender: TObject);
begin
