...
var
  setruleadjust3: Tsetruleadjust3;
...
procedure Tsetruleadjust3.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetruleadjust3.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetruleadjust3.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsetruleadjust3.FormCloseQuery(Sender: TObject;
...
procedure Tsetruleadjust3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetruleadjust3.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetruleadjust3.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with cdsbill_dtl do
...
procedure Tsetruleadjust3.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetruleadjust3.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetruleadjust3.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetruleadjust3.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.comp_id='+inttostr(compid)+' and a.bod_type_id=23 and a.bod_status_id in (1,4)';  //3,
...
procedure Tsetruleadjust3.FormCreate(Sender: TObject);
begin
...
procedure Tsetruleadjust3.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetruleadjust3.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetruleadjust3.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetruleadjust3.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetruleadjust3.SpeedButton2Click(Sender: TObject);
begin
if cdsbill_dtl.RecordCount=0 then raise Exception.Create('无明细数据，不可送审核');
if MessageBox(0,'确定本单通过财务审批','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
setprogress(1);
SpeedButton2.enabled:=false; // 避免连续按两次
with dm.pubqry do
begin
    if active=true then close;
    commandtext:='sp_updatestock23 23,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsetruleadjust3.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetruleadjust3.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetruleadjust3.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetruleadjust3.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
...
procedure Tsetruleadjust3.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetruleadjust3.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetruleadjust3.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetruleadjust3.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetruleadjust3.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetruleadjust3.N1Click(Sender: TObject);
...
procedure Tsetruleadjust3.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetruleadjust3.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetruleadjust3.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetruleadjust3.brokermedtlCalcFields(DataSet: TDataSet);
begin
with brokermedtl do
begin
...
procedure Tsetruleadjust3.dxDBGrid2leaderButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickstaff do
begin
...
procedure Tsetruleadjust3.dxDBGrid2brokerButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickbroker do
begin
...
procedure Tsetruleadjust3.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsetruleadjust3.SpeedButton3Click(Sender: TObject);
begin
