...
type
  Tsetmedexport5 = class(TForm)
    ...
    Label23: TLabel;
    ADOQuery1sub_busi_id: TWordField;
...
var
  setmedexport5: Tsetmedexport5;
  ptype: string[20];
  pcent: real;

implementation

uses datamodu, Unit14, Unit16a, Unit18, Unit16e, Unit16c, Unit16f;

{$R *.DFM}

procedure Tsetmedexport5.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetmedexport5.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetmedexport5.FormActivate(Sender: TObject);
begin
{
with dm.broker do
...
procedure Tsetmedexport5.FormCloseQuery(Sender: TObject;
...
procedure Tsetmedexport5.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetmedexport5.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedexport5.dxDBGrid2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
...
procedure Tsetmedexport5.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
exit;
...
begin
    if pickmedinfo.active=True then pickmedinfo.Close;
    exit;
end;
...
procedure Tsetmedexport5.dxDBGrid2Enter(Sender: TObject);
begin
...
procedure Tsetmedexport5.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetmedexport5.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetmedexport5.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetmedexport5.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    close;
    commandtext:='select a.*,mate_name1=b.mate_name,mate_name2=c.mate_name,creater=d.zname,checker=e.zname,broker=f.zname,';
    ...
    commandtext:=commandtext+' where a.bod_type_id=2 and a.cate_id=4';   // a.comp_id='+inttostr(compid)+' and
...
procedure Tsetmedexport5.FormCreate(Sender: TObject);
begin
...
procedure Tsetmedexport5.SpeedButton6Click(Sender: TObject);
var mid : integer;
begin
...
begin
    ...
    commandtext:=commandtext+' values ('+inttostr(compid)+',2,0,4,1,'+inttostr(curuserid)+',getdate(),cast(convert(char(10),getdate(),20) as datetime))';
...
procedure Tsetmedexport5.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetmedexport5.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetmedexport5.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
procedure Tsetmedexport5.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedexport5.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedexport5.SpeedButton2Click(Sender: TObject);
begin
with cdsbill do
begin
    if fieldbyname('src_id').asinteger=0 then raise Exception.Create('请选择商业公司');
    if fieldbyname('dst_id').asinteger=0 then raise Exception.Create('请选择药店');
    if fieldbyname('busi_type').asinteger=0 then raise Exception.Create('请选择出库类别');
    if fieldbyname('sub_busi_id').asinteger=0 then raise Exception.Create('请选择流向类别');
...
procedure Tsetmedexport5.SpeedButton4Click(Sender: TObject);
begin
...
procedure Tsetmedexport5.SpeedButton3Click(Sender: TObject);
var s: string;
begin
with dm.pubqry do
begin
...
procedure Tsetmedexport5.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedexport5.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedexport5.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetmedexport5.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
    then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
...
procedure Tsetmedexport5.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetmedexport5.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedexport5.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetmedexport5.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetmedexport5.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedexport5.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetmedexport5.dxDBGrid2DblClick(Sender: TObject);
var key:char;
begin
exit;
key:=#32;
...
procedure Tsetmedexport5.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
    SFileName,sbodcd,sb: string;
    J,mi,mid,id1,id2,id4,id8,id9,id10,id11: integer;
    rate: single;
    dt1,dt2: TDatetime;
    qty: real;
begin
    ...
    sbodcd:='';
    j:=2; mi:=0;  //第2行开始 每行一单
//格式:1商业公司id1, 2药店名称id2, 3发生日期dt1, 4药品编码id4, 5数量, 6批号, 7有效期dt2, 8流向类别(纯销/分销)id8, 9单据摘要, 10销售单价
    while (sheet.cells[j,1].text<>'') and (mi<10) do
    begin
        id1:=0; id2:=0; id4:=0; id8:=0; id9:=0; id10:=0; qty:=0;

        if sheet.cells[j,3].text='' then dt1:=dt0 else
        try
            dt1:=strtodatetime(Trim(sheet.cells[j,3].text));
        except
            dt1:=dt0;
        end;
        if sheet.cells[j,7].text='' then dt2:=dt0 else
        try
            dt2:=strtodatetime(Trim(sheet.cells[j,7].text));
        except
            dt2:=dt0;
        end;
        with dm.pubqry do
        begin
            ...
            begin
                if active=true then close;
                commandtext:='select top 1 mate_id,district from tb_busimate where mate_type_id=5 and mate_name ='''+Trim(sheet.cells[j,1].text)+'''';
                open;
                if recordcount>0 then
                begin
                    id1:=fieldbyname('mate_id').asinteger;
                    id9:=fieldbyname('district').asinteger;
                end;
            end;
            if active=true then close;
            ...
            begin
                if active=true then close;
                commandtext:='select top 1 mate_id,district from tb_busimate where mate_type_id=6 and mate_name ='''+Trim(sheet.cells[j,2].text)+'''';
                open;
                if recordcount>0 then id2:=fieldbyname('mate_id').asinteger;
            end;
{
                if active=true then close;
            ...
            begin
                ...
                commandtext:='select top 1 med_id from tb_medicine where med_code='''+Trim(sheet.cells[j,4].text)+'''';
{
                commandtext:='select top 1 a.med_id from tb_linkword a,tb_medicine b';  //,a.rate
                commandtext:=commandtext+' where a.type_id=1'; // and a.comp_id='+inttostr(compid);
                commandtext:=commandtext+'  and a.med_id=b.med_id and (a.zdesc='''+Trim(sheet.cells[j,4].text)+''' or b.med_code='''+Trim(sheet.cells[j,4].text)+''')';
                commandtext:=commandtext+'  and a.mate_id1='+inttostr(id1);
}
                open;
                if recordcount>0 then id4:=fieldbyname('med_id').asinteger;
            end;
            if (id9>0) and (id4>0) then
            begin
                if active=true then close;
                commandtext:='select top 1 sta_id from tb_busiframe1 where district_id='+inttostr(id9)+' and med_id='+inttostr(id4);
                commandtext:=commandtext+' and valid_dt<='''+datetimetostr(dt1)+''' order by valid_dt';
                open;
                if recordcount>0 then id10:=fieldbyname('sta_id').asinteger; // 区域经理
            end;

            if Trim(sheet.cells[j,8].text)='分销' then id8:=2 else id8:=1;

            if (id2=0) or (id2=0) or (id4=0) or (id8=0) or (id10=0) or (sheet.cells[j,4].text='') then
            begin
                sbodcd:=sbodcd+#13#10+'第'+inttostr(j)+'行:';
                if id1=0 then sbodcd:=sbodcd+',未取得商业公司';
                if id2=0 then sbodcd:=sbodcd+',未取得药店';
                if id4=0 then sbodcd:=sbodcd+',未取得药品';
                if id8=0 then sbodcd:=sbodcd+',未取得流向类别';
                if id10=0 then sbodcd:=sbodcd+',未取得区域经理';
                if sheet.cells[j,5].text='' then sbodcd:=sbodcd+',无数量';
                mi:=mi+1;
            end;
        end;
        inc(j);
...
//开始导入
//格式:1商业公司id1, 2药店名称id2, 3发生日期dt1, 4本公司产品编码id4, 5数量, 6批号, 7有效期dt2, 8流向类别(纯销/分销)id8, 9单据摘要, 10销售单价
                ...
                commandtext:='select top 1 med_id from tb_medicine where med_code='''+Trim(sheet.cells[j,4].text)+'''';
{
            ...
            if Trim(sheet.cells[j,8].text)='分销' then id8:=2 else id8:=1;
            if strtoint(cleardeli(sheet.cells[j,5].text))<0 then id11:=2 else id11:=1; //id11=1 销售，id11=2 退货

            sb:=formatfloat('00000000',getbillid(compid,1)); //notice:此处单据类别(bod_type_id)应取2,因发现时已发生大量单据，为避免单号重复，故沿用1
            sbodcd:=sbodcd+#13#10+sb;

            if active=true then close;
            commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,cate_id,busi_type,sub_busi_id,creat_by,creat_dt,src_id,dst_id,district_id,bod_desc,carry_dt)';  //lead_id,
            commandtext:=commandtext+' values ('+inttostr(compid)+','''+sb+''',2,0,4,'+inttostr(id11)+','+inttostr(id8)+','+inttostr(curuserid)+',getdate(),';
            commandtext:=commandtext+inttostr(id1)+','+ inttostr(id2)+','+inttostr(id9)+','''+Trim(sheet.cells[j,9].text)+''','''+datetimetostr(dt1)+''')';
            commandtext:=commandtext+' select @@identity';
            open;
            mid:=fields[0].asinteger;
//格式:1商业公司id1, 2药店名称id2, 3发生日期dt1, 4本公司产品编码id4, 5数量, 6批号, 7有效期dt2, 8流向类别(纯销/分销)id8, 9单据摘要, 10销售单价
            if active=true then close;
            commandtext:='insert into tb_bill_dtl (bod_id,lead_id,med_id,price,qty,amot,bat_cd,valid_dt)';
            commandtext:=commandtext+' values ('+inttostr(mid)+','+inttostr(id10)+','+inttostr(id4)+',';
            if sheet.cells[j,10].text='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,10].text)+','; //销售单价
            if sheet.cells[j,5].text='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,5].text)+','; //销售数量
            if (sheet.cells[j,5].text='') or (sheet.cells[j,10].text='')
                then commandtext:=commandtext+'0,'
                else commandtext:=commandtext+'cast('+cleardeli(sheet.cells[j,5].text)+'*'+cleardeli(sheet.cells[j,10].text)+' as decimal(15,2)),';  //金额
            commandtext:=commandtext+''''+sheet.cells[j,6].text+''',';  //批号
            commandtext:=commandtext+''''+datetimetostr(dt2)+''')';  //有效期

            commandtext:=commandtext+' update tb_bill_dtl set amot=round(price*qty,2) where bod_id='+inttostr(mid);
            execute;
{
...
procedure Tsetmedexport5.N1Click(Sender: TObject);
...
{       ///后面取中标价
        ...
        commandtext:=commandtext+'  and not exists (select 1 from tb_busiframe1_dtl f where b.med_id=f.med_id';  // f.comp_id='+inttostr(compid)+' and
        ...
        if active=true then close;  //按业务规则补 业务员、区域经理 数据
...
{      //后面取业务规则中的区域经理、业务员
        ...
        if active=true then close;  //检查中标价
        commandtext:='select top 5 a.bod_cd,m.med_code,m.med_name,m.specifi,m.pdt_place';
        commandtext:=commandtext+' from tB_bill a,tb_bill_dtl b,tb_busimate c,tb_medicine m';
    ...
    try
        try
            ...
            if sender= N1 then
            begin
                if active=true then close;  //取中标价
...
procedure Tsetmedexport5.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetmedexport5.N3Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t : string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetmedexport5.dxDBButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetmedexport5.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetmedexport5.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetmedexport5.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tsetmedexport5.Button1Click(Sender: TObject);
begin
...
procedure Tsetmedexport5.N5Click(Sender: TObject);
begin
if pubqry.active=true then pubqry.close;
pubqry.commandtext:='select bod_id from tb_bill where bod_type_id=2 and isnull(mate_id1,0)=0 and isnull(mate_id2,0)=0 and isnull(mate_id3,0)=0 and isnull(mate_id4,0)=0 and isnull(mate_id5,0)=0';
pubqry.open;
...
procedure Tsetmedexport5.dxEdit1Click(Sender: TObject);
begin
dxEdit1.SelectAll;
dxEdit1.SetFocus;
end;

procedure Tsetmedexport5.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetmedexport5.dxDBButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedexport5.dxDBButtonEdit2KeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#32) and ((Sender as TdxDBButtonEdit).Text<>'') then //
begin
    pickbusimate5.Edit1.Text:=(Sender as TdxDBButtonEdit).Text;
    pickbusimate5.Edit1.OnKeyPress(Sender,Key);
    (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
    (Sender as TdxDBButtonEdit).SelectAll;
end
end;

procedure Tsetmedexport5.dxDBButtonEdit4KeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#32) and ((Sender as TdxDBButtonEdit).Text<>'') then //
begin
    pickstore.Edit1.Text:=(Sender as TdxDBButtonEdit).Text;
    pickstore.Edit1.OnKeyPress(Sender,Key);
    (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
    (Sender as TdxDBButtonEdit).SelectAll;
end
end;

procedure Tsetmedexport5.dxDBButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickstore do
begin
