...
type
  Tsettruststopapply3 = class(TForm)
...
var
  settruststopapply3: Tsettruststopapply3;
...
procedure Tsettruststopapply3.setupdatestatus;    // 编辑状态
begin
...
procedure Tsettruststopapply3.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsettruststopapply3.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsettruststopapply3.FormCloseQuery(Sender: TObject;
...
procedure Tsettruststopapply3.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsettruststopapply3.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsettruststopapply3.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with cdsbill_dtl do
...
procedure Tsettruststopapply3.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsettruststopapply3.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.comp_id='+inttostr(compid)+' and a.bod_type_id=26 and a.bod_status_id in (1,4)';
    commandtext:=commandtext+'  and a.creat_dt>= '''+datetostr(dxdateedit1.date)+''' and a.creat_dt< dateadd(day,1,'''+datetostr(dxdateedit4.date)+''')';
    if fdistltd then commandtext:=commandtext+' and not exists (select 1 from tb_bill_dtl c,tb_busimate i,tb_trustdistrict t where t.sta_id='+inttostr(curuserid)+' and a.bod_id=c.bod_id and c.mate_id1=i.mate_id and dbo.fn_ischildnode(i.district,t.dist_id)=0)';
...
procedure Tsettruststopapply3.FormCreate(Sender: TObject);
begin
...
procedure Tsettruststopapply3.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
begin
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active=true then close;
                commandtext:='select b.bod_id,b.dtl_id,b.valid_dt,a.rec_id,a.mate_id,e.mate_name,a.mate_id1,mate_name1=dbo.fn_mate_name(a.mate_id1),a.med_id,m.med_code,m.med_name,m.chm_name,m.specifi,m.pdt_place,';
                ...
                commandtext:=commandtext+'	b.price,b.price1,b.price2,b.rate,b.rela_value,b.cost,b.qty,a.f1,a.f2,a.f3,a.f4,a.level_id,a.valid';
                commandtext:=commandtext+'	from tb_bill_dtl b';
                commandtext:=commandtext+'  inner join tb_sysrule a on b.med_id=a.rec_id';
...
procedure Tsettruststopapply3.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsettruststopapply3.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsettruststopapply3.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsettruststopapply3.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsettruststopapply3.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsettruststopapply3.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsettruststopapply3.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsettruststopapply3.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsettruststopapply3.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsettruststopapply3.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsettruststopapply3.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsettruststopapply3.N1Click(Sender: TObject);
...
procedure Tsettruststopapply3.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsettruststopapply3.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsettruststopapply3.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsettruststopapply3.brokermedtlCalcFields(DataSet: TDataSet);
begin
with brokermedtl do
begin
...
procedure Tsettruststopapply3.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsettruststopapply3.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsettruststopapply3.SpeedButton2Click(Sender: TObject);
begin
...
begin
    if active=true then close;
    commandtext:='sp_updatestock26 26,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsettruststopapply3.SpeedButton3Click(Sender: TObject);
begin
