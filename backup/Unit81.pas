...
type
  Tsetreport17 = class(TForm)
    ...
    qryCqty: TFloatField;
    ADOQuery1q1: TBCDField;
    ADOQuery1q2: TBCDField;
    ADOQuery1q3: TBCDField;
    ADOQuery1q4: TBCDField;
    qryq1: TBCDField;
    qryq2: TBCDField;
    qryq3: TBCDField;
    qryq4: TBCDField;
    ADOQuery2dt3: TDateTimeField;
...
var
  setreport17: Tsetreport17;
  frecid: integer;

implementation

uses datamodu, Unit7, Unit28, Unit28a, Unit16f, Unit16c, Unit28c,
  Unit28d, Unit16;

{$R *.DFM}

procedure Tsetreport17.dxButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetreport17.SpeedButton2Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'1.xls', dxDBGrid1.SaveToXLS, self.tag)
end;

procedure Tsetreport17.DSqryDataChange(Sender: TObject;
  Field: TField);
begin
with qry do
begin
...
begin
    ...
    commandtext:=commandtext+' from tb_report04dtl a left join tb_busimate b on a.mate_id1=b.mate_id left join tb_staff c on a.creat_by=c.sta_id';
...
procedure Tsetreport17.qryBeforeDelete(DataSet: TDataSet);
begin
if qry.fieldbyname('valid').asboolean=True then raise Exception.Create('本记录已生效，不可删除');
with dm.pubqry do
begin
    if active=True then close;
    commandtext:='select top 1 1 from tb_report04dtl where rec_id='+qry.fieldbyname('rec_id').asstring;
    open;
...
procedure Tsetreport17.qryAfterPost(DataSet: TDataSet);
begin
if qry.ChangeCount>0 then myupdaterefresh(qry,'tb_report04','rec_id','');
//refreshmyrecord(qry,dm.Refreshcds,0,'a.rec_id',qry.fieldbyname('rec_id').asinteger);
end;

procedure Tsetreport17.SpeedButton3Click(Sender: TObject);
var mi: integer;
begin
with qry do
begin
    ...
    commandtext:='select a.*,signer=d.zname,level1=dbo.fn_obj_desc(e.level_id1),level2=dbo.fn_obj_desc(e.level_id2),level3=dbo.fn_obj_desc(e.level_id3),b.mate_name,mate_name1=f.mate_name,';
    ...
    commandtext:=commandtext+' from tb_report04 a';
    ...
    commandtext:=commandtext+' left join tb_staff d on a.sign_by=d.sta_id';
{
    if (dxCheckEdit1.Checked) and ((Trim(dxLookupEdit2.text)<>'') or (Trim(dxButtonEdit6.text)<>''))
        then commandtext:=commandtext+' inner join tb_busiframe1 e on a.level_id1=e.level_id1 and a.lead_id=e.sta_id and a.med_id=e.med_id and dbo.fn_treeischild(a.district_id,e.district_id)=1' // a.district_id=e.district_id
        else commandtext:=commandtext+' left join tb_busiframe1 e on a.level_id1=e.level_id1 and a.lead_id=e.sta_id and a.med_id=e.med_id and dbo.fn_treeischild(a.district_id,e.district_id)=1';
}
    if (dxCheckEdit1.Checked) and ((Trim(dxLookupEdit2.text)<>'') or (Trim(dxButtonEdit6.text)<>''))
        then commandtext:=commandtext+' inner join tb_busiframe1 e on a.med_id=e.med_id and dbo.fn_treeischild(a.district_id,e.district_id)=1' // a.district_id=e.district_id
        else commandtext:=commandtext+' left join tb_busiframe1 e on a.med_id=e.med_id and dbo.fn_treeischild(a.district_id,e.district_id)=1';
...
procedure Tsetreport17.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetreport17.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetreport17.FormCloseQuery(Sender: TObject;
...
procedure Tsetreport17.FormCreate(Sender: TObject);
begin
...
procedure Tsetreport17.N3Click(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure Tsetreport17.N1Click(Sender: TObject);
begin
with dxdbgrid1 do Columns[GetAbsoluteColumnIndex(FocusedColumn)].Visible:=false;
end;

procedure Tsetreport17.N2Click(Sender: TObject);
var i,mi: integer;
begin
setprogress(1);
with dxdbgrid1 do
...
procedure Tsetreport17.N6Click(Sender: TObject);
begin
if FileExists(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini')
    then dxDBGrid1.LoadFromIniFile(extractfilepath(Application.ExeName)+self.name+'-dxDBGrid10.ini');//加载
end;

procedure Tsetreport17.FormActivate(Sender: TObject);
begin
with dm.district do
...
with type2 do  //所属 一品红药业/康乃馨
begin
    if active=False then
    begin
        commandtext:='select obj_id=0,zdesc='''' union select obj_id=1,zdesc=''一品红药业'' union select obj_id=2,zdesc=''康乃馨''';
        open;
    end;
end;
with type3 do  //合同类型 战略KA/普通KA/区域代理/三方合作
begin
    if active=False then
    begin
        commandtext:='select obj_id=0,zdesc='''' union select obj_id=1,zdesc=''战略KA'' union select obj_id=2,zdesc=''普通KA'' union select obj_id=3,zdesc=''区域代理'' union select obj_id=4,zdesc=''三方合作''';
        open;
    end;
end;
end;

procedure Tsetreport17.dxLookupTreeView1CloseUp(Sender: TObject;
  Accept: Boolean);
begin
speedbutton4.Enabled:=(dxbuttonedit1.tag<>0) and (dxLookupTreeView1.text<>'');
end;

procedure Tsetreport17.dxLookupTreeView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if key=vk_delete then dxLookupTreeView1.text:='';
end;

procedure Tsetreport17.qryBeforePost(DataSet: TDataSet);
begin
with qry do
begin
...
procedure Tsetreport17.SpeedButton5Click(Sender: TObject);
begin
with qry do
begin
...
begin
    if active=true then close;
    commandtext:='update tb_report04 set modify_by='+inttostr(curuserid)+',modify_dt=getdate()';
...
procedure Tsetreport17.dxButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetreport17.dxButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickbusimate5 do
begin
...
procedure Tsetreport17.dxButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetreport17.Button1Click(Sender: TObject);
begin
with dm.pubqry do
begin
    if active=True then close;
    commandtext:='update tb_report04 set creat_by='+inttostr(curuserid);
    execute;
end;
end;

procedure Tsetreport17.qryBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetreport17.N7Click(Sender: TObject);
begin
with qry do
begin
...
procedure Tsetreport17.N8Click(Sender: TObject);
var mi: integer;
begin
with qry do
begin
...
begin
    if active=True then close;
    commandtext:='insert into tb_report04 (comp_id,fyear,type_id1,type_id2,sign_dt,level_id1,lead_id,broker_id,district_id,mate_id,med_id,';
    ...
    commandtext:=commandtext+' from tb_report04 where rec_id='+inttostr(frecid);
...
procedure Tsetreport17.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetreport17.SpeedButton6Click(Sender: TObject);
var
    XL,Sheet: Variant;
    SFileName: string;
...
//11 药品编码,12 供货价,13 结算方式,14 返利政策,15 铺底货数量,16 铺底货回款期限,
//17 1季度协议量,18 2季度协议量,19 3季度协议量,20 4季度协议量,21 起始日期,22 结束日期
...
//  11药品编码id11
            if (id4=0) or (id5=0) or (id6=0) or (id8=0) or (id9=0) or (id11=0) then
            begin
...
//从第2行开始导入，导入格式: 1配送商编码,2配送商名称
    ...
    begin
        ...
        begin
            ...
            if active=true then close;
            commandtext:='select top 1 rec_id from tb_report04 where comp_id= '+inttostr(compid)+' and fyear='+sheet.cells[j,1].text;
...
//  26首批提货量,27目标销量,28预计开发月份,29起始日期,30结束日期,31其他约定,32保证金
                if active=true then close;
                commandtext:='insert into tb_report04 (comp_id,fyear,type_id2,sign_dt,level_id1,lead_id,district_id,broker_id,mate_id,mate_id1,med_id,';
                commandtext:=commandtext+'price,contact1,appoint,qty1,dt3,q1,q2,q3,q4,dt1,dt2,creat_by,creat_dt)';
                ...
                commandtext:=commandtext+''''+Trim(sheet.cells[j,13].text)+''','; //结算方式
                commandtext:=commandtext+''''+Trim(sheet.cells[j,14].text)+''','; //返利政策
                if sheet.cells[j,15].text='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,15].text)+','; //铺底货数量
                if sheet.cells[j,16].text='' then dt:=0 else
                try
                    dt:=strtodatetime(Trim(sheet.cells[j,16].text));
                except
                    dt:=0;
                end;
                if dt=0 then commandtext:=commandtext+'null,' else commandtext:=commandtext+''''+datetimetostr(dt)+''',';
                if sheet.cells[j,17].text='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,17].text)+','; //1季度协议量
                if sheet.cells[j,18].text='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,18].text)+','; //2季度协议量
                if sheet.cells[j,19].text='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,19].text)+','; //3季度协议量
                if sheet.cells[j,20].text='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,20].text)+','; //4季度协议量
                if sheet.cells[j,21].text='' then dt:=0 else
                try
                    dt:=strtodatetime(Trim(sheet.cells[j,21].text));
                except
                    dt:=0;
                end;
                if dt=0 then commandtext:=commandtext+'null,' else commandtext:=commandtext+''''+datetimetostr(dt)+''',';
                if sheet.cells[j,22].text='' then dt:=0 else
                try
                    dt:=strtodatetime(Trim(sheet.cells[j,22].text));
                except
                    dt:=0;
                end;
                if dt=0 then commandtext:=commandtext+'null,' else commandtext:=commandtext+''''+datetimetostr(dt)+''',';
    ...
    //exit;
            ...
            begin
                if active=true then close;
                commandtext:='if not exists (select 1 from tb_report04dtl where rec_id='+inttostr(mi)+' and mate_id1='+inttostr(id10)+')';
                commandtext:=commandtext+' insert into tb_report04dtl (rec_id,mate_id1,qty2,creat_dt,creat_by)';
                ...
                commandtext:=commandtext+' else update tb_report04dtl set qty2=';
                if sheet.cells[j,27].text='' then commandtext:=commandtext+'0,' else commandtext:=commandtext+cleardeli(sheet.cells[j,27].text)+',';
...
procedure Tsetreport17.qrydtlAfterPost(DataSet: TDataSet);
begin
if qrydtl.ChangeCount>0 then myupdaterefresh(qrydtl,'tb_report04dtl','dtl_id','');
end;

procedure Tsetreport17.qrydtlBeforeDelete(DataSet: TDataSet);
begin
if qry.fieldbyname('valid').asboolean=True then raise Exception.Create('本记录已生效，不可删除');
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetreport17.qrydtlBeforePost(DataSet: TDataSet);
begin
with qrydtl do
begin
    if state=dsinsert then
...
procedure Tsetreport17.dxDBGrid2mate_name1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetreport17.DSqrydtlStateChange(Sender: TObject);
begin
if qrydtl.state in [dsinsert,dsedit] then qry.edit;
end;

procedure Tsetreport17.wwDBNavigator1PostClick(Sender: TObject);
begin
...
procedure Tsetreport17.wwDBNavigator1CancelClick(Sender: TObject);
begin
if (qrydtl.State in [dsinsert,dsedit]) then qrydtl.cancelUpdates;
end;

procedure Tsetreport17.dxDBGrid2Enter(Sender: TObject);
begin
with qrydtl do
    if recordcount=0 then append;
end;

procedure Tsetreport17.dxButtonEdit5ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickmanager do
begin
...
procedure Tsetreport17.dxDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetreport17.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetreport17.MenuItem9Click(Sender: TObject);
begin
...
procedure Tsetreport17.dxDBGrid1signerButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickmanager do
begin
...
procedure Tsetreport17.dxButtonEdit6ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with picklevel3 do
begin
...
procedure Tsetreport17.N10Click(Sender: TObject);
...
begin
    if active=true then close;
    commandtext:='select top 1 bod_cd from tb_bill a,tb_bill_dtl b where a.bod_type_id=33'; // and bod_status_id<>1';
    ...
    if active=true then close;
    commandtext:='select top 1 1 from tb_report04 where rec_id in ('+t+') and valid=1';
    open;
    if recordcount>0 then raise Exception.Create('所选合同中有已生效合同,不可再次申请');
{
    if active=true then close;
    commandtext:='select top 1 1 from tb_report04 where rec_id in ('+t+') and isnull(creat_by,0)<>'+inttostr(curuserid);
    open;
...
mcd:=formatfloat('00000000',getbillid(0,33)); //bod_type_id=33: 终端合同申请单
with dm.pubqry do
begin
    setprogress(1);
    if active=true then close;
    ...
    commandtext:=commandtext+' values ('+inttostr(compid)+','''+mcd+''',33,0,'+inttostr(curuserid)+',getdate())';
    commandtext:=commandtext+' select @@identity';
//edit1.text:=commandtext;
    open;
    mi:=fields[0].asinteger;
    ...
    commandtext:=commandtext+' from tb_report04 where rec_id in ('+t+')';
    execute;
    if active=True then close;
    setprogress(0);
...
procedure Tsetreport17.qryAfterInsert(DataSet: TDataSet);
begin
with qry do
begin
...
procedure Tsetreport17.N11Click(Sender: TObject);
var i,mi,l,m,n,k,o,p,q,i1,i2,i3,i4,i5,i6,m1,m2,m3 : integer;
    s,t: string;
    mcd: string[20];
begin
...
begin
    if active=true then close;
    commandtext:='select top 1 bod_cd from tb_bill a,tb_bill_dtl b where a.bod_type_id=34'; // and bod_status_id<>1';
    commandtext:=commandtext+' and a.bod_id=b.bod_id and b.med_id in ('+t+')';
    open;
    if recordcount>0 then raise Exception.Create('所选合同已生成'+fieldbyname('bod_cd').asstring+'号合同失效申请单尚未完成审批,不可再次生成');
    if active=true then close;
    commandtext:='select top 1 1 from tb_report04 where rec_id in ('+t+') and isnull(valid,0)=0';
    open;
...
mcd:=formatfloat('00000000',getbillid(0,34)); //bod_type_id=28: 合同申请单
with dm.pubqry do
begin
    setprogress(1);
    if active=true then close;
    ...
    commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,creat_by,creat_dt)';
    commandtext:=commandtext+' values ('+inttostr(compid)+','''+mcd+''',34,0,'+inttostr(curuserid)+',getdate())';
    commandtext:=commandtext+' select @@identity';
//edit1.text:=commandtext;
    open;
    mi:=fields[0].asinteger;
...
procedure Tsetreport17.qryCalcFields(DataSet: TDataSet);
begin
with qry do
begin
    fieldbyname('cqty').asfloat:=fieldbyname('q1').asfloat +fieldbyname('q2').asfloat +fieldbyname('q3').asfloat +fieldbyname('q4').asfloat;
end;
end;
