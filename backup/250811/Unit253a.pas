...
begin
    ...
    close;
    commandtext:='select top 5000 a.*,g.mate_name,creater=d.zname,checker=e.zname,bank_name=dbo.fn_obj_desc(a.stock_id),broker=c.zname,agent=f.mate_name,m.material_code1,m.med_code,m.med_name,m.specifi,m.pdt_place';
    ...
    begin
        ...
        speedbutton3.enabled:=(state=dsbrowse) and (active=true) and (recordcount>0)
            ...
            and ((fieldbyname('check_by').asinteger=curuserid) and (datetostr(fieldbyname('check_dt').asdatetime)=datetostr(dt0))
                or (curuserid=19232)); //19232 -> 左志华  and (datetostr(fieldbyname('check_dt').asdatetime)>=datetostr(dt_1))); // 审核当天可反审
