...
type
  Tsetmedexport = class(TForm)
    ...
    procedure dxDBGrid2material_codeButtonClick(Sender: TObject;
      AbsoluteIndex: Integer);
    procedure cdsbill_dtlprice3Change(Sender: TField);
  private
...
var
  setmedexport: Tsetmedexport;
  ptype: string[20];
  pcent: real;

implementation

uses datamodu, Unit14, Unit16a, Unit18, Unit16b, Unit61, Unit7;

{$R *.DFM}

procedure Tsetmedexport.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetmedexport.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetmedexport.FormActivate(Sender: TObject);
begin
{
with dm.district do
...
procedure Tsetmedexport.FormCloseQuery(Sender: TObject;
...
procedure Tsetmedexport.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetmedexport.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedexport.dxDBGrid2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
...
1级可配送 1级、2级、医院;2级可配送 2级、3级、医院;依此类推
            ...
            begin
                ...
                fieldbyname('material_code').asstring:=pickmed_bat.query1.FieldByName('material_code').asstring;
...
//                    if fmatetypeid=2 then //商业公司
                    ...
                    cdsbill_dtl.fieldbyname('material_code').asstring:=pickmed_bat.query1.FieldByName('material_code').asstring;
...
procedure Tsetmedexport.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
exit;
...
procedure Tsetmedexport.dxDBGrid2Enter(Sender: TObject);
begin
...
procedure Tsetmedexport.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetmedexport.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin

if (edit1.text='') or (cdsbill.fieldbyname('dst_id').asinteger=0) then raise Exception.Create('请选择购货单位');
...
procedure Tsetmedexport.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetmedexport.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    close;
    commandtext:='select c.district,a.*,mate_name1=b.mate_name,mate_name2=c.mate_name,c.mate_type_id,creater=d.zname,checker=e.zname,broker=f.zname,';
    ...
    commandtext:=commandtext+' agent=j.zname,level1=g.zdesc,level2=h.zdesc,level3=i.zdesc,cdistrict=dbo.fn_getdistrict1(c.district)';
//    commandtext:=commandtext+' platform_id=dbo.fn_getdtlplatformid(a.bod_id)';
    ...
    commandtext:=commandtext+' where a.bod_type_id=2 and a.cate_id=1'; // cate_id 1:商业 2:分销 3:终端战略 4:终端普通 5:两票商业 6:连锁终端销售出库单  7:零售销售出库单
...
//查询调用销货单 用 setmedexport3
{
    ...
    if Trim(dxEdit1.text)='' then commandtext:=commandtext+'  and a.creat_dt>= '''+datetostr(dxdateedit1.date)+''' and a.creat_dt< dateadd(day,1,'''+datetostr(dxdateedit4.date)+''')'
...
procedure Tsetmedexport.FormCreate(Sender: TObject);
begin
...
procedure Tsetmedexport.SpeedButton6Click(Sender: TObject);
var mid : integer;
begin
// insert speedbutton
...
begin
    ...
    commandtext:=commandtext+' values ('+inttostr(compid)+',2,0,1,1,'+inttostr(curuserid)+',getdate(),cast(convert(char(10),getdate(),20) as datetime))';
...
procedure Tsetmedexport.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetmedexport.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
//            and ((fieldbyname('creat_by').asinteger=curuserid) or (fieldbyname('check_by').asinteger=curuserid))
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active then close;
//                commandtext:='select a.*,leader=e.zname,broker=d.zname,b.med_name,b.specifi,b.pdt_place,b.med_code,b.material_code,b.material_code1,b.site_code,b.qtyperpack,b.qtyperbox,med_unit=c.zdesc,'; //dbo.fn_obj_desc(m.unit_id),'; //';
                commandtext:='select a.*,leader=j.BU_SORT1,broker=dbo.fn_staff_name(h.sta_id),b.med_name,b.specifi,b.pdt_place,b.med_code,b.material_code,b.material_code1,b.site_code,b.qtyperpack,b.qtyperbox,med_unit=c.zdesc';
//                commandtext:=commandtext+' cate_id=dbo.fn_getmatemedcateid('+inttostr(compid)+','+cdsbill.fieldbyname('src_id').asstring+',a.med_id)';
                commandtext:=commandtext+' ,channel=dbo.fn_obj_desc1(11,a.channel_id),channel_dtl=dbo.fn_obj_desc1(12,a.channel_dtl_id)';
                commandtext:=commandtext+' from tb_bill_dtl a';
                commandtext:=commandtext+' inner join tb_bill i on a.bod_id=i.bod_id';
                commandtext:=commandtext+' left join tb_busimate x on i.dst_id=x.mate_id';
...
//                commandtext:=commandtext+' left join tb_brokermed h on h.rec_id =dbo.fn_getbrokermedrecid('+inttostr(cdsbill.fieldbyname('src_id').asinteger)+','+inttostr(cdsbill.fieldbyname('dst_id').asinteger)+',a.med_id,'''+cdsbill.fieldbyname('carry_dt').asstring+''')';
                commandtext:=commandtext+' left join tb_brokermed h on h.rec_id =dbo.fn_getbrokermedrecid(i.src_id,i.dst_id,a.med_id,i.carry_dt)';
                // dbo].[fn_getbrokermedrecid] (@mateid1 int,@mateid int,@medid int,@dt datetime)
                commandtext:=commandtext+' left join SAP_ZSD_BUS j on j.rec_id =dbo.fn_getSAP_ZSD_BUSrecid(a.channel_id,a.channel_dtl_id,x.district,x.mate_code,b.material_code,i.carry_dt)';
...
procedure Tsetmedexport.Edit1KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
    fwid: boolean; //  东方语言 宽字符 (2bytes)
...
procedure Tsetmedexport.Edit1Click(Sender: TObject);
begin
(sender as TEdit).selectall;
end;

procedure Tsetmedexport.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
procedure Tsetmedexport.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedexport.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedexport.SpeedButton2Click(Sender: TObject);
var s: string;
begin
with cdsbill do
begin
...
{
    if active then close;
    commandtext:='select top 3 b.med_id,m.material_code,m.med_code,m.med_name,m.specifi,m.pdt_place from tb_bill a,tb_bill_dtl b,tb_medicine m where a.bod_id='+ cdsbill.fieldbyname('bod_id').asstring;
    commandtext:=commandtext+' and a.bod_id=b.bod_id and b.med_id=m.med_id';
    commandtext:=commandtext+' and not exists (select 1 from tb_sysrule c,tb_medicine n where c.type_id=2 and c.mate_id=a.src_id and c.med_id=n.med_id and m.material_code1=n.material_code1 and c.valid_dt<=a.carry_dt)'; // c.comp_id='+inttostr(compid);
edit5.Text:=commandtext;
    try
        setprogress(1);
        open;
    ...
    begin
        ...
        while not eof do
        begin
            s := s+ #13#10 +'['+fieldbyname('material_code').asstring+']'+fieldbyname('med_name').asstring+' '+fieldbyname('specifi').asstring+' '+fieldbyname('pdt_place').asstring;
            next;
        end;
    ...
    if active then close;
{
    commandtext:='select top 3 m.material_code,m.med_code,m.med_name,m.specifi,m.pdt_place';
...
//    commandtext:=commandtext+' left join (select i.rec_id,i.type_id2,i.price1,j.rate from tb_brokermed i inner join tb_brokermedtl j on i.rec_id=j.rela_id) d on d.rec_id =dbo.fn_getbrokermedrecid(a.src_id,a.dst_id,b.med_id,a.carry_dt)'; //b.rule_id=d.rec_id';
...
{
        if active then close;
        commandtext:='select top 5 m.material_code,m.med_code,m.med_name,m.specifi,m.pdt_place';
        commandtext:=commandtext+' from tb_bill a,tb_bill_dtl b,tb_medicine m';
        commandtext:=commandtext+' where a.bod_id='+ cdsbill.fieldbyname('bod_id').asstring;
        commandtext:=commandtext+'  and a.bod_id=b.bod_id and b.med_id=m.med_id';
...
procedure Tsetmedexport.SpeedButton4Click(Sender: TObject);
begin
...
procedure Tsetmedexport.SpeedButton3Click(Sender: TObject);
var s: string;
begin
with dm.pubqry do
begin
...
procedure Tsetmedexport.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedexport.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedexport.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetmedexport.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1 then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
...
procedure Tsetmedexport.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
    checkmatemed(cdsbill.fieldbyname('src_id').asinteger,cdsbill.fieldbyname('dst_id').asinteger,FieldByName('med_id').asinteger);
    if (fieldbyname('price3').asfloat=0) or (fieldbyname('qty').asfloat=0)
...
procedure Tsetmedexport.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedexport.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetmedexport.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetmedexport.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedexport.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetmedexport.dxDBGrid2DblClick(Sender: TObject);
var key:char;
begin
exit;
key:=#32;
...
procedure Tsetmedexport.Edit2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
    fwid: boolean; //  东方语言 宽字符 (2bytes)
...
procedure Tsetmedexport.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
    J: integer;
...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1发生日期, 2业务类别, 3商业公司, 4单据摘要, 5商务人员, 6物料编码, 7批号, 8数量, 9单价, 10渠道编码, 11子渠道编码
//新格式:1供货单位, 2购货单位, 3单据摘要, 4发生日期, 5单据类别, 6物料编码, 7数量, 8批号, 9有效期至, 10商业价, 11渠道名称, 12子渠道名称
    j:=2;  //第1行开始 每行一单   //mi:=0;
    sql:='declare @tab table (line_no varchar(3),f1 varchar(300),f2 varchar(300),f3 varchar(100),f4 varchar(100),f5 varchar(100),f6 varchar(100),f7 varchar(100),f8 varchar(100),f9 varchar(100),f10 varchar(100),f11 varchar(100),f12 varchar(100))';
    sql:=sql+' insert into @tab (line_no,f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12)';
    ...
    begin
        ...
        sql:=sql+''','''+Trim(sheet.cells[j,6].text)+''','''+Trim(sheet.cells[j,7].text)+''','''+Trim(sheet.cells[j,8].text)+''','''+Trim(sheet.cells[j,9].text)+''','''+Trim(sheet.cells[j,10].text)+''','''+Trim(sheet.cells[j,11].text)+''','''+Trim(sheet.cells[j,12].text)+'''';
        j:=j+1;
    ...
    begin
        setprogress(0);
        MessageBox(0,pchar('导入记录数不可超过1000行'),'请注意',MB_OK+MB_IconError);
        abort;
    end;
//从第2行开始导入，格式: 1发生日期, 2业务类别, 3商业公司, 4单据摘要, 5商务人员, 6物料编码, 7批号, 8数量, 9单价, 10渠道, 11子渠道
//新格式:1供货单位, 2购货单位, 3单据摘要, 4发生日期, 5单据类别, 6物料编码, 7数量, 8批号, 9有效期至, 10商业价, 11渠道名称, 12子渠道名称
    sql1:=sql;  //导入数据无错误时，用sql1 插入数据
    sql:=sql+'select top 10 * from ( select top 5 info=''第''+line_no+''行 无供货单位或名称超长或数据无效'' from @tab where f1='''' or len(f1)>300 or not exists (select 1 from tb_busimate where mate_type_id=2 and f1=mate_name)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无购货单位或名称超长或数据无效'' from @tab where f2='''' or len(f2)>300 or not exists (select 1 from tb_busimate where mate_type_id in (1,2) and f2=mate_name)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无发生日期或数据无效'' from @tab where f4='''' or try_cast(f4 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无单据类别或数据无效'' from @tab where f5='''' or f5 not in (''销售'',''退货'',''调退'',''调销'')';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无物料编码或数据无效'' from @tab where f6='''' or not exists (select 1 from tb_medicine where material_code=f6)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无数量或数据无效(需为整数)'' from @tab where f7='''' or try_cast(f7 as int) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无有效期至数据无效'' from @tab where f9<>'''' and try_cast(f9 as datetime) is null';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行 无有效期至或数据无效'' from @tab where f9='''' or try_cast(f9 as datetime) is null';
//    sql:=sql+' union all select top 5 ''第''+line_no+''行 无业务类别或数据无效'' from @tab where f10='''' or f10 not in (''商业'',''分销'')';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无商业价或数据无效'' from @tab where f10='''' or try_cast(f10 as decimal(15,4)) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无渠道名称或数据无效'' from @tab where f11='''' or not exists (select 1 from tb_object where obj_type_id=11 and zdesc= f11)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无子渠道名称或数据无效'' from @tab where f12='''' or not exists (select 1 from tb_object where obj_type_id=12 and zdesc= f12)';
{
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无渠道(名称)或数据无效'' from @tab a where f12='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.zdesc=a.f12)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无子渠道(名称)或数据无效'' from @tab a where f13='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.zdesc=a.f13)';
}
    sql:=sql+' ) a order by info';
//从第2行开始导入，格式: 1发生日期, 2业务类别, 3商业公司, 4单据摘要, 5商务代表, 6物料编码, 7批号, 8数量, 9单价, 10渠道, 11子渠道
...
//    commandtext:=commandtext+' where a.bod_type_id=2 and a.cate_id=1'; // cate_id 1:商业 2:分销 3:终端战略 4:终端普通 5:两票商业 6:连锁终端销售出库单  7:零售销售出库单
        ...
        sql:=sql+'  exec sp_getbillid2 '+inttostr(compid)+',2,@bodcd output';

//新格式:1供货单位, 2购货单位, 3单据摘要, 4发生日期, 5单据类别, 6物料编码, 7数量, 8批号, 9有效期至, 10商业价, 11渠道, 12子渠道
        sql:=sql+'  insert into tb_bill (comp_id,bod_type_id,bod_status_id,cate_id,busi_type,bod_cd,creat_by,creat_dt,carry_dt,src_id,dst_id,bod_desc)';
//        sql:=sql+'      values ('+inttostr(compid)+',2,0,1,1,'+inttostr(curuserid)+',getdate(),cast(convert(char(10),getdate(),20) as datetime))';
        sql:=sql+'      select top 1 '+inttostr(compid)+',bod_type_id=2,bod_status_id=0,cate_id=1,busi_type=case a.f5 when ''销售'' then 1 when ''退货'' then 2 when ''调退'' then 3 when ''调销'' then 4 end,';
        sql:=sql+'      bod_cd=@bodcd,creat_by='+inttostr(curuserid)+',creat_dt=getdate(),carry_dt=a.f4,'; //cast(convert(char(10),getdate(),20) as datetime),';
        sql:=sql+'      src_id=b.mate_id,dst_id=c.mate_id,bod_desc=a.f3';
        sql:=sql+'      from @tab a';
        sql:=sql+'      left join tb_busimate b on b.mate_type_id=2 and b.mate_name=a.f1';
        sql:=sql+'      left join tb_busimate c on c.mate_type_id in (1,2) and c.mate_name=a.f2';
        sql:=sql+'      order by a.f1,a.f2,a.f3,a.f4,a.f5';
        sql:=sql+'  set @bodid=@@identity';
        sql:=sql+'  select top 1 @lineno=line_no from @tab order by f1,f2,f3,f4,f5';
//        sql:=sql+'  insert into tb_bill_dtl (bod_id,med_id,bat_cd,valid_dt,price,qty,amot,channel_id,channel_dtl_id)';

        sql:=sql+'  insert into tb_bill_dtl (bod_id,med_id,bat_cd,valid_dt,price,price3,qty,amot,channel_id,channel_dtl_id)';
        sql:=sql+'  select @bodid,m.med_id,bat_cd=a.f8,valid_dt=case when a.f9='''' then null else a.f9 end,price=cast(a.f10 as decimal(15,4)),price3=cast(a.f10 as decimal(15,4)),qty=cast(a.f7 as decimal(15,4)),';
        sql:=sql+'      amot=round(cast(a.f10 as decimal(15,4))*cast(a.f7 as decimal(15,4)),2),c.obj_code,d.obj_code';
        sql:=sql+'      from @tab a inner join (select top 1 * from @tab b where line_no=@lineno) b on a.f1=b.f1 and a.f2=b.f2 and a.f3=b.f3 and a.f4=b.f4 and a.f5=b.f5';
        sql:=sql+'      inner join tb_medicine m on a.f6=m.material_code';
        sql:=sql+'      inner join (select obj_id,obj_code,zdesc from tb_object where obj_type_id=11) c on c.zdesc=a.f11';
        sql:=sql+'      inner join (select obj_id,obj_code,zdesc from tb_object where obj_type_id=12) d on d.zdesc=a.f12';
        sql:=sql+'  delete from @tab where line_no in (select b.line_no from @tab b,(select top 1 * from @tab where line_no=@lineno) a where a.f1=b.f1 and a.f2=b.f2 and a.f3=b.f3 and a.f4=b.f4 and a.f5=b.f5)';
...
procedure Tsetmedexport.N1Click(Sender: TObject);
        ...
        if active then close;
{
    ...
    commandtext:=commandtext+'  and not exists (select 1 from tb_medstock_bat f where f.mate_id='+cdsbill.fieldbyname('src_id').asstring+' and f.cate_id='+cdsbill.fieldbyname('cate_id').asstring+' and f.med_id=b.med_id and f.qty>=b.qty)';
        ...
        commandtext:='select top 5 a.bod_cd,m.material_code,m.med_code,m.med_name,m.specifi,m.pdt_place';
        commandtext:=commandtext+' from tb_bill a,tb_bill_dtl b,tb_medicine m';
    ...
    //edit3.text:=commandtext;
        ...
        begin
            ...
            while not eof do
            begin
                s := s+ #13#10 +fieldbyname('bod_cd').asstring+'单 ['+fieldbyname('material_code').asstring+']'+fieldbyname('med_name').asstring+' '+fieldbyname('specifi').asstring+' '+fieldbyname('pdt_place').asstring;
                next;
            end;
            MessageBox(0,pchar(s),'请注意',MB_OK+MB_IconError);
            abort;
...
{       ///后面取中标价
        ...
        if active then close;
        commandtext:='select top 5 a.bod_cd,b.med_id,m.material_code,m.med_code,m.med_name,m.specifi,m.pdt_place';
        commandtext:=commandtext+' from tb_bill a,tb_bill_dtl b,tb_medicine m';
...
procedure Tsetmedexport.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetmedexport.N3Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t : string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetmedexport.dxDBButtonEdit3ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedexport.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetmedexport.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetmedexport.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tsetmedexport.Button1Click(Sender: TObject);
begin
...
procedure Tsetmedexport.N5Click(Sender: TObject);
begin
...
procedure Tsetmedexport.dxEdit1Click(Sender: TObject);
begin
dxEdit1.SelectAll;
dxEdit1.SetFocus;
end;

procedure Tsetmedexport.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetmedexport.N6Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'列表1.xls', dxDBGrid1.SaveToXLS, self.tag)
end;

procedure Tsetmedexport.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
{
with cdsbill_dtl do
begin
...
procedure Tsetmedexport.N9Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t : string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetmedexport.dxDBGrid2material_codeButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedexport.cdsbill_dtlprice3Change(Sender: TField);
begin
cdsbill_dtl.fieldbyname('price').asfloat:=cdsbill_dtl.fieldbyname('price3').asfloat;
end;
