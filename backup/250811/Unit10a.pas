...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1交货单号, 2订单编号, 3渠道名称, 4子渠道名称, 5业务负责人, 6供货单位, 7商业公司, 8销售人员, 9单据摘要, 10发运日期, 11物料编码, 12单价, 13数量, 14批号, 15有效期至
//    sbodcd:='导入数据已生成下列单据'+#13#10+'---------------------------';
    ...
    XL.Quit;
//从第2行开始导入，格式: 1交货单号, 2订单编号, 3渠道名称, 4子渠道名称, 5业务负责人, 6供货单位, 7商业公司, 8销售人员, 9单据摘要, 10发运日期, 11定价参考物料编码, 12单价, 13数量, 14批号, 15有效期至
    sql1:=sql;  //导入数据无错误时，用sql1 插入数据
    sql:=sql+'select top 5 * from ( select top 5 info=''第''+line_no+''行无交货单号或已有该单号'' from @tab a where f1='''' or not exists (select 1 from vi_ZSD_015 b where b.GBELN=a.f1)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无订单编号'' from @tab where f2=''''';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无渠道名称或数据无效'' from @tab a where f3='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.zdesc=a.f3)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无子渠道名称或数据无效'' from @tab a where f4='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.zdesc=a.f4)';
    ...
    sql:=sql+' union all select top 5 ''第''+line_no+''行无销售人员或数据无效'' from @tab a where f8='''' or not exists (select 1 from tb_staff b where sta_type_id=0 and dept_id=43 and b.zname=a.f8)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无发运日期或数据无效'' from @tab where f10='''' or try_cast(f10 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无物料编码或数据无效'' from @tab a where f11='''' or not exists (select 1 from tb_medicine b where b.material_code=a.f11)';
