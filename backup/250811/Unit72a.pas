...
var
  setbrokerfeepay1: Tsetbrokerfeepay1;
...
procedure Tsetbrokerfeepay1.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetbrokerfeepay1.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetbrokerfeepay1.FormActivate(Sender: TObject);
begin
{
with homebank do
begin
...
procedure Tsetbrokerfeepay1.FormCloseQuery(Sender: TObject;
...
procedure Tsetbrokerfeepay1.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetbrokerfeepay1.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetbrokerfeepay1.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetbrokerfeepay1.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
procedure Tsetbrokerfeepay1.FormCreate(Sender: TObject);
begin
...
procedure Tsetbrokerfeepay1.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetbrokerfeepay1.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetbrokerfeepay1.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetbrokerfeepay1.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetbrokerfeepay1.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetbrokerfeepay1.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetbrokerfeepay1.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
...
procedure Tsetbrokerfeepay1.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetbrokerfeepay1.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetbrokerfeepay1.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetbrokerfeepay1.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetbrokerfeepay1.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetbrokerfeepay1.N1Click(Sender: TObject);
...
begin
    ...
    begin
        if ((sender=N1) and (dxDBGrid1.SelectedNodes[i].Values[j]<>1))
    ...
    try
        try
            ...
            begin
                ...
                begin
                    if active=true then close;
                    if sender= N1 then commandtext:='sp_updatestock18 18,'+ dm.pubqry.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
                    if sender= N2 then commandtext:='sp_updatestock18 -18,'+ dm.pubqry.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
                    execute;
                end;
                next;
            end;
...
procedure Tsetbrokerfeepay1.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetbrokerfeepay1.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetbrokerfeepay1.SpeedButton2Click(Sender: TObject);
begin
...
begin
    if active=true then close;
    commandtext:='sp_updatestock18 18,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsetbrokerfeepay1.SpeedButton3Click(Sender: TObject);
begin
...
procedure Tsetbrokerfeepay1.SpeedButton6Click(Sender: TObject);
begin
