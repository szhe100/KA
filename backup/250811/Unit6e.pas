...
//从第2行开始导入，格式: 1渠道名称,2子渠道名称,3业务省份,4业务城市,5物料编码,6中标价/终端开票价,7开票扣率,8考核价,9推广比率,10金额,11定价单位,12启用日期, 13备注
    ...
    sql:=sql+'select top 10 * from ( select top 5 info=''第''+line_no+''行无渠道名称或数据无效'' from @tab a where f1='''' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.zdesc=a.f1)';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无子渠道名称或数据无效'' from @tab a where f2='''' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.zdesc=a.f2)';
