...
//        if (busimate.active=false) or (busimate.recordcount=0) then exit;
        ...
        commandtext:=commandtext+' dist1=dbo.fn_getdistrictname(case type_id when 1 then b.district when 2 then c.district end,1),';
        commandtext:=commandtext+' dist2=dbo.fn_getdistrictname(case type_id when 1 then b.district when 2 then c.district end,2),';
...
//从第2行开始导入，格式: 1推广服务商名称, 2纳税类型, 3准入状态, 4使用状态, 5确认时间, 6编码
    ...
    sql:=sql+'select top 10 * from ( select top 5 info=line_no+''行 无推广服务商名称'' from @tab where f1=''''';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无纳税类型或无效(小规模纳税人/一般纳税人)'' from @tab where f2='''' or (f2<>''小规模纳税人'' and f2<>''一般纳税人'')';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无准入状态或无效(准入/终止)'' from @tab where f3='''' or (f3<>''准入'' and f3<>''终止'')';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无使用状态或无效(正常/暂停/终止)'' from @tab where f4='''' or (f4<>''正常'' and f4<>''暂停'' and f4<>''终止'')';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无确认时间或数据无效'' from @tab where f5='''' or try_cast(f5 as datetime) is null';
    sql:=sql+' union all select top 5 ''第''+line_no+''行 无编码'' from @tab where f6=''''';
...
//从第2行开始导入，格式: 1推广服务商名称, 2纳税类型, 3准入状态, 4使用状态, 5确认时间, 6编码
        sql:=sql1;
        sql:=sql+' declare @tab1 table (f1 varchar(100),f2 varchar(100),f3 varchar(100),f4 varchar(100),f5 varchar(100),f6 varchar(100))';
        sql:=sql+' insert into @tab1 (f1,f2,f3,f4,f5,f6) select f1,f2,f3,f4,f5=max(f5),f6 from @tab group by f1,f2,f3,f4,f6'; // 排除导入数据的重复

        sql:=sql+' declare @cnt1 int,@cnt2 int';

        sql:=sql+' update a set mate_name=f1,tax_type_id=case f2 when ''小规模纳税人'' then 1 when ''一般纳税人'' then 2 end,';
        ...
        sql:=sql+' from tb_busimate a,@tab1 b';
...
//从第2行开始导入，格式: 1推广服务商名称, 2纳税类型, 3准入状态, 4使用状态, 5确认时间
        ...
        sql:=sql+'  select distinct comp_id='+inttostr(compid)+',mate_type_id=10,mate_name=f1,';
