...
type
  Tqryreport24 = class(TForm)
    ...
    qry2material_code: TStringField;
    qry1agent_code: TStringField;
...
if PageControl1.ActivePage=TabSheet1
	then label4.Caption:='区    域' else label4.Caption:='品    种';
end;
...
with pickmedicine do
    if showmodal= mrOK then
    begin
        dxbuttonedit3.text:=query1.fieldbyname('material_code').asstring+' '+query1.fieldbyname('med_name').asstring+' '+query1.fieldbyname('specifi').asstring+' '+query1.fieldbyname('pdt_place').asstring+' '+query1.fieldbyname('med_unit').asstring+' 件装:'+formatfloat('####',query1.fieldbyname('qtyperpack').asinteger);
