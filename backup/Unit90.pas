...
procedure Tsetmedimport5.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetmedimport5.setunupdatestatus;  // 浏览状态
begin
...
begin
    ...
    commandtext:=commandtext+' where a.bod_type_id=41';     // cate_id 1:商业 2:分销 3:终端战略 4:终端普通  a.comp_id='+inttostr(compid)+' and
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                ...
                commandtext:=commandtext+' left join (select c.order_id,d.rela_id1,d.qty2 from tb_bill c,tb_bill_dtl d'; //生成的两票销售单的销售数量，两票销售单可能退货(冲减两票销售单的tb_bill_dtl.qty2)
...
begin
{
    ...
    if recordcount>0 then
    begin
        MessageBox(0,pchar(fieldbyname('bod_cd').asstring+'单已完成销售,不可再次生成销售单'),'请注意',MB_OK+MB_IconError);
...
//    showmessage(inttostr(recordcount));
    ...
    begin
        ...
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select top 1 '+inttostr(compid)+','''+mcd+''',42,0,cate_id=1,busi_type=1,src_id,dst_id,agent_id,broker_id,district_id,bod_id,''由''+bod_cd+''号商业采购计划单生成'','+inttostr(curuserid)+',getdate()';
        ...
        dm.pubqry.commandtext:='insert into tb_bill_dtl (bod_id,med_id,price,price1,price2,qty,qty1,qty2,rela_id1)';
{
        ...
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' left join (select c.order_id,d.med_id,qty2=sum(d.qty2) from tb_bill c,tb_bill_dtl d'; //生成的两票销售单的销售数量，两票销售单可能退货(冲减两票销售单的tb_bill_dtl.qty2)
        ...
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' where b.bod_id='+fieldbyname('bod_id').asstring;
{
        ...
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' from tb_bill_dtl b';
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' left join (select c.order_id,d.rela_id1,d.qty2 from tb_bill c,tb_bill_dtl d'; //生成的两票销售单的销售数量，两票销售单可能退货(冲减两票销售单的tb_bill_dtl.qty2)
...
setprogress(0);
MessageBox(0,pchar('已成功生成'+mcd+'号两票销售出库单'),'请注意',MB_OK+MB_ICONInformation);
end;
...
begin
    ...
    begin
        ...
        if dm.pubqry.active=True then dm.pubqry.close;
        dm.pubqry.commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,cate_id,busi_type,src_id,dst_id,agent_id,broker_id,district_id,order_id,bod_desc,creat_by,creat_dt)';
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select top 1 '+inttostr(compid)+','''+mcd+''',42,0,cate_id=1,busi_type=2,src_id,dst_id,agent_id,broker_id,district_id,bod_id,''由''+bod_cd+''号商业采购计划单生成'','+inttostr(curuserid)+',getdate()';
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' from tb_bill where bod_id='+fieldbyname('bod_id').asstring;
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select @@identity';
//edit1.text:=dm.pubqry.commandtext;
        dm.pubqry.open;
