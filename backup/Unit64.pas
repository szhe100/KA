...
type
  Tsetruleadjust = class(TForm)
    ...
    dxDBGrid2broker: TdxDBGridButtonColumn;
    Edit2: TEdit;
    dxDBGrid2cost: TdxDBGridColumn;
    ...
    procedure dxDBGrid2leaderButtonClick(Sender: TObject;
      AbsoluteIndex: Integer);
    procedure dxDBGrid2brokerButtonClick(Sender: TObject;
      AbsoluteIndex: Integer);
    procedure brokermedtlAfterPost(DataSet: TDataSet);
    procedure DSbrokermedtlStateChange(Sender: TObject);
  private
...
var
  setruleadjust: Tsetruleadjust;

implementation

uses datamodu, Unit16, Unit28a, Unit28;

{$R *.DFM}

procedure Tsetruleadjust.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetruleadjust.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetruleadjust.FormActivate(Sender: TObject);
begin
{
with dm.fee_type do
...
procedure Tsetruleadjust.FormCloseQuery(Sender: TObject;
...
procedure Tsetruleadjust.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetruleadjust.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetruleadjust.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with cdsbill_dtl do
...
procedure Tsetruleadjust.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetruleadjust.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
begin
    if active=True then close;
    commandtext:='update b set price=c.rate from tb_bill_dtl b,';
    commandtext:=commandtext+'(select b.med_id,rate=sum(c.rate) from tb_brokermedtl c,tb_bill_dtl b where b.bod_id='+cdsbill.fieldbyname('bod_id').asstring+' and b.med_id=c.rela_id group by b.med_id) c';
    commandtext:=commandtext+' where b.bod_id='+cdsbill.fieldbyname('bod_id').asstring+' and b.med_id=c.med_id';
...
procedure Tsetruleadjust.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetruleadjust.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.comp_id='+inttostr(compid)+' and a.bod_type_id=23';
...
procedure Tsetruleadjust.FormCreate(Sender: TObject);
begin
...
procedure Tsetruleadjust.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetruleadjust.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
begin
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active=true then close;
                commandtext:='select b.bod_id,b.dtl_id,b.med_id,b.price,b.cost,b.price1,b.valid_dt,';
                commandtext:=commandtext+' a.*,leader=f.zname,m.med_code,m.med_name,m.chm_name,m.specifi,m.pdt_place,med_unit=c.zdesc,m.qtyperpack,m.qtyperbox,';
                ...
                commandtext:=commandtext+' inner join tb_brokermed a on b.med_id=a.rec_id';
                ...
                commandtext:=commandtext+' left join tb_staff d on b.sta_id=d.sta_id';
                ...
                commandtext:=commandtext+' left join tb_staff f on b.lead_id=f.sta_id';
...
procedure Tsetruleadjust.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetruleadjust.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetruleadjust.SpeedButton2Click(Sender: TObject);
var s: string;
begin
{
with cdsbill do
...
procedure Tsetruleadjust.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetruleadjust.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetruleadjust.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetruleadjust.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
...
procedure Tsetruleadjust.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetruleadjust.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetruleadjust.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetruleadjust.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetruleadjust.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetruleadjust.N1Click(Sender: TObject);
...
procedure Tsetruleadjust.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetruleadjust.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetruleadjust.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetruleadjust.brokermedtlCalcFields(DataSet: TDataSet);
begin
with brokermedtl do
begin
...
procedure Tsetruleadjust.dxDBGrid2leaderButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickstaff do
begin
    ...
    if showmodal= mrOK then
    begin
        cdsbill_dtl.fieldbyname('lead_id').asinteger:= query1.fieldbyname('sta_id').asinteger;
        cdsbill_dtl.fieldbyname('leader').asstring:= query1.fieldbyname('zname').asstring;
    end
    else
    begin
        cdsbill_dtl.fieldbyname('lead_id').asinteger:=0;
        cdsbill_dtl.fieldbyname('leader').asstring:='';
    end;
end;
end;

procedure Tsetruleadjust.dxDBGrid2brokerButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickbroker do
begin
    ...
    if showmodal= mrOK then
    begin
        cdsbill_dtl.fieldbyname('sta_id').asinteger:= query1.fieldbyname('sta_id').asinteger;
        cdsbill_dtl.fieldbyname('broker').asstring:= query1.fieldbyname('zname').asstring;
    end
    else
    begin
        cdsbill_dtl.fieldbyname('sta_id').asinteger:=0;
        cdsbill_dtl.fieldbyname('broker').asstring:='';
    end;
end;
end;

procedure Tsetruleadjust.brokermedtlAfterPost(DataSet: TDataSet);
begin
if brokermedtl.ChangeCount>0 then myupdaterefresh(brokermedtl,'tb_brokermedtl','rec_id','');
end;

procedure Tsetruleadjust.DSbrokermedtlStateChange(Sender: TObject);
begin
with brokermedtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;
