...
procedure Tsetmedimport4.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetmedimport4.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetmedimport4.FormActivate(Sender: TObject);
begin
//with dm.imp_type do
...
procedure Tsetmedimport4.FormCloseQuery(Sender: TObject;
...
procedure Tsetmedimport4.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetmedimport4.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedimport4.dxDBGrid2KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
...
procedure Tsetmedimport4.dxDBGrid2Enter(Sender: TObject);
begin
...
procedure Tsetmedimport4.FormKeyPress(Sender: TObject; var Key: Char);
//var fapp: boolean;
begin
...
procedure Tsetmedimport4.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
if cdsbill.fieldbyname('dst_id').asinteger=0 then raise Exception.Create('请选择商业公司');
...
procedure Tsetmedimport4.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetmedimport4.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
//    commandtext:=commandtext+' c.zname1,c.address1,c.phone1,c.zname2,c.address2,c.phone2,cdistrict=dbo.fn_getdistrict(f.district),finished=dbo.fn_orderisfinished(a.bod_id),';
    ...
    commandtext:=commandtext+' where a.bod_type_id=39';     // cate_id 1:商业 2:分销 3:终端战略 4:终端普通  a.comp_id='+inttostr(compid)+' and
...
procedure Tsetmedimport4.FormCreate(Sender: TObject);
begin
...
procedure Tsetmedimport4.SpeedButton6Click(Sender: TObject);
var mid :integer;
begin
setprogress(1);
with dm.pubqry do
...
procedure Tsetmedimport4.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetmedimport4.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetmedimport4.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
procedure Tsetmedimport4.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedimport4.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedimport4.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport4.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport4.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetmedimport4.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
    then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
...
procedure Tsetmedimport4.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetmedimport4.cdsbillBeforePost(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport4.cdsbillReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedimport4.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedimport4.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetmedimport4.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetmedimport4.PageControl1Change(Sender: TObject);
begin
...
procedure Tsetmedimport4.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetmedimport4.dxDBGrid2DblClick(Sender: TObject);
begin
{
if pickmedicine.showmodal= mrOK then
begin
...
procedure Tsetmedimport4.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
    SFileName,sbodcd,sb: string;
    J,mi,mid, id2,id3,id4,id5,id8,typeid: integer;
    price1,price2: real;
    dt: TDatetime;
begin
    ...
    j:=2; mi:=0;  //第2行开始 每行一单
//格式:1收款日期, 2供货单位id2, 3商业公司id3, 4代理人,5药品编码id5,6.采购数量（根据数量正负判断类别为采购还是退货）,7单据摘要,8省份 城市
    while (sheet.cells[j,1].text<>'') and (mi<10) do
    begin
        id2:=0;id3:=0;id4:=0;id5:=0;id8:=0;typeid:=0; price1:=0; price2:=0; //s6:='';s11:='';
        ...
        begin
            ...
            begin
                if active=true then close;
                commandtext:='select top 1 rec_id from tb_treedata where dbo.fn_getdistrict(rec_id)='''+Trim(sheet.cells[j,8].text)+'''';
                open;
                if recordcount>0 then id8:=fieldbyname('rec_id').asinteger;
            end;
...
edit1.text:=commandtext;            
            ...
            if (id2=0) or (id3=0) or (id4=0) or (id5=0) or (id8=0) or (Trim(cleardeli(sheet.cells[j,6].text))='') or (price1=0) or (price2=0) then
            begin
                ...
                if id8=0 then sbodcd:=sbodcd+',未取得省份 城市';
...
//格式:1收款日期, 2供货单位id2, 3商业公司id3, 4代理人,5药品编码id5,6.采购数量（根据数量正负判断类别为采购还是退货）,7单据摘要,8省份 城市
            if active=true then close;
            commandtext:='insert into tb_bill (comp_id,bod_cd,bod_type_id,bod_status_id,busi_type,creat_by,creat_dt,src_id,dst_id,agent_id,district_id,bod_desc,carry_dt)';  // ,district_id cate_id 1:商业 2:分销 3:终端
            commandtext:=commandtext+' values ('+inttostr(compid)+','''+sb+''',39,0,'+inttostr(typeid)+','+inttostr(curuserid)+',getdate(),';
            commandtext:=commandtext+ inttostr(id2)+','+ inttostr(id3)+','+ inttostr(id4)+','+ inttostr(id8)+','''+Trim(sheet.cells[j,7].text)+''','''+datetimetostr(dt)+''')';
            commandtext:=commandtext+' select @@identity';
edit1.text:=commandtext;
            open;
            mid:=fields[0].asinteger;
...
procedure Tsetmedimport4.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetmedimport4.N3Click(Sender: TObject);
...
procedure Tsetmedimport4.dxDBButtonEdit3Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetmedimport4.dxDBButtonEdit3KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetmedimport4.dxDBButtonEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tsetmedimport4.dxDBButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetmedimport4.dxDBButtonEdit1KeyPress(Sender: TObject;
  var Key: Char);
begin
if (key=#32) and ((Sender as TdxDBButtonEdit).Text<>'') then //
begin
...
procedure Tsetmedimport4.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetmedimport4.dxEdit1Click(Sender: TObject);
begin
dxEdit1.SelectAll;
dxEdit1.SetFocus;
end;

procedure Tsetmedimport4.dxDBButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedimport4.dxDBButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedimport4.SpeedButton2Click(Sender: TObject);
begin
with cdsbill do
begin
    if fieldbyname('src_id').asinteger=0 then raise Exception.Create('请选择供货单位');
    ...
    if fieldbyname('carry_dt').asfloat=0 then raise Exception.Create('请输入收款日期');
...
procedure Tsetmedimport4.dxDBGraphicEdit1AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport4.dxDBGraphicEdit2AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport4.dxDBGraphicEdit3AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport4.dxDBGraphicEdit4AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport4.dxDBGraphicEdit5AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport4.dxDBGraphicEdit1Change(Sender: TObject);
begin
setupdatestatus;
//cdsbill.edit;
end;

procedure Tsetmedimport4.dxDBGraphicEdit1GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass1').asstring));
end;

procedure Tsetmedimport4.dxDBGraphicEdit2GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass2').asstring));
end;

procedure Tsetmedimport4.dxDBGraphicEdit3GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass3').asstring));
end;

procedure Tsetmedimport4.dxDBGraphicEdit4GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass4').asstring));
end;

procedure Tsetmedimport4.dxDBGraphicEdit5GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass5').asstring));
end;

procedure Tsetmedimport4.dxDBGraphicEdit1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
...
procedure Tsetmedimport4.qryimageAfterPost(DataSet: TDataSet);
begin
qryimage.ApplyUpdates(-1);
end;

procedure Tsetmedimport4.N2Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t,mcd: string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
begin
{
    ...
    if recordcount>0 then
    begin
        MessageBox(0,pchar(fieldbyname('bod_cd').asstring+'单已完成销售'),'请注意',MB_OK+MB_IconError);
...
procedure Tsetmedimport4.N4Click(Sender: TObject);
...
procedure Tsetmedimport4.N5Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t,mcd: string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetmedimport4.N7Click(Sender: TObject);
begin
