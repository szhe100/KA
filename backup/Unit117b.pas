...
procedure Tsetmedimport6.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetmedimport6.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsetmedimport6.FormActivate(Sender: TObject);
begin
//with dm.imp_type do
...
procedure Tsetmedimport6.FormCloseQuery(Sender: TObject;
...
procedure Tsetmedimport6.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetmedimport6.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetmedimport6.dxDBGrid2Enter(Sender: TObject);
begin
...
procedure Tsetmedimport6.FormKeyPress(Sender: TObject; var Key: Char);
//var fapp: boolean;
begin
...
procedure Tsetmedimport6.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetmedimport6.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetmedimport6.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
//    commandtext:=commandtext+' left join (select bod_cd,order_id from tb_bill where bod_type_id=1) h on a.bod_id =h.order_id';
    commandtext:=commandtext+' where a.bod_type_id=52 and a.busi_type=3 and a.comp_id='+inttostr(compid);
...
procedure Tsetmedimport6.FormCreate(Sender: TObject);
begin
...
procedure Tsetmedimport6.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetmedimport6.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsetmedimport6.dxDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var fapp: boolean;
begin
...
procedure Tsetmedimport6.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedimport6.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetmedimport6.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport6.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport6.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetmedimport6.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
    then raise Exception.Create('本单已审核，不可更改。'+#13#10+'如需更改，请先反审核');
...
procedure Tsetmedimport6.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetmedimport6.cdsbillBeforePost(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetmedimport6.cdsbillReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedimport6.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetmedimport6.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetmedimport6.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
    fieldbyname('amot').asfloat:=round(fieldbyname('price').asfloat*fieldbyname('qty').asfloat*100+0.01)/100;
end;

procedure Tsetmedimport6.PageControl1Change(Sender: TObject);
begin
...
procedure Tsetmedimport6.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetmedimport6.dxDBGrid2DblClick(Sender: TObject);
begin
...
procedure Tsetmedimport6.SpeedButton1Click(Sender: TObject);
var
    XL,Sheet: Variant;
...
procedure Tsetmedimport6.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetmedimport6.N3Click(Sender: TObject);
...
procedure Tsetmedimport6.dxEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tsetmedimport6.dxEdit1Click(Sender: TObject);
begin
dxEdit1.SelectAll;
dxEdit1.SetFocus;
end;

procedure Tsetmedimport6.dxDBButtonEdit2ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedimport6.dxDBButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetmedimport6.SpeedButton2Click(Sender: TObject);
var s: string;
begin
...
begin
    if active=True then close;
    commandtext:='select top 3 m.med_code,m.material_code1,m.med_name,m.specifi,m.pdt_place,b.bat_cd from tb_bill_dtl b,tb_medicine m';
    commandtext:=commandtext+' where b.bod_id='+cdsbill.fieldbyname('bod_id').asstring+' and (isnull(b.discount,0)=0) and b.med_id=m.med_id';
    open;
    if recordcount>0 then
    begin
        s:='下列品种无调后价，本单不可送审核'+#13+#10+'------------------------------------------------';
        first;
        while not eof do
        begin
            s:=s+#13+#10+fieldbyname('material_code1').asstring+' '+fieldbyname('med_name').asstring+' '+fieldbyname('specifi').asstring+' '+fieldbyname('bat_cd').asstring;
            next;
        end;
...
{
    if active=True then close;
    commandtext:='select top 3 b.med_id,b.rela_id1,m.med_code,m.material_code1,m.med_name,m.specifi,m.pdt_place,qty1=sum(isnull(qty1,0)),qty=sum(qty)';   //b.bat_cd,
...
{
    if active=True then close;
    commandtext:='select top 5 m.med_code,m.material_code1,m.med_name,m.specifi,m.pdt_place,h.qty1,h.qty from (select b.med_id,qty1=avg(b.qty1),qty=sum(qty) from tb_bill a,tb_bill_dtl b';
    commandtext:=commandtext+' where a.bod_type_id=52 and a.order_id='+cdsbill.fieldbyname('order_id').asstring+' and a.bod_id=b.bod_id';
...
procedure Tsetmedimport6.dxDBGraphicEdit1AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport6.dxDBGraphicEdit2AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport6.dxDBGraphicEdit3AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport6.dxDBGraphicEdit4AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport6.dxDBGraphicEdit5AssignPicture(Sender: TObject;
  var Picture: TPicture);
begin
...
procedure Tsetmedimport6.dxDBGraphicEdit1Change(Sender: TObject);
begin
setupdatestatus;
//cdsbill.edit;
end;

procedure Tsetmedimport6.dxDBGraphicEdit1GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass1').asstring));
end;

procedure Tsetmedimport6.dxDBGraphicEdit2GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass2').asstring));
end;

procedure Tsetmedimport6.dxDBGraphicEdit3GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass3').asstring));
end;

procedure Tsetmedimport6.dxDBGraphicEdit4GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass4').asstring));
end;

procedure Tsetmedimport6.dxDBGraphicEdit5GetGraphicClass(Sender: TObject;
  var GraphicClass: TGraphicClass);
begin
GraphicClass := TGraphicClass(GetClass(qryimage.fieldbyname('graphclass5').asstring));
end;

procedure Tsetmedimport6.dxDBGraphicEdit1KeyDown(Sender: TObject;
...
procedure Tsetmedimport6.qryimageAfterPost(DataSet: TDataSet);
begin
qryimage.ApplyUpdates(-1);
end;

procedure Tsetmedimport6.MenuItem1Click(Sender: TObject);
var mi: integer;
begin
cdsbillBeforeEdit(nil);
...
procedure Tsetmedimport6.N2Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t,mcd: string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
procedure Tsetmedimport6.N4Click(Sender: TObject);
var mi,j,k,l,i : integer;
    s,t,mcd: string;
begin
if (cdsbill.active=false) or (cdsbill.recordcount=0) then exit;
...
begin
    ...
    begin
        ...
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select top 1 '+inttostr(compid)+','''+mcd+''',52,0,cate_id,2,src_id,dst_id,agent_id,broker_id,district_id,order_id,bod_id,''由''+bod_cd+''号两票销售出库单生成'','+inttostr(curuserid)+',getdate()';
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' from tb_bill where bod_id='+fieldbyname('bod_id').asstring;
        dm.pubqry.commandtext:=dm.pubqry.commandtext+' select @@identity';
//edit1.text:=dm.pubqry.commandtext;
        dm.pubqry.open;
...
procedure Tsetmedimport6.N6Click(Sender: TObject);
begin
dm.Save1('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', self.caption+'列表1.xls', dxDBGrid1.SaveToXLS, self.tag)
end;

procedure Tsetmedimport6.SpeedButton6Click(Sender: TObject);
var mid : integer;
begin
...
begin
    ...
    commandtext:=commandtext+' values ('+inttostr(compid)+',52,0,3,'+inttostr(curuserid)+',getdate(),cast(convert(char(10),getdate(),20) as datetime))';
    commandtext:=commandtext+'select @@identity';
    open;
    mid:=fields[0].asinteger;
    close;
...
procedure Tsetmedimport6.SpeedButton3Click(Sender: TObject);
begin
with qry do
begin
	if active=True then close;
...
procedure Tsetmedimport6.qryCalcFields(DataSet: TDataSet);
begin
with qry do
begin
    case fieldbyname('cate_id').asinteger of
...
procedure Tsetmedimport6.PageControl2Change(Sender: TObject);
begin
...
procedure Tsetmedimport6.DSqrydtlDataChange(Sender: TObject;
  Field: TField);
begin
SpeedButton10.Enabled:=(PageControl2.ActivePage=TabSheet5) and (qrydtl.active=True) and (qrydtl.recordcount>0);
end;

procedure Tsetmedimport6.dxDBGrid4CustomDrawCell(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
  AColumn: TdxTreeListColumn; ASelected, AFocused, ANewItemRow: Boolean;
  var AText: String; var AColor: TColor; AFont: TFont;
  var AAlignment: TAlignment; var ADone: Boolean);
...
procedure Tsetmedimport6.SpeedButton10Click(Sender: TObject);
var mi: integer;
begin
...
begin
	if active=True then close;
    commandtext:='select top 1 a.bod_id,a.bod_cd,a.bod_status_id from tb_bill a,tb_bill_dtl b where bod_type_id=52 and a.busi_type=3 and a.bod_id=b.bod_id and b.rela_id1='+qrydtl.fieldbyname('dtl_id').asstring;
//edit2.text:=commandtext;    
    open;
    if recordcount>0 then
    begin
...
begin
	if active=True then close;
    commandtext:='update a set src_id=c.src_id,dst_id=c.dst_id,agent_id=c.agent_id,broker_id=c.broker_id,cate_id=c.cate_id,carry_dt=c.carry_dt,bod_desc=''由'+qry.fieldbyname('bod_cd').asstring+'号两票销售单生成'',relative_id=c.bod_id,order_id=c.order_id';
...
procedure Tsetmedimport6.cdsbill_dtldiscountChange(Sender: TField);
begin
with cdsbill_dtl do
begin
...
procedure Tsetmedimport6.dxEdit2Click(Sender: TObject);
begin
dxEdit2.SelectAll;
dxEdit2.SetFocus;
end;

procedure Tsetmedimport6.dxEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key=vk_return then // <Enter> key is pressed
begin
...
procedure Tsetmedimport6.Button1Click(Sender: TObject);
begin
SpeedButton2Click(nil);
end;

procedure Tsetmedimport6.dxDBButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
with pickagent do
begin
...
procedure Tsetmedimport6.dxDBButtonEdit1Enter(Sender: TObject);
begin
(Sender as TdxDBButtonEdit).SelectAll;
end;

procedure Tsetmedimport6.dxDBButtonEdit1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
if (key=VK_RETURN) and (Shift=[ssCtrl]) then (Sender as TdxDBButtonEdit).OnButtonClick(nil,0);
end;

procedure Tsetmedimport6.dxDBButtonEdit1KeyPress(Sender: TObject;
  var Key: Char);
begin
