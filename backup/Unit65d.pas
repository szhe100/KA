...
type
  Tsettruststopapply = class(TForm)
...
var
  settruststopapply: Tsettruststopapply;
//  ptype: string[20];
//  pcent: real;
...
procedure Tsettruststopapply.setupdatestatus;    // 编辑状态
begin
...
procedure Tsettruststopapply.setunupdatestatus;  // 浏览状态
begin
...
procedure Tsettruststopapply.FormActivate(Sender: TObject);
begin
{
with dm.fee_type do
...
procedure Tsettruststopapply.FormCloseQuery(Sender: TObject;
...
procedure Tsettruststopapply.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsettruststopapply.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsettruststopapply.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with cdsbill_dtl do
...
procedure Tsettruststopapply.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsettruststopapply.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsettruststopapply.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsettruststopapply.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    commandtext:=commandtext+' where a.comp_id='+inttostr(compid)+' and a.bod_type_id=26';
...
procedure Tsettruststopapply.FormCreate(Sender: TObject);
begin
...
procedure Tsettruststopapply.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsettruststopapply.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
procedure Tsettruststopapply.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsettruststopapply.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsettruststopapply.SpeedButton2Click(Sender: TObject);
var s: string;
begin
{
with cdsbill do
...
with cdsbill do
begin
    if fieldbyname('allo_desc').asstring='' then raise Exception.Create('请填写停用原因');
end;
...
procedure Tsettruststopapply.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsettruststopapply.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsettruststopapply.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsettruststopapply.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
...
procedure Tsettruststopapply.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsettruststopapply.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsettruststopapply.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsettruststopapply.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsettruststopapply.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsettruststopapply.N1Click(Sender: TObject);
...
procedure Tsettruststopapply.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsettruststopapply.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsettruststopapply.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsettruststopapply.brokermedtlCalcFields(DataSet: TDataSet);
begin
with brokermedtl do
begin
...
procedure Tsettruststopapply.cdsbill_dtlBeforePost(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
