...
procedure Tsetrulestop.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetrulestop.setunupdatestatus;  // 浏览状态
begin
...
begin
    ...
    open;
    if recordcount>0 then raise Exception.Create('本单有未设置停用日期记录，不可送审');
end;
{
with dm.pubqry do
begin
...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1政策ID, 2业务员, 3商业公司, 3医院, 4药品编码, 5明细业余员, 6申请摘要, 7停用原因, 8停用日期
    mi:=0;j:=2;  //第1行开始 每行一单
...
//从第2行开始导入，格式: 1政策ID, 2业务员, 3商业公司, 3医院, 4药品编码, 5明细业余员, 6申请摘要, 7停用原因, 8停用日期
    ...
    sql:=sql+' select top 5 * from ( select top 5 ''第''+line_no+''行无政策ID或无效'' from @tab where f1='''' or try_cast(f1 as int) is null or not exists (select 1 from tb_brokermed where rec_id=cast(f1 as int))';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无停用日期或数据无效'' from @tab where f8='''' or try_cast(f8 as datetime) is null';
    sql:=sql+' ) a';
//从第2行开始导入，格式: 1政策ID, 2业务员, 3商业公司, 3医院, 4药品编码, 5明细业余员, 6申请摘要, 7停用原因, 8停用日期
    with dm.pubqry do
    begin
...
//edit1.text:=commandtext;
    ...
    begin
        SpeedButton5Click(nil);
        if cdsbill.Locate('bod_id',mi,[]) then MessageBox(0,pchar('已成功导入'+cdsbill.fieldbyname('bod_cd').asstring+'号停用申请单'),'请注意',MB_OK+MB_ICONInformation);
    end;
end;
