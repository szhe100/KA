...
var
  setagentaccount1: Tsetagentaccount1;
...
procedure Tsetagentaccount1.setupdatestatus;    // 编辑状态
begin
speedbutton2.enabled:=false;    // examine disable
speedbutton5.enabled:=false;   // refresh disable
end;

procedure Tsetagentaccount1.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetagentaccount1.FormActivate(Sender: TObject);
begin
if cdsbill.active=False then SpeedButton5Click(nil);
end;

procedure Tsetagentaccount1.FormCloseQuery(Sender: TObject;
...
procedure Tsetagentaccount1.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetagentaccount1.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetagentaccount1.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetagentaccount1.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetagentaccount1.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetagentaccount1.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    close;
    commandtext:='select a.*,agent=c.mate_name,broker=f.zname,creater=d.zname,checker=e.zname,carryer=g.zname,receiver=h.zname';
    ...
    commandtext:=commandtext+' where a.bod_type_id=14 and a.bod_status_id in (1,2,3,4) and busi_type=1';   // a.comp_id='+inttostr(compid)+' and
    commandtext:=commandtext+'  and a.creat_dt>= '''+datetostr(dxdateedit1.date)+''' and a.creat_dt< dateadd(day,1,'''+datetostr(dxdateedit4.date)+''')';
    if fdistltd then
    begin
        commandtext:=commandtext+' and exists (select 1 from tb_trustdistrict t where t.sta_id='+inttostr(curuserid)+' and dbo.fn_ischildnode(f.district,t.dist_id)=1)';
    end;
...
procedure Tsetagentaccount1.FormCreate(Sender: TObject);
begin
N2.Visible:=False;
...
procedure Tsetagentaccount1.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetagentaccount1.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
begin
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                if active=true then close;
                commandtext:='select top 3 b.bod_id,b.dtl_id,b.med_id,med_name=case med_id when 1 then ''收款人'' when 2 then ''开户银行'' when 3 then ''银行账户'' end,b.bat_cd,b.zdesc';
...
procedure Tsetagentaccount1.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetagentaccount1.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetagentaccount1.SpeedButton2Click(Sender: TObject);
begin
...
procedure Tsetagentaccount1.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetagentaccount1.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetagentaccount1.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetagentaccount1.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
...
procedure Tsetagentaccount1.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetagentaccount1.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetagentaccount1.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetagentaccount1.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetagentaccount1.dxDBGrid1CustomDrawCell(Sender: TObject;
...
begin
    if (ANode.Values[dxdbgrid1cbod_status.Index] ='制单') or (ANode.Values[dxdbgrid1cbod_status.Index] ='业务审核') then Acolor := $00A3FEFC;
...
procedure Tsetagentaccount1.N1Click(Sender: TObject);
...
begin
    ...
    begin
        if active=true then close;
        commandtext:='update tb_bill set bod_status_id=3,carry_by='+inttostr(curuserid)+',carry_dt=getdate() where bod_id in ('+t+')';
        execute;
        SpeedButton5Click(nil);
        ...
        try
            try
                ...
                begin
                    ...
                    begin
                        if active=true then close;
                        commandtext:='sp_updatestock14 -14,'+ dm.pubqry.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
                        execute;
                    end;
                    next;
                end;
...
procedure Tsetagentaccount1.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
try
...
procedure Tsetagentaccount1.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetagentaccount1.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
{
with cdsbill_dtl do
begin
...
procedure Tsetagentaccount1.SpeedButton6Click(Sender: TObject);
begin
