...
uses
  ...
  DBClient, KAserver_TLB, StdVcl, Db, ADODB, Provider, common, Variants;

type
  TKA = class(TRemoteDataModule, IKA)
...
class procedure TKA.UpdateRegistry(Register: Boolean; const ClassID, ProgID: string);
begin
  if Register then
  begin
...
procedure TKA.RemoteDataModuleCreate(Sender: TObject);
begin
  { Update the client counter }
//  MainForm.UpdateClientCount(1);
end;

procedure TKA.RemoteDataModuleDestroy(Sender: TObject);
begin
  { Update the client counter }
//  MainForm.UpdateClientCount(-1);
end;

procedure TKA.pvdbill_dtl1BeforeUpdateRecord(Sender: TObject;
  SourceDS: TDataSet; DeltaDS: TClientDataSet; UpdateKind: TUpdateKind;
  var Applied: Boolean);
begin
...
procedure TKA.pvdbill_dtl2BeforeUpdateRecord(Sender: TObject;
  SourceDS: TDataSet; DeltaDS: TClientDataSet; UpdateKind: TUpdateKind;
  var Applied: Boolean);
begin
  // 避免增加、修改、删除 非tb_bill_dtl表的数据
...
procedure TKA.pvdbrokermedBeforeUpdateRecord(Sender: TObject;
  SourceDS: TDataSet; DeltaDS: TClientDataSet; UpdateKind: TUpdateKind;
  var Applied: Boolean);
begin
...
/////////////////////////////////////////////////////////////////////////////////////////////////
function TKA.VarToStr(const V: Variant): string;
begin
  Result := VarToStrDef(V, '');
end;

function TKA.VarToStrDef(const V: Variant; const ADefault: string): string;
begin
...
function TKA.vartosql(value: Variant): wideString;
var tmp:widestring;
begin
...
function TKA.ExeSql(SQLSTR:String; Query: TADOQuery; Flag: Boolean = true) : Boolean;
begin
    Query.close;
    Query.SQL.clear;
...
function TKA.applyupdate(pdelta: OleVariant; const ptablename,
...
//更新数据库，并自动返回新插入自增长字段
function TKA.updateRefresh(pdelta: OleVariant; const ptablename, pkeyfield,
...
procedure TKA.ADOCon_KABeforeConnect(Sender: TObject);
begin
ADOCon_KA.ConnectionString:=MainForm.pubcom.ConnectionString;
end;

initialization
  TComponentFactory.Create(ComServer, TKA,
    Class_KA, ciMultiInstance, tmApartment);
end.
