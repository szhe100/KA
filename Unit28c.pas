...
procedure Tpickmanager.Edit1KeyPress(Sender: TObject; var Key: Char);
var sc: string;
    mi: integer;
    sk: widestring;
    fwid: boolean;
...
begin
    ...
    begin
        ...
        commandtext:=commandtext+' sta_type_id,qry_code from tb_staff';
        commandtext:=commandtext+' where comp_id='+inttostr(compid)+' and sta_type_id=0 and manager=1 and isnull(stop_yn,0)=0';
...
//        commandtext:=commandtext+' order by zname';
        open;
        if recordcount=0 then begin beep; raise Exception.Create('无所查询区域经理'); end;
    end;
end;
end;

procedure Tpickmanager.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickmanager.DBGrid1DblClick(Sender: TObject);
begin
   modalresult:=mrOK;
end;

procedure Tpickmanager.wwDBNavigator1FirstClick(Sender: TObject);
begin
edit1.setfocus;
end;

procedure Tpickmanager.FormCloseQuery(Sender: TObject;
...
procedure Tpickmanager.FormActivate(Sender: TObject);
begin
edit1.setfocus;
edit1.selectall;
end;

procedure Tpickmanager.BitBtn3Click(Sender: TObject);
begin
self.tag:=1;  // <Esc> (cancel) 退出标志
end;

procedure Tpickmanager.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tpickmanager.DSquery1DataChange(Sender: TObject; Field: TField);
begin
if activecontrol=dbgrid1 then
edit1.text:=query1.fieldbyname('pick_ord').asstring;
end;

procedure Tpickmanager.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
