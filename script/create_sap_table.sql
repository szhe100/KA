...
select count(*) from SAP_ZSD_015

select * from SAP_ZSD_MARA
select * into tb_medicine_250514 from tb_medicine
...
select count(1) from SAP_ZSD_PR02
select count(1) from SAP_ZSD_PR03
select * from SAP_ZSD_PR03
...
select top 20000 med_unit=dbo.fn_obj_desc(m.unit_id),a.*,m.*, dist1=d.prov_name, dist2=d.city_name,  creater=f.zname 
from tb_medprice a 
left join tb_medicine m on a.med_id =m.med_id 
left join tb_staff f on a.creat_by =f.sta_id 
left join (select prov_code,prov_name,city_name from tb_district where level_id=1) d on ZREGIO=d.prov_code  
where type_id=2

alter table tb_busimate add BEZEI varchar(50),CITY1 varchar(50)

select mate_id,mate_code,mate_name,a.region,a.BEZEI,a.CITY1,b.BEZEI,b.CITY1,tax_code,TAXNUMXL from tb_busimate a,sap_zsd_z001 b where a.mate_type_id=2 and a.mate_code=b.PARTNER

update a set BEZEI=b.BEZEI,CITY1=b.CITY1 from tb_busimate a,sap_zsd_z001 b where a.mate_type_id=2 and a.mate_code=b.PARTNER

select count(1) from tb_busimate where mate_type_id=2
...
select * into tb_busimate_250513 from tb_busimate
de
...
select a.mate_id,a.mate_name,a.REGION,a.BEZEI,a.CITY1,b.REGION,b.BEZEI,b.CITY1 from tB_busimate a,SAP_ZSD_Z005 b where a.mate_type_id=4 and a.mate_code=partner and a.mate_code='0001000003'

update a set REGION=b.REGION,BEZEI=b.BEZEI,CITY1=b.CITY1 from tB_busimate a,SAP_ZSD_Z005 b where a.mate_type_id=4 and a.mate_code=partner 
-- and a.mate_code='0001000003'

declare @bodcd varchar(12),@bcd varchar(2000)
set @bcd=''
set @bodcd='12345678'
if @bcd<>'' set @bcd=@bcd+','
set @bcd=@bcd+@bodcd
...
select top 10 channel_id from tb_busiframe3


select * from tb_busimate where mate_type_id=4 and mate_code='0001000003'

alter table tb_busimate add CITY1 varchar(50)
REGION
BEZEI
CITY1
...
select *,MATNR from SAP_ZSD_PR02
truncate table SAP_ZSD_PR02

select * from SAP_ZSD_PR03 
truncate table SAP_ZSD_PR03
select count(*) from SAP_ZSD_BUS

select top 10 * from SAP_ZSD_015 where ZKONV<>''
...
select * into tb_medprice_250513 from tb_medprice
truncate table tb_medprice

select * from tb_medprice where type_id=1 and zcitynum<>''

select * from tb_district where prov_code='44'

update a set valid_from=b.valid_to,valid_to=b.valid_from from tb_medprice a,tb_medprice b where a.type_id in (1,2) and a.rec_id=b.rec_id

alter table tb_medprice add MATNR varchar(20)

中标价
共有4306条记录，zcitynum有值的有263条记录

delete from tb_medprice where type_id=1

insert into tb_medprice (type_id,KNUMH,MATNR,ZCOUNTRY,ZREGIO,ZCITYNUM,district_id,med_id,price,currency,price_unit,valid_from,valid_to)
select type_id=1,KNUMH,MATNR,ZCOUNTRY,ZREGIO,ZCITYNUM,district=b.rec_id,m.med_id,price=KBETR,currency=KONWA,price_unit=KMEIN,valid_from=DATAB,valid_to=DATBI
 from SAP_ZSD_PR02 a
...
select * into tb_medprice_250508a from tb_medprice

update a set valid_from = b.valid_to,valid_to=b.valid_from from tb_medprice a,tb_medprice_250508a b where a.rec_id=b.rec_id
...
select top 5 * from SAP_ZSD_PR02
select top 5 * from SAP_ZSD_PR03 
select top 5 * from SAP_ZSD_PR03 a,tb_district b
 where a.zregio=b.prov_code

alter table tb_medprice add KONDA varchar(20)
	[ZCOUNTRY] [varchar](20) NULL,
	[ZREGIO] [varchar](20) NULL,
	[ZCITYNUM] [varchar](20) NULL 

select distinct type_id from tb_medprice

select top 10 * from SAP_ZSD_PR03

select * from tb_busimate where mate_type_id=1


select * from tb_medprice a,SAP_ZSD_PR02 b where a.type_id=1 and a.KNUMH=b.KNUMH
update a set MATNR=b.MATNR from tb_medprice a,SAP_ZSD_PR02 b where a.type_id=1 and a.KNUMH=b.KNUMH

select * from tb_medprice a,SAP_ZSD_PR03 b where a.type_id=2 and a.KNUMH=b.KNUMH
update a set MATNR=b.MATNR from tb_medprice a,SAP_ZSD_PR03 b where a.type_id=2 and a.KNUMH=b.KNUMH

select * into tb_medprice_250515 from tb_medprice

delete from tb_medprice where type_id=2

select * from tb_medprice where type_id=2

insert into tb_medprice (type_id,KNUMH,MATNR,ZREGIO,KONDA,VKORG,channel_id,channel_dtl_id,sale_team_id,cust_group_id,district_id,med_id,price,currency,price_unit,valid_from,valid_to)
select type_id=2,KNUMH,MATNR,ZREGIO,KONDA,VKORG,channel_id=c.obj_id,channel_dtl_id=d.obj_id,sale_team_id=f.obj_id,cust_group_id=e.obj_id,district=a.zregio,m.med_id,price=KBETR,currency=KONWA,price_unit=KMEIN,valid_from=DATAB,valid_to=DATBI
 from SAP_ZSD_PR03 a
...
select top 20000 med_unit=dbo.fn_obj_desc(m.unit_id),a.*,m.*, dist1=d.prov_name, dist2=d.city_name, creater=c.zname from tb_medprice a left join tb_medicine m on a.med_id =m.med_id left join tb_staff c on a.creat_by =c.sta_id 
left join (select prov_code,prov_name,city_name from tb_district where level_id=1) d on ZREGIO=d.prov_code  where type_id=1
...
select * from tb_linkword

select * into tb_linkword_250512 from tb_linkword

truncate table tb_linkword


select med_id,med_name,material_code,material_code1 from tb_medicine where material_code1 like material_code+'%'
...
select len('1000001400'),substring('1000001400',1,len('1000001400')-1)
...
select 2,'测试','10','10','刘永华','广州医药股份有限公司','白云区鹤龙街社区卫生服务中心','1000010000','','9.8','28.02','','2025-1-1' 
union all  
select 3,'测试','10','10','周先福(周菊梅)','国药集团一致药业股份有限公司','广东省第二人民医院','1000010000','','8.4','28.02','','2025-1-1' 
union all  
select 4,'测试','10','10','周先福(周菊梅)','国药集团一致药业股份有限公司','广东省第二人民医院','1000010000','冯强(王雪霜)','1.4','28.02','','2025-1-1' 
union all  
select 5,'测试','10','10','刘永华','广州医药股份有限公司','广州市第十二人民医院','1000010000','','8.4','28.02','5%','2025-1-2'
select top 5 * from ( 
select top 5 info='第'+line_no+'行无渠道' from @tab a where f2='' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.zdesc=a.f2) 
union all 
select top 5 '第'+line_no+'行无子渠道' from @tab a where a.f3='' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.zname=a.f3) union all select top 5 '第'+line_no+'行无业务员或数据无效' from @tab a where f4='' or not exists (select 1 from tb_staff b where sta_type_id=1 and b.zname =a.f4) union all select top 5 '第'+line_no+'行无商业公司或数据无效' from @tab a where f5='' or not exists (select 1 from tb_busimate b where mate_type_id=2 and b.mate_name =a.f5) union all select top 5 '第'+line_no+'行无医院或数据无效' from @tab a where f6='' or not exists (select 1 from tb_busimate b where mate_type_id=1 and b.mate_name =a.f6) union all select top 5 '第'+line_no+'行无药品编码或数据无效' from @tab a where f7='' or not exists (select 1 from tb_medicine m where m.med_name =a.f7) union all select top 5 '第'+line_no+'行无明细业务员或数据无效' from @tab a where f8='' or not exists (select 1 from tb_staff b where sta_type_id=1 and b.zname =a.f8) union all select top 5 '第'+line_no+'行无费用金额或数据无效' from @tab a where f9='' or try_cast(f9 as decimal(15,4)) is null union all select top 5 '第'+line_no+'行无中标价或数据无效' from @tab where f10='' or try_cast(f10 as decimal(15,4)) is null union all select top 5 '第'+line_no+'行无医院托管结算率或数据无效' from @tab where f11='' or try_cast(f11 as decimal(15,4)) is null union all select top 5 '第'+line_no+'行无启用日期或数据无效' from @tab where f12='' or try_cast(f2 as datetime) is null ) a

select * from tb_object where obj_type_id=11

declare @tab table (line_no varchar(3),f1 varchar(100),f2 varchar(100),f3 varchar(100),f4 varchar(100),f5 varchar(100),f6 varchar(100),f7 varchar(100),f8 varchar(100),f9 varchar(100),f10 varchar(100),f11 varchar(100),f12 varchar(100), channel_id int,channel_dtl_id int,sta_id int,sta_id1 int,mate_id int,mate_id1 int,med_id int) insert into @tab (line_no,f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12) select 2,'测试','10','10','刘永华','广州医药股份有限公司','白云区鹤龙街社区卫生服务中心','1000010000','','9.8','28.02','','2025-1-1' union all  select 3,'测试','10','10','周先福(周菊梅)','国药集团一致药业股份有限公司','广东省第二人民医院','1000010000','','8.4','28.02','','2025-1-1' union all  select 4,'测试','10','10','周先福(周菊梅)','国药集团一致药业股份有限公司','广东省第二人民医院','1000010000','冯强(王雪霜)','1.4','28.02','','2025-1-1' union all  select 5,'测试','10','10','刘永华','广州医药股份有限公司','广州市第十二人民医院','1000010000','','8.4','28.02','5%','2025-1-2' select top 5 * from ( select top 5 info='第'+line_no+'行无渠道' from @tab a where a.f2='' or not exists (select 1 from tb_object b where b.obj_type_id=11 and b.obj_code=a.f2) union all select top 5 '第'+line_no+'行无子渠道' from @tab a where a.f3='' or not exists (select 1 from tb_object b where b.obj_type_id=12 and b.obj_code=a.f3) union all select top 5 '第'+line_no+'行无业务员或数据无效' from @tab a where f4='' or not exists (select 1 from tb_staff b where sta_type_id=1 and b.zname =a.f4) union all select top 5 '第'+line_no+'行无商业公司或数据无效' from @tab a where f5='' or not exists (select 1 from tb_busimate b where mate_type_id=2 and b.mate_name =a.f5) union all select top 5 '第'+line_no+'行无医院或数据无效' from @tab a where f6='' or not exists (select 1 from tb_busimate b where mate_type_id=1 and b.mate_name =a.f6) union all select top 5 '第'+line_no+'行无药品编码或数据无效' from @tab a where f7='' or not exists (select 1 from tb_medicine m where m.med_name =a.f7) union all select top 5 '第'+line_no+'行无明细业务员或数据无效' from @tab a where f8='' or not exists (select 1 from tb_staff b where sta_type_id=1 and b.zname =a.f8) union all select top 5 '第'+line_no+'行无费用金额或数据无效' from @tab a where f9='' or try_cast(f9 as decimal(15,4)) is null union all select top 5 '第'+line_no+'行无中标价或数据无效' from @tab where f10='' or try_cast(f10 as decimal(15,4)) is null union all select top 5 '第'+line_no+'行无医院托管结算率或数据无效' from @tab where f11='' or try_cast(f11 as decimal(15,4)) is null union all select top 5 '第'+line_no+'行无启用日期或数据无效' from @tab where f12='' or try_cast(f2 as datetime) is null ) a
...
select * into tb_sysrule_250508 from tb_sysrule
...
left join tb_staff c on a.creat_by =c.sta_id 
left join tb_district d on a.district_id =d.rec_id 
where type_id=1 and dbo.fn_treeischild(a.district_id,18)=1 
...
select * into tb_staff_250516 from tb_staff
