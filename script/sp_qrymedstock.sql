...
if exists (select 1 from tempdb..sysobjects where id=object_id('tempdb..#tmp'))
  ...
  select a.mate_id,a.med_id,a.bat_cd,
		 ...
		 imp_qty=sum(cast(imp_qty as decimal(15,2)))
   into #tmp
   from 
  ( 
   ---SAP商业采购
  		select a.carry_dt,b.mate_id,m.med_id,bat_cd=a.CHARG,qty1=isnull(a.qty,0),qty2=0,qty3=0,imp_qty=0
			from SAP_ZSD_015 a
   ...
   ---KA商业采购
		union all
  		select a.carry_dt,c.mate_id,b.med_id,bat_cd,qty1=isnull(b.qty,0),qty2=0,qty3=0,imp_qty=0
			from tb_bill a
   ...
   ---KA商业销售 - 销售 售出方
		union all
  		select a.carry_dt,c.mate_id,b.med_id,bat_cd,qty1=0,qty2=isnull(b.qty,0),qty3=0,imp_qty=0
			from tb_bill a
			inner join tb_bill_dtl b on a.bod_id=b.bod_id
   ...
   ---KA商业盘点单
		union all
  		select a.carry_dt,c.mate_id,b.med_id,bat_cd,qty1=0,qty2=0,qty3=isnull(b.qty,0),imp_qty=0
			from tb_bill a
			inner join tb_bill_dtl b on a.bod_id=b.bod_id
			inner join (select mate_id,mate_code,mate_name from tb_busimate where mate_type_id=2) c on a.dst_id=c.mate_id
   ...
   ---KA 导入商业库存
		union all
  		select carry_dt=occur_dt,mate_id,med_id,bat_cd,qty1=0,qty2=0,qty3=0,imp_qty=qty
			from tb_matestock
		where year(occur_dt)=@year and month(occur_dt)=@month

	) a 
	where (@mateid=0 or a.mate_id=@mateid) and (@medid=0 or a.med_id=@medid)
	group by a.mate_id,a.med_id,bat_cd

	insert tb_matestock_desc (fyear,fmonth,mate_id,med_id,bat_cd,qty0,qty1,qty2,qty3,imp_qty) --,adj_qty1,adj_qty2,desc1,desc2,adj_diff,
	select @year int,@month,mate_id,med_id,bat_cd,qty0,qty1,qty2,qty3,imp_qty
	from #tmp a 
	where not exists (select 1 from tb_matestock_desc b where b.fyear=@year and b.fmonth=@month and a.mate_id=b.mate_id and a.med_id=b.med_id and a.bat_cd=b.bat_cd)

	update b set qty0=a.qty0,qty1=a.qty1,qty2=a.qty2,qty3=a.qty3,imp_qty=a.imp_qty
		from #tmp a 
		inner join tb_matestock_desc b on b.fyear=@year and b.fmonth=@month and a.mate_id=b.mate_id and a.med_id=b.med_id and a.bat_cd=b.bat_cd
