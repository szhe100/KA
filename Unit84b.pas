...
var
  setpromoapply2: Tsetpromoapply2;
...
procedure Tsetpromoapply2.setupdatestatus;    // 编辑状态
begin
...
procedure Tsetpromoapply2.setunupdatestatus;  // 浏览状态
begin
speedbutton5.enabled:=true;   // refresh enable
end;

procedure Tsetpromoapply2.FormActivate(Sender: TObject);
begin
with fee_type do
...
procedure Tsetpromoapply2.FormCloseQuery(Sender: TObject;
...
procedure Tsetpromoapply2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure Tsetpromoapply2.dxDBGrid1DblClick(Sender: TObject);
begin
pagecontrol1.activepage:=tabsheet2;
DSbillDataChange(PageControl1,nil);
end;

procedure Tsetpromoapply2.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tsetpromoapply2.SpeedButton8Click(Sender: TObject);
var mid: integer;
begin
...
procedure Tsetpromoapply2.SpeedButton9Click(Sender: TObject);
begin
// cancel speedbutton
...
procedure Tsetpromoapply2.SpeedButton5Click(Sender: TObject);
var bi: integer;
begin
// refresh speedbutton
...
begin
    ...
    close;
    commandtext:='select a.*,mate_name=dbo.fn_mate_name(c.mate_id),mate_name1='''',agent=dbo.fn_mate_name(c.agent_id),med_name=m.material_code+'' ''+m.med_name+'' ''+m.specifi+'' ''+m.pdt_place,';
    commandtext:=commandtext+' creater=d.zname,checker=e.zname,broker=f.zname,carryer=g.zname,receiver=h.zname,';
    ...
    commandtext:=commandtext+' inner join (select bod_id,rec_id=min(med_id) from tb_bill_dtl group by bod_id) b on a.bod_id=b.bod_id';
    commandtext:=commandtext+' inner join tb_busiframe3 c on b.rec_id=c.rec_id';
    ...
    commandtext:=commandtext+' where a.bod_type_id=35 and a.bod_status_id in (1,3)';
...
procedure Tsetpromoapply2.FormCreate(Sender: TObject);
begin
...
procedure Tsetpromoapply2.SpeedButton7Click(Sender: TObject);
begin
...
procedure Tsetpromoapply2.DSbillDataChange(Sender: TObject; Field: TField);
begin
...
begin
    ...
    begin
        ...
        SpeedButton6.enabled:=(state=dsbrowse) and (active) and (recordcount>0) and (fieldbyname('bod_status_id').asinteger=3);
    ...
    with cdsbill_dtl do
        if tag<>cdsbill.fieldbyname('bod_id').asinteger then
            ...
            begin
                ...
                commandtext:=commandtext+' c.channel_id,channel=dbo.fn_obj_desc1(11,c.channel_id),c.channel_dtl_id,channel_dtl=dbo.fn_obj_desc1(12,c.channel_dtl_id),';
                ...
                commandtext:=commandtext+' med_code,material_code,material_code1,med_name,specifi,pdt_place,med_unit=dbo.fn_obj_desc(m.unit_id),med_type=dbo.fn_med_type(m.med_id)';
...
procedure Tsetpromoapply2.DSbillStateChange(Sender: TObject);
begin
with cdsbill do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetpromoapply2.DSbill_dtlStateChange(Sender: TObject);
begin
with cdsbill_dtl do
    if state in [dsinsert,dsedit] then setupdatestatus;
end;

procedure Tsetpromoapply2.SpeedButton2Click(Sender: TObject);
begin
...
begin
    ...
    open;
    if recordcount=0 then raise Exception.Create('无明细数据，不可送审核');
end;
...
begin
    if active then close;
    commandtext:='sp_updatestock35 35,'+cdsbill.fieldbyname('bod_id').asstring+','+inttostr(curuserid);
    execute;
    close;
end;
...
procedure Tsetpromoapply2.cdsbillBeforeEdit(DataSet: TDataSet);
begin
with cdsbill do
begin
...
procedure Tsetpromoapply2.cdsbillCalcFields(DataSet: TDataSet);
begin
with cdsbill do
...
procedure Tsetpromoapply2.cdsbill_dtlBeforeDelete(DataSet: TDataSet);
begin
cdsbillBeforeEdit(nil);
if MessageBox(0,'确定删除本记录','请注意',MB_YESNO+MB_ICONQUESTION)<>IDYES then abort;
end;

procedure Tsetpromoapply2.cdsbill_dtlBeforeEdit(DataSet: TDataSet);
begin
if cdsbill.fieldbyname('bod_status_id').asinteger=1
...
procedure Tsetpromoapply2.cdsbill_dtlReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure Tsetpromoapply2.cdsbill_dtlAfterInsert(DataSet: TDataSet);
begin
with cdsbill_dtl do
    fieldbyname('bod_id').asinteger:=cdsbill.fieldbyname('bod_id').asinteger;
end;

procedure Tsetpromoapply2.cdsbill_dtlpriceChange(Sender: TField);
begin
with cdsbill_dtl do
...
procedure Tsetpromoapply2.PageControl1Change(Sender: TObject);
begin
if pagecontrol1.activepage=TabSheet2 then DSbillDataChange(PageControl1,nil);
end;

procedure Tsetpromoapply2.dxDBGrid1CustomDrawCell(Sender: TObject;
...
procedure Tsetpromoapply2.N1Click(Sender: TObject);
...
procedure Tsetpromoapply2.cdsbill_dtlAfterPost(DataSet: TDataSet);
begin
if cdsbill_dtl.ChangeCount>0 then myupdaterefresh(cdsbill_dtl,'tb_bill_dtl','dtl_id','');
end;

procedure Tsetpromoapply2.dxDBGrid2mate_nameButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure Tsetpromoapply2.cdsbill_dtlCalcFields(DataSet: TDataSet);
begin
with cdsbill_dtl do
begin
...
procedure Tsetpromoapply2.SpeedButton6Click(Sender: TObject);
begin
...
procedure Tsetpromoapply2.SpeedButton3Click(Sender: TObject);
begin
...
procedure Tsetpromoapply2.DSbill_dtlDataChange(Sender: TObject;
  Field: TField);
begin
with dtl do
begin
...
procedure Tsetpromoapply2.dtlCalcFields(DataSet: TDataSet);
begin
