...
type
  Tsetagent = class(TForm)
    ...
    agentBEZEI: TStringField;
    agentCITY1: TStringField;
    ...
    procedure agentCalcFields(DataSet: TDataSet);
    procedure agentdistrictChange(Sender: TField);
...
//    commandtext:='select cdistrict=dbo.fn_getdistrict(a.district),a.*,mate_name1=e.mate_name,';
    commandtext:='select a.*,mate_name1=e.mate_name,';  //cdistrict='''',
...
//    if Trim(dxLookupTreeView1.text)<>'' then commandtext:=commandtext+' and a.region='+dm.district.fieldbyname('code').asstring;
    if Trim(dxLookupTreeView1.text)<>'' then commandtext:=commandtext+' and (a.region like '''+dm.district.fieldbyname('code').asstring+'%'' or a.BEZEI='''+dm.district.fieldbyname('name').asstring+''' or a.city1='''+dm.district.fieldbyname('name').asstring+''')';
...
begin
    ...
    commandtext:=commandtext+' and district='+agent.fieldbyname('district').asstring+' and mate_name='''+ agent.fieldbyname('mate_name').asstring+'''';
    if agent.fieldbyname('mate_id').asinteger<>0 then commandtext:=commandtext+'  and mate_id<>'+agent.fieldbyname('mate_id').asstring;
{
    ...
    open;
    if recordcount>0 then raise Exception.Create('该地区已有此业务负责人，不可重复录入');
end;
with agent do
begin
...
procedure Tsetagent.dxLookupTreeView1DropDown(Sender: TObject);
begin
setprogress(1);
(Sender as TdxLookupTreeView).Width:=180;
end;

procedure Tsetagent.dxLookupTreeView1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
setprogress(0);
end;

procedure Tsetagent.dxLookupTreeView1CloseUp(Sender: TObject;
  Accept: Boolean);
begin
(Sender as TdxLookupTreeView).Width:=120;
end;

procedure Tsetagent.agentCalcFields(DataSet: TDataSet);
begin
with agent do
    fieldbyname('Cdistrict').asstring:=fieldbyname('BEZEI').asstring+' '+fieldbyname('CITY1').asstring;
end;

procedure Tsetagent.agentdistrictChange(Sender: TField);
begin
with agent do
begin
...
procedure Tsetagent.dxDBLookupTreeView1DropDown(Sender: TObject);
begin
setprogress(1);
(Sender as TdxDBLookupTreeView).Width:=180;
end;

procedure Tsetagent.dxDBLookupTreeView1CloseUp(Sender: TObject;
  Accept: Boolean);
begin
(Sender as TdxDBLookupTreeView).Width:=120;
end;
