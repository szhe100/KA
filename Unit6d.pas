...
begin
    ...
    setprogress(1);
//从第2行开始导入，格式: 1渠道名称,2子渠道名称,3业务省份,4业务城市,5关联编码, 6中标价/开票价, 7金额,8定价单位,9启用日期,10 备注
    mi:=0;j:=2;  //第1行开始 每行一单
    ...
    begin
        ...
        sql:=sql+''','''+Trim(sheet.cells[j,6].text)+''','''+Trim(sheet.cells[j,7].text)+''','''+Trim(sheet.cells[j,8].text)+''','''+Trim(sheet.cells[j,9].text)+''','''+Trim(sheet.cells[j,10].text)+'''';      //+''','''+Trim(sheet.cells[j,10].text
        j:=j+1;
    end;
    XL.Quit;
...
//    sql:=sql+' union all select top 5 ''第''+line_no+''行无 业务省份或数据无效'' from @tab a where f3='''' or not exists (select 1 from tb_district b where b.prov_name=f3 )';
    sql:=sql+' union all select top 5 ''第''+line_no+''行无 省份城市或数据无效'' from @tab a where f3='''' or not exists (select 1 from tb_district b where (b.prov_name=f3 and (b.city_name=f4 or b.county_name=f4)))';
...
//rec_id,comp_id,type_id,channel_id,channel_dtl_id,cust_group_id,district_id,med_id,price,price_unit,price1,valid_from,valid_to,creat_by,creat_dt
        sql:=sql1+' insert into tb_medprice (comp_id,type_id,channel_id,channel_dtl_id,district_id,med_id,price,price_unit,price1,valid_from,zdesc,creat_by,creat_dt)';   //valid_to,
        ...
        sql:=sql+'      m.med_id,price=f7,price_unit=f8,price1=f6,valid_from=cast(f9 as datetime),zdesc=f10,'; //,valid_to=cast(f10 as datetime),';
